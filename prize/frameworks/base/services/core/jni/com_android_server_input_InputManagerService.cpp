...
static struct {
    ...
    jmethodID interceptMotionBeforeQueueingInteractive;
} gServiceClassInfo;
...
/// M: KeyDispatchingTimeout predump mechanism
    ...
    /* Dynamically hiding nav bar feature. prize-linkh-20150805 */
    virtual void interceptMotionBeforeQueueing(const MotionEvent* motionEvent, nsecs_t when, uint32_t& policyFlags);
...
/* Dynamically hiding nav bar feature. prize-linkh-20150805 */
void NativeInputManager::interceptMotionBeforeQueueing(const MotionEvent* motionEvent, nsecs_t when, uint32_t& policyFlags) {
    bool interactive = mInteractive.load();
    if(interactive && (policyFlags & POLICY_FLAG_TRUSTED)/* && !(policyFlags & POLICY_FLAG_INJECTED)*/ ) {
            policyFlags |= POLICY_FLAG_INTERACTIVE;
            ...
            if(motionEventObj) {
                wmActions = env->CallIntMethod(mServiceObj,
                            gServiceClassInfo.interceptMotionBeforeQueueingInteractive,
                            motionEventObj, policyFlags);
                if (checkAndClearExceptionFromCallback(env,
                        "interceptMotionBeforeQueueingInteractive")) {
                    wmActions = 0;
                }
                android_view_MotionEvent_recycle(env, motionEventObj);
                env->DeleteLocalRef(motionEventObj);
            } else {
                ALOGE("Failed to obtain motion event object for interceptMotionBeforeQueueing.");
                wmActions = 0;
            }

            handleInterceptActions(wmActions, when, /*byref*/ policyFlags);            
    } else {
        interceptMotionBeforeQueueing(when, policyFlags);
    }
...
int register_android_server_InputManager(JNIEnv* env) {
    ...
    /* Dynamically hiding nav bar feature. prize-linkh-20150805 */
    GET_METHOD_ID(gServiceClassInfo.interceptMotionBeforeQueueingInteractive, clazz,
            "interceptMotionBeforeQueueingInteractive", "(Landroid/view/MotionEvent;I)I");
