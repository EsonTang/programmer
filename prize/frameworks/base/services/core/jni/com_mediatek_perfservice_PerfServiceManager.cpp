...
static int (*perfSetUeventIndex)(int, int) = NULL;
static int (*perfLevelBoost)(int) = NULL;
static int (*perfReloadWhiteList)(void) = NULL;
static int (*perfGetClusterInfo)(int, int) = NULL;
static int (*perfGetPackAttr)(const char*, int) = NULL;
static char *(*perfGetGiftAttr)(const char*, const char*) = NULL;
static int (*perfSetExclusiveCore)(int, int) = NULL;
static int (*perfSetUidInfo)(int, int) = NULL;
...
static void init()
{
    ...
    func = dlsym(handle, "perfReloadWhiteList");
    perfReloadWhiteList = reinterpret_cast<reload_white_list>(func);

    if (perfReloadWhiteList == NULL) {
        ALOGE("perfReloadWhiteList error: %s", dlerror());
        perfReloadWhiteList = NULL;
        dlclose(handle);
        return;
    }
    ...
    func = dlsym(handle, "perfGetGiftAttr");
    perfGetGiftAttr = reinterpret_cast<get_gift_attr>(func);

    if (perfGetGiftAttr == NULL) {
        ALOGE("perfGetGiftAttr error: %s", dlerror());
        perfGetGiftAttr = NULL;
        dlclose(handle);
        return;
    }
    ...
    if (perfSetExclusiveCore == NULL) {
        ...
        perfSetExclusiveCore = NULL;
        dlclose(handle);
        return;
    }

    func = dlsym(handle, "perfSetUidInfo");
    perfSetUidInfo = reinterpret_cast<set_uid_info>(func);

    if (perfSetUidInfo == NULL) {
        ALOGE("perfSetUidInfo error: %s", dlerror());
        perfSetUidInfo = NULL;
        dlclose(handle);
        return;
    }
                                     ...
                                     jint handle, jint cmd, jint param_1, jint param_2, jint param_3, jint param_4)
{
    ...
    if (perfUserRegScnConfig && handle >= 0 && param_1 >= 0)
...
static int
android_server_PerfSetUeventIndex(JNIEnv *env, jobject thiz, jint index, jint action)
{
#if defined(MTK_PERFSERVICE_SUPPORT)
    if (!inited)
        init();
    ...
    if (perfSetUeventIndex)
        return perfSetUeventIndex(index, action);
...
static int
android_server_PerfReloadWhiteList(JNIEnv *env, jobject thiz)
{
#if defined(MTK_PERFSERVICE_SUPPORT)
    if (!inited)
        init();

    if (perfReloadWhiteList)
        return perfReloadWhiteList();

    ALOGE("perfReloadWhiteList bypassed!");
#endif
    return -1;
}
...
static jstring
android_server_PerfGetGiftAttr(JNIEnv *env, jobject thiz, jstring packName, jstring attrName)
{
#if defined(MTK_PERFSERVICE_SUPPORT)
    char *returnBuf;
    ...
    if(GiftResult != nullptr){
        env->DeleteGlobalRef(GiftResult);
        GiftResult = nullptr;
    }

    if (perfGetGiftAttr) {
        const char *nativePack = (packName) ? env->GetStringUTFChars(packName, 0) : NULL;
        const char *nativeAttr = (attrName) ? env->GetStringUTFChars(attrName, 0) : NULL;

        if(nativePack != NULL && nativeAttr != NULL) {
            returnBuf = perfGetGiftAttr(nativePack, nativeAttr);
        GiftResult = reinterpret_cast<jstring>(env->NewGlobalRef(env->NewStringUTF(returnBuf)));
        if(returnBuf != NULL)
            free(returnBuf);

            env->ReleaseStringUTFChars(packName, nativePack);
            env->ReleaseStringUTFChars(attrName, nativeAttr);
...
static int
android_server_PerfSetUidInfo(JNIEnv *env, jobject thiz, jint uid, jint fromUid)
{
#if defined(MTK_PERFSERVICE_SUPPORT)
    if (!inited)
        init();
    ...
    if (perfSetUidInfo) {
        return perfSetUidInfo(uid, fromUid);
    }

    ALOGE("perfSetUidInfo bypassed!");
#endif
    return -1;
}
...
static JNINativeMethod sMethods[] = {
    ...
    {"nativePerfSetUeventIndex","(II)I",  (int *)android_server_PerfSetUeventIndex},
    {"nativePerfLevelBoost", "(I)I",  (int *)android_server_PerfLevelBoost},
    {"nativePerfReloadWhiteList", "()I",  (int *)android_server_PerfReloadWhiteList},
    ...
    {"nativePerfGetGiftAttr", "(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;", (void *)android_server_PerfGetGiftAttr},
    {"nativePerfSetExclusiveCore", "(II)I", (int *)android_server_PerfSetExclusiveCore},
    {"nativePerfSetUidInfo", "(II)I", (int *)android_server_PerfSetUidInfo},
};
