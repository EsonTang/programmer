...
import com.mediatek.common.prizeoption.PrizeOption; // prize liyongli add for split screen
...
final class TaskRecord {
    ...
    int appInstanceIndex = 0; // 0 represents this activity is original.
    //used for task persister feature.
    private static final String ATTR_APP_INSTANCE_INDEX = "appInstIdx";
    //end...
    ...
    final ActivityRecord performClearTaskLocked(ActivityRecord newR, int launchFlags) {
        ...
        for (int activityNdx = numActivities - 1; activityNdx >= 0; --activityNdx) {
            ...
            if (r.realActivity.equals(newR.realActivity) 
                    && mService.isSameAppInstance(r.realActivity.getPackageName(), r.appInstanceIndex, newR.appInstanceIndex)) {/* app multi instances feature. prize-linkh-20151028 */
    ...
    void saveToXml(XmlSerializer out) throws IOException, XmlPullParserException {
        ...
        /* app multi instances feature. prize-linkh-20151111 */
        out.attribute(null, ATTR_APP_INSTANCE_INDEX, String.valueOf(appInstanceIndex));
    ...
    static TaskRecord restoreFromXml(XmlPullParser in, ActivityStackSupervisor stackSupervisor)
        ...
        for (int attrNdx = in.getAttributeCount() - 1; attrNdx >= 0; --attrNdx) {
            ...
            } else if(ATTR_APP_INSTANCE_INDEX.equals(attrName)) {// app multi instances feature. prize-linkh-20151111
                appInsIdx = Integer.valueOf(attrValue);
            } else {
        ...
        /* app multi instances feature. prize-linkh-20151111 */
        task.appInstanceIndex = appInsIdx;
        if(activities.size() == 0 && task.intent != null) {
            task.intent.setAppInstanceIndex(appInsIdx);
        } //end...
    ...
    Configuration updateOverrideConfiguration(Rect bounds, @Nullable Rect insetBounds) {
        ...
        if (mFullscreen) {
            // The stack will be null when reinstall apk in some case, PackageManager will call
            // killApplication function, it will cause NullPointerException seldom.
            /// M: ALPS03505142. Avoid NullPointerException
            if (mBounds != null && (stack != null && StackId.persistTaskBounds(stack.mStackId))) {
            ...
            mTmpRect.set(bounds);
            /* prize-add-split screen we drag change size, so no need adjust to the mini rect -liyongli-20171121-start */
            if(!PrizeOption.PRIZE_SPLIT_SCREEN_DRAG) {//fix bug  42046
            adjustForMinimalTaskDimensions(mTmpRect); //orginal code, after drag will adjust the rect to mMinHeight
            }/* prize-add-split screen-liyongli-20171121-end */
            if (mBounds == null) {
    ...
    Rect getLaunchBounds() {
        ...
        /// M: BMW. [ALPS03010130] Fix a google issue, the freeform task's launchBounds @{
        ///     is error in some cases.
        if (MultiWindowManager.isSupported() && stackId == FREEFORM_WORKSPACE_STACK_ID
