...
class AutomaticBrightnessController {
    ...
    //prize-chengdong-20180329 0.10f-> 0.40f  0.20f-> 0.50f start
    private static final float BRIGHTENING_LIGHT_HYSTERESIS = 0.40f;
    private static final float DARKENING_LIGHT_HYSTERESIS = 0.50f;
    private static final float BRIGHTENING_LIGHT_HYSTERESIS_LARGE = 1.20f;
    private static final float DARKENING_LIGHT_HYSTERESIS_LARGE = 0.90f;
	//prize-chengdong-20180329 010f-> 0.40f  0.20f-> 0.50f end
    ...
    //prize-chendong-20180329 Proximity Sensor State true positive -start
    private boolean mProximitySensorPositive = false;
    //prize-wulaing-20180418 default update Brightness
    private boolean mDefaultLuxThreshold = true;
    // Mediatek AAL support
    ...
    public int getAutomaticScreenBrightness() {
        if (DEBUG) {
            Slog.d(TAG, "getAutomaticScreenBrightness: brightness=" + mScreenAutoBrightness +
                ", dozing=" + mDozing + ", factor=" + mDozeScaleFactor);
        }
        if (mDozing) {
    ...
    private void setAmbientLux(float lux, boolean defaultLuxThreshold) {
        mAmbientLux = lux;
        //prize-wuliang-20180418 start
        ...
        Slog.d("wuliang", "setAmbientLux = " + mAmbientLux);
        setDefaultLuxThreshold(defaultLuxThreshold);
        //prize-wuliang-20180418 end
    ...
    private void updateAmbientLux(long time) {
        ...
        if (!mAmbientLuxValid) {
            ...
            //prize-wuliang-20180418 AutoMode Brightness start
            //setAmbientLux(calculateAmbientLux(time));
            setAmbientLux(calculateAmbientLux(time),mDefaultLuxThreshold);
            ...
            if(mDefaultLuxThreshold){
                updateAutoBrightness(true);
            } else {
                updateAutoModeBrightness(mScreenAutoBrightness);
            }
            //prize-wuliang-20180418 AutoMode Brightness end
        }

        long nextBrightenTransition = nextAmbientLightBrighteningTransition(time);
        long nextDarkenTransition = nextAmbientLightDarkeningTransition(time);
        float ambientLux = calculateAmbientLux(time);
        //prize-wuliang-201800419-start && mBrightnessState
        boolean notCanBrighten = false;
        if(!mDefaultLuxThreshold && ambientLux >= mBrighteningLuxThreshold && nextBrightenTransition <= time){
            float value = mScreenAutoBrightnessSpline.interpolate(ambientLux);
            int newScreenAutoBrightness = clampScreenBrightness(Math.round(value * PowerManager.BRIGHTNESS_ON));
            notCanBrighten = newScreenAutoBrightness < mScreenAutoBrightness;
            if (MTK_DEBUG) {
                Slog.d(TAG, "updateAutoBrightness: notCanBrighten=" + notCanBrighten + " newScreenAutoBrightness = " + newScreenAutoBrightness + " mScreenAutoBrightness = "+mScreenAutoBrightness);
            }
        }
        if (((ambientLux >= mBrighteningLuxThreshold && nextBrightenTransition <= time && !notCanBrighten)
                || ambientLux <= mDarkeningLuxThreshold && nextDarkenTransition <= time) && !mProximitySensorPositive) {
        //prize-wuliang-201800419-end
            //prize-wuliang-20180418 AutoMode Brightness
            //setAmbientLux(ambientLux);
            setAmbientLux(ambientLux, true);
            if (MTK_DEBUG) {
    ...
    private void updateAutoBrightness(boolean sendUpdate) {
        ...
        Slog.d("wuliang", "updateAutoBrightness");
     ...
     */
    public void updateAutoModeBrightness(int newScreenAutoBrightness) {
        mScreenAutoBrightness = newScreenAutoBrightness;
        mCallbacks.updateBrightness();
    }
     ...
     * prize-wuliang 20180227 for auto brightness
     */
    public void setDefaultLuxThreshold(boolean defaultThreshold) {
        mDefaultLuxThreshold = defaultThreshold;
        if(mDefaultLuxThreshold){
            mBrighteningLuxThreshold = mAmbientLux * (1.0f + BRIGHTENING_LIGHT_HYSTERESIS);
            mDarkeningLuxThreshold = mAmbientLux * (1.0f - DARKENING_LIGHT_HYSTERESIS);
        } else {
            mBrighteningLuxThreshold = mAmbientLux * (1.0f + BRIGHTENING_LIGHT_HYSTERESIS_LARGE);
            mDarkeningLuxThreshold = mAmbientLux * (1.0f - DARKENING_LIGHT_HYSTERESIS_LARGE);
        }
        Slog.d("wuliang", "setDefaultLuxThreshold mDefaultLuxThreshold = " + mDefaultLuxThreshold);
    }
     ...
     * prize-wuliang 20180227 for auto brightness
     */
    public void onApplyAmbientLux() {
        Slog.d("wuliang", "onApplyAmbientLux mAmbientLuxValid = " + mAmbientLuxValid);
        if(mAmbientLuxValid){
            long time = SystemClock.uptimeMillis();
            mAmbientLightRingBuffer.prune(time - mAmbientLightHorizon);
            setAmbientLux(calculateAmbientLux(time),true);
            updateAutoBrightness(true);
        } else {
            setDefaultLuxThreshold(true);
        }
    }
     ...
     * prize-wuliang 20180227 for auto brightness
     */
    public void updateProximitySensorPositive(boolean positive){
        mProximitySensorPositive = positive;
    }
        ...
        public void onSensorChanged(SensorEvent event) {
            if (MTK_DEBUG) {
                Slog.d(TAG, "onSensorChanged: mLightSensorEnabled=" + mLightSensorEnabled +
                        ", mAmbientLuxValid=" + mAmbientLuxValid + ", lux=" +
                        (mLightSensorEnabled ? event.values[0] : "null"));
            }
