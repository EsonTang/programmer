...
public final class PinnerService extends SystemService {
    ...
    // M: Fix file path to arm 32-bit files when 64-bit boot files not exist.
    private void tryFixPinFilesPath(String[] filesToPin) {
        for (int i = 0; i < filesToPin.length; i++){
            if (filesToPin[i].contains("/arm64/")) {
                File file = new File(filesToPin[i]);
                if (!file.exists()) {
                    Slog.i(TAG, "Pin file: " + filesToPin[i] +
                                " from config does not exist, replace with 32-bit version");
                    filesToPin[i] = filesToPin[i].replace("/arm64/", "/arm/");
                }
            }
        }
    }
    ...
    private void handlePinOnStart() {
        ...
        // M: Minimun effort to fit for 32-bit projects
        tryFixPinFilesPath(filesToPin);
