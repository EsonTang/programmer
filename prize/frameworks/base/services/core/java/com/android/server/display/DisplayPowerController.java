...
final class DisplayPowerController implements AutomaticBrightnessController.Callbacks {
    ...
    //prize-chendong-20180329-start
    private static final int BRIGHTNESS_RAMP_RATE_DIM = 45;
    private static final int BRIGHTNESS_RAMP_RATE_FAST = 90;
    private static final int BRIGHTNESS_RAMP_RATE_SLOW = 6;
    //prize-chendong-20180329-end
    ...
    //prize-chendong-20180329-start
    private int mLastPolicy = -1;
    private boolean mBrightnessLitup;
    private boolean mProximitySensorForAutoBrightnessEnabled = false;
    //prize-chendong-20180329-end
    ...
    public DisplayPowerController(Context context,
        ...
        BrightnessObserver brightnessObserver = new BrightnessObserver(mHandler);
    ...
    private class BrightnessObserver extends ContentObserver {
        ...
        private final Uri AUTO_MODE_BRIGHTNESS_URI =
                Settings.System.getUriFor(Settings.System.SCREEN_AUTO_MODE_TEMP_BRIGHTNESS);
        ...
        public void onChange(boolean selfChange, Uri uri) {
            Slog.d("wuliang", "onChange selfChange = " + selfChange + " uri = " + uri.toString());
            if (selfChange) return;
            if(mAutomaticBrightnessController != null){
                if (AUTO_MODE_BRIGHTNESS_URI.equals(uri)) {
                    ...
                    int value = Settings.System.getIntForUser(mContext.getContentResolver(),
                            Settings.System.SCREEN_AUTO_MODE_TEMP_BRIGHTNESS, mMaximumBacklight,
                            UserHandle.USER_CURRENT);
                    mAutoModeBrightnessTuningQuicklyApply = true;
                    mAutomaticBrightnessController.updateAutoModeBrightness(value);
                    mAutomaticBrightnessController.setDefaultLuxThreshold(false);
                } else if(BRIGHTNESS_MODE_URI.equals(uri) && isBrightnessAutoMode()){
					mTuningQuicklyApply = true;
                    mAutomaticBrightnessController.onApplyAmbientLux();
                }
            }
        }

        private boolean isBrightnessAutoMode(){
            int automatic = Settings.System.getIntForUser(mContext.getContentResolver(),
                        Settings.System.SCREEN_BRIGHTNESS_MODE, 
                        Settings.System.SCREEN_BRIGHTNESS_MODE_MANUAL,
                        UserHandle.USER_CURRENT);
            return automatic != Settings.System.SCREEN_BRIGHTNESS_MODE_MANUAL;
        }
    ...
    private final RampAnimator.Listener mRampAnimatorListener = new RampAnimator.Listener() {
        ...
        public void onAnimationEnd() {
            ...
            //wuliang-prize 20180228 auto brightness
            mAutoModeBrightnessTuningQuicklyApply = false;
            sendUpdatePowerState();
        }

        //wuliang-prize 20180228 auto brightness
        @Override
        public void onAnimationUpdate(int brightness) {
            if (mAutomaticBrightnessController != null
                && mPowerRequest != null
                && mPowerRequest.useAutoBrightness ) {

                setAutomodeBrightness(brightness);
            }
        }
    };
    ...
    private void updatePowerState() {
        ...
        switch (mPowerRequest.policy) {
            case DisplayPowerRequest.POLICY_OFF:
                ...
                /*PRIZE-mtk-fix-ALPS03800004-liufan-2018-03-08-start*/
                //performScreenOffTransition = true;
                performScreenOffTransition = false;
                /*PRIZE-mtk-fix-ALPS03800004-liufan-2018-03-08-end*/
                break;
        ...
        if (mProximitySensor != null) {
            if (mPowerRequest.useProximitySensor && state != Display.STATE_OFF) {
                ...
                if (mPowerRequest.useProximitySensor) {
                    ...
                    //prize-chendong-20180329-start
                    if(mAutomaticBrightnessController != null && mPowerRequest.useAutoBrightness && state != Display.STATE_OFF){
                        setProximitySensorForAutoBrightnessEnabled(true);
                    } else {
                        setProximitySensorForAutoBrightnessEnabled(false);
                    }
                    //prize-chendong-20180329-end
                }
                mWaitingForNegativeProximity = false;
                Slog.d(TAG,"mWaitingForNegativeProximity");
            }
            if (mScreenOffBecauseOfProximity
                    && mProximity != PROXIMITY_POSITIVE) {
            ...
            if (state == Display.STATE_ON || state == Display.STATE_DOZE) {
                ...
                if (MTK_AAL_SUPPORT) {
                    ...
                    if (mPowerState.getScreenBrightness() < brightness) {
                        ...
                        mBrightnessLitup = false;
                    }

                    if(mLastPolicy == DisplayPowerRequest.POLICY_DIM && mPowerRequest.policy == DisplayPowerRequest.POLICY_BRIGHT) {
                        rate = mBrightnessRampRateFast;
                        if (mPowerState.getScreenBrightness() < brightness){
                            mBrightnessLitup = true;
                        } else {
                            mBrightnessLitup = false;
                        }
                    }
                    mLastPolicy = mPowerRequest.policy;
                }
                ...
                if (mPowerRequest.policy == DisplayPowerRequest.POLICY_DIM){
                    rate = BRIGHTNESS_RAMP_RATE_DIM;
                }
                /// M: dismiss ColorFade when IPO boot up
                if(mBrightnessLitup){
                    rate = mBrightnessRampRateFast;
                }
                //prize-wuliang-20180411 Sync Now
                if(mAutoModeBrightnessTuningQuicklyApply){
                    rate = 0;
                }
                Slog.d("wuliang", "animateScreenBrightness = " + brightness + " rate = " + rate);
    ...
    private void setProximitySensorEnabled(boolean enable) {
        if (enable) {
            //priz-chengdong-20180413 diable ps
            setProximitySensorForAutoBrightnessEnabled(false);
    ...
    private void debounceProximitySensor() {
        if (mProximitySensorEnabled
            ...
            if (mPendingProximityDebounceTime <= now) {
                ...
                //prize-wuliang 20180225 bug 50370 wakeUp system for screen sleep 
                if(!isScreenStateBright() && mPendingProximity == PROXIMITY_NEGATIVE){
            ...
            switch (msg.what) {
                ...
                case MSG_SCREEN_ON_UNBLOCKED:
                    if (mPendingScreenOnUnblocker == msg.obj) {
                        unblockScreenOn();
                        updatePowerState();
                        SystemProperties.set("persist.pafd.delayon", "0"); //PRIZE-for FaceID unlock, push power key delay ScreenOn -liyongli-2018-01-25
                    }
                    break;
    ...
    //prize-chendong-20180329-start
    private void setProximitySensorForAutoBrightnessEnabled(boolean enable) {
        if (enable) {
            if(!mProximitySensorForAutoBrightnessEnabled) {
                mProximitySensorForAutoBrightnessEnabled = true;
                mSensorManager.registerListener(mProximitySensorForAutoBrightnessListener, mProximitySensor,
                        SensorManager.SENSOR_DELAY_NORMAL);
                Slog.d(TAG,"registerlistener for auto brightness PS");
            }
        } else {
            if(mProximitySensorForAutoBrightnessEnabled) {
                mProximitySensorForAutoBrightnessEnabled = false;
                mSensorManager.unregisterListener(mProximitySensorForAutoBrightnessListener);
                Slog.d(TAG,"unregisterlistener for auto brightness PS");
            }
        }
    }

    private final SensorEventListener mProximitySensorForAutoBrightnessListener = new SensorEventListener() {
        @Override
        public void onSensorChanged(SensorEvent event) {
            if(mProximitySensorForAutoBrightnessEnabled) {
                final long time = SystemClock.uptimeMillis();
                final float distance = event.values[0];
                boolean positive = distance >= 0.0f && distance < mProximityThreshold;
                Slog.d(TAG,"positive = " + positive);

                if(mAutomaticBrightnessController != null){
                    Slog.d(TAG,"updateProximitySensorPositive positive = " + positive);
                    mAutomaticBrightnessController.updateProximitySensorPositive(positive);
                }
            }
        }
        @Override
    ...
    private final class ScreenOnUnblocker implements WindowManagerPolicy.ScreenOnListener {
        ...
        public void onScreenOn() {
            ...
            /* PRIZE-for FaceID unlock, push power key delay ScreenOn -liyongli-2018-01-22 -start
            persist.sys.faceid, 1 faceid open camera, 0 clse camera
            */
            if(false&&SystemProperties.getInt("persist.sys.faceid", 0)==1){    
                Slog.d(TAG, "FaceId delay 300ms screen on ");
                SystemProperties.set("persist.pafd.delayon", "1"); //PRIZE-for FaceID unlock, push power key delay ScreenOn -liyongli-2018-01-25
                mHandler.sendMessageDelayed(msg, 300);  //liyongli  add delay 
            }else{
            mHandler.sendMessage(msg); //android org code
            }/* PRIZE-for FaceID unlock, push power key delay ScreenOn -liyongli-2018-01-22 -end*/
    ...
    //prize-wuliang 20180228 auto brightness
    private boolean mAutoModeBrightnessTuningQuicklyApply = false;
     ...
     */
    public void setAutomodeBrightness(int val) {
        if(!mAutoModeBrightnessTuningQuicklyApply && val >= 0 && val <= 255 ){
            Slog.d("wuliang", "setAutomodeBrightness = " + val);
            Settings.System.putIntForUser(mContext.getContentResolver(),
                Settings.System.SCREEN_AUTO_MODE_BRIGHTNESS, val,
                UserHandle.USER_CURRENT);
            Settings.System.putIntForUser(mContext.getContentResolver(),
