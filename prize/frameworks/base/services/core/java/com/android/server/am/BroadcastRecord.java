...
/* app multi instances feature. prize-linkh-20160413 */
...
final class BroadcastRecord extends Binder {
    ...
    /* app multi instances feature. prize-linkh-20160412 */
    int appInstanceIndex = 0; // 0 represents this app is original.
    ...
    boolean cleanupDisabledPackageReceiversLocked(
        ...
        if(ActivityManagerService.ENABLE_LOG_SERVICE) {
            ActivityManagerService.logAppInstancesInfo(ActivityManagerService.LOG_BROADCAST_TAG, 
                "cleanupDisabledPackageReceiversLocked(): name=" + packageName + ", filterByClasses=" + filterByClasses + ", userId=" + userId 
                + ", doIt=" + doit + ", appInst=" + appInst);
        }
        //end...
        ...
        for (int i = receivers.size() - 1; i >= 0; i--) {
            ...
            final int appInstFromResolveInfo = ((ResolveInfo)o).appInstanceIndex;
            if(PrizeOption.PRIZE_APP_MULTI_INSTANCES) {
                //Note: we can't remove the record with zero app inst index independently!!!
                if(appInst != PrizeAppInstanceUtils.ALL_APP_INSTANCES) {
                    if(appInst == PrizeAppInstanceUtils.ALL_APP_INSTANCES_EXCEPT_ORIGINAL
                        && appInstFromResolveInfo < 1) {
                        sameInstance = false;
                    } else if(appInst > 0 && appInst != appInstFromResolveInfo) {
                        sameInstance = false;
                    }
                }
            } //end...
            ...
            final boolean sameComponent = packageName == null
                    ...
                    && (filterByClasses == null || filterByClasses.contains(info.name)) && sameInstance/* app multi instances feature. prize-linkh-20160412 */);
            if (sameComponent) {
                if (!doit) {
                    return true;
                }
    ...
    public String toString() {
        ...
        if(ActivityManagerService.DUMP_APP_INST && appInstanceIndex != 0) {
            return "BroadcastRecord{"
                        ...
                        + " u" + userId + " " + intent.getAction() + " aii-" + appInstanceIndex + "}";
        }
        //end...
