...
public class NetworkMonitor extends StateMachine {
    ...
    /// M: Add new fallback server for China.
    private static final String FALLBACK_URL_FOR_CHINA =
    ...
    protected CaptivePortalProbeResult isCaptivePortal() {
        ...
        URL pacUrl = null, httpsUrl = null, httpUrl = null, fallbackUrl = null, fallbackUrlOther = null;
        /// M: Add for Chian Region
        URL fallbackChinaUrl = null;
        ...
        if (pacUrl == null) {
            ...
            fallbackUrlOther = makeURL(DEFAULT_OTHER_FALLBACK_URLS);

            /// M: Add for Chian region
            fallbackChinaUrl= makeURL(FALLBACK_URL_FOR_CHINA);
        ...
        if (pacUrl != null) {
            ...
            validationLog("1st probe result = " + result.mHttpResponseCode);
            /// M: Add for China region
            if (result.mHttpResponseCode == 599 && fallbackChinaUrl != null) {
                result = sendHttpProbe(fallbackChinaUrl, ValidationProbeEvent.PROBE_FALLBACK);
                validationLog("2nd probe result with China Url = " + result.mHttpResponseCode);
            }
            if (result.mHttpResponseCode == 599 && fallbackUrl != null) {
                result = sendHttpProbe(fallbackUrl, ValidationProbeEvent.PROBE_FALLBACK);
                validationLog("3rd probe result = " + result.mHttpResponseCode);
            }
            // If a fallback url is specified, use a fallback probe to try again portal detection.
            if (result.mHttpResponseCode == 599 && fallbackUrlOther != null) {
                result = sendHttpProbe(fallbackUrlOther, ValidationProbeEvent.PROBE_FALLBACK);
                validationLog("4th probe result = " + result.mHttpResponseCode);
            }
        }
    ...
    protected CaptivePortalProbeResult sendHttpProbe(URL url, int probeType) {
        ...
        try {
            ...
            log("this is not right: exception " + e);
        } finally {
