...
//                    mLastFocusedStack == null ? -1 : mLastFocusedStack.getStackId(), reason);
            int focusedStackId = mFocusedStack == null ? -1 : mFocusedStack.getStackId();
            int lastFocusedStackId = mLastFocusedStack == null ? -1 : mLastFocusedStack.getStackId();
            EventLogTags.writeAmFocusedStack(
                    mCurrentUser, focusedStackId, lastFocusedStackId, reason);
                    
            /* prize-add-split screen-liyongli-20170708-start */
            if(PrizeOption.PRIZE_SPLIT_SCREEN_FOCUS) {
                if( focusedStackId!=-1 && lastFocusedStackId!=-1
                    && 
                    (focusedStackId==DOCKED_STACK_ID || lastFocusedStackId==DOCKED_STACK_ID)
                    //&&  getStack(DOCKED_STACK_ID) != null
                ){
                	Slog.w(TAG, "--lyl-- WMS--prizeFocusStackChange  reason= "+reason );
                	if( reason !=null                	
                	   && (reason.indexOf("moveTaskToDockedStack") < 0) //not is open divider screen
                	   && (reason.indexOf("swapDockedAndFullscreenStack") < 0) //not is change screen position
                	   && (reason.indexOf("finishActivity") < 0) //not is finish,
                	 ){
                        mWindowManager.prizeFocusStackChange(focusedStackId, lastFocusedStackId);
                	}
                }
            }
        ...
        for (int displayNdx = mActivityDisplays.size() - 1; displayNdx >= 0; --displayNdx) {
            ...
            for (int stackNdx = stacks.size() - 1; stackNdx >= 0; --stackNdx) {
                ...
                if (r != null && r.state != PAUSED && r.state != STOPPED && r.state != STOPPING) {
                    /* prize-add-split screen fix bug 37484 -liyongli-20171120-start */
                    if( PrizeOption.PRIZE_SPLIT_SCREEN_MODIFY_CODE_LOGIC && r.app==null && r.state==DESTROYED ){
                        Slog.d(TAG_STATES, "prize liyongli  mdf---- r.state==DESTROYED, do as  PAUSED -----" );
                        Slog.d(TAG_STATES, "prize liyongli allPausedActivitiesComplete  : "+r );
                        Slog.d(TAG_STATES, "prize liyongli  mdf---------------------------------" );
                        return true;
                    }/* prize-add-split screen-liyongli-20171120-end */
    ...
    ActivityInfo resolveActivity(Intent intent, ResolveInfo rInfo, int startFlags,
        ...
        if (aInfo != null) {
            /* app multi instances feature. prize-linkh-20151106 */
            if(PrizeOption.PRIZE_APP_MULTI_INSTANCES 
                && mPrizeAppInstanceUtils.supportMultiInstance(aInfo.applicationInfo.packageName)) {
                aInfo.appInstanceIndex = rInfo.appInstanceIndex;
            } //end... 
    ...
    void startSpecificActivityLocked(ActivityRecord r,
        ...
        ProcessRecord app = mService.getProcessRecordLocked(r.processName,
                r.info.applicationInfo.uid, true, r.appInstanceIndex/* app multi instances feature. prize-linkh-20151031 */);

        r.task.stack.setLaunchTime(r);

        /* app multi instances feature. prize-linkh-20151027 */
        if(PrizeOption.PRIZE_APP_MULTI_INSTANCES) {            
            if(app != null && 
                    mPrizeAppInstanceUtils.supportMultiInstance(r.info.packageName) 
                        && app.appInstanceIndex != r.appInstanceIndex) {
                if(mService.ENABLE_LOG_ACTIVITY) {
                    mService.logAppInstancesInfo(mService.LOG_ACTIVITY_TAG, 
                        "Ignore app-" + app + " because of different app instance index");                  
                }              
                app = null;
            }

            //for starting new process.
            r.info.applicationInfo.appInstanceIndex = r.appInstanceIndex;          
        } //end...
        ...
        for (int displayNdx = mActivityDisplays.size() - 1; displayNdx >= 0; --displayNdx) {
            ...
            for (int stackNdx = stacks.size() - 1; stackNdx >= 0; --stackNdx) {
                ...
                if (stack.finishDisabledPackageActivitiesLocked(
                        packageName, filterByClasses, doit, evenPersistent, userId, appInst/* app multi instances feature. prize-linkh-20151225 */)) {
                    didSomething = true;
                }
            }
        }
    ...
    void moveTasksToFullscreenStackLocked(int fromStackId, boolean onTop) {
        ...
        try {
            ...
            if (onTop) {
                ...
                /* prize-add-split screen, keycode_back  error MM/momo --> MM-->momo -liyongli-20170911-start */
                if (PrizeOption.PRIZE_SPLIT_SCREEN_RETURN && fromStackId == DOCKED_STACK_ID && size>0) { //liyongli add
                    tasks.get(0).setTaskToReturnTo(HOME_ACTIVITY_TYPE);
                }
                /* prize-add-split screen, keycode_back  error MM/momo --> MM-->momo -liyongli-20170911-end */
    ...
    void handleNonResizableTaskIfNeeded(
        ...
        if (!task.canGoInDockedStack() || forceNonResizable) {
            if( PrizeOption.PRIZE_POWER_EXTEND_MODE && topActivity != null 
                  &&"com.android.superpowersave".equals(topActivity.appInfo.packageName) ){
                Slog.w(TAG, "lyl not send  DISMISSING_DOCKED_STACK "+ topActivity.appInfo.packageName);
            }else{
            ...
            /* prize-add-split screen, fix bugID 41877-liyongli-20171109-start */
            //  taskId=43
            //    appTokens=[AppWindowToken{350ee90 token=Token{b65a142 ActivityRecord{b49db8d u0 com.android.notepad/.NotePadActivity t43}}}
            //        Activity #2 AppWindowToken{fba11f8 token=Token{b97655b ActivityRecord{2002c6a u0 com.prize.applock.fingerprintapplock/.LockUI t43}}}
            //  applock is not NEW TASK
            if (PrizeOption.PRIZE_FINGERPRINT_APPLOCK && "com.prize.applock.fingerprintapplock".equals(packageName)) {
                mService.mHandler.sendEmptyMessage(NOTIFY_ACTIVITY_DISMISSING_DOCKED_STACK_MSG);
                moveTasksToFullscreenStackLocked(DOCKED_STACK_ID, actualStackId == DOCKED_STACK_ID);
                return;
            }/* prize-add-split screen-liyongli-20171109-end */
                ...
                case LAUNCH_TIMEOUT_MSG: {
                    ...
                    synchronized (mService) {
                        if (mLaunchingActivity.isHeld()) {
                            ...
                            Slog.d(TAG, "mLaunchingActivity.release() in LAUNCH_TIMEOUT_MSG");
                            mLaunchingActivity.release();
                            /// M: PerfService @{
                            if(mPerfService != null) {
                                mPerfService.amsBoostStop();
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (mLockTaskModeState == LOCK_TASK_MODE_PINNED && shouldLockKeyguard) {
                                //prize add by xiarui for bug#54749 2018-04-09 start @{
                                if (PrizeOption.PRIZE_FACE_ID) {
                                    Intent intent = new Intent("com.faceid");
                                    intent.putExtra("faceid", 4);
    ...
    final int startActivityFromRecentsInner(int taskId, Bundle bOptions) {
	 ...
	 if (false && PrizeOption.PRIZE_FINGERPRINT_APPLOCK) 
	 {
	 	short fromAppLock = 0;
		boolean canRun = true;
	 	if(bOptions != null)
	 	{
	 		fromAppLock = bOptions.getShort("fromSource",(short)0);
	 	}
		if(bOptions == null || fromAppLock != 1)
		{
			canRun = false;
		}
		TaskRecord taskrecord = anyTaskForIdLocked(taskId, false, launchStackId);
		...
		{
			aInfo = resolveActivity(taskrecord.intent, null, 0, null, 0);
			if(!canRun && !mService.isAppLockByFingerprint(aInfo.packageName))
			{
				canRun =  true;
			}
		}
		Slog.d(TAG,"fingerprintlock startActivityFromRecentsInner canRun:"+canRun+",fromAppLock:"+fromAppLock);
		if(!canRun)
		{
		    Intent targetIntent = new Intent();
		    targetIntent.setClassName("com.prize.applock.fingerprintapplock","com.prize.applock.fingerprintapplock.LockUI");
		    targetIntent.putExtra("startMode",1);
		    targetIntent.putExtra("taskId",taskId);
		    if(bOptions != null)targetIntent.putExtras(bOptions);		    
		    if(aInfo != null)
		    {
			    targetIntent.putExtra("block_pkgname",aInfo.packageName);			    
		    }
				
		    int result = mService.startActivityInPackage(0, null, targetIntent, null,
                	null, null, 0, 0, bOptions, 0, null, null);
    ...
    /* prize-add-split screen-liyongli-20170809-start */
    private void QQresizeStackLocked(int stackId, Rect bounds, Rect tempTaskBounds, Rect tempTaskInsetBounds,
            boolean preserveWindows, boolean allowResizeInDockedMode, boolean deferResume) {
    ...
    private void QQresizeStackUncheckedLocked(ActivityStack stack, Rect bounds, Rect tempTaskBounds,
            Rect tempTaskInsetBounds) {
        bounds = TaskRecord.validateBounds(bounds);
        ...
        // We might trigger a configuration change. Save the current task bounds for freezing.
        mWindowManager.prepareFreezingTaskBounds(stack.mStackId);
        stack.mFullscreen = mWindowManager.QQresizeStack(stack.mStackId, bounds, mTmpConfigs,
                mTmpBounds, mTmpInsetBounds);
        stack.setBounds(bounds);
    }
    
    
    void prizeQQresizeDockedStackLocked(Rect dockedBounds, Rect tempDockedTaskBounds,
        ...
        final ActivityStack stack = getStack(DOCKED_STACK_ID);
        if (stack == null) {
            Slog.w(TAG, "prizeQQresizeDockedStackLocked: docked stack not found");
            return;
        }
        ...
        try {
            ...
            QQresizeStackUncheckedLocked(stack, dockedBounds, tempDockedTaskBounds,
                    tempDockedTaskInsetBounds);
...
/*
            ...
            */
            //replace the up for() ....
            final ActivityStack stackFull = getStack(FULLSCREEN_WORKSPACE_STACK_ID);
            Slog.w(TAG, "prizeQQresizeDockedStackLocked: stackFull="+stackFull);
            
            //if(stackFull!=null){
                //resizeStackUncheckedLocked(stackFull, tempRect, null, null);
                QQresizeStackUncheckedLocked(stackFull, tempRect, tempOtherTaskBounds, tempOtherTaskInsetBounds);
            //}
            //2.
            final ActivityStack stackHome = getStack(HOME_STACK_ID);
            if( stackHome!=null){
                QQresizeStackUncheckedLocked(stackHome, tempRect, tempOtherTaskBounds, tempOtherTaskInsetBounds);
/*                resizeStackLocked(HOME_STACK_ID, tempRect, tempOtherTaskBounds,
                                tempOtherTaskInsetBounds, preserveWindows,
                                true, // allowResizeInDockedMode 
                                !DEFER_RESUME);*/
            }
    ...
    void prizeMoveFullScrennStackToFront(String reason) {
        final ActivityStack stack = getStack(FULLSCREEN_WORKSPACE_STACK_ID);
        if(stack!=null){
    ...
    private ActivityStack QQmoveTaskToStackUncheckedLocked(
            TaskRecord task, int stackId, boolean toTop, boolean forceFocus, String reason) {
        ...
        task.mTemporarilyUnresizable = false;
        //mWindowManager.moveTaskToStack(task.taskId, stack.mStackId, toTop);
        mWindowManager.prizemoveTaskToStack(task.taskId, stack.mStackId, toTop); //no performSurfacePlacement
        stack.addTask(task, toTop, reason);
    ...
    boolean prizeMoveTaskToStackLocked(int taskId, int stackId, boolean toTop, boolean forceFocus,
            String reason, boolean animate, boolean deferResume) {
        final TaskRecord task = anyTaskForIdLocked(taskId);
        if (task == null) {
            Slog.w(TAG, "moveTaskToStack: no task for id=" + taskId);
        ...
        boolean kept = true;
        try {
            final ActivityStack stack = QQmoveTaskToStackUncheckedLocked(
                    task, stackId, toTop, forceFocus, reason + " moveTaskToStack");
//            stackId = stack.mStackId;
 ...
 //       handleNonResizableTaskIfNeeded(task, preferredLaunchStackId, stackId);
            task.setTaskToReturnTo(HOME_ACTIVITY_TYPE); //2017-9-12 add

        return true;//(preferredLaunchStackId == stackId);
    }

    /* prize-add-split screen-liyongli-20170809-end */
