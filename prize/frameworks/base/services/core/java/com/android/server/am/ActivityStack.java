...
/* app multi instances feature. prize-linkh-201511106 */
import com.mediatek.common.prizeoption.PrizeOption;
import android.util.PrizeAppInstanceUtils;
//END...
import android.view.KeyEvent; // prize-add-split screen, return btn exit split screen-liyongli-20170724
import com.mediatek.common.prizeoption.PrizeOption; // prize-add-split screen,  fix 53696 -liyongli-20180408
...
final class ActivityStack {
    ...
    void findTaskLocked(ActivityRecord target, FindTaskResult result) {
        ...
        /* app multi instances feature. prize-linkh-20151028 */
        int appInstanceIndex = intent.getAppInstanceIndex();
        if(PrizeOption.PRIZE_APP_MULTI_INSTANCES) {
            if(appInstanceIndex < 0) {
                appInstanceIndex = (target.appInstanceIndex > 0) ?  (target.appInstanceIndex) : (info.appInstanceIndex);
                appInstanceIndex = Math.max(appInstanceIndex, 0);
            }
            
            if(ActivityManagerService.ENABLE_LOG_ACTIVITY) {
                mService.logAppInstancesInfo("findTaskLocked() target=" + target);
                mService.logAppInstancesInfo("findTaskLocked() appInstanceIndex=" + appInstanceIndex);
            }
        }
        //end...
        ...
        for (int taskNdx = mTaskHistory.size() - 1; taskNdx >= 0; --taskNdx) {
            ...
            /* app multi instances feature. prize-linkh-20151028 */
            if(PrizeOption.PRIZE_APP_MULTI_INSTANCES) {
                if(mPrizeAppInstanceUtils.supportMultiInstance(info.packageName) 
                        && (appInstanceIndex != task.appInstanceIndex)) {
                    //Perhaps they have the same affinity, so we don't break and keep going.
                    if(!isDocument && !taskIsDocument
                        && result.r == null && task.canMatchRootAffinity()) {
                        if(mPrizeAppInstanceUtils.supportMultiInstance(task.rootAffinity)) {
                            if(ActivityManagerService.ENABLE_LOG_ACTIVITY) {
                                mService.logAppInstancesInfo("Ignore the task rootAffinity. Skipping " + task + ": different app instance index.");
                            }
                        ...
                        if(ActivityManagerService.ENABLE_LOG_ACTIVITY) {
                            mService.logAppInstancesInfo("Skipping " + task + ": different app instance index.");
                        }
    ...
    ActivityRecord findActivityLocked(Intent intent, ActivityInfo info,
        ...
        if(PrizeOption.PRIZE_APP_MULTI_INSTANCES) {
            if(appInstanceIndex < 0) {
                appInstanceIndex = Math.max(info.appInstanceIndex, 0);
            }
            
            if (ActivityManagerService.ENABLE_LOG_ACTIVITY) {
                mService.logAppInstancesInfo("findActivityLocked() intent=" + intent + ", info=" + info);
                mService.logAppInstancesInfo("findActivityLocked() appInstanceIndex=" + appInstanceIndex);
            }
        }//end.....
        ...
        for (int taskNdx = mTaskHistory.size() - 1; taskNdx >= 0; --taskNdx) {
            ...
            for (int activityNdx = activities.size() - 1; activityNdx >= 0; --activityNdx) {
                ...
                if (!r.finishing && r.userId == userId) {
                    /* app multi instances feature. prize-linkh-20151028 */
                    if(PrizeOption.PRIZE_APP_MULTI_INSTANCES) {
                        if(mPrizeAppInstanceUtils.supportMultiInstance(cls.getPackageName()) 
                                && (appInstanceIndex != r.appInstanceIndex)) {
                            if (ActivityManagerService.ENABLE_LOG_ACTIVITY) {
                                mService.logAppInstancesInfo("findActivityLocked() act= " + r + ", different app instance index. skip.");
                            }
    ...
    void minimalResumeActivityLocked(ActivityRecord r) {
        ...
        if (appProc != null) {
            ...
            eventData.set(appProc.pid, r.info.name, r.info.packageName,
              r.mActivityType, appProc.uid, r.launchedFromUid);
    ...
    public final Bitmap screenshotActivitiesLocked(ActivityRecord who) {
        ...
        // PRIZE BEGIN
        // ID : 58683 and 59477
        // DESCRIPTION : reset to origin source code; mService.screenWidth = 0; mService.screenHeight = 0;
        // AUTHOR : xiarui
        /**prize-update screenshot bitmap width and height-liufan-20151027-start*/
        int w = mService.mThumbnailWidth;   //int w = (int)(mService.screenWidth * 0.65f);
        int h = mService.mThumbnailHeight;  //int h = (int)(mService.screenHeight * 0.65f);
        /**prize-update screenshot bitmap width and height-liufan-20151027-end*/
        // PRIZE END
        ...
        if (next.app != null && next.app.thread != null) {
            ...
            if (appProc != null) {
                ...
                aarEventData.set(appProc.pid, next.info.name, next.info.packageName,
                        next.mActivityType, appProc.uid, next.launchedFromUid);
        ...
        if (!isHomeStack() || numActivities() > 0) {
            ...
            if (proc == null) {
                ...
                if(PrizeOption.PRIZE_APP_MULTI_INSTANCES && r.appInstanceIndex > 0) {
                    proc = mService.mProcessNames.get(r.appInstanceIndex, r.processName, r.info.applicationInfo.uid);
                    if(ActivityManagerService.ENABLE_LOG_ACTIVITY) {
                        mService.logAppInstancesInfo("retrieve the process map again for " + r);
                        mService.logAppInstancesInfo("result: proc= " + proc);
                    }                    
                } //end...
            }
    ...
    private void adjustFocusedActivityLocked(ActivityRecord r, String reason) {
        /*prize modify for monkey test report many anr liuwei 2018-04-23 begin */
        //if (!mStackSupervisor.isFocusedStack(this) || mService.mFocusedActivity != r) {
        /*prize modify for diar err  liuwei 2018-06-12 begin */
        if (r == null || !mStackSupervisor.isFocusedStack(this) ||(mService.mFocusedActivity != r && mService.mFocusedActivity != null)) {
        /*prize modify for diar err  liuwei 2018-06-12 end */
        /*prize modify for monkey test report many anr liuwei 2018-04-23 end */
            return;
        }
        ...
        /*prize modify for back key can't work liuwei 20180413 start*/
        //mService.setFocusedActivityLocked(mStackSupervisor.topRunningActivityLocked(), myReason);
        final ActivityRecord nextTo = mStackSupervisor.topRunningActivityLocked();

        if(nextTo == null && "finishActivity".equals(reason)){
            Slog.e(TAG, "adjustFocusedActivityLocked finishActivity setFocusedActivityLocked ActivityRecord nextTo: null");
        }
        mService.setFocusedActivityLocked(nextTo, myReason);
        /*prize modify for back key can't work liuwei 20180413 end*/
    }
    ...
    boolean ensureActivityConfigurationLocked(
        ...
        /* prize-fix 53696 -split screen tencent.av.ui.AVActivity drag to big size, black screen and button no useful -liyongli-20180408-start */
        if( PrizeOption.PRIZE_SPLIT_SCREEN_DRAG && 
              ("com.tencent.mobileqq/com.tencent.av.ui.AVActivity".equals(r.shortComponentName)
                 || "com.tencent.mm/.plugin.gallery.ui.AlbumPreviewUI".equals(r.shortComponentName)
              )
          ){
            if(mStackSupervisor.getStack(DOCKED_STACK_ID) != null){
                taskChanges &= ~ CONFIG_SCREEN_LAYOUT;
            }
        }/* prize-fix 53696 -split screen -liyongli-20180408-end */
    ...
    boolean finishDisabledPackageActivitiesLocked(String packageName, Set<String> filterByClasses,
            boolean doit, boolean evenPersistent, int userId, int appInst/* app multi instances feature. prize-linkh-20151225 */) {
        boolean didSomething = false;
        TaskRecord lastTask = null;
        ComponentName homeActivity = null;
        /* app multi instances feature. prize-linkh-20151225 */
        if(PrizeOption.PRIZE_APP_MULTI_INSTANCES) {
            int index = mService.ensureAppInstIndexForStoppingPackge(appInst);
            if(index != appInst) {
                if(ActivityManagerService.ENABLE_LOG_ACTIVITY) {
                    ActivityManagerService.logAppInstancesInfo(ActivityManagerService.LOG_ACTIVITY_TAG,
                        "Fetal! finishDisabledPackageActivitiesLocked() in ActivityStack. Invalid app Inst index. fix it.(" 
                            + appInst + " -> " + index + ")");
                }
                appInst = index;
            }
        ...
        for (int taskNdx = mTaskHistory.size() - 1; taskNdx >= 0; --taskNdx) {
            ...
            for (int activityNdx = 0; activityNdx < numActivities; ++activityNdx) {
                ...
                if(PrizeOption.PRIZE_APP_MULTI_INSTANCES) {
                    //Note: we can't kill the record with zero app inst index independent!!!
                    if(appInst != PrizeAppInstanceUtils.ALL_APP_INSTANCES) {
                        if(appInst == PrizeAppInstanceUtils.ALL_APP_INSTANCES_EXCEPT_ORIGINAL
                            && r.appInstanceIndex < 1) {
                            sameInstance = false;
                        } else if(appInst > 0 && appInst != r.appInstanceIndex) {
                            sameInstance = false;
                        }
                    }
                } //end...
                ...
                if ((userId == UserHandle.USER_ALL || r.userId == userId)
                        && (sameComponent || r.task == lastTask) && sameInstance /* app multi instances feature. prize-linkh-20151225 */
    ...
    // modify from --- void adjustFocusedActivityLocked(ActivityRecord r, String reason) 
    public int prizeReturnExitSplit(IBinder token) {
        if(!PrizeOption.PRIZE_SPLIT_SCREEN_RETURN) {
            return -3;
        }
        
        ActivityRecord r = isInStackLocked(token);
        //Slog.w(TAG, "prizeReturnExitSplit 2 "+r );//+" "+r.packageName );
        if( r==null ){
            return -2;
        }
        
        if("com.android.systemui/.stackdivider.ForcedResizableInfoActivity".equals(r.shortComponentName)
            ||"com.tencent.mtt/.SplashActivity".equals(r.shortComponentName) 
        ){//the if() ,  fix bug 61999  2018/6/13
            return -2;
        }
        ...
        TaskRecord task = r.task;
        if(dockedStack != null  && mTaskHistory!=null && task!=null){
            
            Slog.w(TAG, "prizeReturnExitSplit mTaskHistory.size = "+mTaskHistory.size());
            //Slog.w(TAG, "prizeReturnExitSplit task.mActivities = "+task.mActivities);
            
            //for debug see the info
            /*
            int taskNdx = 0;
            for (taskNdx = mTaskHistory.size() - 1; taskNdx >= 0; --taskNdx) {
              TaskRecord t = mTaskHistory.get(taskNdx);
              Slog.w(TAG, "prizeReturnExitSplit  "+taskNdx +"  "+t );
            }*/
            // end for debug
            
            //1. 
            if( mTaskHistory.size() == 1 && task.mActivities!=null){
                Slog.w(TAG, "prizeReturnExitSplit task.mActivities.size() = "+task.mActivities.size() );
                if(task.mActivities.size()==1){
                    return 0;
                }
                if(task.mActivities.size()==2){//QQ-->tecent news, fast touch BACK botton
                                      //r1 is finishing,   now r is do finish(), so r,r1 all exit, need exit split-screen mode
                    ActivityRecord r1 = task.mActivities.get(1);
                    Slog.w(TAG, "prizeReturnExitSplit r1 = "+r1+", "+r1.finishing );
                    if (r1.finishing && r.hashCode()!=r1.hashCode() ) {
                        return 0;
                    }
                }
            }
            ...
            if( next!=null && next.packageName.equals(r.packageName) ){
                //Slog.d(TAG, "prizeReturnExitSplit task "+r +" to task "+next );
                if( false && r.task!=null && next.task!=null ){//2017/10/31 no need these codes
                    boolean isStartNewTask = (r.task.taskId < next.task.taskId);
                    if( r.task.taskId==next.task.taskId ){
                        //this if(), fix bug : 
                        //1. reboot,  entry split screen and select MOMO, welcome finish, exit split screen
                        //                                 momo  WelcomeActivity t52  to  MaintabActivity t52
                        return -1;//no need exit split screen; 
                    }else if( isStartNewTask ){
                        //2 com.android.notepad/.NotePadActivity
                        //Share TO BlueTooth,  BluetoothOppLauncherActivity t64  to  BluetoothOppBtEnableActivity t65
                        //Share TO QQ,  com.tencent.mobileqq/  MessageShareActivity t66  to  SplashActivity t67
                        return -1;//no need exit split screen
                    }
                    //meituan Share to PengYouQuan, exit split
                    //com.sankuai.meituan/.wxapi.WXEntryActivity t109} to DealDetailActivity t101}
                    //Slog.d(TAG, "prizeReturnExitSplit task "+r +" to task "+next );
                    Slog.d(TAG, "prizeReturnExitSplit task "+r.task.taskId +" to task "+next.task.taskId );
                    
                    //for  PRIZE_SYSTEMUI_QQPOPICON_OPEN_SPLIT_SCREEN  bug
                    return 0;//exit split screen, for bug :   this bug shuld modify in pop QQ.    2017/10/31 
                                      //MM -->Home,  open sohuvideo play avi file, then POP mm  click enter split screen,
                                      //click Return , the screen show err.  (bug modify/9-12 Return)
                }
                
                Slog.d(TAG, "prizeReturnExitSplit next="+next );
                return -1;//no need exit split screen
            }
            
            //3.
            boolean  bb = task == topTask();
            Slog.d(TAG, "prizeReturnExitSplit  r.frontOfTask="+r.frontOfTask+ ", "+bb+", "+task.isOverHomeStack());
            // setting ---> wifi ,  return,  false true false
            // setting  , return  , true true false
            ...
            if (r.frontOfTask && bb /*&& task.isOverHomeStack()*/) {
                if(  task.isOverHomeStack() ){
                    Slog.d(TAG, "prizeReturnExitSplit will goto Launch " );
                    return 0;//will goto Launch
                }
                
                //A. the bellow code causing bug :  share to MM QQ will exit split, so not use the if return 0.
                //B.  ActivityStackSupervisor.java   moveTasksToFullscreenStackLocked()  add code
                //      if (PrizeOption.PRIZE_SPLIT_SCREEN_RETURN) tasks.get(0).setTaskToReturnTo(HOME_ACTIVITY_TYPE);
                //      so, no need use this code To fix bug.
                //A and B, so not use the bellow if codes    2017/10/31
 /*               if("com.tencent.mm".equals(r.packageName) ||"com.tencent.mobileqq".equals(r.packageName) ){
                    Slog.d(TAG, "prizeReturnExitSplit will goto Launch " );
                    return 0; //because float MM QQ enter split screen,  not do TaskRecord.setTaskToReturnTo(HOME_ACTIVITY_TYPE)
                }                     //so , here not check MM QQ task.isOverHomeStack(), defaut return Home
    ...
    /* prize-add-split screen-liyongli-20170921-start */
    // modify from --- prizeReturnExitSplit   for MIGU/QQ, migu share to QQ, QQ click back to MIGU bug
    public int prizeIsNotSameStackId(IBinder token) {
        if(!PrizeOption.PRIZE_SPLIT_SCREEN_RETURN) {
            return -3;
        }

        ActivityRecord r = ActivityRecord.forTokenLocked(token);
        Slog.w(TAG, "prizeIsNotSameStackId 1 "+r );
        if( r==null ){
            return -2;
        }
        //ActivityRecord r = isInStackLocked(r0);
        TaskRecord task = r.task;
        Slog.w(TAG, "prizeIsNotSameStackId 1task= "+task );
        if(task==null){
            return -2;
        }
        Slog.w(TAG, "prizeIsNotSameStackId 1task.stack= "+task.stack );
        if(task.stack==null){
            return -2;
        }
        
        //for debug show log        
/*        Slog.w(TAG, "prizeReturnExitSplit task.mActivities.contains(r) = "+task.mActivities.contains(r) );
        Slog.w(TAG, "prizeReturnExitSplit mTaskHistory.contains(task)= "+mTaskHistory.contains(task) );
        for (int activityNdx = task.mActivities.size() - 1; activityNdx >= 0; --activityNdx) {
                    ActivityRecord rd = task.mActivities.get(activityNdx);
                    Slog.w(TAG, "prizeIsNotSameStackId activity "+activityNdx+" , "+rd );
                }
        for (int taskNdx = mTaskHistory.size() - 1; taskNdx >= 0; --taskNdx) {
            final TaskRecord tr = mTaskHistory.get(taskNdx);
            Slog.w(TAG, "prizeIsNotSameStackId task "+taskNdx+" , "+tr );
        }*/
        //end for debug show
...
//                && task.mActivities.contains(r) && mTaskHistory.contains(task)) {  
        if (task.mActivities.contains(r) && mTaskHistory.contains(task)) {
            if (task.stack != this) Slog.w(TAG,
                    "prizeIsNotSameStackId Illegal state! task does not point to stack it is in.");
        }else{
            if( mTaskHistory.size()>0){
                final TaskRecord tr0 = mTaskHistory.get(0);
                if(tr0.stack!=null && tr0.stack.mStackId!=task.stack.mStackId){
                    Slog.w(TAG, "prizeIsNotSameStackId ExitSplitScreen, task StackId != mTaskHistory StackId");
                    return 0;
                }
            }            
            return -2;
    ...
    */
    //replace up sentence    
        if( mTaskHistory.size()>0 && "com.tencent.mobileqq".equals(task.affinity) ){
            final TaskRecord tr0 = mTaskHistory.get(0);
            if(tr0.stack!=null && tr0.stack.mStackId!=task.stack.mStackId){
                Slog.w(TAG, "prizeIsNotSameStackId  task StackId != mTaskHistory StackId");
                return task.stack.mStackId;
            }
        }
        return -1;
    }
    ...
    /* prize-add-split screen-liyongli-20180523-start */
    //for moveActivityTaskToBack()
    // modify from --- final boolean moveTaskToBackLocked(int taskId) 
    // split app, itself call moveActivityTaskToBack() back to HOME. e.g. 139mail
    public int prizeTaskToBack2Home(IBinder token) {
        if(!PrizeOption.PRIZE_SPLIT_SCREEN_RETURN) {
            return -3;
        }
        ...
        TaskRecord task = r.task;
        if(mTaskHistory!=null && task!=null){
            int numTasks = mTaskHistory.size();
            final boolean canGoHome = !task.isHomeTask() && task.isOverHomeStack();
            boolean prevIsHome = false;
            if (canGoHome) {
                final TaskRecord nextTask = getNextTask(task);
                if (nextTask == null) {
            ...
            final TaskRecord resumeTask = mResumedActivity != null ? mResumedActivity.task : null;
            //Slog.i(TAG, "prizeTaskToBack2Home  prevIsHome " + prevIsHome);    
            //Slog.i(TAG, "prizeTaskToBack2Home   " + (task == resumeTask)+ "&&"+ canGoHome);                
            //Slog.i(TAG, "prizeTaskToBack2Home   " +numTasks+ "<=1&&"+ isOnHomeDisplay() );                 
            if (prevIsHome || (task == resumeTask && canGoHome) || (numTasks <= 1 && isOnHomeDisplay())) {
                return 0;
            }
        }
        return 1;
    }
    /* prize-add-split screen-liyongli-20180523-end */
