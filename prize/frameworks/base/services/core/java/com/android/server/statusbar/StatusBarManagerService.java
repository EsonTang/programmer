...
//prize add by xiarui 2018-04-29 start @{
...
public class StatusBarManagerService extends IStatusBarService.Stub {
    ...
    //prize add by xiarui 2018-04-29 start@{
    private static int mStatusBarStylePriority = 0;
    private LinkedHashMap<String, StatusBarStyle> mStatusBarStyle = new LinkedHashMap<>();

    private class StatusBarStyle {
        public String packageName;
        public int color;
        public Intent intent;
        public PendingIntent pendingIntent;
        public String text;
        public int priority;    //The higher the value, the higher the priority.

        public StatusBarStyle(String packageName,
                              int color,
                              Intent intent,
        ...
        public String toString() {
            return new StringBuilder("StatusBarStyle[")
                    .append("packageName=").append(packageName)
                    .append(",priority=").append(priority)
                    .append(",color=").append(color)
                    .append(",text=").append(text)
                    .append(",intent=").append(intent)
                    .append(",pendingIntent=").append(pendingIntent)
                    .append(']').toString();
        }
    }
    //---end by xiarui 2018-04-29 @}
    ...
    public void onStatusBarInverseChanged(int style) {
        ...
        if (mBar != null) {
            try {
                mBar.onStatusBarInverseChanged(style);
            } catch (RemoteException ex) {
            }
        }
    }
    ...
    public void updateNavBarColor(int color, boolean enableNavBarColor, boolean enableSystemUINavBarColorBackground) {
        ...
        if (mBar != null) {
            try {
                mBar.updateNavBarColor(color, enableNavBarColor, enableSystemUINavBarColorBackground);
            } catch (RemoteException ex) {
            }
        }
    }
    ...
    protected void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        synchronized (mLock) {
            ...
            //prize add by xiarui 2018-04-29 start @{
            for (Map.Entry<String, StatusBarStyle> styleEntry : mStatusBarStyle.entrySet()) {
                pw.println("  " + styleEntry.getValue().toString());
            }
            //@}
        }
    }
    ...
    public void setStatusBarBackgroundColor(String pkg, int color) {
        if (mBar != null) {
            ...
            if (mStatusBarStyle != null && !mStatusBarStyle.containsKey(pkg)) {
                int priority = mStatusBarStylePriority++;
                if ("com.android.dialer".equals(pkg)) {
                    priority = Integer.MAX_VALUE;
                }
                StatusBarStyle style = new StatusBarStyle(pkg, color, null, null, null, priority);
                mStatusBarStyle.put(pkg, style);
            }
            if (mStatusBarStyle != null && !mStatusBarStyle.isEmpty() && mStatusBarStyle.containsKey(pkg)) {
                StatusBarStyle priStyle = mStatusBarStyle.get(pkg);
                for (Map.Entry<String, StatusBarStyle> value : mStatusBarStyle.entrySet()) {
                    StatusBarStyle temp = value.getValue();
                    if (temp.priority > priStyle.priority) {
                        priStyle = temp;
                    }
                }
                pkg = priStyle.packageName;
                color = priStyle.color;
            }
            //--end by xiarui @}
            try {
                mBar.setStatusBarBackgroundColor(pkg, color);
            } catch (RemoteException ex) {
            }
        }
    }
    ...
    @Override
    public void setStatusBarBackgroundClickEvent(String pkg, Intent intent) {
        if (mBar != null) {
            //prize add by xiarui 2018-04-28 start@{
            if (mStatusBarStyle != null && !mStatusBarStyle.isEmpty() && mStatusBarStyle.containsKey(pkg)) {
                StatusBarStyle style = mStatusBarStyle.get(pkg);
                style.intent = intent;

                StatusBarStyle priStyle = style;
                for (Map.Entry<String, StatusBarStyle> value : mStatusBarStyle.entrySet()) {
                    StatusBarStyle temp = value.getValue();
                    if (temp.priority > priStyle.priority) {
                        priStyle = temp;
                ...
                intent = priStyle.intent;
            //@}
                try {
                    mBar.setStatusBarBackgroundClickEvent(intent);
                } catch (RemoteException ex) {
                }
            }
        }
    ...
    @Override
    public void setStatusBarBackgroundClickPendingEvent(String pkg, PendingIntent pendingIntent) {
        if (mBar != null) {
            if (mStatusBarStyle != null && !mStatusBarStyle.isEmpty() && mStatusBarStyle.containsKey(pkg)) {
                StatusBarStyle style = mStatusBarStyle.get(pkg);
                style.pendingIntent = pendingIntent;
                ...
                pendingIntent = priStyle.pendingIntent;

                try {
                    mBar.setStatusBarBackgroundClickPendingEvent(pendingIntent);
                } catch (RemoteException ex) {
                }
            }
        }
    ...
    public void clearStatusBarBackgroundColor() {
        if (mBar != null) {
            try {
                mBar.clearStatusBarBackgroundColor();
            } catch (RemoteException ex) {
            }
        }
    }
    ...
    @Override
    public void setStatusBarText(String pkg, String str) {
        if (mBar != null) {
            //prize add by xiarui 2018-04-28 start@{
            if (mStatusBarStyle != null && !mStatusBarStyle.isEmpty() && mStatusBarStyle.containsKey(pkg)) {
                StatusBarStyle style = mStatusBarStyle.get(pkg);
    ...
    // prize add by xiarui 2018-03-01 @{
    @Override
    public void clearStatusBarBackgroundColors(String pkg) {
        if (mStatusBarStyle != null && mStatusBarStyle.containsKey(pkg)) {
            mStatusBarStyle.remove(pkg);
            if (mBar != null) {
                try {
                    mBar.clearStatusBarBackgroundColor();
                } catch (RemoteException ex) {
            ...
            if (!mStatusBarStyle.isEmpty()) {
                StatusBarStyle priStyle = null;
                for (Map.Entry<String, StatusBarStyle> style : mStatusBarStyle.entrySet()) {
                    StatusBarStyle lastStyle = style.getValue();
                    Log.d(TAG, lastStyle.toString());
                    priStyle = lastStyle;
                }
                for (Map.Entry<String, StatusBarStyle> value : mStatusBarStyle.entrySet()) {
                    StatusBarStyle temp = value.getValue();
                ...
                if (priStyle != null) {
                    if (mBar != null) {
                        try {
                            mBar.setStatusBarBackgroundColor(priStyle.packageName, priStyle.color);
                            mBar.setStatusBarBackgroundClickEvent(priStyle.intent);
                            if (priStyle.pendingIntent != null) {
                                mBar.setStatusBarBackgroundClickPendingEvent(priStyle.pendingIntent);
                            }
                            mBar.setStatusBarText(priStyle.text);
                        } catch (RemoteException ex) {
