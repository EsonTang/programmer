...
public class NetworkStatsService extends INetworkStatsService.Stub {
   ...
   public static boolean USE_TRUESTED_TIME = false;
    ...
    public static final String ACTION_IGNORE_DATA_USAGE_ALERT =
            "android.intent.action.IGNORE_DATA_USAGE_ALERT";
    public static final String EXTRA_IGNORE_ENABLED =
            "ignore";
    ...
    // M: Add support for Multiple ViLTE
    /** Set of currently subId SubscriberID pairs */
    private final ArrayMap<String, Integer> mActiveSubscriberSubIdMap = new ArrayMap<>();
    private boolean mMultipleViLteSupport;
    ...
    NetworkStatsService(Context context, INetworkManagementService networkManager,
        ...
        mIgnoreAlert = false;
        // M: Add support for Multiple ViLTE
        int multipleImsSupport = SystemProperties.getInt("ro.mtk_multiple_ims_support", 1);
        Slog.i(TAG, "multipleImsSupport:" + multipleImsSupport);
        mMultipleViLteSupport = ((multipleImsSupport > 1) ? true : false);
        Slog.i(TAG, "mMultipleViLteSupport: " + mMultipleViLteSupport);
    }
    ...
    public void systemReady() {
        ...
        final IntentFilter ignoreAlertFilter =
            new IntentFilter(ACTION_IGNORE_DATA_USAGE_ALERT);
    ...
    private BroadcastReceiver mIgnoreAlertReceiver = new BroadcastReceiver() {
        ...
        Log.i(TAG, "mIgnoreAlertReceiver onReceive action:" + action);
        if (action.equals(ACTION_IGNORE_DATA_USAGE_ALERT))
            Log.e(TAG, "[powerdebug]ignoreAlertFilter received");
            boolean enabled = intent.getBooleanExtra(EXTRA_IGNORE_ENABLED, true);
            if (enabled) {
                mIgnoreAlert = true;
            } else {
                mIgnoreAlert = false;
    ...
    // M: Add support for Multiple ViLTE
    private String getVtInterface(int subId) {
        return new String(VT_INTERFACE + String.valueOf(subId));
    }

    private void rebuildActiveSubscriberSubIdMap(ArrayMap<String, Integer> activeSubscriberSubIdMap) {
        activeSubscriberSubIdMap.clear();
        ...
        for (int subId : subIds) {
            ...
            activeSubscriberSubIdMap.put(subscriberId, new Integer(subId));
            Slog.v(TAG, "activeSubscriberSubIdMap put:" +
                subscriberId + ":" + subId);
        }
    }
    ...
    private void updateIfacesLocked() {
        ...
        // M: Add support for Multiple ViLTE
        if (mMultipleViLteSupport) {
            rebuildActiveSubscriberSubIdMap(mActiveSubscriberSubIdMap);
        }
            ...
            if (state.networkInfo.isConnected()) {
                ...
                if (baseIface != null) {
                    ...
                    if (state.networkCapabilities.hasCapability(
                        ...
                        // M: Add support for Multiple ViLTE, Separate VT_INTERFACE according to subId
                        int subId = -1;
                        if (mMultipleViLteSupport) {
                            Integer entry = mActiveSubscriberSubIdMap.get(ident.getSubscriberId());
                            if (entry != null) {
                                subId = entry.intValue();
                                Slog.v(TAG, "Get subId=" + subId + " for sbscriberId:"
                                    + ident.getSubscriberId());
                            } else {
                                Slog.v(TAG, "Get null entry for sbscriberId:" + ident.getSubscriberId());
                            }

                            findOrCreateNetworkIdentitySet(mActiveIfaces,
                                getVtInterface(subId)).add(vtIdent);
    ...
    private NetworkStats getNetworkStatsUidDetail() throws RemoteException {
        ...
        for (int subId : subIds) {
            ...
            try {
                if (telephonyEx != null) {
                    usage = telephonyEx.getVtDataUsage(subId);
                } else {
                    Slog.e(TAG, "telephonyEx == null");
                }
            } catch (RemoteException e) {
                e.printStackTrace();
            }
            ...
            if (LOGV) {
                Slog.d(TAG, "VT call data usage subId" + subId +" = " + usage);
            }
            final NetworkStats.Entry entry = new NetworkStats.Entry();
            entry.iface = getVtInterface(subId);
            entry.uid = UID_VILTE;
