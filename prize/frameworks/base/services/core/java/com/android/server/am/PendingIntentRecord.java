...
/* app multi instances feature. prize-linkh-20151213 */
...
final class PendingIntentRecord extends IIntentSender.Stub {
    ...
    /* app multi instances feature. prize-linkh-20151203 */
    // Note: If the type of PendingIntentRecord is Broadcast type, 
    // We can't pass the callerApp to mService.broadcastIntentInPackage().
    //WeakReference<ProcessRecord> callerAppRef;
    ...
    final static class Key {
        ...
        /* app multi instances feature. prize-linkh-20151203 */
        final int appInstanceIndex;
        
        private static final int ODD_PRIME_NUMBER = 37;

        Key(int _t, String _p, ActivityRecord _a, String _w,
                int _r, Intent[] _i, String[] _it, int _f, Bundle _o, int _userId, int appInst/* app multi instances feature. prize-linkh-20151203 */) {
            type = _t;
            packageName = _p;
            activity = _a;
            who = _w;
            ...
            /* app multi instances feature. prize-linkh-20151203 */
            appInstanceIndex = appInst;
            hash = (ODD_PRIME_NUMBER*hash) + appInst;
            //end...
        ...
        public boolean equals(Object otherObj) {
            ...
            try {
                ...
                /* app multi instances feature. prize-linkh-20151203 */
                if(appInstanceIndex != other.appInstanceIndex) {
                    return false;
                } //end...
        ...
        public String toString() {
            /* app multi instances feature. prize-linkh-20151203 */
            if(ActivityManagerService.DUMP_APP_INST && appInstanceIndex != 0) {
                return "Key{" + typeName() + " pkg=" + packageName
                    + " intent="
                    + (requestIntent != null
                            ? requestIntent.toShortString(false, true, false, false) : "<null>")
                    + " flags=0x" + Integer.toHexString(flags) + " u=" + userId + " aii="+ appInstanceIndex + "}";
            } //end...
    ...
    void setCallerAppInfoForAppInst(ProcessRecord app) {
        if(ActivityManagerService.ENABLE_LOG_PENDING_INTENT) {
            owner.logAppInstancesInfo(ActivityManagerService.LOG_PENDING_INTENT_TAG, 
                "setCallerAppInfoForAppInst() app=" + app);
        }
        ...
        callerProcessName = app.processName;
        callerUid = app.info != null ? app.info.uid : 0;
        callerAppInst = app.appInstanceIndex;
    } //END..
    ...
    {
    		int fromSource = 0;
		if(code == -1)fromSource = Intent.FROM_SYSTEMUI_NOTIFICATION;
    		return sendInner(code,intent,resolvedType,finishedReceiver,requiredPermission,
				resultTo,resultWho,requestCode,flagsMask,flagsValues,options,container,fromSource);
    }
    /*prize-add by lihuangyuan,for fingerapplock-2017-07-04-end*/
    int sendInner(int code, Intent intent, String resolvedType, IIntentReceiver finishedReceiver,
            String requiredPermission, IBinder resultTo, String resultWho, int requestCode,
            int flagsMask, int flagsValues, Bundle options, IActivityContainer container,int fromSource) {
        ...
        synchronized (owner) {
            ...
            if (!canceled) {
                ...
                if (PrizeOption.PRIZE_APP_MULTI_INSTANCES) {
                    if (callerProcessName != null && callerUid > 0 && callerAppInst >= 0) {
                        callerApp = owner.getProcessRecordLocked(callerProcessName, 
                                        callerUid, true, callerAppInst);
                        if (callerApp != null) {
                            appInst = callerApp.appInstanceIndex;
                        }
                    }
                    
                    if (callerApp == null) {
                        appInst = key.appInstanceIndex;                      
                    }
                    
                    if(ActivityManagerService.ENABLE_LOG_PENDING_INTENT) {
                        ActivityManagerService.logAppInstancesInfo(ActivityManagerService.LOG_PENDING_INTENT_TAG, 
                            "sendInner: caller=" + callerApp + ", intent=" + intent + ", type=" + key.typeName() 
                            + ", appInst=" + appInst + ", this=" + this);
                    }                    
                } else {
                    appInst = 0;
                }
			   ...
			   /*prize-add by lihuangyuan,for fingerapplock-2017-07-04-start*/			   
			   finalIntent.setIntentFromSource(fromSource);//add by lihuangyuan
			   /*prize-add by lihuangyuan,for fingerapplock-2017-07-04-end*/
                        try {
                            if (key.allIntents != null && key.allIntents.length > 1) {
                                ...
                                owner.startActivitiesInPackage(callerApp, /* app multi instances feature. prize-linkh-20151203 */uid, key.packageName, allIntents,
                                        allResolvedTypes, resultTo, options, userId);
                            } else {
                                owner.startActivityInPackage(callerApp,/* app multi instances feature. prize-linkh-20151203 */uid, key.packageName, finalIntent,
                    ...
                    case ActivityManager.INTENT_SENDER_BROADCAST:
                        try {
                            ...
                            int sent = owner.broadcastIntentInPackage(key.packageName, uid,
                                    ...
                                    false, userId, appInst/* app multi instances feature. prize-linkh-20151203 */);
                    ...
                    case ActivityManager.INTENT_SENDER_SERVICE:
                        try {
                            owner.startServiceInPackage(callerApp,/* app multi instances feature. prize-linkh-20151203 */ uid, finalIntent,
    ...
    void dump(PrintWriter pw, String prefix) {
        ...
        if(ActivityManagerService.DUMP_APP_INST) {
            if (key.appInstanceIndex != 0) {            
                pw.print(prefix); pw.print("aii="); pw.println(key.appInstanceIndex);
            }
            if (callerProcessName != null || callerUid > 0
                || callerAppInst >= 0) {
                pw.print(prefix); pw.print("callerProcessName="); pw.println(callerProcessName);
                pw.print(prefix); pw.print("callerUid="); pw.println(callerUid);
                pw.print(prefix); pw.print("callerAppInst="); pw.println(callerAppInst);
            }
        } //end...
    }
    ...
    public String toString() {
        ...
        if (ActivityManagerService.DUMP_APP_INST) {
            if (key.appInstanceIndex != 0) {
                sb.append(" aii-");
                sb.append(key.appInstanceIndex);
            }

            if (callerProcessName != null || callerUid > 0
                || callerAppInst >= 0) {
                sb.append(" (").append(callerProcessName)
                  .append("|").append(callerUid)
                  .append("|").append(callerAppInst);                  
            }
        }//end...
        sb.append('}');
