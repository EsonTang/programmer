...
import android.animation.ObjectAnimator;/* PRIZE- add launch anim - zhouerlong - 20160310 */
...
/* PRIZE- add launch anim - zhouerlong - 20160310 begin */
import android.view.animation.DecelerateInterpolator;
/* PRIZE- add launch anim - zhouerlong - 20160310 end */
...
public class AppTransition implements Dump {
    ...
    private static final int NEXT_TRANSIT_TYPE_SCALE_DOWN = 9;/* PRIZE- add launch anim - zhouerlong - 20160310 */
...
/* PRIZE- add launch anim - zhouerlong - 20160310 begin */
    private final float TRANSITION_BG_MAX=1f;
    private final float SCANLE_BG_MAX=6.5f;
    private int mDuration= DEFAULT_APP_TRANSITION_DURATION;
    private int mLastBackDuration =DEFAULT_APP_TRANSITION_DURATION;
...
/* PRIZE- add launch anim - zhouerlong - 20160310 end */
	...
	private boolean oppo=true;/* PRIZE- add launch anim - zhouerlong - 20160310 */
    ...
    void goodToGo(AppWindowAnimator topOpeningAppAnimator, AppWindowAnimator topClosingAppAnimator,
        ...
        mService.getDefaultDisplayContentLocked().getDockedDividerController()
                    .notifyAppTransitionStarting(openingApps, appTransition);//exit split screen,  do exit anim
                   // .notifyAppVisibilityChanged();  exit split screen, not do exit anim. use this will cause problem HOME not exit split screen.
    ...
    void clear() {
        ...
        mDuration = DEFAULT_APP_TRANSITION_DURATION;/* PRIZE- add launch anim - zhouerlong - 20160310 */
    ...
    private Animation createScaleUpAnimationLocked(int transit, boolean enter,
        ...
        if (enter) {
            ...
            // element fade out.
			/* PRIZE- mdy launch anim - zhouerlong - 20160310  begin*/
//            a = new AlphaAnimation(1, 0);
//            a.setDetachWallpaper(true);
            
            ////////////////////////

    		int toX = (int) ((0 - mNextAppTransitionStartX)*TRANSITION_BG_MAX);
    		int toY = (int) ((0 - mNextAppTransitionStartY)*TRANSITION_BG_MAX);
    		int fromX=0;
    		int fromY=0;
            // Entering app zooms out from the center of the initial rect.
            float scaleW = mNextAppTransitionStartWidth / (float) appWidth;
            float scaleH = mNextAppTransitionStartHeight / (float) appHeight;
            Animation scale = new ScaleAnimation(1,  SCANLE_BG_MAX, 1,SCANLE_BG_MAX,
                    computePivot(mNextAppTransitionStartX, scaleW),
                    computePivot(mNextAppTransitionStartY, scaleH));
            scale.setInterpolator(mDecelerateInterpolator);
            
            Animation traAnimation = new TranslateAnimation(fromX, toX, fromY, toY);
            traAnimation.setInterpolator(mDecelerateInterpolator);
...
//            set.addAnimation(traAnimation);
    		...
    		int toX = (int) ((appWidth/2-mNextAppTransitionStartX/2)*TRANSITION_BG_MAX);
    		int toY = (int) ((appHeight/2-mNextAppTransitionStartY/2)*TRANSITION_BG_MAX*appHeight/appWidth);
    		int fromX=0;
    		int fromY=0;
            // Entering app zooms out from the center of the initial rect.
            float scaleW = mNextAppTransitionStartWidth / (float) appWidth;
...
//            set.addAnimation(traAnimation);
            a = set;
            a.setZAdjustment(Animation.ZORDER_BOTTOM);
            ////////////////////////
//            a = new AlphaAnimation(1, 1);
        ...
        switch (transit) {
            ...
            default:
            	...
            	if (mDuration != DEFAULT_APP_TRANSITION_DURATION) {
            		duration=mDuration;
            	}
                break;
        }
        ...
        return a;
			...
			/* PRIZE- mdy launch anim - zhouerlong - 20160310  end*/
    }
...
/* PRIZE- add launch anim - zhouerlong - 20160310  begin*/
    ...
    private Animation createScaleDownAnimationLocked(int transit, boolean enter,
        ...
        if (appWidth==0|| appHeight == 0) {
        	...
        	appHeight = mContext.getResources().getDisplayMetrics().heightPixels;
        }
        if (!enter) {
            // Entering app zooms out from the center of the initial rect.
            float scaleW = mNextAppTransitionStartWidth / (float) appWidth;
...
//            a.setDetachWallpaper(true);
			...
			////////////////////////
	    		...
	    		int fromX = (int) ((0 - mNextAppTransitionStartX)*TRANSITION_BG_MAX);
	    		int fromY = (int) ((0 - mNextAppTransitionStartY)*TRANSITION_BG_MAX);
				int toX=0;
				int toY=0;
			  // Entering app zooms out from the center of the initial rect.
			  float scaleW = mNextAppTransitionStartWidth / (float) appWidth;
			  float scaleH = mNextAppTransitionStartHeight / (float) appHeight;
	            Animation scale = new ScaleAnimation(  SCANLE_BG_MAX,1, SCANLE_BG_MAX,1,
			          computePivot(mNextAppTransitionStartX, scaleW),
			          computePivot(mNextAppTransitionStartY, scaleH));
			  scale.setInterpolator(mDecelerateInterpolator);
...
//            a = new AlphaAnimation(0, 1);
    		...
    		int fromX = (int) ((appWidth/2-mNextAppTransitionStartX/2)*TRANSITION_BG_MAX);
    		int fromY = (int) ((appHeight/2-mNextAppTransitionStartY/2)*TRANSITION_BG_MAX*appHeight/appWidth);
				int toX=0;
				int toY=0;
			  // Entering app zooms out from the center of the initial rect.
			  float scaleW = mNextAppTransitionStartWidth / (float) appWidth;
...
//			  set.addAnimation(traAnimation);
    ...
    Animation loadAnimation(WindowManager.LayoutParams lp, int transit, boolean enter, int uiMode,
        ...
        if (isVoiceInteraction && (transit == TRANSIT_ACTIVITY_OPEN
            ...
            if (DEBUG_APP_TRANSITIONS || DEBUG_ANIM) Slog.v(TAG,
                    ...
                    + " Callers=" + Debug.getCallers(3));
			/* PRIZE- add launch anim - zhouerlong - 20160310  begin*/
        } 	else if (mNextAppTransitionType == NEXT_TRANSIT_TYPE_SCALE_DOWN) {
        	try {
                a = createScaleDownAnimationLocked(transit, enter, frame);
			} catch (Exception e) {
				a=null;
				e.printStackTrace();
			}
    ...
    void overridePendingAppTransition(String packageName, int enterAnim, int exitAnim,
        ...
        if (isTransitionSet()) {
			...
			/* PRIZE- add launch anim - zhouerlong - 20160310  begin*/
    void overridePendingAppTransitionScaleUpbyDuration(int startX, int startY, int startWidth,
        ...
        if (isTransitionSet()) {
            ...
            mDuration = duration!=0?duration:DEFAULT_APP_TRANSITION_DURATION;
            mLastBackDuration = mDuration;
    ...
    void overridePendingAppTransitionScaleUp(int startX, int startY, int startWidth,
        ...
        if (isTransitionSet()) {
			...
			/* PRIZE- add launch anim - zhouerlong - 20160310  begin*/
    void overridePendingAppTransitionScaleDownWithLauncher() {
    	if(mLastBackDuration != DEFAULT_APP_TRANSITION_DURATION) {
    		mDuration = mLastBackDuration;
    		mLastBackDuration=DEFAULT_APP_TRANSITION_DURATION;
    	}
    	if(mDuration !=DEFAULT_APP_TRANSITION_DURATION) {
