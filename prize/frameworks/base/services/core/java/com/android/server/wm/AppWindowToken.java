...
/// @}
// Nav bar color customized feature. prize-linkh-20171024 @{
...
class AppWindowToken extends WindowToken {
    ...
    AppWindowToken(WindowManagerService _service, IApplicationToken _token,
        ...
        /// M: add for fullscreen switch feature @{
        if (service.mSupportFullscreenSwitch) {
            isFullscreenOn = initFullscreenSwitchState();
        }
        /// @}
    }
    ...
    void sendAppVisibilityToClients() {
        ...
        for (int i=0; i<N; i++) {
            ...
            if (PrizeOption.PRIZE_NAVBAR_COLOR_CUST) {
                // If this win becomes visibile, then we send this changed event
                // to client before client fires relayout window event. Client can
                // change nav bar color in time.
                if (!clientHidden && win.mPendingNavBarColorChanged) {
                    try {
                        if (WindowManagerService.DBG_NAV_BAR_COLOR) {
                            Slog.d(TAG, "sendAppVisibilityToClients() dispatch nav bar color 0x" + Integer.toHexString(win.mPendingNavBarColor)
                                    + " to " + win);
                        }
                        win.mClient.dispatchNavBarColorChanged(win.mPendingNavBarColor);
                    } catch (RemoteException e) {
                        if (WindowManagerService.DBG_NAV_BAR_COLOR) {
                            Slog.d(TAG, "sendAppVisibilityToClients() Can't dispatch: " + e);
                        }
                    } finally {
    ...
    void removeAllWindows() {
        for (int winNdx = allAppWindows.size() - 1; winNdx >= 0;
            ...
            /// M:[ALPS03256869]It is a timing issue for window removed before calling
            /// method clearTimedoutReplacesLocked, so force mWillReplaceWindow to false.
            /// if clearTimedoutReplacesLocked called after allAppWindows removed,
            /// so mWillReplaceWindow will always true, and DimLayer will not disappear@{
            if (service.mH.hasMessages(service.mH.WINDOW_REPLACEMENT_TIMEOUT)) {
    ...
    private boolean initFullscreenSwitchState() {
        ...
        if (ar != null && ar.packageName != null) {
            on = service.getFullscreenMode(ar.packageName);
