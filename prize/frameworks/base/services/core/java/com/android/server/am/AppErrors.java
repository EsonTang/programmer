...
/* app multi instances feature. prize-linkh-20161028 */
...
class AppErrors {
    ...
    boolean dumpLocked(FileDescriptor fd, PrintWriter pw, boolean needSep,
        ...
        /* app multi instances feature. prize-linkh-20151218 */
        if(mService.DUMP_APP_INST && mProcessCrashTimes.getAppInstancesCount() > 0) {
            SparseArray<ArrayMap<String, SparseArray<Long>>> mapForAppInst = mProcessCrashTimes.getMapForAppInstances();
            for(int idx = 0; idx < mapForAppInst.size(); ++idx) {
                boolean printed = false;
                long now = SystemClock.uptimeMillis();
                final ArrayMap<String, SparseArray<Long>> pmap = mapForAppInst.valueAt(idx);
                ...
                for (int ip=0; ip<NP; ip++) {
                    ...
                    for (int i=0; i<N; i++) {
                        ...
                        ProcessRecord r = mService.mProcessNames.get(mapForAppInst.keyAt(idx), pname, puid);
                        if (dumpPackage != null && (r == null
                                || !r.pkgList.containsKey(dumpPackage))) {
                            continue;
                        }
                        if (!printed) {
                            if (needSep) pw.println();
                            needSep = true;
                            pw.println("  Time since processes crashed for AppInst " + mapForAppInst.keyAt(idx) + " :");
                            printed = true;
                        }
                        pw.print("    Process "); pw.print(pname);
                                pw.print(" uid "); pw.print(puid);
        ...
        /* app multi instances feature. prize-linkh-20151218 */
        if(mService.DUMP_APP_INST && mBadProcesses.getAppInstancesCount() > 0) {
            SparseArray<ArrayMap<String, SparseArray<BadProcessInfo>>> mapForAppInst = mBadProcesses.getMapForAppInstances();
            for(int idx = 0; idx < mapForAppInst.size(); ++idx) {
                boolean printed = false;
                final ArrayMap<String, SparseArray<BadProcessInfo>> pmap = mapForAppInst.valueAt(idx);
                final int NP = pmap.size();
                for (int ip=0; ip<NP; ip++) {
                    String pname = pmap.keyAt(ip);
                    ...
                    for (int i=0; i<N; i++) {
                        ...
                        if (!printed) {
                            if (needSep) pw.println();
                            needSep = true;
                            pw.println("  Bad processes for AppInst " + mapForAppInst.keyAt(idx) + " :");
                        }
    ...
    boolean isBadProcessLocked(ApplicationInfo info) {
        return isBadProcessLocked(info, 0);
    } //End..
    boolean isBadProcessLocked(ApplicationInfo info, int appInst/* app multi instances feature. prize-linkh-20161028 */) {
        return mBadProcesses.get(appInst/* app multi instances feature. prize-linkh-20161028 */, info.processName, info.uid) != null;        
    }
    
    /* app multi instances feature. prize-linkh-20161028 */
    void clearBadProcessLocked(ApplicationInfo info) {
        isBadProcessLocked(info, 0);
    } //End..
    void clearBadProcessLocked(ApplicationInfo info, int appInst/* app multi instances feature. prize-linkh-20161028 */) {
        mBadProcesses.remove(appInst/* app multi instances feature. prize-linkh-20161028 */, info.processName, info.uid);
    }
    
    /* app multi instances feature. prize-linkh-20161028 */
    void resetProcessCrashTimeLocked(ApplicationInfo info) {
        resetProcessCrashTimeLocked(info, 0);
    } //End..
    void resetProcessCrashTimeLocked(ApplicationInfo info, int appInst/* app multi instances feature. prize-linkh-20161028 */) {
        mProcessCrashTimes.remove(appInst/* app multi instances feature. prize-linkh-20161028 */, info.processName, info.uid);
    }
    
    /* app multi instances feature. prize-linkh-20161028 */
    void resetProcessCrashTimeLocked(boolean resetEntireUser, int appId, int userId) {
        resetProcessCrashTimeLocked(resetEntireUser, appId, userId, PrizeAppInstanceUtils.ALL_APP_INSTANCES);
    } //End..
    void resetProcessCrashTimeLocked(boolean resetEntireUser, int appId, int userId, int appInst/* app multi instances feature. prize-linkh-20161028 */) {
        final ArrayMap<String, SparseArray<Long>> pmap = mProcessCrashTimes.getMap();
        ...
        /* app multi instances feature. prize-linkh-20161028 */
        // remove for other app instances!
        if(PrizeOption.PRIZE_APP_MULTI_INSTANCES) {
            final int count = mProcessCrashTimes.getAppInstancesCount();
            if(count > 0) {
                for(int index = 1; index <= count; ++index) {
                    ...
                    final ArrayMap<String, SparseArray<Long>> pmapForAppInstance = mProcessCrashTimes.getMap(index);
                    
                    for (int ip=pmapForAppInstance.size()-1; ip>=1; ip--) {
                        SparseArray<Long> ba = pmapForAppInstance.valueAt(ip);
                        for (int i=ba.size()-1; i>=0; i--) {
                            boolean remove = false;
                            final int entUid = ba.keyAt(i);
                            if (!resetEntireUser) {
                        ...
                        if (ba.size() == 0) {
                            pmapForAppInstance.removeAt(ip);
                        }
                    }
                }
            }
    ...
    void crashApplicationInner(ProcessRecord r, ApplicationErrorReport.CrashInfo crashInfo) {
        ...
        synchronized (mService) {
            ...
            if (r != null && !r.isolated && res != AppErrorDialog.RESTART) {
                ...
                mProcessCrashTimes.put(r.appInstanceIndex /*app multi instances feature. prize-linkh-20161029*/, r.info.processName, r.uid,
    ...
    boolean handleAppCrashLocked(ProcessRecord app, String reason,
        ...
        if (!app.isolated) {
            crashTime = mProcessCrashTimes.get(app.appInstanceIndex,/* app multi instances feature. prize-linkh-20161029 */ app.info.processName, app.uid);
            crashTimePersistent = mProcessCrashTimesPersistent.get(app.appInstanceIndex,/* app multi instances feature. prize-linkh-20161029 */ app.info.processName, app.uid);
        } else {
        ...
        if (crashTime != null && now < crashTime+ProcessList.MIN_CRASH_INTERVAL) {
            ...
            if (!app.persistent) {
                ...
                if (!app.isolated) {
                    ...
                    mBadProcesses.put(app.appInstanceIndex,/* app multi instances feature. prize-linkh-20161029 */ app.info.processName, app.uid,
                            new BadProcessInfo(now, shortMsg, longMsg, stackTrace));
                    mProcessCrashTimes.remove(app.appInstanceIndex,/* app multi instances feature. prize-linkh-20161029 */ app.info.processName, app.uid);
                }
                app.bad = true;
                app.removed = true;
        ...
        if (!app.isolated) {
            ...
            mProcessCrashTimes.put(app.appInstanceIndex,/* app multi instances feature. prize-linkh-20161029 */ app.info.processName, app.uid, now);
            mProcessCrashTimesPersistent.put(app.appInstanceIndex,/* app multi instances feature. prize-linkh-20161029 */ app.info.processName, app.uid, now);
        }
