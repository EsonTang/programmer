...
import com.mediatek.common.prizeoption.PrizeOption; // prize-add-split screen-liyongli-20170909
import android.os.SystemProperties; // prize-add-split screen, add flag for camera check split mode-liyongli-20180621
                                                                            //SystemProperties.getInt("persist.split.stack", 0)==1
...
public class TaskStack implements DimLayer.DimLayerUser,
    ...
    TaskStack(WindowManagerService service, int stackId) {
        ...
        /* prize-add-split screen, add flag for camera check split mode-liyongli-20180621-start */
        if (mStackId == DOCKED_STACK_ID) {
            SystemProperties.set("persist.split.stack", "1");
        }else{
            SystemProperties.set("persist.split.stack", "0");
        }/* prize-add-split screen, add flag for camera check split mode-liyongli-20180621-end */
    }
    ...
    private boolean adjustForMinimizedDockedStack(float minimizeAmount) {
        ...
        if (dockSide == DOCKED_TOP) {
            ...
            /* prize-add-split screen, for HOME exit split screen animation -liyongli-20170909-start */
            if(PrizeOption.PRIZE_SPLIT_SCREEN_HOME && minimizeAmount>=0.9f){
                mTmpRect.top = -40;
                mBounds.set(0,-80,0,-20);
                mTmpAdjustedBounds.set(mBounds);
                return true;
            }
            /* prize-add-split screen, for HOME exit split screen animation -liyongli-20170909-end */
    ...
    public String prizeGetTask0Name() {
        if(mTasks.size()<1){
            return null;
        }
        
        final Task task = mTasks.get(mTasks.size() - 1);
        String out = task.prizeGetTaskName();
        
        return out;
    }
    
    private void MMalignTasksBounds(Rect bounds) {        
        // Update bounds of containing tasks.
        for (int taskNdx = mTasks.size() - 1; taskNdx >= 0; --taskNdx) {
            final Task task = mTasks.get(taskNdx);
            task.MM2setBounds(bounds);
        }
    }
    
    int MMQQsetAdjustedBounds(Rect bounds) {
        mBounds.set(bounds);
        MMalignTasksBounds(mBounds);
        return 1;
    }
    /* prize-add-split screen, for com.tencent.mm  adjust height when pop ime -liyongli-20170812-start */
