...
import android.view.Surface; //liyongli add for NOTCH_SCREEN 2018/5/4 10:07
import com.mediatek.common.prizeoption.PrizeOption; //liyongli add for split screen 2018/3/20 16:24
...
final class WindowState implements WindowManagerPolicy.WindowState {
    ...
    boolean mPendingNavBarColorChanged;
    int mPendingNavBarColor;
    // @}
    ...
    WindowState(WindowManagerService service, Session s, IWindow c, WindowToken token,
        ...
        /// M: add for fullscreen switch feature @{
        if (mService.mSupportFullscreenSwitch) {
            mIsFullscreenOn = initFullscreenSwitchState();
        }
        /// @}
    }
    ...
    public void computeFrameLw(Rect pf, Rect df, Rect of, Rect cf, Rect vf, Rect dcf, Rect sf,
        ...
        if (mAttrs.type == TYPE_DOCK_DIVIDER) {
            ...
            /* prize-add-landscape cut the lcd ears, modify for split screen, calc mid position-liyongli-20180504-start */
            if( PrizeOption.PRIZE_NOTCH_SCREEN){
                int cutEarsWidth = mStableFrame.top; //is status bar hight
                if(mService.mRotation==Surface.ROTATION_270){
                    mStableInsets.right += cutEarsWidth;//ears width (put right)
                }else if( mService.mRotation==Surface.ROTATION_90 ){
                    mStableInsets.left += cutEarsWidth; //ears width (put left)
                }                
            }/* prize-add-landscape cut the lcd ears, modify for split screen, calc mid position-liyongli-20180504-end */
    ...
    /// M: add for fullscreen switch feature @{
    private boolean mIsFullscreenOn = true;
    public boolean isFullscreenOn() {
        return mIsFullscreenOn;
    }
    private boolean initFullscreenSwitchState() {
        if (mAppToken != null) {
            return mAppToken.isFullscreenOn;
        }
        return true;
    }
    /// @}
    ...
    public boolean isAppFullScreen() {
        return mAppToken != null ? mAppToken.appFullscreen : false;
    }
    ...
    @Override
    public WindowState getAttachedWindow() {
    ...
    /* prize-add- 0 180 rotation, fullscreen app,  cut lcd ears-liyongli-20180628-begin */
    /** {@hide} */
    @Override
    public boolean prizeAnimatingExit() {
        return mAnimatingExit;
    }    
    /* prize-add- 0 180 rotation, fullscreen app,  cut lcd ears-liyongli-20180628-end */
