...
//prize add by xiarui 2018-06-06 @{
...
public class AudioService extends IAudioService.Stub {
    ...
    private int getNewRingerMode(int stream, int index, int flags) {
        ...
        if (((flags & AudioManager.FLAG_ALLOW_RINGER_MODES) != 0) ||
            ...
            int newRingerMode;
            if (index == 0) {
                //prize modify by xiarui for bug61451 bug61809 2018-06-11 @{
                if (PrizeOption.PRIZE_VIBRATE_CONTROL) {
                    boolean isVibrateInSilent = Settings.System.getInt(mContentResolver, Settings.System.PRIZE_VIBRATE_IN_SILENT, 0) != 0;
                    newRingerMode = (mHasVibrator && isVibrateInSilent) ? AudioManager.RINGER_MODE_VIBRATE
    ...
    private int setModeInt(int mode, IBinder cb, int pid, String caller) {
		...
		if (caller.equals("com.qihoo360.antilostwatch")) {
		    qihoopid = pid;
			Log.d(TAG, "qihoopid = " + qihoopid);
		}
		if (caller.equals("com.tencent.mobileqq")) {
			if (mode == 1)
				mode = 0;
		}
        do {
            if (mode == AudioSystem.MODE_NORMAL) {
                ...
                if (!mSetModeDeathHandlers.isEmpty()) {
                    ...
                    //M:chenteng-20180307-qihoo360
					Log.d(TAG, "qihoopid =  " + qihoopid + "hdlr.mPid = " + hdlr.mPid);
					if (qihoopid == hdlr.mPid) {
						  mode = 0;
						  hdlr.setMode(mode);
						  Log.d(TAG, "qihoo360 setmode =  " + mode);
					}
                }
            } else {
                if (hdlr == null) {
                        ...
                        if ((mSetModeDeathHandlers.isEmpty() ||
                            ...
                            if (mScoAudioState == SCO_STATE_INACTIVE) {
                                ...
                                if (scoAudioMode == SCO_MODE_UNDEFINED) {
                                    ///M:ALPS03008725, get headset devcie.
                                    if (DEBUG_DEVICES) {
                                        Log.d(TAG, "requestScoState:mBluetoothHeadsetDevice="
                                            + mBluetoothHeadsetDevice + ",mBluetoothHeadset:"
                                            + mBluetoothHeadset);
                                    }
                                    if (mBluetoothHeadsetDevice == null) {
                                        if (mBluetoothHeadset != null) {
                                            ...
                                            if (devices.size() > 0) {
                                                mBluetoothHeadsetDevice = devices.get(0);
                                            }
                                        }
                                    }
        ...
        public void onServiceConnected(int profile, BluetoothProfile proxy) {
            ...
            case BluetoothProfile.HEADSET:
                synchronized (mScoClients) {
                    ...
                    ///M:ALPS03008725, add debug info
                    if (DEBUG_DEVICES) {
                        Log.d(TAG, "BluetoothProfile.ServiceListener " + BluetoothProfile.HEADSET
                        + ",mScoAudioState: " + mScoAudioState + ",mBluetoothHeadsetDevice:"
                            + mBluetoothHeadsetDevice + ",mScoAudioMode:" + mScoAudioMode
    ...
    private int checkForRingerModeChange(int oldIndex, int direction, int step, boolean isMuted,
        ...
        case RINGER_MODE_NORMAL:
            if (direction == AudioManager.ADJUST_LOWER) {
                //prize add by xiarui 2018-06-05 normal to silent or vibrate @{
                boolean isVibrateInSilent = true;
                if (PrizeOption.PRIZE_VIBRATE_CONTROL) {
                    isVibrateInSilent = Settings.System.getInt(mContentResolver,
                            Settings.System.PRIZE_VIBRATE_IN_SILENT, 0) != 0;
                }
                //@}
                if (mHasVibrator && isVibrateInSilent) {
                ...
                if (isTv && oldIndex >= 2 * step && isMuted) {
                    ...
                    if (mVolumePolicy.volumeDownToEnterSilent) {
                        ...
                        if (diff > mVolumePolicy.vibrateToSilentDebounce
                            ...
                            //ringerMode = RINGER_MODE_SILENT;//remove liup 20171218 zen
                        }
                    } else {
            ...
            if (isTv && direction == AudioManager.ADJUST_LOWER && oldIndex >= 2 * step && isMuted) {
                ...
                if (!mVolumePolicy.volumeUpToExitSilent) {
                  ...
                  if (mHasVibrator && direction == AudioManager.ADJUST_RAISE) {
                      //prize modify by xiarui 2018-06-04 for silent to normal @{
                      if (PrizeOption.PRIZE_VIBRATE_CONTROL) {
                          ringerMode = RINGER_MODE_NORMAL;
                      } else {
                          ringerMode = RINGER_MODE_VIBRATE;
    ...
    */
    ...
    private class AudioServiceBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            ///M:ALPS03008725, add debug info
            if (DEBUG_DEVICES) {
                Log.d(TAG, "BluetoothHeadset.action:" + action
                    + ",intent:" + intent);
            }
            ///M.
