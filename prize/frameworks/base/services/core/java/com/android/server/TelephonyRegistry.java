...
//        mConnectedApns = new ArrayList<String>();
    ...
    public void notifySubscriptionInfoChanged() {
        // MTK-START  delete because log too much
    ...
    public void notifyCallStateForPhoneInfo(int phoneType, int phoneId, int subId, int state,
        ...
        if (VDBG) {
            log("notifyCallStateForPhoneInfo: phoneType=" + phoneType + " phoneId =" + phoneId
                + " subId=" + subId
                + " state=" + state + " incomingNumber=" + Rlog.pii(SDBG, incomingNumber));
        }
        synchronized (mRecords) {
            if (validatePhoneId(phoneId)) {
                mCallState[phoneId] = state;
            ...
            if (validatePhoneId(phoneId)) {
                ...
                for (Record r : mRecords) {
                    if (r.matchPhoneStateListenerEvent(
                            PhoneStateListener.LISTEN_PRECISE_DATA_CONNECTION_STATE) &&
                            (idMatch(r.subId, subId) ||
                            isEmergencyWithoutSim(r.subId, subId, apnType))) {
                        try {
                            if (VDBG) {
    ...
    // M: In order to reduce log
    private LogSSC logSSC [] = new LogSSC[1];
    private int next = 0;
    ...
    boolean idMatchForNetwork(int rSubId, int subId, int rPhoneId, int phoneId) {
        if(subId < 0) {
            // Invalid case, we need compare phoneId with Record's
    ...
    // ALPS03067851: Can listen Emergency data connection state without SIM inserted.
    boolean isEmergencyWithoutSim(int rSubId, int subId, String apnType) {
        if (PhoneConstants.APN_TYPE_EMERGENCY.equalsIgnoreCase(apnType) &&
                rSubId == SubscriptionManager.DEFAULT_SUBSCRIPTION_ID &&
                !SubscriptionManager.isValidSubscriptionId(subId)) {
            log("isEimsWithoutSim() rSubId: " + rSubId + " subId: " +
                    subId + " apnType: " + apnType);
            return true;
        }
        return false;
    }
