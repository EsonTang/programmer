...
/* app multi instances feature. prize-linkh-20160815 */
...
class RecentTasks extends ArrayList<TaskRecord> {
    ...
    final void addLocked(TaskRecord task) {
	 ...
	 /*prize-add by lihuangyuan,for fingerapplock qq,weixin video/audio call task-2017-10-06-start*/
	 if(task.intent != null
	      && ("com.tencent.av.ui.VideoInviteFull".equals(task.intent.getComponent().getClassName()) 
	     ||"com.tencent.av.ui.AVActivity".equals(task.intent.getComponent().getClassName()) 
	     ||"com.tencent.av.ui.VideoInviteLock".equals(task.intent.getComponent().getClassName())
	     ||"com.tencent.mm.plugin.voip.ui.VideoActivity".equals(task.intent.getComponent().getClassName())))
	 {
	 	Slog.d(TAG_RECENTS,"addLocked "+",task:"+task+task.intent+" not add to the recenttask!");
		return;
	 }
	 /*prize-add by lihuangyuan,for fingerapplock qq,weixin video/audio call task-2017-10-06-end*/
	 /*prize-add by lihuangyuan,for kugou LockScreenActivity task not save recenttask-2017-10-06-start*/
	 if( task.intent != null &&  "com.kugou.android.app.lockscreen.LockScreenActivity".equals(task.intent.getComponent().getClassName()))
	 {
	 	Slog.d(TAG_RECENTS,"addLocked "+"kugou LockScreenActivity not add to the recenttask");
		return;
	 }	 
        /*prize-add by lihuangyuan,for kugou LockScreenActivity task not save recenttask-2017-10-06-end*/
	 /*-prize-add by lihuangyuan,for applock 2017-10-19-start*/
	 if( task.intent != null &&  "com.prize.applock.fingerprintapplock".equals(task.intent.getComponent().getPackageName()))
	 {
	 	Slog.d(TAG_RECENTS,"addLocked "+"fingerprintapplock  not add to the recenttask");
		return;
	 }
	 if( task.intent != null &&  "com.android.settings.applock.PrizeAppLockManagerActivity".equals(task.intent.getComponent().getClassName()))
	 {
	 	Slog.d(TAG_RECENTS,"addLocked "+"PrizeAppLockManagerActivity  not add to the recenttask");
		return;
	 }
        ...
        for (int i = 0; i < recentsCount; i++) {
            ...
            if (task != tr) {
                ...
                if(PrizeOption.PRIZE_APP_MULTI_INSTANCES) {
                    if(task.appInstanceIndex != tr.appInstanceIndex) {
                        continue;
                    }
                } //end..
