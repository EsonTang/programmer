...
public class MediaSessionService extends SystemService
    implements Monitor,MediaSessionStack.OnMediaButtonSessionChangedListener {
    ...
    public MediaSessionService(Context context) {
        ...
        mPriorityStack = new MediaSessionStack(this);
    ...
    public void updateSession(MediaSessionRecord record) {
        synchronized (mLock) {
            Log.d(TAG, "updateSession : record : " + record);
            ...
            //if (mPriorityStack.contains(record)) {
                ...
                //Log.d(TAG, "updateSession : Found it.");
                //mPriorityStack.removeSession(record);
            //}
    ...
    public void onSessionPlaystateChange(MediaSessionRecord record, int oldState, int newState) {
        ...
        Log.d(TAG, "onSessionPlaystateChange : record : " + record +
        ...
        synchronized (mLock) {
            ...
            Log.d(TAG, "onSessionPlaystateChange");
    ...
    private void destroySessionLocked(MediaSessionRecord session) {
        ...
        UserRecord user = mUserRecords.get(userId);
        if (user != null) {
            Log.d(TAG, "destroySessionLocked : pushAddressedPlayerChangedLocked.");
            user.pushAddressedPlayerChangedLocked();
    ...
    private void rememberMediaButtonReceiverLocked(MediaSessionRecord record) {
        ...
        Log.d(TAG, "rememberMediaButtonReceiverLocked : " + receiver);
    ...
    public void onMediaButtonSessionChanged(MediaSessionRecord oldMediaButtonSession,
        ...
        synchronized (mLock) {
            if (oldMediaButtonSession != null) {
                Log.d(TAG, "old media session : " + oldMediaButtonSession.getUserId());
                mHandler.post(MessageHandler.MSG_SESSIONS_CHANGED,
                    oldMediaButtonSession.getUserId(), 0);
            }
            if (newMediaButtonSession != null) {
                rememberMediaButtonReceiverLocked(newMediaButtonSession);
                Log.d(TAG, "new media session : " + newMediaButtonSession.getUserId());
                mHandler.post(MessageHandler.MSG_SESSIONS_CHANGED,
                    newMediaButtonSession.getUserId(), 0);
                int userId = newMediaButtonSession.getUserId();
                UserRecord user = mUserRecords.get(userId);
                if (user != null) {
                    Log.d(TAG, "onMediaButtonSessionChanged : pushAddressedPlayerChangedLocked.");
                    user.pushAddressedPlayerChangedLocked();
                }
            }
        }
    ...
    final class UserRecord{
        ...
        private void pushAddressedPlayerChangedLocked() {
            ...
            try {
                MediaSessionRecord mediaButtonSession = getMediaButtonSession();
                if (mediaButtonSession != null) {
                    Log.d(TAG, "pushAddressedPlayer : mediaButtonSession "
                        + mediaButtonSession);
                    mCallback.onAddressedPlayerChangedToMediaSession(
                            new MediaSession.Token(mediaButtonSession.getControllerBinder()));
                } else if (mLastMediaButtonReceiver != null) {
                    Log.d(TAG, "pushAddressedPlayer : mLastMediaButtonReceiver "
                        + mLastMediaButtonReceiver);
                    mCallback.onAddressedPlayerChangedToMediaButtonReceiver(
                            mLastMediaButtonReceiver
                                    .getIntent().getComponent());
                } else if (mRestoredMediaButtonReceiver != null) {
                    Log.d(TAG, "pushAddressedPlayer : mRestoredMediaButtonReceiver "
                        + mRestoredMediaButtonReceiver);
                    mCallback.onAddressedPlayerChangedToMediaButtonReceiver(
                            mRestoredMediaButtonReceiver);
                }
            } catch (RemoteException e) {
        ...
        private MediaSessionRecord getMediaButtonSession() {
            return mPriorityStack.isGlobalPriorityActive()
                    ? mPriorityStack.getDefaultMediaButtonSession(mCurrentUserIdList, false)
        ...
        private void restoreMediaButtonReceiver() {
            ...
            if (!TextUtils.isEmpty(receiverName)) {
                ...
                Log.d(TAG, "restoreMediaButtonReceiver : mRestoredMediaButtonReceiver "
                    + mRestoredMediaButtonReceiver);
        ...
        public void setCallback(ICallback callback) {
            ...
            try {
                ...
                synchronized (mLock) {
                    ...
                    UserRecord user = mUserRecords.get(userId);
                    if (user == null || user.mUserId != userId) {
                        Log.w(TAG, "Only the current user can set the callback"
                                + ", userId=" + userId);
                        return;
                    }
                    user.mCallback = callback;
                    Log.d(TAG, "The callback " + user.mCallback
                            + " is set by user ID " + uid);
                    ...
                    try {
                        if (user.mLastMediaButtonReceiver != null) {
                            ...
                            if (user.mCallback != null) {
                                user.mCallback
                                        .onMediaKeyEventDispatchedToMediaButtonReceiver(
                                                keyEvent, user.mRestoredMediaButtonReceiver);
                            }
                        }
