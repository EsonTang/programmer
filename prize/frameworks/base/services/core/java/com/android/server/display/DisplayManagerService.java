...
public final class DisplayManagerService extends SystemService {
    ...
    private DisplayInfo mCropInfo = null;
    /// @}
    ...
    private void setDisplayInfoOverrideFromWindowManagerInternal(
        ...
        synchronized (mSyncRoot) {
            ...
            if (mSupportFullscreenSwitch) {
                if (info != null && info.cropWidth != 0 && info.cropHeight != 0) {
                    mCropInfo = new DisplayInfo();
                    mCropInfo.cropWidth = info.cropWidth;
                    mCropInfo.cropHeight = info.cropHeight;
                    Slog.d(TAG, "fullscreen switch, crop display:"
                            + info.cropWidth + ", " + info.cropHeight
                            + " mCropInfo " + mCropInfo);
                } else {
                    mCropInfo = null;
                }
    ...
    private int mLastSleepBrightness = 0;
    private Object mSyncSetDiaplayState = new Object();
    public class GlobalDisplayState
    {
        public int wakeupReason;
	 public int state;
	 public int brightness;
    }
    private void setDisplayOn(int state)
    {
            final int count = mDisplayDevices.size();
	     for (int i = 0; i < count; i++) 
	     {
	            DisplayDevice device = mDisplayDevices.get(i);
	            Slog.d(TAG, "setDisplayOn global display " + device.getDisplayDeviceInfoLocked()+",state:"+state);
		     device.setDisplayOn(state);
	     }	  
    }
    private void setGlobalDisplayStateLock(GlobalDisplayState displaystate)
    {
        /* prize modify solve the system server crash yueliu start */
        //synchronized (mSyncSetDiaplayState)
        synchronized (mSyncRoot)
	      {
              /* prize modify solve the system server crash yueliu end */
	             if(displaystate.wakeupReason == 1)
		      	{	      	   
		      	  
		          if(displaystate.state == Display.STATE_ON)
		          {
		              if(displaystate.brightness == 0)
		              {
		                  //Slog.d(TAG, "setGlobalDisplayStateLock brightness:"+displaystate.brightness+",mLastSleepBrightness:"+mLastSleepBrightness);
		                  //displaystate.brightness = mLastSleepBrightness;
		              }
		          }
		     }
	            final int count = mDisplayDevices.size();
		     ...
		     {
		            DisplayDevice device = mDisplayDevices.get(i);
		            Slog.d(TAG, "setGlobalDisplayStateLock global display " + device.getDisplayDeviceInfoLocked()+",state:"+displaystate.state+",brigthness:"+displaystate.brightness);
			     device.setDisplayStateLock(displaystate.state, displaystate.brightness);
		     } 
		     //mDisplayPowerController.prizeNotifyScreenOn();		     
             	}
    }
    private void setNormalGlobalDisplayStateLock(GlobalDisplayState displaystate)
    {
        /* prize modify solve the system server crash yueliu start */
        //synchronized (mSyncSetDiaplayState)
             synchronized (mSyncRoot)
	      {
		     ...
		     {
		            DisplayDevice device = mDisplayDevices.get(i);
		            Slog.d(TAG, "setNormalGlobalDisplayStateLock global display " + device.getDisplayDeviceInfoLocked()+",state:"+displaystate.state+",brigthness:"+displaystate.brightness);
			     device.setDisplayStateLock(displaystate.state, displaystate.brightness);
		     }	     
             	}
    }
    private void setRewakdupGlobalDisplayStateLock()
    {
        /* prize modify solve the system server crash yueliu start */
        //synchronized (mSyncSetDiaplayState)
        synchronized (mSyncRoot)
	      {
              /* prize modify solve the system server crash yueliu end */
	            if(mGlobalDisplayState == Display.STATE_ON && mGlobalDisplayBrightness > 0 && mWakeupReason != 1)
	            {
			     ...
			     {
			            DisplayDevice device = mDisplayDevices.get(i);
			            Slog.d(TAG, "setRewakdupGlobalDisplayStateLock global display " + device.getDisplayDeviceInfoLocked()+",state:"+mGlobalDisplayState+",brigthness:"+mGlobalDisplayBrightness);
				     device.setDisplayStateLock(mGlobalDisplayState, mGlobalDisplayBrightness);
			     }	   
	            }
             	}
    }
    private static final int MSG_SET_GLOBAL_DISPLAY_STATE = 100;
    private static final int MSG_SET_NORMAL_GLOBAL_DISPLAY_STATE = 101;
    private static final int MSG_SET_DISPLAY_ON = 102;
    private static final int MSG_REWAKEUP_TURNONSCREEN = 103;
    private class PowerMangerHanlder extends Handler
    {
        public PowerMangerHanlder(Looper looper) 
	 {
        ...
        public void handleMessage(Message msg) 
        {
            switch (msg.what) 
	     {	     
	    case MSG_SET_GLOBAL_DISPLAY_STATE:		  	
	  	//if(mWakeupReason == 1)
	  	{
	  	       GlobalDisplayState state = (GlobalDisplayState)msg.obj;
	  		setGlobalDisplayStateLock(state);				
	  	}
	  	break;
	     case MSG_SET_NORMAL_GLOBAL_DISPLAY_STATE:
		 {
	  	       GlobalDisplayState state = (GlobalDisplayState)msg.obj;
	  		setNormalGlobalDisplayStateLock(state);				
	  	}
		 break;
	     case MSG_SET_DISPLAY_ON:
		 {
		 	 GlobalDisplayState displaystate = new GlobalDisplayState();
			displaystate.wakeupReason = 0;
		       displaystate.state = msg.arg1;
			displaystate.brightness = msg.arg2;
		 	setNormalGlobalDisplayStateLock(displaystate);
	     	 }
		 break;
	     case MSG_REWAKEUP_TURNONSCREEN:
		 setRewakdupGlobalDisplayStateLock();		 
		 break;	     
            }
        }
    }
    private PowerMangerHanlder mPowerHandler = null;
    private HandlerThread mPowerHandlerThread = null;
    /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-end*/
    private void requestGlobalDisplayStateInternal(int state, int brightness) {
		...
		if(true)
		{
		    synchronized (mSyncRoot) {
		     ...
		     Slog.d(TAG,"requestGlobalDisplayStateInternal state:"+state+",brightness:"+brightness+",mWakeupReason:"+mWakeupReason);
		     if(brightness > 35)
		     {
		         mLastSleepBrightness = brightness;
		     }
		     synchronized (mSyncSetDiaplayState) 
	           {
			     if(mWakeupReason == 1 && state == Display.STATE_ON)//not fingerprint
			     {			         
	        	         Slog.d(TAG,"requestGlobalDisplayStateInternal wait for fingerprint result");
				   //setDisplayOn(Display.STATE_ON);
				   Message msg = mPowerHandler.obtainMessage(MSG_SET_DISPLAY_ON, Display.STATE_ON,0);
            	                 mPowerHandler.sendMessage(msg);
			     }
			     else
			     {
			         GlobalDisplayState displaystate = new GlobalDisplayState();
				  displaystate.wakeupReason = mWakeupReason;
				  displaystate.state = mGlobalDisplayState;
				  displaystate.brightness = mGlobalDisplayBrightness;
				  //setGlobalDisplayStateLock(displaystate);
	         	         Message msg = mPowerHandler.obtainMessage(MSG_SET_NORMAL_GLOBAL_DISPLAY_STATE, displaystate);
            	                mPowerHandler.sendMessage(msg);
			     }
		     }
            ...
            if (display != null) {
                ...
                if (mSupportFullscreenSwitch) {
                    if (mCropInfo != null
                                ...
                                && mCropInfo.cropWidth != 0 && mCropInfo.cropHeight != 0) {
                        boolean logicalCompare = info.logicalWidth > info.logicalHeight;
                        info.logicalWidth = logicalCompare
                                ? Math.max(mCropInfo.cropWidth, mCropInfo.cropHeight)
                                : Math.min(mCropInfo.cropWidth, mCropInfo.cropHeight);
                        info.logicalHeight = logicalCompare
                                ? Math.min(mCropInfo.cropWidth, mCropInfo.cropHeight)
                                : Math.max(mCropInfo.cropWidth, mCropInfo.cropHeight);
                    }
                }
    ...
    private final class LocalService extends DisplayManagerInternal {
        ...
        public void initPowerManagement(final DisplayPowerCallbacks callbacks, Handler handler,
		  ...
		  /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-start*/
		  mPowerHandlerThread = new HandlerThread("PowerThread");
		  mPowerHandlerThread.start();
		  mPowerHandler = new PowerMangerHanlder(mPowerHandlerThread.getLooper());
		  /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-end*/
            }
        }
	 ...
	 public void setWakeupReason(int reason)
	 {
	     synchronized (mSyncSetDiaplayState) 
	     {
	       //mPowerHandler.removeMessages(MSG_SET_GLOBAL_DISPLAY_STATE);
	     	mWakeupReason = reason;
	     }
	 }
	 @Override
	 public  void turnOnScreenNow()
	 {
	     Slog.d(TAG,"turnOnScreenNow mWakeupReason:"+mWakeupReason);
	     if(mWakeupReason == 1)
	     {
	     	  GlobalDisplayState displaystate = new GlobalDisplayState();
		  displaystate.wakeupReason = mWakeupReason;
		  displaystate.state = mGlobalDisplayState;
		  displaystate.brightness = mGlobalDisplayBrightness;
	         Message msg = mPowerHandler.obtainMessage(MSG_SET_GLOBAL_DISPLAY_STATE, displaystate);
            	  //mPowerHandler.sendMessageDelayed(msg,40);
            	  mPowerHandler.sendMessage(msg);
		  mWakeupReason = 0;
	     }
	 }	 
	 ...
	 @Override
	 public void rewakeupTurnOnScreenNow()
	 {
	     Message msg = mPowerHandler.obtainMessage(MSG_REWAKEUP_TURNONSCREEN);
            mPowerHandler.sendMessage(msg);
	 }
	 /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-end*/
    }
