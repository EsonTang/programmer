...
public final class PowerManagerService extends SystemService
    ...
    private boolean wakeUpNoUpdateLocked(long eventTime, String reason, int reasonUid,
	 ...
	 int wakeupreason = 0;
	 if(reason != null && reason.equals("android.policy:FINGER"))
	 {
	     wakeupreason = 1;
	 }
	 //rewakeup by other reason	 
	 int curwakeupreason = mDisplayManagerInternal.wakeupReason();
	 mDisplayManagerInternal.setWakeupReason(wakeupreason);
        mPolicy.setWakeupReason(wakeupreason);//for ficeid
	 if(curwakeupreason == 1 && wakeupreason != 1)
	 {
	     mDisplayManagerInternal.rewakeupTurnOnScreenNow();
	 }
        /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-end*/
	 /*-prize-add by lihuangyuan,for faceid -2018-03-07-start*/
	 mPolicy.setWakeOrSleep(true);
    ...
    private void goToSleepInternal(long eventTime, int reason, int flags, int uid) {
        synchronized (mLock) {
            ...
            if(reason == 100 && mDisplayManagerInternal.wakeupReason() != 1)
            {
                Slog.d(TAG,"goToSleepInternal reason:"+reason+",uid:"+uid+",wakeup by other reason");
                return;
            }
    ...
    private boolean goToSleepNoUpdateLocked(long eventTime, int reason, int flags, int uid) {
        ...
        /* PRIZE-for FaceID unlock, push power key delay ScreenOn -liyongli-2018-01-25 -start*/
        //fix, in sleep, fast push power key twice, lcd not on;
        if(reason==PowerManager.GO_TO_SLEEP_REASON_POWER_BUTTON){
            if( SystemProperties.getInt("persist.pafd.delayon", 0) !=0){
                Slog.d(TAG, "lyl FaceID fast push power key twice  not do Going to sleep, power button");
                return false;
            }
        }/* PRIZE-for FaceID unlock, push power key delay ScreenOn -liyongli-2018-01-25 -end*/
        
        /*-prize-add by lihuangyuan,for faceid -2018-03-07-start*/
        mPolicy.setWakeOrSleep(false);
        /*-prize-add by lihuangyuan,for faceid -2018-03-07-end*/
    ...
    private int getDesiredScreenPolicyLocked() {
        //fix issue that can be waken up on CAT test
        if (mWakefulness == WAKEFULNESS_ASLEEP) {
    ...
    /**
    * Switch super saver mode and reboot the device.
    * </p><p>
    * {@samplecode
    * 	PowerManager pm = (PowerManager) getSystemService(Context.POWER_SERVICE);
    * 	pm.switchSuperSaverMode(true);
    * }
    * </p>
    *
    * @return whether the device is super saver mode
    */
    /*@Override // Binder call
    public void switchSuperSaverMode(boolean intoSuperSaver) {
        switchSuperSaverModeInternal(intoSuperSaver);
    }*/

    /// M: power-off alarm
    ///    add for power-off alarm Check the boot mode whether alarm boot or
    ///    normal boot (including ipo boot). {@
    ///@hide prize-wuliang 47965 modify AlarmBoot phonewindowmanager.java
    ...
    private void switchSuperSaverModeInternal(final boolean intoSuperSaver){
        Slog.i(TAG,"PowerExtendMode switchSuperSaverModeInternal intoSuperSaver is " + intoSuperSaver);
        if (mHandler == null || !mSystemReady) {
            throw new IllegalStateException("Too early to call shutdown() or reboot()");
        }
        ///prize-wuliang 47965 modify AlarmBoot 
        if(isAlarmBoot()){
            Slog.e(TAG, "isAlarmBoot not to SuperSaverMode");
            return; 
        }
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                synchronized (this) {
                    if (intoSuperSaver) {
                        ShutdownThread.switchSuperSaverMode(mContext, null, true);
                    } else {
                        ShutdownThread.switchSuperSaverMode(mContext, null, false);
                    }
                }
            }
        };
            ...
            if (mDeviceIdleMode) {
                ...
                if (appid >= Process.FIRST_APPLICATION_UID &&
                        ...
                        mUidState.get(wakeLock.mOwnerUid,
                                ...
                                //modify by lihuangyuan,for doze do not release some app wakelock,2018-07-27-start
                                >ActivityManager.PROCESS_STATE_TOP) {
                                //> ActivityManager.PROCESS_STATE_FOREGROUND_SERVICE) {
                                //modify by lihuangyuan,for doze do not release some app wakelock,2018-07-27-end
                    disabled = true;
                }
            }
    ...
    private final class BinderService extends IPowerManager.Stub {
	 ...
	 {
	     mDisplayManagerInternal.turnOnScreenNow();
	 }	
	 @Override // Binder call
	 public int   isSleep()
	 {
	     //return mDisplayManagerInternal.wakeupReason();
	     return (WAKEFULNESS_AWAKE != mWakefulness?1:0);
	 }
	 @Override // Binder call
	 public int   wakeupReason()
	 {
        ...
        @Override // Binder call
        public void switchSuperSaverMode(boolean intoSuperSaver) {
            Slog.d(TAG, "PowerExtendMode switchSuperSaverMode is " + intoSuperSaver);
            switchSuperSaverModeInternal(intoSuperSaver);
        }
        /*PRIZE-PowerExtendMode-wangxianzhen-2015-05-30-end*/
    }
