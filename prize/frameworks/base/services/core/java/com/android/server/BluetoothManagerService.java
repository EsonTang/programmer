...
class BluetoothManagerService extends IBluetoothManager.Stub {
    ...
    private static final boolean USER_MODE = SystemProperties.get("ro.build.type").equals("user");
    ...
    //M: [ALPS03524591] Make user enable force to On state
            ...
            /// M: ALPS00868801 IPO reboot
                ...
                synchronized(mReceiver) {
                    ...
                    if (ACTION_BOOT_IPO.equals(action)) {
                        ...
                        if (DBG) Slog.d(TAG, "Wait for nvram daemon.");
                        if (isBluetoothPersistedStateOn()) {
    ...
    private void loadStoredNameAndAddress() {
        ...
        if (!USER_MODE || mAddress == null || mAddress.length() < ADDRESS_LENGTH) {
            if (DBG) Slog.d(TAG, "Stored bluetooth Name=" + mName + ",Address=" + mAddress);
        } else {
            if (DBG) Slog.d(TAG, "Stored bluetooth Name=" + mName + ",Address=" +
                    mAddress.substring(0,9) + "XX:XX:XX");
        }
    }
    ...
    private void onBluetoothGattServiceUp() {
        ...
        try {
            mBluetoothLock.readLock().lock();
            //check isbluetoothpersistedstateonbluetooth to prevent BT can't turn
            // on when enableBLE update app count.
            if ((isBleAppPresent() == false || mEnableExternal ||
                 isBluetoothPersistedStateOnBluetooth()) && mBluetooth != null
                  && mBluetooth.getState() == BluetoothAdapter.STATE_BLE_ON) {
    ...
    final private class ProfileServiceConnections implements ServiceConnection,
        ...
        public void onServiceDisconnected(ComponentName className) {
            ...
            try {
                ...
                //M: ALPS03484437: Handle NoSuchElementException
                Slog.e(TAG, "NoSuchElementException when unlinkToDeath", e);
                return;
            }
            // @}
    ...
    private void waitForNvramDaemonReady() {
        ...
        for (int i = 0; i < MAX_RETRY; i++) {
            if (SystemProperties.get("service.nvram_init").equals("Ready")) {
                if (DBG) Slog.d(TAG, "Nvram daemon is ready.");
                // Enable BT if necessary
                boolean bluetoothStateBluetooth = isBluetoothPersistedStateOnBluetooth();
                if (mEnableExternal && bluetoothStateBluetooth) {
                    //Enable
                    if (DBG) Slog.d(TAG, "Auto-enabling Bluetooth.");
