...
import com.mediatek.common.prizeoption.PrizeOption; // prize-add-split screen-liyongli-20180106
...
class WindowStateAnimator {
    ...
    void prepareSurfaceLocked(final boolean recoveringMemory) {
        ...
        if (displayed) {
            if (w.mOrientationChanging) {
                /* prize-add-split screen, for LANDSPACE  HOME fast show lancher -liyongli-20180106-start */
                boolean prizeUseAnim = true;
                if(PrizeOption.PRIZE_SPLIT_SCREEN_LANDSCAPE_HOME_FAST) {
                    final boolean dockVisible = mService.isStackVisibleLocked(DOCKED_STACK_ID);
                    if(dockVisible){
                        boolean pressHome = mService.getDefaultDisplayContentLocked()
                                                                         .mDividerControllerLocked
                                                                         .prizeIsToLauncherExitSplit();
                        if( pressHome ){
                            prizeUseAnim = false;
                            String str = ""+w.getWindowTag();
                            if( str.contains("com.android.launcher3")
                             || str.contains("com.android.systemui.ImageWallpaper") 
                             || str.contains("DockedStackDivider") 
                            ){
                                prizeUseAnim = true;
                            }
                             Slog.v(TAG,"lyl  " + str);
                ...
                /* prize-add-split screen, for LANDSPACE  HOME fast show lancher -liyongli-20180106-end */
                
                if (prizeUseAnim&&!w.isDrawnLw()) {
    ...
    boolean applyAnimationLocked(int transit, boolean isEntrance) {
        ...
        if (mService.okToDisplay()) {
            ...
            if (a != null) {
                ...
                /// @}
                // solve-bug-43086. prize-linkh @{
                // Keep animation and set 0 duration for Hiding Nav Bar Feature and this 
                // can avoid  that nav bar can't update to right state.
                if (com.mediatek.common.prizeoption.PrizeOption.PRIZE_DYNAMICALLY_HIDE_NAVBAR) {
                    if (mWin.mAttrs.type == WindowManager.LayoutParams.TYPE_NAVIGATION_BAR && 
                        (transit == WindowManagerPolicy.TRANSIT_EXIT || transit == WindowManagerPolicy.TRANSIT_HIDE)) {
                        a.setDuration(0);
                        a.setStartOffset(0);
                        if (DEBUG_ANIM) {
                            Slog.d(TAG, "applyAnimation: force set 0ms duration for nav bar. transit=" + transit);
                        }
                    }
                } // @}
                setAnimation(a);
