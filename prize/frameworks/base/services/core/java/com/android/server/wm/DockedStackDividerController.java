...
import static com.android.server.wm.WindowManagerService.H.PRIZE_NOTIFY_DOCKED_STACK_MINIMIZED_CHANGED_END;
...
import com.mediatek.common.prizeoption.PrizeOption; // prize-add-split screen-liyongli-20170708
...
public class DockedStackDividerController implements DimLayerUser {
    ...
    private static final int PRIZE_DIVIDER_WIDTH_INACTIVE_DP = 2; // prize-add-split screen-liyongli-20170828
    ...
    private boolean mMinimizedDock = false;
    ...
    // prize-add-split screen, open launcher exit split screen -liyongli-20170911
    private boolean prizeToLauncherExitSplit = false;  // prize-add-split screen, open launcher exit split screen -liyongli-20170911
    private boolean prizeDimCoverUpScreen = false;  //Dim layer cover the up screen (bottom screen app pop the soft keyboard)
    private boolean prizeHomeDownKB = false; //the bottom screen app pop the soft keyboard, and press HOME
    // prize-add-split screen -liyongli-20170911  end
    ...
    private void loadDimens() {
        ...
        if(PrizeOption.PRIZE_SPLIT_SCREEN_BG_HINT_FOCUS) {
            ...
            mDividerWindowWidthInactive = WindowManagerService.dipToPixel(
                    PRIZE_DIVIDER_WIDTH_INACTIVE_DP, mDisplayContent.getDisplayMetrics()); 
    ...
    void prizeNotifyDockedFocusChanged(int focusedStackId, int lastFocusedStackId) {
        ...
        for (int i = 0; i < size; ++i) {
            ...
            try {
                listener.prizeOnFocusStackChanged(focusedStackId, lastFocusedStackId);
            } catch (RemoteException e) {
                Slog.e(TAG_WM, "Error listener.prizeOnFocusStackChanged.", e);
            }
        }
    ...
    void prizeNotifyRequestExitSplitScreen(int btnType, int focusedStackId) {
        ...
        for (int i = 0; i < size; ++i) {
            final IDockedStackListener listener = mDockedStackListeners.getBroadcastItem(i);
            try {
                listener.prizeOnRequestExitSplitScreen(btnType, focusedStackId);
            } catch (RemoteException e) {
                Slog.e(TAG_WM, "Error listener.prizeOnRequestExitSplitScreen.", e);
            }
        }
        mDockedStackListeners.finishBroadcast();
    }
    ...
    void notifyDockedStackExistsChanged(boolean exists) {
        final int size = mDockedStackListeners.beginBroadcast();
        /* prize-add-split screen, fix 55136 mm die exit split statubar background is black -liyongli-20180413-start 
             PhoneWindowManager.java   updateSystemUiVisibilityLw() if(isResizing) return 0; not change the statusbar background color
             for fix 47847  swip the split window, navBar blur.  55136 is caused by  (isResizing==true) , 
             so split screen exit , set isResizing to false.
        */
        if(!exists){
            setResizing(false);
        }/* prize-add-split screen, fix 55136 -liyongli-20180413-end */
        ...
        /* prize-add-split screen, for HOME exit split screen -liyongli-20171220-start */
        if(PrizeOption.PRIZE_SPLIT_SCREEN_KEYBOARD_HOME){
            prizeHomeDownKB = false; //fix blink bug -- bottom screen pop keyboard, press HOME
        }/* prize-add-split screen-liyongli-20171220-end */
        
        /* prize-add-split screen, fix 58302 -liyongli-20180516-start */
        if(!exists){
            setResizing(false);
        }/* prize-add-split screen, fix 58302 -liyongli-20180516-end */
    }
        ...
        if (visibleAndValid) {
            ...
            if (mTmpRect.height() > 0 && mTmpRect.width() > 0) {
                ...
                mDimLayer.show(mService.mLayersController.getResizeDimLayer(),
                        alpha, 0 /* duration */);
                /* prize-add-split screen, for HOME exit split screen -liyongli-20171220-start */
                //fix blink bug -- bottom screen pop keyboard, press HOME
                if(PrizeOption.PRIZE_SPLIT_SCREEN_KEYBOARD_HOME && stack.mStackId==DOCKED_STACK_ID){
                    prizeDimCoverUpScreen = true;
                }/* prize-add-split screen-liyongli-20171220-end */
            } else {
    ...
    void notifyAppTransitionStarting(ArraySet<AppWindowToken> openingApps, int appTransition) {
        ...
        checkMinimizeChanged(true /* animate */); //tigger the split screen TO mini state

        /* prize-add-split screen-liyongli-20171212-start */
        if(PrizeOption.PRIZE_SPLIT_SCREEN_RETURN||PrizeOption.PRIZE_SPLIT_SCREEN_HOME) {
            return;
        }/* prize-add-split screen-liyongli-20171212-end */
    ...
    private void checkMinimizeChanged(boolean animate) {
        ...
        if( PrizeOption.PRIZE_SPLIT_SCREEN_HOME){
            boolean tominiDock = (homeVisible && !homeBehind);
            Slog.d(TAG_WM, "lylanimate  start  miniDock anim  : "+mMinimizedDock + " !=  "+tominiDock );
            if( mMinimizedDock != tominiDock ){//2017/12/4 add for HOME not exit split
                //because  mMinimizedDock == tominiDock setMinimizedDockedStack() do nothing,
                //so no need change the prizeToLauncherExitSplit value
                Slog.d(TAG_WM, "lylanimate  will mini Dock : "+prizeToLauncherExitSplit );
                if( !prizeIsToLauncherExitSplit() ){
                    return;
                }
//                prizeSetToLauncherExitSplit(false);  liyongli 2018/1/6  move to animateForMinimizedDockedStack
                                                                                             //for landspace HOME fast show Launch
            }
        }
    ...
    public boolean animate(long now) {
        ...
        if (mAnimatingForMinimizedDockedStack) {
            ...
            /* prize-add-split screen, for HOME exit split screen -liyongli-20171221-start */
            if(PrizeOption.PRIZE_SPLIT_SCREEN_KEYBOARD_HOME &&prizeToLauncherExitSplit) {
                //add &&prizeToLauncherExitSplit fix 49734, true --> pushed HOME key
                final TaskStack imeTargetStack = mService.getImeFocusStackLocked();
                if(imeTargetStack!=null && 
                    (imeTargetStack.mStackId==DOCKED_STACK_ID||imeTargetStack.mStackId==HOME_STACK_ID)
                ){
                    mLastAnimationProgress = mAnimationTarget;
                    mLastDividerProgress = mDividerAnimationTarget;
                    mAnimatingForIme = false;
                    return false; //fix blink bug -- top screen pop keyboard, press HOME
                }
            }
            /* prize-add-split screen -liyongli-20171221-end */
    ...
    private boolean animateForIme(long now) {
        ...
        /* prize-add-split screen, for HOME exit split screen -liyongli-20171220-start */
        if(PrizeOption.PRIZE_SPLIT_SCREEN_KEYBOARD_HOME && prizeHomeDownKB){
            updated = false; //fix blink bug -- bottom screen pop keyboard, press HOME
            //Note : if normal show(hide) keyboard, set updated = false will cause display black strip
        }/* prize-add-split screen-liyongli-20171220-end */
    ...
    private boolean animateForMinimizedDockedStack(long now) {
        ...
        t = (isAnimationMaximizing() ? TOUCH_RESPONSE_INTERPOLATOR : mMinimizedDockInterpolator)
            ...
            if( PrizeOption.PRIZE_SPLIT_SCREEN_HOME && mMinimizedDock && t>=0.9f){
                Slog.d(TAG_WM, "lylanimate  mini Dock change to exitsplit screen" );
                 if (stack != null) {
                    if (stack.setAdjustedForMinimizedDock(getMinimizeAmount(stack, 1.0f))) {
                        mService.mWindowPlacerLocked.performSurfacePlacement();
                    }
                }
                
                mService.mH.removeMessages(PRIZE_NOTIFY_DOCKED_STACK_MINIMIZED_CHANGED_END);
                mService.mH.obtainMessage(PRIZE_NOTIFY_DOCKED_STACK_MINIMIZED_CHANGED_END, 1, 0).sendToTarget();
                mMinimizedDock = false;
                //mAnimatingForMinimizedDockedStack = false;
                
                prizeSetToLauncherExitSplit(false);     // liyongli 2018/1/6  from checkMinimizeChanged move here
                                                                                             //for landspace HOME fast show Launch  
    ...
    void dump(String prefix, PrintWriter pw) {
        ...
        pw.println(prefix + "  mResizing=" + mResizing);
        pw.println(prefix + "  prizeToLauncherExitSplit=" + prizeToLauncherExitSplit);
    ...
    /* prize-add-split screen, for HOME exit split screen animation -liyongli-20170911-start */
    void prizeSetToLauncherExitSplit( boolean openLauncherExitSplit ) {
        prizeToLauncherExitSplit = openLauncherExitSplit;
        
        /* prize-add-split screen, for HOME exit split screen -liyongli-20171220-start */
        if(PrizeOption.PRIZE_SPLIT_SCREEN_KEYBOARD_HOME) {
            //fix blink bug -- bottom screen pop keyboard, press HOME
            if( prizeDimCoverUpScreen ){//dim layer cover top screen
                prizeDimCoverUpScreen = false;
                prizeHomeDownKB = true;
            }else{
                prizeHomeDownKB = false;
            }

            InputMethodManagerInternal inputMethodManagerInternal =
                         LocalServices.getService(InputMethodManagerInternal.class);
            if (inputMethodManagerInternal != null && prizeHomeDownKB) {
                inputMethodManagerInternal.hideCurrentInputMethod();
                //mDimLayer.hide();
                
                //no need call the ime anim
                //DisplayContent displayContent = mService.getDefaultDisplayContentLocked();
                //mMinimizedDock = false;
                //mService.adjustForImeIfNeeded(displayContent);
            }                         
        }/* prize-add-split screen -liyongli-20171220-end */
    }
    
    boolean prizeIsToLauncherExitSplit() {
        return prizeToLauncherExitSplit;
    }
    /* prize-add-split screen, for HOME exit split screen animation -liyongli-20170911-end */
}
