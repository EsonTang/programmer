...
public final class JobSchedulerService extends com.android.server.SystemService
    ...
    /** 
      * Manage jobs of third apps. prize-linkh-20161214 
      */
    // when receive package restarted broadcast, we will cleanup jobs of that package.
    static final boolean CLEANUP_JOBS_OF_THRID_APPS_WHEN_FORCE_STOP = true;
    // when reboot, we will cleanup jobs of third apps to avoid auto-running.
    static final boolean CLEANUP_JOBS_OF_THRID_APPS_WHEN_REBOOT = true;
    ...
    /*prize-add by lihuangyuan,for smart-killed app clear jobservice-2018-03-30-start*/
    ...
    private static final String TAG_KILL = "KillRestart-job";
    /*prize-add by lihuangyuan,for smart-killed app clear jobservice-2018-03-30-end*/
    /**
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_PACKAGE_CHANGED.equals(action)) {
                    ...
                    if (apps != null) 
                    {
                        ...
                        if (apps.length > 0) 
                        {
                            ...
                            for (int k=0;k<apps.length;k++) 
                            {
                                ...
                                int pkgUid = 0;
                                try {
                                    pkgUid = context.getPackageManager().getPackageUid(pkg,
                                            PackageManager.MATCH_UNINSTALLED_PACKAGES);
                                }
                                catch(Exception e)
                                {
                                    Slog.e(TAG_KILL," get uid :"+pkg+" error!");
                                }
                                if(pkgUid != 0)
                                {
                                    Slog.d(TAG_KILL,"cancel jobs for "+pkg+","+pkgUid);
                                    cancelJobsForPackageAndUid(pkg, pkgUid);
                                }                                    
                            }
                        }
                ...
                if (pkgUid != -1) {
                    ...
                    if (!(PrizeOption.PRIZE_SMART_CLEANER && "com.prize.smartcleaner".equals(pkgName))) {
                        cancelJobsForPackageAndUid(pkgName, pkgUid);
                    }
                    //@}
                }
            }// Manage jobs of third apps. prize-linkh-20161214
            else if (CLEANUP_JOBS_OF_THRID_APPS_WHEN_FORCE_STOP && Intent.ACTION_PACKAGE_RESTARTED.equals(intent.getAction())) {
                final String pkgName = getPackageName(intent);
                final int pkgUid = intent.getIntExtra(Intent.EXTRA_UID, 0);
                Slog.d(TAG, "Receive ACTION_PACKAGE_RESTARTED " + pkgName + ":" + pkgUid);
                if (pkgName != null && pkgUid > 0) {
                    final int userId = UserHandle.getUserId(pkgUid);
                    if (shouldCleanupJob(pkgName, userId)) {
                        Slog.d(TAG, "Removing jobs for " + pkgName + ":" + pkgUid);
                        cancelJobsForUid(pkgUid, true);
                    }
                }
            } //END...
    ...
    // Manage jobs of third apps. prize-linkh-20161214
    // Only take effect for third apps.
    static boolean shouldCleanupJob(String pkgName, int userId) {
        boolean cleanup = false;
        try {
            IPackageManager pm = AppGlobals.getPackageManager();
            if (DEBUG) {
                Slog.d(TAG, "Retrieve " + pkgName + " in user " + userId);
            }
            // check if it's a third app.
            final android.content.pm.ApplicationInfo ai = pm.getApplicationInfo(pkgName, 0, userId);
            if (DEBUG) {
                Slog.d(TAG, "Application info: " + ai);
            }
            if (ai != null && (!ai.isSystemApp() || ai.isPrebuiltThirdApp())) {
                cleanup = true;
            }
        } catch (Exception e) {
            Slog.w(TAG, "Can't retrieve for " + pkgName + "in user " + userId);
        }

        if (DEBUG) {
            Slog.d(TAG, "Should cleanup for " + pkgName + " in user " + userId + " ? " + cleanup);
        }
        return cleanup;
    } //END....
    ...
    public void onBootPhase(int phase) {
        if (PHASE_SYSTEM_SERVICES_READY == phase) {
            ...
            // Manage jobs of third apps. prize-linkh-20161214
            if (CLEANUP_JOBS_OF_THRID_APPS_WHEN_FORCE_STOP) {
            ...
            {
                userFilter.addAction(KILLED_APPS_INTENT_ACTION);
            }
            /*prize-add by lihuangyuan,for smart-killed app clear jobservice-2018-03-30-end*/
            getContext().registerReceiverAsUser(
