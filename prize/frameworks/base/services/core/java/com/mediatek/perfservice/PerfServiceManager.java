...
public class PerfServiceManager implements IPerfServiceManager {
    ...
    public static native int nativePerfSetUeventIndex(int index, int action);
    public static native int nativePerfGetPackAttr(String packName, int cmd);
    public static native String nativePerfGetGiftAttr(String packName, String attrName);
    ...
    public static native int nativePerfSetUidInfo(int uid, int fromUid);
    ...
    public void appBoostEnable(String packName) {
        int timeout = -1, mode = -1;
        //log("appBoostEnable " + packName);
        timeout = nativePerfGetPackAttr(packName, IPerfServiceWrapper.CMD_GET_PACK_BOOST_TIMEOUT);
        mode = nativePerfGetPackAttr(packName, IPerfServiceWrapper.CMD_GET_PACK_BOOST_MODE);
        if(timeout > 0 && mode != -1) {
    ...
    public void setUeventIndex(int index, int action) {
        ...
        if(index == 6) {
          ...
          nativePerfNotifyUserStatus(IPerfServiceWrapper.NOTIFY_USER_TYPE_CORE_ONLINE, 10);
          log("cpuset L");
        } else if (index == 11) {
          nativePerfNotifyUserStatus(IPerfServiceWrapper.NOTIFY_USER_TYPE_CORE_ONLINE, 11);
          log("cpuset LL");
        } else if (index == 12) {
          nativePerfNotifyUserStatus(IPerfServiceWrapper.NOTIFY_USER_TYPE_CORE_ONLINE, 12);
          log("cpuset L+LL");
        } else if (index == 13) {
          nativePerfNotifyUserStatus(IPerfServiceWrapper.NOTIFY_USER_TYPE_CORE_ONLINE, 13);
          log("idle_prefer 1");
        } else if (index == 14) {
          nativePerfNotifyUserStatus(IPerfServiceWrapper.NOTIFY_USER_TYPE_CORE_ONLINE, 14);
          log("idle_prefer 0");
        } else {
            msg.sendToTarget();
        }
        ...
        public void handleMessage(Message msg) {
            try {
                switch (msg.what) {
                    ...
                    {
                        log("MESSAGE_SET_UEVENT_INDEX: " + msg.arg1 + ", " + msg.arg2);
                        nativePerfSetUeventIndex(msg.arg1, msg.arg2);
                        break;
                    }
    ...
    private class SmartObserver extends UEventObserver {
        ...
        public void onUEvent(UEventObserver.UEvent event) {
            ...
            int action = Integer.parseInt(action_name);
            //log("ACTION=" + action + ", DETECT=" + index_name);
            setUeventIndex(index, action);
        }
