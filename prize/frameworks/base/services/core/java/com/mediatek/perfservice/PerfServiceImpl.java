...
public class PerfServiceImpl extends IPerfService.Stub {
    ...
    class PerfServiceBroadcastReceiver extends android.content.BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            else if (("mediatek.intent.action.PACKAGE_INSTALL_BEGIN").equals(action)) {
                log("Intent.ACTION_PACKAGE_INSTALL_BEGIN");
                perfServiceMgr.boostEnableTimeout(IPerfServiceWrapper.SCN_APP_INSTALL, 10);
                return;
            }
            else if (Intent.ACTION_PACKAGE_ADDED.equals(action)) {
                log("Intent.ACTION_PACKAGE_ADDED");
                perfServiceMgr.boostDisable(IPerfServiceWrapper.SCN_APP_INSTALL);
                return;
            }
            else {
    ...
    public PerfServiceImpl(Context context, IPerfServiceManager pm) {
        ...
        broadcastFilter.addAction("mediatek.intent.action.PACKAGE_INSTALL_BEGIN");
    ...
    public void appBoostEnable(String packName) {
        ...
        perfServiceMgr.appBoostEnable(packName);
    }

    public String getGiftAttr(String packName, String attrName) {
        //log("PerfServiceImpl getGiftAttr"+packName+attrName );
        return perfServiceMgr.getGiftAttr(packName, attrName);
    }

    public int reloadWhiteList() {
        //log("reloadWhiteList");
        perfServiceMgr.reloadWhiteList();
        return 1;
    }
    ...
    public void setUidInfo(int uid, int fromUid) {
        //log("setExclusiveCore");
        perfServiceMgr.setUidInfo(uid, fromUid);
    }
