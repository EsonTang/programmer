...
//prize-wuliang-20170918 modify mtp name
...
public class MtpDatabase implements AutoCloseable {
    ...
    private int beginSendObject(String path, int format, int parent,
        ...
        if (path != null) {
            if (DEBUG) Log.d(TAG, "beginSendObject: path = " + path);
            Cursor c = null;
            try {
    ...
    private boolean getObjectInfo(int handle, int[] outStorageFormatParent,
        ...
        Cursor c = null;
        if (DEBUG) Log.d(TAG, "getObjectInfo");
        try {
    ...
    private int getObjectFilePath(int handle, char[] outFilePath, long[] outFileLengthFormat) {
        ...
        try {
            ...
            if (c != null && c.moveToNext()) {
                ...
                if (DEBUG) Log.d(TAG, "getObjectFilePath RESPONSE_OK: path = " + path);
    ...
    private int deleteFile(int handle) {
        ...
        if (DEBUG) Log.d(TAG, "deleteFile: handle = 0x" + Integer.toHexString(handle));
    ...
    private void sessionStarted() {
        if (DEBUG) Log.d(TAG, "sessionStarted");
        mDatabaseModified = false;
    }

    private void sessionEnded() {
        if (DEBUG) Log.d(TAG, "sessionEnded, mDatabaseModified = " + mDatabaseModified);
