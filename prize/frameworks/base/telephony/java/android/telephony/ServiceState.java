...
public class ServiceState implements Parcelable {
    ...
    //MTK-START to support CA's Component Carrier Count
    private int mCaDlCc = 0;
    private int mCaUlCc = 0;
    ...
    public void setProprietaryDataRadioTechnology(int rt) {
        if (rt > ServiceState.RIL_RADIO_TECHNOLOGY_MTK) {
            ...
            // if rt == LTEA, dl/ul component carrier(CC) count may be inserted at higher bits
            // ex: 0x23...8B    2 is dl CC count, 3 is ul, 8B is LTEA
            if ((rt & ServiceState.RIL_RADIO_TECHNOLOGY_LTEA) != 0) {
                mCaDlCc = (rt >> ((Integer.SIZE) - 4)) & 0xf;
                mCaUlCc = (rt >> ((Integer.SIZE) - 8)) & 0xf;
                // clear the higher bits
                rt &= ~(mCaDlCc << ((Integer.SIZE) - 4));
                rt &= ~(mCaUlCc << ((Integer.SIZE) - 8));
                if (DBG) Rlog.d(LOG_TAG, "ca speed: " + mCaDlCc + "/" + mCaUlCc + "/" + rt);
            }
