...
public class SignalStrength implements Parcelable {
    ...
    public int getLevel() {
		...
		// prize liguizeng - 2015-11-4 - set lte as the standard of signal strength
		int lteLevel = getLteLevel();
		//level = level > lteLevel ? level : lteLevel;
        return level;
    }
    ...
    public int getCdmaLevel() {
		...
		if (cdmaDbm >= -94) levelDbm = SIGNAL_STRENGTH_GREAT;
        else if (cdmaDbm >= -100) levelDbm = SIGNAL_STRENGTH_GOOD;
        else if (cdmaDbm >= -106) levelDbm = SIGNAL_STRENGTH_MODERATE;
        else if (cdmaDbm >= -113) levelDbm = SIGNAL_STRENGTH_POOR;
	
		/** prize liguizeng - 2015-11-4 - end*/
    ...
    public int getEvdoLevel() {
		...
		if (evdoDbm >= -91) levelEvdoDbm = SIGNAL_STRENGTH_GREAT;
        else if (evdoDbm >= -97) levelEvdoDbm = SIGNAL_STRENGTH_GOOD;
        else if (evdoDbm >= -110) levelEvdoDbm = SIGNAL_STRENGTH_MODERATE;
        else if (evdoDbm >= -114) levelEvdoDbm = SIGNAL_STRENGTH_POOR;
...
/*** prize liguizeng - 2015-11-4  - only consider dDm  */
        if (mLteRsrp > -44) {
            rsrpIconLevel = -1;
        } else if (mLteRsrp >= -104) {
            rsrpIconLevel = SIGNAL_STRENGTH_GREAT;
        } else if (mLteRsrp >= -112) {
            rsrpIconLevel = SIGNAL_STRENGTH_GOOD;
        } else if (mLteRsrp >= -120) {
            rsrpIconLevel = SIGNAL_STRENGTH_MODERATE;
        } else if (mLteRsrp >= -127) {
    ...
    public int getTdScdmaLevel() {
        ...
        /*priz-gxy-modifiy Signal optimization----start*/
        if ((tdScdmaDbm > -25) || (tdScdmaDbm == SignalStrength.INVALID))
               level = SIGNAL_STRENGTH_NONE_OR_UNKNOWN;
        else if (tdScdmaDbm >= -91) level = SIGNAL_STRENGTH_GREAT;
        else if (tdScdmaDbm >= -97) level = SIGNAL_STRENGTH_GOOD;
        else if (tdScdmaDbm >= -110) level = SIGNAL_STRENGTH_MODERATE;
        else if (tdScdmaDbm >= -114) level = SIGNAL_STRENGTH_POOR;
        else level = SIGNAL_STRENGTH_NONE_OR_UNKNOWN;
        /*priz-gxy-modifiy Signal optimization----end*/
