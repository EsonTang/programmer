...
public class FontListParser {
    ...
    private static Family readFamily(XmlPullParser parser)
        ...
        String path = android.os.SystemProperties.get("persist.sys.founder.fontpath", "system_fonts");
        boolean bool = false;
        ...
        while (parser.next() != XmlPullParser.END_TAG) {
            ...
            if (tag.equals("font")) {
            	///================founder font add start================
                if(bool && ("sans-serif".equals(name) || "sans-serif-condensed".equals(name))) {
                    lang = "zh-Hans";
                    hasChange = true;
                }
                //fonts.add(readFont(parser));      ///====original code
                fonts.add(readFont(parser, hasChange, path));
                hasChange = false;
                ///================founder font add end================
            } else {
                skip(parser);
    ...
    //private static Font readFont(XmlPullParser parser)		///====original code
    private static Font readFont(XmlPullParser parser, boolean change, String changePath)
            throws XmlPullParserException, IOException {
