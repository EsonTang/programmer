...
class InteractionController {
    ...
    /** M: change uiautomator inject touch move event to 100Hz @{ */
    private static final long TOUCH_MOVE_EVENT_INTERVAL = 10; // 100 Hz ==> 10ms
    /** @} */
    ...
    public boolean swipe(int downX, int downY, int upX, int upY, int steps, boolean drag) {
        ...
        for(int i = 1; i < swipeSteps; i++) {
            /** M: change uiautomator inject touch move event to 100Hz @{ */
            long eventInjectStartTime = SystemClock.uptimeMillis();
            /** @} */
            ret &= touchMove(downX + (int)(xStep * i), downY + (int)(yStep * i));
            /** M: change uiautomator inject touch move event to 100Hz @{ */
            long eventInjectExpiredTime = SystemClock.uptimeMillis() - eventInjectStartTime;
            if (DEBUG) {
                Log.i(LOG_TAG, "inject touch move event spent time : "  + eventInjectExpiredTime);
            }
            /** @} */
            if(ret == false)
                break;
            ...
            /** M: change uiautomator inject touch move event to 100Hz @{ */
            // 10ms between events
            long sleep_time =  TOUCH_MOVE_EVENT_INTERVAL - eventInjectExpiredTime;
            if(sleep_time > 0 ){
                if (DEBUG) {
                    Log.i(LOG_TAG, "inject event sleep  : "  + sleep_time + " ms ");
                }
                SystemClock.sleep(sleep_time);
            }else{
                if (DEBUG) {
                    Log.i(LOG_TAG, "inject event spent time : "  + eventInjectExpiredTime
                          + " > 10 ms , no sleep");
                }
            }
            /** @} */
    ...
    /** M: change uiautomator inject touch move event to 100Hz @{ */
    private boolean injectEventAsync(InputEvent event) {
        return mUiAutomatorBridge.injectInputEvent(event, false);
    }
    /** @} */
