...
public class SoterUtil {
    ...
    public static SoterRSAKeyGenParameterSpec convertKeyNameToParameterSpec(String name) {
        ...
        //Log.i(TAG, "hy: spec: " + spec.toString());  //Microtrust-Delete wechat Sensitive Information;
        return spec;
    }
    ...
    public static String getPureKeyAliasFromKeyName(String name) {
        //Log.i(TAG, "hy: retrieving pure name from: " + name); //Microtrust-Delete wechat Sensitive Information;
        if(isNullOrNil(name)) {
        ...
        String[] splits = name.split("\\.");
        if(splits == null || splits.length <= 1) {
            //Log.d(TAG, "hy: pure alias");  //Microtrust-Delete wechat Sensitive Information;
            return name;
        }
        return splits[0];
    }
    ...
    public static byte[] getDataFromRaw(byte[] origin, String jsonKey) throws JSONException {
        ...
        if (jsonObj != null && jsonObj.has(jsonKey)) {
			...
			//Log.d("Soter", "base64 encoded public key: " + base64pubkey);  //Microtrust-Delete wechat Sensitive Information;
			
			//filter head and tail and lf
			String pureBase64pubkey = base64pubkey.replace("-----BEGIN PUBLIC KEY-----", "").replace("-----END PUBLIC KEY-----", "").replace("\\n","");
			//Log.d("Soter", "pure base64 encoded public key: " + pureBase64pubkey);  //Microtrust-Delete wechat Sensitive Information;
    ...
    private static JSONObject retriveJsonFromExportedData(byte[] origin) {
		...
		//Log.d("Soter", "parsed raw length: " + rawLength);  //Microtrust-Delete wechat Sensitive Information;
        ...
        //Log.d("Soter", "to convert json: " + jsonStr);  //Microtrust-Delete wechat Sensitive Information;
        try {
