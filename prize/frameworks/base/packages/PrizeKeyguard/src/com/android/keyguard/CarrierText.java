...
/**PRIZE import package liyao 2015-05-28 start*/
import android.os.AsyncTask;
import android.provider.Settings;
/**PRIZE import package liyao 2015-05-28 end*/
public class CarrierText extends TextView {
    ...
    /*PRIZE--show Airplane mode on the lockscreen-liyao-2015-08-04-start */
    private boolean mAirPlaneModeEnabled = false;
    /*PRIZE--show Airplane mode on the lockscreen-liyao-2015-08-04-end */
    ...
    protected void updateCarrierText() {
        /*PRIZE--show Airplane mode on the lockscreen-liyao-2015-07-16-start */
        //PRIZE-liyao-first boot,mAirPlaneModeEnabled is false,can't get the correct Mode
        /**prize delete by xiarui hide carrier text ued for bug#43621 2017-12-06 start**/
        //boolean isAirplaneModeOn = mAirPlaneModeEnabled ? mAirPlaneModeEnabled : KeyguardUtils.isAirplaneModeOn(mContext);
        //Log.d(TAG,"updateCarrierText isAirplaneModeOn "+isAirplaneModeOn);
        //if(isAirplaneModeOn ){
        //    setText(R.string.carrier_text_airplane);
        //    setVisibility(View.VISIBLE);
        //    return;
        //}
        /**prize delete by xiarui hide carrier text ued for bug#43621 2017-12-06 end**/
        /*PRIZE--show Airplane mode on the lockscreen-liyao-2015-07-16-end */
        ...
        for (int i = 0 ; i < mNumOfPhone ; i++) {
            ...
            if (mCarrierNeedToShow[i] && (mCarrier[i] != null)) {
                /**-- PRIZE-show ZhongGuoYiDong when mCarrier[i] contains CMCC-liyao-2015-05-15-start */
                if(mCarrier[i].contains("cmcc") || mCarrier[i].contains("CMCC")){
                     mCarrier[i] = getContext().getString(R.string.carrier_text_cmcc);
                } 
                /**-- PRIZE-show ZhongGuoYiDong when mCarrier[i] contains CMCC-liyao-2015-05-15-end */
                /**PRIZE-Bug 2101-show CUCC on the lockscreen-liyao-2015-07-02-start   */          
                else if(mCarrier[i].contains("cu") || mCarrier[i].contains("CU")){
                    mCarrier[i] = getContext().getString(R.string.carrier_text_cucc);
                }
                /**PRIZE-Bug 2101-show CUCC on the lockscreen-liyao-2015-07-02-end */
     ...
     */
    /**PRIZE refresh the Dataconnection State when there is no SIM card or have two SIM card(bug 1468) liyao 2015-05-28 start*/
    private CharSequence getCarrierTextForSimState(final int phoneId,final IccCardConstants.State simState,
    /**PRIZE refresh the Dataconnection State when there is no SIM card or have two SIM card(bug 1468) liyao 2015-05-28 end*/
        ...
        AsyncTask.execute(new Runnable() {
            ...
            public void run() {
                switch (simState) {
                    case READY:
                        Settings.Global.putInt(mContext.getContentResolver(),phoneId+"_slot_ready" ,1);
                        return;
                    case ABSENT:
                    case NETWORK_LOCKED:
                    case NOT_READY:
                    ...
                    case PERM_DISABLED:
                    case UNKNOWN:
                        Settings.Global.putInt(mContext.getContentResolver(),phoneId+"_slot_ready" ,0);
                        return;
                }
