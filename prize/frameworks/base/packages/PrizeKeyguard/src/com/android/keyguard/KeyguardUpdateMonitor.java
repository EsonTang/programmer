...
import com.prize.faceunlock.service.FaceUnlockCallback;
import com.prize.faceunlock.service.FaceVerifyManager;
import com.mediatek.common.prizeoption.PrizeOption;
//2018-04-09 liuwei add for faceid end

/*prize add for faceid can use while sim pin lock 2018-04-25 begin */
import com.android.keyguard.KeyguardSecurityModel.SecurityMode;
/*prize add for faceid can use while sim pin lock 2018-04-25 end */
...
public class KeyguardUpdateMonitor implements TrustManager.TrustListener {
    ...
    /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-start*/
    private static final int MSG_DO_FINGERPRING_HELP =400;	
    /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-end*/
    ...
    private FaceUnlockCallback mFaceUnlockCallback = new FaceUnlockCallback()
    {
        ...
        @Override
        public void onFaceVerifyChanged(int resultCode, String msg)
        {
             Log.i(TAG, "lava_faceunlock  onFaceVerifyChanged resultCode = " + resultCode + " , msg = " + msg);
             //handleFaceUnlockResult(resultCode,msg);
             handleFaceUnlockResult(resultCode,msg);
        }
    ...
    private void handleFaceUnlockResult(int resultCode, String msg)
    {
        Log.i(TAG, "lava_faceunlock  startLavaFaceUnlock start" );
        if(resultCode == 0)
        {
            mFingerprintAuthCallback.onFingerprintAuthenticated(0);
        }
        Log.i(TAG, "lava_faceunlock  handleFaceUnlock end" );
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_USER_UNLOCKED:
                    ...
                    break;
		  /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-start*/
		  case MSG_DO_FINGERPRING_HELP:		  	
		  	prizeHandleFingerprintHelp(msg.arg1, (String)msg.obj);
		  	break;
		  /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-end*/
            }
        }
    ...
    private void onFingerprintAuthenticated(int userId) {
	 ...
	 /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-start*/
	 if(mFingerprintAuthCallback != null)mFingerprintAuthCallback.onFingerprintAuthenticated(userId);
	 /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-end*/
        for (int i = 0; i < mCallbacks.size(); i++) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
            if (cb != null) {
    ...
    private void handleFingerprintAuthFailed() {
	 /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-start*/
	 if(mFingerprintAuthCallback != null)mFingerprintAuthCallback.onFingerprintAuthFailed();
	 /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-end*/
        for (int i = 0; i < mCallbacks.size(); i++) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
            if (cb != null) {
    ...
    private void handleFingerprintAcquired(int acquireInfo) {
	 ...
	 /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-start*/
	 if(mFingerprintAuthCallback != null)mFingerprintAuthCallback.onFingerprintAcquired();
	 /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-end*/
        for (int i = 0; i < mCallbacks.size(); i++) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
            if (cb != null) {
    ...
    /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-start*/
    private void prizeHandleFingerprintHelp(int msgId, String helpString)
    {
        for (int i = 0; i < mCallbacks.size(); i++) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
            if (cb != null) {
    ...
    private void handleFingerprintHelp(int msgId, String helpString) {
	 /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-start*/
	 Log.d(TAG,"handleFingerprintHelp msgId:"+msgId);
        if(mFingerprintAuthCallback != null)mFingerprintAuthCallback.onFingerprintHelp(msgId, helpString);
	 mHandler.sendMessage(mHandler.obtainMessage(MSG_DO_FINGERPRING_HELP, msgId,0, helpString));
	 /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-end*/
        /*for (int i = 0; i < mCallbacks.size(); i++) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
            if (cb != null) {
    ...
    private void handleFingerprintError(int msgId, String errString) {
	 ...
	 /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-start*/
	 if(mFingerprintAuthCallback != null)mFingerprintAuthCallback.onFingerprintError(msgId, errString);
	 /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-end*/
        for (int i = 0; i < mCallbacks.size(); i++) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
            if (cb != null) {
    ...
    private void handleFingerprintLockoutReset() {
	 ...
	 if(mFingerprintAuthCallback  != null)
	 {
	     mFingerprintAuthCallback.handleFingerprintLockoutReset();
	 }	 
    ...
    /*prize-add by lihuangyuan,for too many try times lock finger-2017-09-04-start*/
    private LockPatternUtils mLockPatternUtils;
    /*prize-add by lihuangyuan,for too many try times lock finger-2017-09-04-end*/
    public boolean isUnlockingWithFingerprintAllowed() {
        /*return mStrongAuthTracker.isUnlockingWithFingerprintAllowed()
                && !hasFingerprintUnlockTimedOut(sCurrentUser);*/
	 /*prize-add by lihuangyuan,for too many try times lock finger-2017-09-04-start*/
	 return (mLockPatternUtils.getLockoutAttemptDeadline(getCurrentUser()) == 0L);
        /*prize-add by lihuangyuan,for too many try times lock finger-2017-09-04-end*/
    ...
    //add for systemui alarm-liufan-2018-4-2-start
    private PendingIntent mSender;
    //add for systemui alarm-liufan-2018-4-2-end
    ...
    /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-start*/
    public void setFingerprintAuthCallback(KeyguardUpdateMonitorCallback callback)
    {
        mFingerprintAuthCallback = callback;
    }
    private KeyguardUpdateMonitorCallback mFingerprintAuthCallback;
    /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-end*/
    ...
    protected void handleStartedGoingToSleep(int arg1) {
	 ...
	 /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-start*/
	 if(mFingerprintAuthCallback != null)mFingerprintAuthCallback.onStartedGoingToSleep(arg1);
	 /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-end*/
        final int count = mCallbacks.size();
        for (int i = 0; i < count; i++) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
    ...
    protected void handleFinishedGoingToSleep(int arg1) {
        mGoingToSleep = false;
	 /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-start*/
	 if(mFingerprintAuthCallback != null)mFingerprintAuthCallback.onFinishedGoingToSleep(arg1);
	 /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-end*/
        final int count = mCallbacks.size();
        for (int i = 0; i < count; i++) {
            KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
    ...
    private KeyguardUpdateMonitor(Context context) {
	 ...
	 /*prize-add by lihuangyuan,for too many try times lock finger-2017-09-04-end*/
		...
		//2018-04-09 liuwei add for faceid.
        mFaceVerifyManager = new FaceVerifyManager(context);
        mFaceVerifyManager.setFaceUnlockCallback(mFaceUnlockCallback);
        mAM = (ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE);
		//2018-04-09 liuwei add for faceid end.

    }
    /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-start*/
    public void stopFingerprintListeningState()
    {
    ...
    private void updateFingerprintListeningState() {
        ...
        Log.v(TAG,"updateFingerprintListeningState  mFingerprintRunningState:"+mFingerprintRunningState+",shouldListenForFingerprint:"+shouldListenForFingerprint);
    ...
    private boolean shouldListenForFingerprint() {
	 //add moccluded by lihuangyuan
    /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-start*/
	 Log.v(TAG,"shouldListenForFingerprint mKeyguardIsVisible:"+mKeyguardIsVisible+",mBouncer:"+mBouncer+",mFingerprintAlreadyAuthenticated:"+mFingerprintAlreadyAuthenticated);
        return (mKeyguardIsVisible /*|| !mDeviceInteractive*/ || mBouncer || mGoingToSleep||moccluded)
...
/**    private void handleSimStateChange(int subId, int slotId, State state) {
    ...
    private void handleSimStateChange(SimData simArgs) {
        ...
        if (state != IccCardConstants.State.UNKNOWN &&
            ...
            printState() ;
            /*prize-add by lihuangyuan-for update fingerprint listening state when unlock SIM lock-2017-08-03-start*/
            if(state == IccCardConstants.State.READY){
                   updateFingerprintListeningState();
            }
            /*prize-add by lihuangyuan-for update fingerprint listening state when unlock SIM lock-2017-08-03-end*/
            for (int i = 0; i < mCallbacks.size(); i++) {
                KeyguardUpdateMonitorCallback cb = mCallbacks.get(i).get();
                if (cb != null) {
...
/*        int resultId = SubscriptionManager.INVALID_SUBSCRIPTION_ID;
    ...
    */
    ...
    public void nofityPowerKeyPower(boolean interactive) {
        if(!interactive && isEnableFaceUnlock() && !isPinOrPukLock()){
            Log.i(TAG, "faceunlock  startLavaFaceUnlock start" );
            startLavaFaceUnlock();
            return;
        }
    ...
    private boolean isPinOrPukLock(){
        if(mSecurityModel == null) return false;
    ...
    public void setKeyguardSecurityModel(KeyguardSecurityModel model){
        this.mSecurityModel = model;
    }

    private KeyguardSecurityModel mSecurityModel;
    private boolean isEnableFaceUnlock(){
        return PrizeOption.PRIZE_FACE_ID 
        && isFaceUnlockOn()
        && isLockSecurity() 
        && !(mAM.getLockTaskModeState() == ActivityManager.LOCK_TASK_MODE_PINNED);
    }

    private boolean isFaceUnlockOn(){
        int on = 0;
        on = Settings.System.getInt(mContext.getContentResolver(), Settings.System.PRIZE_FACEID_SWITCH,0);
        return on ==1;
    }

    private void startLavaFaceUnlock()
    {
        Log.i(TAG, "faceunlock  startLavaFaceUnlock start" );
        if(mFaceVerifyManager != null)
        {
            mFaceVerifyManager.bindFaceVerifyService();
        }
        Log.i(TAG, "faceunlock  startLavaFaceUnlock end" );
    }
    private void unbindFaceVerifyService()
    {
        Log.i(TAG, "faceunlock  unbindFaceVerifyService start" );
        if(mFaceVerifyManager != null)
        {
            mFaceVerifyManager.unbindFaceVerifyService();
        }
        Log.i(TAG, "faceunlock  unbindFaceVerifyService end" );
    }
    
    public void stopFaceUnlock()
    {
        Log.i(TAG, "faceunlock  stopLavaFaceUnlock start" );
        if(mFaceVerifyManager != null)
        {
            mFaceVerifyManager.stopFaceVerify();
        }
        Log.i(TAG, "faceunlock  stopLavaFaceUnlock end" );
    }

    public boolean isLockSecurity(){
       boolean isSecure = false;
       if(null != mLockPatternUtils){
           isSecure = mLockPatternUtils.isSecure(sCurrentUser);
       }
       Log.w(TAG,"the current lock is secure " +isSecure);
       return isSecure;
    }
	//2018-04-09 liuwei add for faceid end
