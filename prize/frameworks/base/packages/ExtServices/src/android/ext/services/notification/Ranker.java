...
/* app multi instances feature. prize-linkh-20161102 */
...
public final class Ranker extends NotificationRankerService {
    ...
    /* app multi instances feature. prize-linkh-20161102 */
    // the key of SparseArray is app inst index. and its value must be greater than zero.
    SparseArray<Map<Integer, Map<String, LinkedHashSet<String>>>> mUnbundledNotificationsForAppInst;
        ...
        try {
            ...
            if (!sbn.isAppGroup()) {
                ...
                synchronized (mUnbundledNotifications) {
                    ...
                    /* app multi instances feature. prize-linkh-20161102 */
                    if (PrizeOption.PRIZE_APP_MULTI_INSTANCES && sbn.getAppInstanceIndex() > 0) {
                        if (DEBUG) {
                            Log.i(TAG, "onNotificationPosted() A notification from App Inst " + sbn.getAppInstanceIndex()
                                + ": " + sbn);
                        }

                        Map<Integer, Map<String, LinkedHashSet<String>>> unbundledNotificationsByAppInst =
                            mUnbundledNotificationsForAppInst.get(sbn.getAppInstanceIndex());
                        if (unbundledNotificationsByAppInst == null) {
                            unbundledNotificationsByAppInst = new HashMap<>();
                            mUnbundledNotificationsForAppInst.put(sbn.getAppInstanceIndex(), unbundledNotificationsByAppInst);
                        }
                        unbundledNotificationsByUser = unbundledNotificationsByAppInst.get(sbn.getUserId());
                        if (unbundledNotificationsByUser == null) {
                            unbundledNotificationsByUser = new HashMap<>();
                        }
                        unbundledNotificationsByAppInst.put(sbn.getUserId(), unbundledNotificationsByUser);                        
                    } else {
                        if (unbundledNotificationsByUser == null) {
                            unbundledNotificationsByUser = new HashMap<>();
                        }
    ...
    private void maybeUnbundle(StatusBarNotification sbn, boolean notificationGone, int user) {
        ...
        synchronized (mUnbundledNotifications) {
            ...
            if (PrizeOption.PRIZE_APP_MULTI_INSTANCES && sbn.getAppInstanceIndex() > 0) {
                if (DEBUG) {
                    Log.i(TAG, "maybeUnbundle() A notification from App Inst " + sbn.getAppInstanceIndex()
                        + ": " + sbn);
                }

                unbundledNotificationsByUser = null;
                Map<Integer, Map<String, LinkedHashSet<String>>> unbundledNotificationsByAppInst =
                        mUnbundledNotificationsForAppInst.get(sbn.getAppInstanceIndex());
                if (unbundledNotificationsByAppInst != null) {
                    unbundledNotificationsByUser = unbundledNotificationsByAppInst.get(sbn.getUserId());
    ...
    public void onListenerConnected() {
        ...
        if (PrizeOption.PRIZE_APP_MULTI_INSTANCES) {
            mUnbundledNotificationsForAppInst = new SparseArray<>();
        } //END...        
