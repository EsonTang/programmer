...
public class RecentsImpl implements ActivityOptions.OnAnimationFinishedListener {
    ...
    /* prize add for dialog display exception liuwei 2018-04-18 begin*/
    private final static boolean prize_disable_recentAnimation = true; 
    /* prize add for dialog display exception liuwei 2018-04-18 end*/
    ...
    public RecentsImpl(Context context) {
        ...
        if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
            mHeaderBar = (TaskViewHeader) inflater.inflate(R.layout.prize_recents_task_view_header,
                    null, false);
        } else {
    ...
    public void toggleRecents(int growTarget) {
        ...
        try {
            ...
            if (ssp.isRecentsActivityVisible(isHomeStackVisible)) {
                ...
                if (!launchState.launchedWithAltTab && !PrizeOption.PRIZE_SYSTEMUI_RECENTS) { //prize modify by xiarui for Bug#43898 2017-12-01
                    ...
                    LogUtils.d("xx", "ToggleRecentsEvent");
                ...
                /*prize modify by xiarui for Bug#43222、Bug#43364 2017-11-27 start*/
                if (runningTask != null &&
                        (SystemServicesProxy.sRecentsBlacklist.contains(runningTask.topActivity.getClassName()) ||
                                SystemServicesProxy.sRecentsBlacklist.contains(runningTask.topActivity.getPackageName()))) {
                    startRecentsActivity(runningTask, isHomeStackVisible.value, false /* animate */,
                            growTarget);
                } else {
                    startRecentsActivity(runningTask, isHomeStackVisible.value, true /* animate */,
                            growTarget);
                }
                /*prize modify by xiarui for Bug#43222、Bug#43364  2017-11-27 end*/
    ...
    public void qqDockTopTask(int topTaskId, int dragMode,
            int stackCreateMode, Rect initialBounds, int typeQM) {
        SystemServicesProxy ssp = Recents.getSystemServices();

        Log.d(TAG, "QQdockedTopTaskEvent ---moveTaskToDockedStack" ); 
        // Make sure we inform DividerView before we actually start the activity so we can change
        // the resize mode already.
        ssp.QQmoveTaskToDockedStack(topTaskId, stackCreateMode, initialBounds);
        /*
        if (ssp.moveTaskToDockedStack(topTaskId, stackCreateMode, initialBounds)) {
            Log.d(TAG, "QQdockedTopTaskEvent ---send BusEvent" ); 
            //EventBus.getDefault().send(new PrizeOpenPendingIntentEvent(typeQM));
            EventBus.getDefault().send(new QQdockedTopTaskEvent(dragMode, initialBounds));
        }*/
    }
    ...
    private ActivityOptions getThumbnailTransitionActivityOptions(
        ...
        if (runningTask != null && runningTask.stackId == FREEFORM_WORKSPACE_STACK_ID) {
            ...
            if (thumbnail != null) {
				...
				/*prize modify by xiarui {fix Animation up then down} 2017-11-18 start*/
				if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
	                return ActivityOptions.makeThumbnailAspectScaleDownAnimation(mDummyStackView,
	                        thumbnail, (int) toTaskRect.left, (int) toTaskRect.top + mTaskBarHeight,
	                        (int) toTaskRect.width(), (int) toTaskRect.height() - mTaskBarHeight, mHandler, null);
				} else {
	                return ActivityOptions.makeThumbnailAspectScaleDownAnimation(mDummyStackView,
	                        thumbnail, (int) toTaskRect.left, (int) toTaskRect.top,
	                        (int) toTaskRect.width(), (int) toTaskRect.height(), mHandler, null);				
				}
				/*prize modify by xiarui {fix Animation up then down} 2017-11-18 end*/
            }
        ...
        // /@}
            synchronized (mHeaderBarLock) {
                ...
                if (RecentsDebugFlags.Static.EnableTransitionThumbnailDebugMode) {
                    ...
                    /*prize delete by xiarui 2017-11-18 start*/
					if (!PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
                    ...
                    /*prize delete by xiarui 2017-11-18 end*/
                }
            }
    ...
    protected void startRecentsActivity(ActivityManager.RunningTaskInfo runningTask,
        ...
        /* prize modify for dialog display exception liuwei 2018-04-18 begin*/
        if (prize_disable_recentAnimation || !animate) {
        //if (prize_disable_recentAnimation || !animate) {
        /* prize modify for dialog display exception liuwei 2018-04-18 end*/
        ...
        LogUtils.d("xx", "startRecentsActivity opts = " + opts.toString());
    ...
    private void startRecentsActivity(ActivityOptions opts) {
		...
		try {
            SystemServicesProxy ssp = Recents.getSystemServices();
			ActivityManager.RunningTaskInfo runningTask = ssp.getRunningTask();
			String topActivity = runningTask.topActivity.getClassName();
			Log.d(TAG, "topActivity:"+topActivity+",camera_capturing:"+Settings.System.getInt(mContext.getContentResolver(), "camera_capturing", 0));
			if(topActivity.equals("com.android.camera.CameraLauncher") && Settings.System.getInt(mContext.getContentResolver(), "camera_capturing", 0) == 1){
					Log.w(TAG, "camera is capturing , do not exec startRecentsActivity");
					return;
			}
		} catch (Exception e) {
            Log.e(TAG, "Failed to startRecentsActivity:", e);
        }
		/*prize-xuchunming-20180411-bugid:50825-end*/
