...
/*PRIZE-import pkg-liufan-2015-12-14-start*/
import com.android.systemui.statusbar.StatusBarState;
/*PRIZE-import pkg-liufan-2015-12-14-end*/
...
public class KeyguardViewMediator extends SystemUI {
    ...
    private static final int FINGERPRINT_HIDE_KEYGUARD = 100;
    ...
    private void sendPowerKeyScreenOnBroadcast(int screenonoff)
    {
        ...
        //mContext.startService(intent);  //ContextImpl.java will call Debug.getCallers
        mContext.startServiceAsUser(intent, UserHandle.CURRENT);//liyongli 20180402 replace up line
    }
        ...
        public int getBouncerPromptReason() {
            ...
            if (any && !strongAuthTracker.hasUserAuthenticatedSinceBoot()) {
                return KeyguardSecurityView.PROMPT_REASON_RESTART;
                 ...
                 //modify by lihuangyuan,for enable finger always-2017-12-12
                 return KeyguardSecurityView.PROMPT_REASON_RESTART;
    ...
    public void onSystemReady() {
        ...
        synchronized (this) {
            ...
            if(mAsyncHandler == null){
                HandlerThread mHandlerThread = new HandlerThread("AsyncThread");
    ...
    public void onStartedGoingToSleep(int why) {
        ...
        synchronized (this) {
            ...
            if (mPendingLock) {
		  if(why != WindowManagerPolicy.OFF_BECAUSE_OF_TIMEOUT)//prize-add by lihuangyuan,for timeout donot playsound-2018-03-07
                playSounds(true);
            }
        }
    ...
    /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-start*/
    private int mIsFingerWakeup = 0;
    public void setFingerWakeup(int isFingerWakeup)
    {
        //remove last sleep somthing
        if(isFingerWakeup == 1)mHandler.removeMessages(RESET);
        mIsFingerWakeup = isFingerWakeup;
	 mStatusBarKeyguardViewManager.setFingerWakeup(mIsFingerWakeup);
    }
    public int isFingerWakeup()
    {
        return mIsFingerWakeup;
    }
    /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-end*/
    /**
    ...
    private void handleSetOccluded(boolean isOccluded, boolean animate) {
        ...
        synchronized (KeyguardViewMediator.this) {
            // M: ALPS03287480 customrization WFD design when is occluded
            if (isOccluded) {
                if ("1".equals(SystemProperties.get("persist.wfd_when_occluded", "0"))
                        && mLockPatternUtils.isInCall()) {
                    mKeyguardDisplayManager.hide();
                }
                if (mHiding) {
    ...
    private void resetStateLocked() {
	 ...
	 if(isFingerWakeup() == 1)
	 {
	     Log.i(TAG,"resetStateLocked so late, isFingerWakeup:"+isFingerWakeup()+",discard it!");
	     return;
	 }
	 /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-end*/
	...
	/*ScrimController scrimCtrol = mStatusBarKeyguardViewManager.getStatusBar().getScrimController();
	scrimCtrol.setBouncerColor(0);*/
	/*PRIZE-when unlock keguard appear splash screen bugId17935-dengyu-2016-7-6-end*/
        ...
        if(PrizeOption.PRIZE_FACE_ID && isOpen && !(am.getLockTaskModeState() == ActivityManager.LOCK_TASK_MODE_PINNED)) {
    ...
    private Handler mHandler = new Handler(Looper.myLooper(), null, true /*async*/) {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
		   /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-start*/
		  case FINGERPRINT_HIDE_KEYGUARD:
		  	handleHideKeyguard();
		  	break;
		   /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-end*/
                case SHOW:
    ...
    public void handleHideForFingerPrint(){
        handleHide();
    }
    ...
    private void handleHide() {
        ...
        synchronized (KeyguardViewMediator.this) {
            ...
            if (mShowing && !mOccluded) {
                ...
                /*-prize-modify by lihuangyuan,for fingerprint quick unlock-2018-03-05-start*/
                    handleStartKeyguardExitAnimation(SystemClock.uptimeMillis() ,10);
                    /*if(mIsFingerWakeup == 1)
                    {
                     ...
                     {
					    if(PrizeOption.PRIZE_KEYGUARD_UNLOCK_FAST) {//2018-04-02 liyongli add PRIZE_KEYGUARD_UNLOCK_FAST
						    handleStartKeyguardExitAnimation(SystemClock.uptimeMillis(), 0);
						}else{//2018-04-02 liyongli add PRIZE_KEYGUARD_UNLOCK_FAST
                        handleStartKeyguardExitAnimation(SystemClock.uptimeMillis() ,10);
						}
                     }*/
                /*-prize-modify by lihuangyuan,for fingerprint quick unlock-2018-03-05-end*/
                } else {
    ...
    public void nofityPowerKeyPower(boolean interactive) {
        mUpdateMonitor.nofityPowerKeyPower(interactive);
		/*prize-xuchunming-20180413:KeyguradBottomAreaView set faceid info at press powerkey-start*/
        if(mStatusBarKeyguardViewManager != null){
            final boolean interactiveT = interactive;
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    mStatusBarKeyguardViewManager.nofityPowerKeyPower(interactiveT);
                }
            });
        }
		/*prize-xuchunming-20180413:KeyguradBottomAreaView set faceid info at press powerkey-start*/
    ...
    private void handleStartKeyguardExitAnimation(long startTime, long fadeoutDuration) {
        ...
        synchronized (KeyguardViewMediator.this) {
            ...
            //prize -modify by lihuangyuan,for check mShowing must in main thread
            ...
            mWakeAndUnlocking = false;
            setShowingLocked(false);
            /*prize add for ramdom launcher show wallpaper slowly liuwei 20180414 begin*/
            updateActivityLockScreenState();
            /*prize add for ramdom launcher show wallpaper slowly liuwei 20180414 end*/
            mStatusBarKeyguardViewManager.hide(startTime, fadeoutDuration);
            resetKeyguardDonePendingLocked();
            mHideAnimationRun = false;
            /*prize remove for ramdom launcher show wallpaper slowly liuwei 20180414 begin*/
            //updateActivityLockScreenState();
            /*prize remove for ramdom launcher show wallpaper slowly liuwei 20180414 end*/
            adjustStatusBarLocked();
            ...
            /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-start*/
            //if(isFingerWakeup() == 1)
            Log.d(TAG,"handleStartKeyguardExitAnimation isFingerWakeup:"+isFingerWakeup());
            {
               //mPM.turnOnScreenNow();
            }
            ...
            setFingerWakeup(0);
            mFingerprintUnlockControler.clearFingerprintFailTimes();            
            /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-end*/
        }
    ...
    private FingerprintUnlockController mFingerprintUnlockControler = null;
    public void notifiyHideKeyguard()
    {
        //mHandler.sendEmptyMessage(FINGERPRINT_HIDE_KEYGUARD);        
    }
    public void notifiyShowKeyguard()
    {
        //mHandler.sendEmptyMessage(FINGERPRINT_HIDE_KEYGUARD);        
    }
    private void handleHideKeyguard()
    {             
	     if(isFingerWakeup() == 1)
	     {
	         mStatusBarKeyguardViewManager.hideKeyguard();
	     }	    
    }  
    public void handleShowKeyguard()
    {
        mStatusBarKeyguardViewManager.showKeyguard();
    }
    private void showBrouncer()
    {
        Log.d(TAG, "showBrouncer isFingerWakeup:"+isFingerWakeup());
        if(isFingerWakeup() == 3)//show brouncer
	 {
	         setFingerWakeup(0);
		  mHandler.postDelayed(new Runnable()
		 	{
		 	    public void run()
		 	    	{				    
	                        mFingerprintUnlockControler.showBrouncer();
		 	    	}
		 	}, 100);	         		  
	 }
    }
    ...
    /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-start*/
                    if(isFingerWakeup() == 1)
                    	{
                    	    Log.d(TAG, "handleNotifyScreenTurningOn wait keyguardone");
    ...
    public StatusBarKeyguardViewManager registerStatusBar(PhoneStatusBar phoneStatusBar,
	 ...
	 /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-start*/
	 mFingerprintUnlockControler = fingerprintUnlockController;
	 /*-prize-add by lihuangyuan,for fingerprint quick unlock-2018-03-05-end*/
    ...
    private void notifyHasLockscreenWallpaperChanged(boolean hasLockscreenWallpaper) {
     ...
     /* Dynamically hiding nav bar feature. */
    /*PRIZE-refresh virtual key state changed-liufan-2015-12-14-start*/
    private boolean isAllowShowVirtualKey(){
        boolean isAllow = mState == StatusBarState.KEYGUARD && !mStatusBarKeyguardViewManager.isBouncerShowing();
        return !isAllow;
    }

    private int mState = -1;
    private boolean isBouncerShowing = false;
    public void setStatusBarState(int state,boolean isbouncershowing){
        Log.d(TAG, "setStatusBarState() - state = " + state + ", mState = " + mState) ;
        if(mState != state || isBouncerShowing != isbouncershowing){
            mState = state;
            isBouncerShowing = isbouncershowing;
            Log.d(TAG, "setStatusBarState() - set mState = " + mState) ;
            try {
                int size = mKeyguardStateCallbacks.size();
                boolean isAllow = isAllowShowVirtualKey();
                for (int i = 0; i < size; i++) {
                    mKeyguardStateCallbacks.get(i).onAllowShowStateChanged(isAllow);
                }
            } catch (RemoteException e) {
                Slog.w(TAG, "Failed to call onAllowShowStateChanged", e);
            }
        }
    }
    /*PRIZE-refresh virtual key state changed-liufan-2015-12-14-end*/
