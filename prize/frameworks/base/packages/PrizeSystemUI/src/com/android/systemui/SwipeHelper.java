...
/*PRIZE-import package-liufan-2016-05-24-start*/
import com.android.systemui.statusbar.phone.PhoneStatusBar;
/*PRIZE-import package-liufan-2016-05-24-end*/
...
public class SwipeHelper implements Gefingerpoken {
    ...
    private boolean mIsLockDirection = false; //swipe down to lock & unlock task
    private Runnable mRefreshLockUI; //update lock icon
    ...
    /*PRIZE-get NotificationStackScrollLayout obj-liufan-2015-10-10-start*/
    private NotificationStackScrollLayout mNotificationStackScrollLayout;
    public void setNotificationStackScrollLayout(NotificationStackScrollLayout notificationStackScrollLayout){
        this.mNotificationStackScrollLayout = notificationStackScrollLayout;
    }
    /*PRIZE-get NotificationStackScrollLayout obj-liufan-2015-10-10-end*/
    ...
    private View mNotificationScrimView;
    public void setNotificationScrimView(View view){
        mNotificationScrimView = view;
    }

    private void setScrimViewTranslation(float translate) {
        if (mSwipeDirection == X && mNotificationScrimView != null) {
            mNotificationScrimView.setTranslationX(translate);
        }
    }
    ...
    protected void setTranslation(View v, float translate) {
        ...
        if (mSwipeDirection == X) {
            ...
            /*PRIZE-swipe notificationscrimview-liufan-2016-06-18-start*/
            setScrimViewTranslation(translate);
            /*PRIZE-swipe notificationscrimview-liufan-2016-06-18-end*/
        } else {
            v.setTranslationY(translate);
        }
        /*prize add by xiarui 2017-11-09 start*/
        if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
            if (translate > 0) { //down, for lock & unlock task
                mIsLockDirection = true;
            } else { //up, for dismiss task
                mIsLockDirection = false;
            }
        }
        /*prize add by xiarui 2017-11-09 end*/
    }
    ...
    private void updateSwipeProgressFromOffset(View animView, boolean dismissable,
        ...
        if (!mCallback.updateSwipeProgress(animView, dismissable, swipeProgress)) {
            /*PRIZE-cancel the LAYER_TYPE_HARDWARE when bg is color-liufan-2016-05-24-start*/
            if (PhoneStatusBar.BLUR_BG_CONTROL && FADE_OUT_DURING_SWIPE && dismissable) {
            ...
            /*PRIZE-cancel the LAYER_TYPE_HARDWARE when bg is color-liufan-2016-05-24-end*/
        }
            ...
            case MotionEvent.ACTION_MOVE:
                if (mCurrView != null && !mLongPressSent) {
                    ...
                    if (Math.abs(delta) > mPagingTouchSlop
                        ...
                        /*PRIZE-modify overlapping when swip the notification-liufan-2015-9-22-start*/
                        mHandler.removeCallbacks(runnable);
                        NotificationStackScrollLayout.isChildChanged = true;
                        if(this.mNotificationStackScrollLayout!=null){
                            this.mNotificationStackScrollLayout.setAllNotificationRowBg();
                        }
                        /*PRIZE-modify overlapping when swip the notification-liufan-2015-9-22-end*/
                    }
                }
                break;
    ...
    Runnable runnable = new Runnable() {
        ...
        public void run() {
            ...
            if(mNotificationStackScrollLayout!=null){
                mNotificationStackScrollLayout.cancelAllNotificationRowBg();
            }
        }
    };
    /*PRIZE-modify overlapping when swip the notification-liufan-2015-9-22-end*/
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                if (!handleUpEvent(ev, mCurrView, velocity, getTranslation(mCurrView))) {
                    /*prize add by xiarui 2017-11-09 start*/
                    if (PrizeOption.PRIZE_SYSTEMUI_RECENTS && mIsLockDirection) {
                        final TaskView tv = (TaskView) mCurrView;
                        final Task mTask = tv.getTask();
                        mRefreshLockUI = new Runnable() {
                            @Override
                            public void run() {
                                tv.setLockLayoutVisible(mTask.isLock);
                            }
                        };
                        if(mTask != null){
                            new Thread(new Runnable() {
                                ...
                                public void run() {
                                    if (mTask.isLock) {
                                        mTask.setLock(mContext, false);
                                    } else {
                                        mTask.setLock(mContext, true);
                                    }
                                    mHandler.post(mRefreshLockUI);
                                }
                            }).start();
                        }
                        // snappity
