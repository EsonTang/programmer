...
public class Utilities {
    ...
    /*prize add Get from android M by xiarui 2017-11-09 start*/
    // These are all going to change
    static final float StackPeekMinScale = 0.98f; // The min scale of the last card in the peek area
    /** Converts from the progress along the curve to a scale. */
    public static float curveProgressToScale(float p) {
    ...
    static final float StackPeekMinAlpha = 1.0f;
    public static float curveProgressToAlpha(float p) {
        if (p < 0) return StackPeekMinAlpha;
        if (p >= 1) return 0f;
        return StackPeekMinAlpha;
    }
    /*prize add Get from android M by xiarui 2017-11-09 end*/
