...
/*PRIZE-import package- liufan-2015-05-21-start*/
import com.android.systemui.statusbar.phone.FeatureOption;
import android.content.ComponentName;
import android.util.Log;
import com.mediatek.storage.StorageManagerEx;
/*PRIZE-import package- liufan-2015-05-21-end*/

// Nav bar color customized feature. prize-linkh-20171027 @{
...
class SaveImageInBackgroundTask extends AsyncTask<Void, Void, Void> {
    ...
    SaveImageInBackgroundTask(Context context, SaveImageInBackgroundData data,
        ...
        /**PRIZE-Bug 478-Stored in the default memory -liyao-2015-05-21 start */
    	...
    	Log.d(TAG,"SaveImageInBackgroundTask getDefaultPath() "+StorageManagerEx.getDefaultPath()+" getExternalStoragePath():" +StorageManagerEx.getExternalStoragePath()
                    +" getInternalStoragePath():"+StorageManagerEx.getInternalStoragePath());
        Log.d(TAG,"SaveImageInBackgroundTask mScreenshotDir "+mScreenshotDir);
        /**PRIZE-Bug 478-Stored in the default memory-liyao-2015-05-21 end */
...
class GlobalScreenshot {
    ...
    public GlobalScreenshot(Context context) {
        ...
        if(PrizeOption.PRIZE_NAVBAR_COLOR_CUST) {
            // This window doesn't need focus. See bug-41254
            mWindowLayoutParams.flags |= WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE;
            Log.d("GlobalScreenshot", "add FLAG_NOT_FOCUSABLE flag");
        } // @}  
    ...
    private void saveScreenshotInWorkerThread(Runnable finisher) {
        ...
        if(PrizeOption.PRIZE_NEW_FLOAT_WINDOW){
        	...
        	Log.d("snail", "---PrizeSystemUi-----GlobalScreenshot.class----mNewOriginalPrizeFloatWindow==="+String.valueOf(mNewOriginalPrizeFloatWindow));
            if(1 == mNewOriginalPrizeFloatWindow){
            	Intent iNewFloatWindow = new Intent("android.intent.action.PRIZE_NEW_FLOAT");
    ...
    private ScreenShotData mCurScreenShotData = null;
    private Handler mHandler = new Handler();
    public void takeScreenshotPublic(Runnable finisher, boolean statusBarVisible, boolean navBarVisible,
            int x, int y, int width, int height)
    {
        boolean enable = false;//SystemProperties.getInt("ro.prize.screen.circlecorner", 0) == 1 ? true : false;        
        if(!enable)        
        {
            takeScreenshot(finisher,statusBarVisible,navBarVisible,x,y,width,height);
            return ;        
        }
        Log.d("screenshot","takeScreenshotPublic w:"+width+",height:"+height);
        if(mCurScreenShotData != null)
        {
            Log.d("screenshot","takeScreenshotPublic mCurScreenShotData is not null");
            return;
        }
        mCurScreenShotData = new ScreenShotData();
        mCurScreenShotData.finisher = finisher;
        mCurScreenShotData.statusBarVisible = statusBarVisible;
        mCurScreenShotData.navBarVisible = navBarVisible;
        mCurScreenShotData.x = x;
        mCurScreenShotData.y = y;
        mCurScreenShotData.width = width;
        mCurScreenShotData.height = height;

        //hide notchscreen
        try
        {
            ActivityManagerNative.getDefault().showNotchScreen(false);
        }
        catch(Exception e){}
        mHandler.postDelayed(new Runnable()
            {
                ...
                public void run()
                {
                    Log.d("screenshot","takeScreenshot start...");
                    takeScreenshot(mCurScreenShotData.finisher,mCurScreenShotData.statusBarVisible,mCurScreenShotData.navBarVisible,
                        mCurScreenShotData.x,mCurScreenShotData.y,mCurScreenShotData.width,mCurScreenShotData.height);
                    mCurScreenShotData = null;
                    Log.d("screenshot","takeScreenshot end...");
                    //show notchscreen
                    try
                    {
                        ActivityManagerNative.getDefault().showNotchScreen(true);
                    }
                    catch(Exception e){}
                }
            }, 300);
    ...
    void takeScreenshot(Runnable finisher, boolean statusBarVisible, boolean navBarVisible,
        ...
        // Take the screenshot
        /*-prize add by lihuangyuan,for notchscreen -2018-04-24-start*/
        mScreenBitmap = SurfaceControl.screenshot((int) dims[0], (int) dims[1],0,312000,false);
        /*-prize add by lihuangyuan,for notchscreen -2018-04-24-end*/
    ...
    void takeScreenshot(Runnable finisher, boolean statusBarVisible, boolean navBarVisible) {
        mDisplay.getRealMetrics(mDisplayMetrics);
        /*-prize add by lihuangyuan,for notchscreen -2018-04-24-start*/
        takeScreenshotPublic(finisher, statusBarVisible, navBarVisible, 0, 0, mDisplayMetrics.widthPixels,
                        ...
                        if (rect != null) {
                            if (rect.width() != 0 && rect.height() != 0) {
                                ...
                                mScreenshotLayout.post(new Runnable() {
                                    public void run() {
                                        /*-prize add by lihuangyuan,for notchscreen -2018-04-24-start*/
                                        takeScreenshotPublic(finisher, statusBarVisible, navBarVisible,
