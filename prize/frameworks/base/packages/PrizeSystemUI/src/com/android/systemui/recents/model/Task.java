...
/**prize-import pkg-liufan-2015-11-17-start*/
import com.android.systemui.statusbar.phone.BlurPic;
import android.graphics.drawable.BitmapDrawable;
/**prize-import pkg-liufan-2015-11-17-end*/
...
public class Task {
    ...
    /*prize-add attributes-liufan-2015-11-13-start*/
    public Drawable launcherIcon;
    public Bitmap copythumbnail;
    public boolean isLock;
    public boolean isEncryption;
    public Bitmap blurScrimeBitmap;
    public int appInstanceIndex;
    /*prize-add attributes-liufan-2015-11-13-end*/
    /**
    ...
    public void copyFrom(Task o) {
        ...
        /*prize-copy attr-liufan-2015-11-26-start*/
        this.isLock = o.isLock;
        this.launcherIcon = o.launcherIcon;
        this.isEncryption = o.isEncryption;
        this.blurScrimeBitmap = o.blurScrimeBitmap;
        /*prize-copy attr-liufan-2015-11-26-end*/
    }
    ...
    public void notifyTaskDataUnloaded(Bitmap defaultThumbnail, Drawable defaultApplicationIcon) {
        ...
        /*prize-recycle the copythumbnail-liufan-2015-11-16-start*/
        if(launcherIcon !=null && launcherIcon instanceof BitmapDrawable){
            BitmapDrawable bd = (BitmapDrawable)launcherIcon;
        ...
        if(this.copythumbnail != null && !this.copythumbnail.isRecycled()){
            this.copythumbnail.recycle();
            this.copythumbnail = null;
        }
        /*prize-recycle the copythumbnail-liufan-2015-11-16-end*/
    }

    /*prize add by xiarui 2018-01-13 start*/
    public boolean isLock(Context context) {
        AppLockManager lockManager = AppLockManager.getInstance(context);
        if (key != null) {
            String packageName = key.getComponent().getPackageName();
            String className = key.getComponent().getClassName();
            if (PrizeOption.PRIZE_APP_MULTI_INSTANCES && PrizeAppInstanceUtils.getInstance(context).supportMultiInstance(packageName)) {
                if (this.appInstanceIndex != 1) {
                    return lockManager.isLockedApp(packageName, key.userId, className);
                } else {
                    return lockManager.isLockedApp(packageName, 1, className);
                }
            } else {
                return lockManager.isLockedApp(packageName, key.userId, className);
            }
    ...
    public void setLock(Context context, boolean isLock) {
        ...
        if (isLock) {
            if (PrizeOption.PRIZE_APP_MULTI_INSTANCES && PrizeAppInstanceUtils.getInstance(context).supportMultiInstance(packageName)) {
                if (this.appInstanceIndex != 1) {
                    value = lockManager.lockApplication(packageName, key.userId, className);
                } else {
                    value = lockManager.lockApplication(packageName, 1, className);
                }
            } else {
                value = lockManager.lockApplication(packageName, key.userId, className);
            }
            ...
            if (PrizeOption.PRIZE_APP_MULTI_INSTANCES && PrizeAppInstanceUtils.getInstance(context).supportMultiInstance(packageName)) {
                if (this.appInstanceIndex != 1) {
                    value = lockManager.unlockApplication(packageName, key.userId);
                } else {
                    value = lockManager.unlockApplication(packageName, 1);
                }
            } else {
                value = lockManager.unlockApplication(packageName, key.userId);
            }
        }
        this.isLock = value ? isLock : !isLock;
    }

    public static void unlockByRemoveTask(Context context, String packageName) {
        if (packageName != null && !packageName.equals("com.tencent.mm") && !packageName.equals("com.tencent.mobileqq")) {
            AppLockManager lockManager = AppLockManager.getInstance(context);
            lockManager.unlockApplication(packageName, 0);
            Log.d(TAG, "set lock, pkg = " + packageName + ", isLock = false");
        } else {
            Log.d(TAG, "set lock, package name is " + (packageName != null ? packageName : "null"));
        }
    }

    public void unlockByRemoveTask(Context context) {
        if (key != null) {
            String packageName = key.getComponent().getPackageName();
    ...
    /*prize add by xiarui 2018-01-13 end*/
