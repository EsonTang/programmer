...
class TaskStackViewTouchHandler implements SwipeHelper.Callback {
    ...
    int mDownX, mDownY;
    int mLastY;
	int mLastX; //prize add by xiarui 2017-11-06
    ...
    public TaskStackViewTouchHandler(Context context, TaskStackView sv,
        ...
        /*prize modify xiarui 2017-11-06 SwipeHelper.X to Y start*/
        int swipeDirection = SwipeHelper.X;
        if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
            swipeDirection = SwipeHelper.Y;
        }
        /*prize modify xiarui 2017-11-06 SwipeHelper.X to Y end*/

        mSwipeHelper = new SwipeHelper(swipeDirection, this, context) {
            @Override
    ...
    private boolean handleTouchEvent(MotionEvent ev) {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_DOWN: {
                ...
                mDownY = (int) ev.getY();
                mLastY = mDownY;
				/*prize add by xiarui 2017-11-06 start*/
				if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
					mLastX = mDownX;
				}
				/*prize add by xiarui 2017-11-06 end*/
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (!mIsScrolling) {
					...
					/*prize modify set scroll left-right xiarui 20171106 start*/
					boolean isScrolling = false;
                    if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
                        isScrolling = Math.abs(x - mDownX) > mScrollTouchSlop && xDiff > yDiff;
                    } else {
                        isScrolling = Math.abs(y - mDownY) > mScrollTouchSlop && yDiff > xDiff;
                    }
                    //if (Math.abs(y - mDownY) > mScrollTouchSlop && yDiff > xDiff) {
                    if (isScrolling) {
                    /*prize modify set scroll left-right xiarui 20171106 end*/
                        mIsScrolling = true;
                ...
                if (mIsScrolling) {
                    ...
                    /*prize modify Y to X xiarui 2017-11-06 start*/
                    //float deltaP = layoutAlgorithm.getDeltaPForY(mDownY, y);
                    float deltaP = 0.0f;
                    if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
                        deltaP = layoutAlgorithm.getDeltaPForX(mDownX, x);
                    } else {
                        deltaP = layoutAlgorithm.getDeltaPForY(mDownY, y);
                    }
                    /*prize modify Y to X  xiarui 2017-11-06 end*/
                    ...
                    /*prize modify lastX xiarui 2017-11-06 [disable this & next code, OK???] start*/
                    if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
                        mStackViewScrolledEvent.updateY(y - mLastX);
                    } else {
                        mStackViewScrolledEvent.updateY(y - mLastY);
                    }
                    /*prize modify lastX xiarui 2017-11-06 [disable this & next code, OK???]  end*/
            ...
            case MotionEvent.ACTION_UP: {
                ...
                /*prize add fling left & right xiarui 2017-11-06 start*/
                int x = 0;
                if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
                    x = (int) ev.getX(activePointerIndex);
                }
                /*prize add fling left & right xiarui 2017-11-06 end*/

                /*prize modify getYVelocity to getXVelocity xiarui 2017-11-06 start*/
                //int velocity = (int) mVelocityTracker.getYVelocity(mActivePointerId);
                int velocity = 0;
                if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
                    velocity = (int) mVelocityTracker.getXVelocity(mActivePointerId);
                } else {
                    velocity = (int) mVelocityTracker.getYVelocity(mActivePointerId);
                }
                /*prize modify getYVelocity to getXVelocity xiarui 2017-11-06 end*/

                if (mIsScrolling) {
                    if (mScroller.isScrollOutOfBounds()) {
                        mScroller.animateBoundScroll();
                    } else if (Math.abs(velocity) > mMinimumVelocity) {
                        /*prize modify fling left & right xiarui 2017-11-06 start*/
                        ...
                        //        mOverscrollSize);
                        if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
                            float minX = mDownX + layoutAlgorithm.getXForDeltaP(mDownScrollP,
                                    layoutAlgorithm.mMaxScrollP);
                            float maxX = mDownX + layoutAlgorithm.getXForDeltaP(mDownScrollP,
                                    layoutAlgorithm.mMinScrollP);
                            mScroller.fling(mDownScrollP, mDownX, x, velocity, (int) minX, (int) maxX,
                                    mOverscrollSize);
                        } else {
                            float minY = mDownY + layoutAlgorithm.getYForDeltaP(mDownScrollP,
                                    layoutAlgorithm.mMaxScrollP);
                        ...
                        /*prize modify fling left & right xiarui 2017-11-06 end*/
    ...
    void maybeHideRecentsFromBackgroundTap(int x, int y) {
        ...
        //prize add by xiarui for demand bug#52019 2018-03-16 @{
        if(PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
            TaskView taskView = findViewAtPoint(x, y);
            if (taskView != null) {
                return;
            }
        } else {
