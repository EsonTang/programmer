...
public class Clock extends TextView implements DemoMode, Tunable {
    ...
    private final CharSequence getSmallTime() {
        ...
        /*PRIZE-add 12mode and 24mode for statusbar time- liufan-2015-05-25-start*/
        if(FeatureOption.PRIZE_QS_SORT){
            result = result.replace(""+MAGIC1, "");
            result = result.replace(""+MAGIC2, " ").trim();
            
            /*PRIZE-update for liuhai screen-liufan-2018-04-09-start*/
            if(PhoneStatusBar.OPEN_LIUHAI_SCREEN || PhoneStatusBar.OPEN_LIUHAI_SCREEN2){
                result = result.replace("上午", "");
                result = result.replace("PM", "");
                result = result.replace("AM", "");
                result = result.replace("下午", "").trim();
            }
            /*PRIZE-update for liuhai screen-liufan-2018-04-09-end*/
        }else{
            if (mAmPmStyle != AM_PM_STYLE_NORMAL) {
        ...
        /*PRIZE-add 12mode and 24mode for statusbar time- liufan-2015-05-25-end*/
