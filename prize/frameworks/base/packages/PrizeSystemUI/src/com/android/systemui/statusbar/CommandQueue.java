...
public class CommandQueue extends IStatusBar.Stub {
    ...
    private static final int MSG_SET_SB_INVERSE_COLOR = 34 << MSG_SHIFT;

    // add for statubar background color. prize-liufan-20150827
    private static final int MSG_SET_SB_BACKGROUND_COLOR = 35 << MSG_SHIFT;
    private static final int MSG_CLEAR_SB_BACKGROUND_COLOR = 36 << MSG_SHIFT;
    private static final int MSG_SET_SB_BACKGROUND_COLOR_CLICK_EVENT = 37 << MSG_SHIFT;
    private static final int MSG_SET_SB_TEXT = 38 << MSG_SHIFT;
    private static final int MSG_SET_SB_BACKGROUND_COLOR_CLICK_PENDING_EVENT = 40 << MSG_SHIFT;

    // Nav bar color customized feature. prize-linkh-2017.07.10 @{
    private static final int MSG_UPDATE_NAV_BAR_COLOR = 39 << MSG_SHIFT;
    // @}
        ...
        public void handleMessage(Message msg) {
            ...
            switch (what) {
                ...
                case MSG_HANDLE_SYSNAV_KEY:
                    ...
                    break;
                // add for statubar inverse style. prize-linkh-20150907
                case MSG_SET_SB_INVERSE_COLOR:                    
                    mCallbacks.onStatusBarInverseChanged(msg.arg1);
                    break;
                // Nav bar color customized feature. prize-linkh-2017.07.10 @{
                case MSG_UPDATE_NAV_BAR_COLOR:
                    mCallbacks.updateNavBarColor(msg.arg1, msg.arg2 == 1 ? true : false, (Boolean)msg.obj);
                    break;
                // @}
                // add for statubar background color. prize-liufan-20170828
                case MSG_SET_SB_BACKGROUND_COLOR:
                    mCallbacks.setStatusBarBackgroundColor((String)msg.obj, msg.arg1);
                    break;
                case MSG_CLEAR_SB_BACKGROUND_COLOR:
                    mCallbacks.clearStatusBarBackgroundColor();
                    break;
                case MSG_SET_SB_BACKGROUND_COLOR_CLICK_EVENT:
                    mCallbacks.setStatusBarBackgroundClickEvent((Intent) msg.obj);
                    break;
                case MSG_SET_SB_BACKGROUND_COLOR_CLICK_PENDING_EVENT:
                    mCallbacks.setStatusBarBackgroundClickPendingEvent(((PendingIntent) msg.obj));
                    break;
                case MSG_SET_SB_TEXT:
                    mCallbacks.setStatusBarText((String)msg.obj);
                    break;
            }
        }
    }
    ...
    public void onStatusBarInverseChanged(int style) {
        synchronized (mLock) {
            mHandler.obtainMessage(MSG_SET_SB_INVERSE_COLOR, style, 0).sendToTarget();
        }
    } //end...
    ...
    public void updateNavBarColor(int color, boolean enableNavBarColor, boolean enableSystemUINavBarColorBackground) {
        synchronized (mLock) {
            mHandler.obtainMessage(MSG_UPDATE_NAV_BAR_COLOR, color, enableNavBarColor ? 1 : 0,
                enableSystemUINavBarColorBackground).sendToTarget();
        }
    }
    // @}
    ...
    public void setStatusBarBackgroundColor(String pkg, int color) {
        synchronized (mLock) {
            mHandler.removeMessages(MSG_SET_SB_BACKGROUND_COLOR);
            mHandler.obtainMessage(MSG_SET_SB_BACKGROUND_COLOR, color, 0, pkg).sendToTarget();
        }
    }
    ...
    public void setStatusBarBackgroundClickEvent(Intent intent) {
        synchronized (mLock) {
            mHandler.removeMessages(MSG_SET_SB_BACKGROUND_COLOR_CLICK_EVENT);
            mHandler.obtainMessage(MSG_SET_SB_BACKGROUND_COLOR_CLICK_EVENT, intent).sendToTarget();
        }
    }
    ...
    public void setStatusBarBackgroundClickPendingEvent(PendingIntent pendingIntent) {
        synchronized (mLock) {
            mHandler.removeMessages(MSG_SET_SB_BACKGROUND_COLOR_CLICK_PENDING_EVENT);
            mHandler.obtainMessage(MSG_SET_SB_BACKGROUND_COLOR_CLICK_PENDING_EVENT, pendingIntent).sendToTarget();
        }
    }
    ...
    public void setStatusBarText(String str){
        synchronized (mLock) {
            mHandler.removeMessages(MSG_SET_SB_TEXT);
            mHandler.obtainMessage(MSG_SET_SB_TEXT, str).sendToTarget();
        }
    }
    ...
    public void clearStatusBarBackgroundColor() {
        synchronized (mLock) {
            mHandler.removeMessages(MSG_CLEAR_SB_BACKGROUND_COLOR);
            mHandler.obtainMessage(MSG_CLEAR_SB_BACKGROUND_COLOR).sendToTarget();
        }
    }
