...
public abstract class ExpandableView extends FrameLayout {
    ...
    public ExpandableView(Context context, AttributeSet attrs) {
        super(context, attrs);
        /*PRIZE-set background color-liufan-2016-05-24-start*/
        setBackgroundColor(0x00000000);
        mSidePaddings = context.getResources()
                .getDimensionPixelSize(R.dimen.notification_side_padding);
        /*PRIZE-set background color-liufan-2016-05-24-end*/
    }

	
    /*PRIZE-avoid overlap when notifications are refreshing-liufan-2016-05-24-start*/
    private boolean mDrawAsSrc;
    private boolean isNotificationHeadsUp;
    private Bitmap bg;
    public void setDrawAsSrc(boolean isDrawAsSrc){
        mDrawAsSrc = isDrawAsSrc;
    }

    public void setNotificationHeadsUp(boolean isHeadsUp){
        isNotificationHeadsUp = isHeadsUp;
    }
    ...
    protected void onDraw(Canvas canvas) {
        if (mDrawAsSrc && !isNotificationHeadsUp) {
			Log.d("yyxx","---------->canvas");
            int w = getWidth();
            int h = getHeight();
            Rect r  = null;
            if(w != 0 && h != 0){
                r = new Rect((getWidth()-w)/2,0,w,h);
                //setClipBounds(r);
            }
            if(r!=null){
                ...
                if(bg!=null){
                    ...
                    canvas.drawBitmap(bg, -location[0], -location[1], null);
                }
    ...
    /*PRIZE-avoid overlap when notifications are refreshing-liufan-2016-05-24-end*/
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (layoutParams.height != ViewGroup.LayoutParams.MATCH_PARENT) {
                ...
                child.measure(
                        getChildMeasureSpec(widthMeasureSpec, mSidePaddings*2 /* padding */, layoutParams.width),
                        childHeightSpec);
        ...
        for (View child : mMatchParentViews) {
            child.measure(getChildMeasureSpec(
                    widthMeasureSpec, mSidePaddings*2 /* padding */, child.getLayoutParams().width),
                    newHeightSpec);
        }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
			...
			if(child instanceof NotificationSettingsIconRow){
                child.layout(left, top, right, bottom);
			} else if(child.getId() == R.id.notification_scrim_view){
                child.layout(left, top, right, bottom);
			}else{
