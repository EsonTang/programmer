...
public class WindowManagerProxy {
    ...
    /* prize-add-split screen-liyongli-20170809-start */
      private final Runnable QQResizeRunnable = new Runnable() {
        @Override
        public void run() {
            synchronized (mDockedRect) {
                mTmpRect1.set(mDockedRect);
    ...
    public void QQresizeDockedStack(Rect docked, Rect tempDockedTaskRect, Rect tempDockedInsetRect,
            Rect tempOtherTaskRect, Rect tempOtherInsetRect) {
        synchronized (mDockedRect) {
            mDockedRect.set(docked);
            if (tempDockedTaskRect != null) {
        ...
        mExecutor.execute(QQResizeRunnable);
    }
