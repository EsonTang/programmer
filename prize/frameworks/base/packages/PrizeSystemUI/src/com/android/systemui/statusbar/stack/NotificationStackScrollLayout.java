...
/** PRIZE- notification background alpha- liufan-2015-09-03-start */
...
/** PRIZE- notification background alpha- liufan-2015-09-03-end */
...
public class NotificationStackScrollLayout extends ViewGroup
    ...
    public NotificationStackScrollLayout(Context context, AttributeSet attrs, int defStyleAttr,
        ...
        /*PRIZE-set obj-liufan-2015-10-10-start*/
        mSwipeHelper.setNotificationStackScrollLayout(this);
        /*PRIZE-set obj-liufan-2015-10-10-end*/
    ...
    public void setNotificationBgSwipeView(View view){
        mSwipeHelper.setNotificationScrimView(view);
    }
    /*PRIZE-add notificationscrimview-liufan-2016-06-18-end*/

    @Override
    protected void onDraw(Canvas canvas) {
        /*PRIZE-cancel notification white background-liufan-2016-11-07-start*/
        ...
        /*PRIZE-cancel notification white background-liufan-2016-11-07-end*/
        if (DEBUG) {
            int y = mTopPadding;
    ...
    /*PRIZE-modify overlapping when delete the notification-liufan-2015-9-15-start*/
    public static boolean isChildChanged;
    private Bitmap rowBitmap = null;
    private float fraction;
    public void setNotificationRowBg(Bitmap bitmap, float fraction){
        this.rowBitmap = bitmap;
        this.fraction = fraction;
    }

    public void setChildDrawAsArc(View child, boolean isDrawAsArc){
        if(child != mEmptyShadeView && child != mDismissView && child instanceof ExpandableView){
            ExpandableView ev = (ExpandableView)child;
            if(isDrawAsArc){
                if(mPhoneStatusBar.getBarState() == StatusBarState.KEYGUARD){
                    if(mPhoneStatusBar.isQsExpanded()){
                        ev.setDrawAsSrc(isDrawAsArc);//true
                        ImageView view = mPhoneStatusBar.getBackdropBack();
                        Drawable d = view != null ? view.getDrawable() : null;
                        Bitmap bm = (d != null && d instanceof BitmapDrawable) ? ((BitmapDrawable)d).getBitmap(): null;
                        ev.setBgBitmap(bm);
                    }else{
                        ev.setDrawAsSrc(!isDrawAsArc);//false
                        ev.setBgBitmap(null);
                    }
                }else if(mPhoneStatusBar.getBarState() == StatusBarState.SHADE){
                    ev.setDrawAsSrc(isDrawAsArc);//true
                    ev.setBgBitmap(null);
                }else{
                    ev.setDrawAsSrc(isDrawAsArc);//true
    ...
    /**
    * Method Description：crop the background where the child is
    */
    public Bitmap getChildBg(View child){
        if(!PhoneStatusBar.BLUR_BG_CONTROL){
            setChildDrawAsArc(child, true);
            return null;
        }
        if(this.rowBitmap == null || this.rowBitmap.isRecycled()){
            return null;
        }
        if(NotificationPanelView.isHaveHeadsUpNotification) return null;
        if(child == mDismissView || child == mEmptyShadeView) return null;
        int[] location = new int[2];
        child.getLocationOnScreen(location);
        int scale = BlurPic.SCALE_VAL;
        int left = location[0]/scale;
        int top = location[1]/scale;
        left = left < 0 ? 0 : left;
        top = top < 0 ? 0 : top;
        try {
            int width = child.getMeasuredWidth()/scale;
            int height = child.getMeasuredHeight()/scale;
            width = (left+width) > rowBitmap.getWidth() ? rowBitmap.getWidth() - left : width;
            height = (top+height) > rowBitmap.getHeight() ? rowBitmap.getHeight() - top : height;
            width = Math.max(width,1);
            height = Math.max(height,1);
            left = (left+width) > rowBitmap.getWidth() ? 0 : left;
            top = (top+height) > rowBitmap.getHeight() ? 0 : top;
            return Bitmap.createBitmap(rowBitmap, left, top, width, height);
        } catch (Exception e) {
            Log.d(TAG,"getChildBg------->"+e);
            return null;
        }
    }
    /*PRIZE-modify overlapping when delete the notification-liufan-2015-9-15-end*/

    @Override
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        /** PRIZE- notification background alpha - liufan-2015-08-31-start */
        int notificationNum = 0;
        for(int i = 0; i < getChildCount(); i++){
            View child = getChildAt(i);
            if (child instanceof ExpandableNotificationRow && child.getVisibility() == View.VISIBLE){
                View view = child.findViewById(R.id.notification_bottom_line);
                view.setVisibility(View.VISIBLE);
                view = child.findViewById(R.id.notification_head_line);
                view.setVisibility(View.VISIBLE);
                notificationNum++;
            }
        }
        ...
        for (int i = 0; i < getChildCount(); i++) {
            View child = getChildAt(i);
            if (child instanceof ExpandableNotificationRow && child.getVisibility() == View.VISIBLE){//child instanceof NotificationOverflowContainer
                curNum++;
                //if (mPhoneStatusBar.getBarState() == StatusBarState.KEYGUARD){
                    if(curNum!=notificationNum){
                        View view = child.findViewById(R.id.notification_bottom_line);
        ...
        //modify the notification overlapping, crop the background to child--liufan-2015-09-15--start
        for(int i = 0; i < getChildCount(); i++){
            View child = getChildAt(i);
            if(child.getVisibility() == View.VISIBLE){
                boolean isCancel = true;
                if(isChildChanged){
                    Bitmap bm = getChildBg(child);
                    if(bm != null){
                        BitmapDrawable bd = new BitmapDrawable(getResources(), bm);
                        //if(!mPhoneStatusBar.isQsExpanded()){
                            //bd.setAlpha((int)(fraction * 255));
                        //}
                        child.setBackground(bd);
                        isCancel = false;
                        //child.setBackground(new BitmapDrawable(getResources(), bm));
                    }
                }
                if(PhoneStatusBar.BLUR_BG_CONTROL && isCancel){
                    Drawable d = child.getBackground();
                    child.setBackground(null);
                    setChildDrawAsArc(child, false);
        ...
        //modify the notification overlapping, crop the background to child--liufan-2015-09-15--end
        /** PRIZE- notification background alpha - liufan-2015-08-31-end */
    ...
    /*PRIZE-notification bg-liufan-2015-10-10-start*/
    public void setAllNotificationRowBg(){
        for(int i = 0; i < getChildCount(); i++){
            View child = getChildAt(i);
            if(child.getVisibility() == View.VISIBLE){
                Bitmap bm = getChildBg(child);
    ...
    public void cancelAllNotificationRowBg(){
        Log.d("yyxx","cancelAllNotificationRowBg, getChildCount : " + getChildCount());
        for(int i = 0; i < getChildCount(); i++){
            View child = getChildAt(i);
            if(child.getVisibility() == View.VISIBLE){
                if(child instanceof ExpandableNotificationRow){
                    ExpandableNotificationRow enr = (ExpandableNotificationRow)child;
                    NotificationChildrenContainer childContainer = enr.getChildrenContainer();
                    if(childContainer != null && childContainer.getChildCount() != 0){
                        Log.d("yyxx","childContainer.getChildCount()---------->"+childContainer.getChildCount());
                        for(int j = 0;j < childContainer.getChildCount();j++){
                            View rowChild = childContainer.getChildAt(j);
                            clearBackground(rowChild);
                        }
                    }
                }
        ...
        Log.d("yyxx","---------->cancelAllNotificationRowBg");
        requestChildrenUpdate();
    }

    private void clearBackground(View child){
        Drawable d = child.getBackground();
        child.setBackground(null);
        setChildDrawAsArc(child, false);
        child.invalidate();
    ...
    /*PRIZE-notification bg-liufan-2015-10-10-end*/
    ...
    /** PRIZE- lineView Height record- liufan-2015-12-16-start */
    private int lineViewHeight;
    /** PRIZE- lineView Height record- liufan-2015-12-16-end */
    ...
    private void applyCurrentBackgroundBounds() {
        /*PRIZE-cancel notification area in scrim_behind-liufan-2016-11-07-start*/
        /*if (!mFadingOut) {
            mScrimController.setExcludedBackgroundArea(mCurrentBounds);
        }
        invalidate();*/
        /*PRIZE-cancel notification area in scrim_behind-liufan-2016-11-07-end*/
    }
    ...
    private class NotificationSwipeHelper extends SwipeHelper {
        ...
        public void onMoveUpdate(View view, float translation, float delta) {
            ...
            /*PRIZE-cancel swipe event,for bugid:47989- liufan-2018-01-22-start*/
            /*if (!isPinnedHeadsUp(view) && !gutsExposed) {
                // Only show the gear if we're not a heads up view and guts aren't exposed.
                checkForDrag();
            }*/
            /*PRIZE-cancel swipe event,for bugid:47989- liufan-2018-01-22-end*/
        }
