...
public class NetworkControllerImpl extends BroadcastReceiver
        implements NetworkController, DemoMode, DataUsageController.NetworkNameProvider, PrizeStatusBarStyleListener {//add a listener for statusbar inverse. prize-linkh-20150903
    // debug
    ...
    NetworkControllerImpl(Context context, ConnectivityManager connectivityManager,
        ...
        mCallbackHandler.setNetworkControllerImpl(this);
     ...
     */
	/*PRIZE-notify all controllers-liufan-2016-04-20-start*/
    private void notifyAllListeners() {
        notifyListeners();
        for (MobileSignalController mobileSignalController : mMobileSignalControllers.values()) {
            mobileSignalController.onStatusBarStyleChanged(mCurStatusBarStyle);
			mobileSignalController.notifyListeners();
        }
        mWifiSignalController.onStatusBarStyleChanged(mCurStatusBarStyle);
        mWifiSignalController.notifyListeners();
        mEthernetSignalController.notifyListeners();
    }

    public void notifyAllListenersForInverse() {
        notifyListeners();
        for (MobileSignalController mobileSignalController : mMobileSignalControllers.values()) {
            mobileSignalController.onStatusBarStyleChanged(mCurStatusBarStyle);
            mobileSignalController.notifyListenersForInverse();
        }
        mWifiSignalController.onStatusBarStyleChanged(mCurStatusBarStyle);
        mWifiSignalController.notifyListenersForInverse();
        mEthernetSignalController.notifyListeners();
    }
	/*PRIZE-notify all controllers-liufan-2016-04-20-end*/
    ...
    public void onStatusBarStyleChanged(int style) {
        ...
        if(mCurStatusBarStyle != style) {
            ...
            mCallbackHandler.notifyAllSignalCallbacks(style);
            handleIMSAction(null);
            /*PRIZE-notify all Callback-liufan-2016-04-20-end*/
            //new Thread(){
            //    @Override
            //    public void run(){
                    notifyAllListenersForInverse();
            //    }
            //}.start();
        }
    }
    
    public void refreshCurStatusBarStyle(int style){
        Log.d(TAG, "refreshCurStatusBarStyle(). curStyle=" + mCurStatusBarStyle + ", newStyle=" + style);
        mCurStatusBarStyle = style;
        mCallbackHandler.notifyAllSignalCallbacks(style);
        handleIMSAction(null);
        notifyAllListeners();
