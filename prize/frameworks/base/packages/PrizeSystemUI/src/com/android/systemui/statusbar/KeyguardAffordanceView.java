...
public class KeyguardAffordanceView extends ImageView {
    ...
    public KeyguardAffordanceView(Context context, AttributeSet attrs, int defStyleAttr,
        ...
        if(PrizeOption.PRIZE_LOCKSCREEN_INVERSE){
            ...
            setCircleColor();
            //And end
            mNormalColor = PhoneStatusBar.LOCK_LIGHT_COLOR;
            mInverseColor = PhoneStatusBar.LOCK_DARK_COLOR;
        } else {
        ...
        if(PrizeOption.PRIZE_LOCKSCREEN_INVERSE){
            if(colorFlag){
                if(textColor == 1){
                    ...
                    mNormalColor = PhoneStatusBar.LOCK_DARK_COLOR;
                    mInverseColor = PhoneStatusBar.LOCK_LIGHT_COLOR;
                }
            }
        }
	...
	private void setCircleColor(){
		//Add for backgroud_reflect-luyingying-2017-09-09-Start
		Log.d("KAV", "colorFlag: "+ colorFlag);
		if(colorFlag){
			Log.d("KAV", "textColor: "+ textColor);
			if(textColor == 0){
				mCircleColor = PhoneStatusBar.LOCK_DARK_COLOR;
			}else{
				mCircleColor = PhoneStatusBar.LOCK_LIGHT_COLOR;	
			}
        }
		//And end
	}
