    ...
    public void recreateNotificationHeader(OnClickListener listener, StatusBarNotification notification) {
        ...
        if (mNotificationHeader == null) {
            ...
            if(PrizeOption.PRIZE_STATUSBAR_INVERSE_COLOR
                && !PrizeNotificaionIconUtil.shouldIgnoreReplacing(notification.getNotification())) {
                NotificationData.Entry entry = null;
                if(mNotificationParent != null){
                    NotificationGroupManager groupManager = mNotificationParent.getGroupManager();
                    if(groupManager!=null){
                        entry = groupManager.getGroupEntry(notification.getGroupKey());
                    }
                }
                if(entry != null){
                    int iconId = com.android.internal.R.id.icon;
                    View v = mNotificationHeader.findViewById(iconId);
                    if(v!=null)
                        PrizeNotificaionIconUtil.replaceNotificationIcon(mContext, entry, mNotificationHeader, iconId, false);
                }
            }
            /*PRIZE-change group notification icon,bugid:44276-liufan-2017-12-26-end*/
