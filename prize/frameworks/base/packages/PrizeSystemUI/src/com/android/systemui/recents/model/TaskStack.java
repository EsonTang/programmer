...
public class TaskStack {
        ...
        public static class ViewState {
            ...
            private ViewState(int areaAlpha, int hintAlpha, @TextOrientation int hintOrientation,
				...
				if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
                	dockAreaOverlay = new ColorDrawable(0xFF000000);
				} else {
    ...
    /**prize add by xiarui  for Bug#45761 2017-12-16 start**/
    /**
     * Removes a task from the stack, with an additional {@param animation} hint to the callbacks on
     * how they should update themselves.
     */
    public void removeTask(Task t, AnimationProps animation, boolean fromDockGesture, boolean forceStop) {
        if (mStackTaskList.contains(t)) {
            ...
            if (mCb != null) {
                ...
                mCb.onStackTaskRemoved(this, t, newFrontMostTask, animation,
                        fromDockGesture, forceStop);
            }
        }
        mRawTaskList.remove(t);
    }
    /**prize add by xiarui  for Bug#45761 2017-12-16 end**/
    ...
    public void removeTask(Task t, AnimationProps animation, boolean fromDockGesture) {
        if (mStackTaskList.contains(t)) {
            ...
            if (mCb != null) {
                // Notify that a task has been removed
                mCb.onStackTaskRemoved(this, t, newFrontMostTask, animation,
                        fromDockGesture, false); //prize add by xiarui 2017-12-16 for Bug#45761
            }
        }
        mRawTaskList.remove(t);
    }
     ...
     */
    public void removeAllTasks(Context context) {   //prize modify by xiarui 2017-12-04
    ...
    public void setTasks(Context context, List<Task> tasks, boolean notifyStackChanges) {
        ...
        for (int i = 0; i < removedTaskCount; i++) {
            mCb.onStackTaskRemoved(this, removedTasks.get(i), newFrontMostTask,
                    AnimationProps.IMMEDIATE, false /* fromDockGesture */, false); //prize add by xiarui 2017-12-16 for Bug#45761
        }
    ...
    public Task getLaunchTarget() {
        ArrayList<Task> tasks = mStackTaskList.getTasks();
        int taskCount = tasks.size();
        /*prize modify by xiarui for Bug#44761 2017-12-11 start*/
        if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
            if (taskCount == 0) {
                return null;
            }
            Task task = tasks.get(taskCount - 1);
            if (!task.isLaunchTarget) { //bug#57323
                return null;
            }
            // Remove the task if it is marked as excluded, unless it is the first most task and we
            // are requested to include it
            boolean isExcluded = (task.key.baseIntent.getFlags() & Intent.FLAG_ACTIVITY_EXCLUDE_FROM_RECENTS)
                    == Intent.FLAG_ACTIVITY_EXCLUDE_FROM_RECENTS;
            //bugid:50751
            if (isExcluded && !task.key.baseIntent.toString().contains("com.adups.fota")) {
                return null;
            }
            return task;
        } else {
        ...
        /*prize modify by xiarui Bug#44761 2017-12-11 end*/
    }
