...
public class StatusBarWindowManager implements RemoteInputController.Callback {
    ...
    private void applyHeight(State state) {
        ...
        if (expanded) {
            ...
            Log.d("StatusBarWindowManager","applyHeight(1) Height = " + mLpChanged.height);
        } else {
            mLpChanged.height = mBarHeight;
            Log.d("StatusBarWindowManager","applyHeight(2) Height = " + mLpChanged.height);
        }
        /*PRIZE-add for haokan-liufan-2017-11-17-start*/
        if(mStatusBar != null) {
            if(barHeight != mLpChanged.height){
                barHeight = mLpChanged.height;
                mStatusBar.onStatusBarHeightChange(mLpChanged.height);
            }
        }
