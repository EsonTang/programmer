...
public class PrizeFastBlur {
    private static final String TAG = "PrizeFastBlur";

    /**
     * 方法描述：缩小图片模糊法，速度快，性能好，当前采用的算法
     */
    public static Bitmap blurScale(Bitmap bmp, float scaleFactor,float radius) throws IllegalArgumentException{
        ...
        if(bmp!=null&&!bmp.isRecycled()&&bmp!=overlay) {
            bmp.recycle();
            bmp=null;
            // System.gc();
        }
    ...
    private static Bitmap doBlur(Bitmap sentBitmap, int radius,
        ...
        if (canReuseInBitmap) {
            ...
            if(sentBitmap!=null&&sentBitmap!=bitmap)
                sentBitmap.recycle();
        }
...
//        bitmap = adjustBrightness(bitmap);

        return bitmap;
    }

    private static Bitmap adjustBrightness(Bitmap srcBitmap){
