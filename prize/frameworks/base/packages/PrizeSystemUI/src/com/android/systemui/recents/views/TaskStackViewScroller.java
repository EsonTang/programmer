...
public class TaskStackViewScroller {
    ...
    int mFlingDownY;
	int mFlingDownX; //add by xiar20171106
     ...
     */
    public void flingY(float downScrollP, int downY, int y, int velY, int minY, int maxY,
            int overscroll) {
        if (DEBUG) {
     ...
     */
    public void flingX(float downScrollP, int downX, int x, int velX, int minX, int maxX,
            int overscroll) {
        if (DEBUG) {
            Log.d(TAG, "fling: " + downScrollP + ", downX: " + downX + ", x: " + x +
                    ", velX: " + velX + ", minX: " + minX + ", maxX: " + maxX);
        }
        mFlingDownScrollP = downScrollP;
        mFlingDownX = downX;
        mScroller.fling(x, 0, velX, 0, minX, maxX, 0, 0, overscroll, 0);
    }

    /**
     * redefine fling function, for fling X & Y
     */
    public void fling(float downScrollP, int down, int start, int vel, int min, int max,
                       int overscroll) {
        if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
            flingX(downScrollP, down, start, vel, min, max, overscroll);
        } else {
            flingY(downScrollP, down, start, vel, min, max, overscroll);
        }
    }
    ...
    boolean computeScroll() {
        if (mScroller.computeScrollOffset()) {
			/*prize modify by xiar 20171106 start*/
			float deltaP = 0.0f;
			if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
				deltaP = mLayoutAlgorithm.getDeltaPForX(mFlingDownX, mScroller.getCurrX());
			} else {
				deltaP = mLayoutAlgorithm.getDeltaPForY(mFlingDownY, mScroller.getCurrY());
			}
            //float deltaP = mLayoutAlgorithm.getDeltaPForY(mFlingDownY, mScroller.getCurrY());
			/*prize modify by xiar 20171106 end*/
