...
//prize add by xiarui 2018-06-05 @{
...
public class VolumeDialog implements TunerService.Tunable {
        ...
        if (mRows.isEmpty()) {
            ...
            if (PrizeOption.PRIZE_VIBRATE_CONTROL) {
                addRow(AudioManager.STREAM_RING,
                        R.drawable.ic_volume_ringer_prize2, R.drawable.ic_volume_ringer_mute, true);
            } else {
                addRow(AudioManager.STREAM_RING,
            ...
            public void onClick(View v) {
                ...
                if (row.stream == AudioManager.STREAM_RING) {
                    ...
                    if (mState.ringerModeInternal == AudioManager.RINGER_MODE_NORMAL) {
                        if (hasVibrator) {
                            //prize modify by xiarui 2018-05-31 @{
                            if (PrizeOption.PRIZE_VIBRATE_CONTROL) {
                                if (mController.isVibrateInSilentOpen()) {
    ...
    private void updateExpandedH(final boolean expanded, final boolean dismissing) {
        ...
        /*PRIZE-add for bugid:49676- liufan-2018-02-02-start*/
        mController.refreshStreamTitles();
        /*PRIZE-add for bugid:49676- liufan-2018-02-02-end*/
        updateExpandButtonH();
        updateFooterH();
    ...
    private void updateExpandButtonH() {
        ...
        if (!(mExpandButtonAnimationRunning && isAttached())) {
            ...
            if (hasTouchFeature()) {
                ...
                //prize modify by xiarui 2018-06-28 start @{
                if (PrizeOption.PRIZE_VIBRATE_CONTROL) {
                    mExpandButton.setImageResource(R.drawable.ic_volume_ringer_prize2);
                } else {
    ...
    private void updateFooterH() {
        ...
        //prize modify by xiarui 2018-06-01 for set zen footer gone @{
        if (PrizeOption.PRIZE_VIBRATE_CONTROL) {
        ...
        //prize modify by xiarui 2018-06-01 for set zen panel gone
        if (PrizeOption.PRIZE_VIBRATE_CONTROL) {
            Util.setVisOrGone(mZenPanel, false);
        } else {
    ...
    private void updateVolumeRowH(VolumeRow row) {
        ...
        boolean zenMuted = isZenAlarms ? (isRingStream || isSystemStream)
                : isZenNone ? (isRingStream || isSystemStream || isAlarmStream || isMusicStream)
                : false;
        //prize modify by xiarui 2018-06-01 @{
        if (PrizeOption.PRIZE_VIBRATE_CONTROL) {
            zenMuted = false;
        }
        //@}
        ...
        if (PrizeOption.PRIZE_VIBRATE_CONTROL) {
            iconRes =
                    isRingVibrate ? R.drawable.ic_volume_ringer_vibrate_prize2
                            : isRingSilent ? R.drawable.ic_volume_ringer_silent_prize2
        ...
        int vlevel = 0;
        //prize add by xiarui vlevel to 0 when ring silent 2018-06-05 @{
        if (PrizeOption.PRIZE_VIBRATE_CONTROL) {
            vlevel = row.ss.muted && (isRingVibrate || isRingSilent || !isRingStream && !zenMuted) ? 0
                    : row.ss.level;
        } else {
            vlevel = row.ss.muted && (isRingVibrate || !isRingStream && !zenMuted) ? 0
                    : row.ss.level;
        }
        //@}
    ...
    private void updateVolumeRowSliderTintH(VolumeRow row, boolean isActive) {
        ...
        ColorStateList tint = isActive && row.slider.isEnabled() ? mActiveSliderTint
                : mInactiveSliderTint;
        //prize add by xiarui 2018-06-05 @{
        if (PrizeOption.PRIZE_VIBRATE_CONTROL) {
            tint = mActiveSliderTint;
        }
        //@}
