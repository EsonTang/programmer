...
/* Reposition nav bar back key feature. prize-linkh-20161115 */
...
public class NavigationBarInflaterView extends FrameLayout implements TunerService.Tunable {
    ...
    final static boolean DEBUG_NAV_BAR = NavigationBarView.DEBUG_NAV_BAR;
    ...
    private boolean mIsAttachingToWin;
    //END...
    ...
    /* Reposition nav bar back key feature & Dynamically hiding nav bar feature. prize-linkh-20161115 */
    public static String getStyleDecription(int style) {
        String str = null;

        switch(style) {
        case STYLE_BACK_AT_LEFT: {
            str = "BACK_AT_LEFT";
            break;
        }
        case STYLE_BACK_AT_RIGHT: {
            str = "BACK_AT_RIGHT";
            break;
        }
        case STYLE_BACK_AT_LEFT_WITH_HIDE: {
            str = "BACK_AT_LEFT_WITH_HIDE";
            break;
        }
        case STYLE_BACK_AT_RIGHT_WITH_HIDE: {
            str = "BACK_AT_RIGHT_WITH_HIDE";
            break;
        }
        default: {
            str = "Unknown";
    ...
    public int fixStyle(int style) {
        if (DEBUG_NAV_BAR) {
            Log.d(TAG, "fixStyle() style=" + style);
        }

        if (PrizeOption.PRIZE_REPOSITION_BACK_KEY ||
            PrizeOption.PRIZE_DYNAMICALLY_HIDE_NAVBAR) {
            ...
            } else if (!PrizeOption.PRIZE_REPOSITION_BACK_KEY) {
    ...
    public boolean canHideNavBar(int style) {
        if (style == STYLE_BACK_AT_LEFT_WITH_HIDE
    ...
    public String getLayout(int style) {
        if (DEBUG_NAV_BAR) {
            Log.d(TAG, "getLayout() ori style=" + style);
        }
        ...
        switch(style) {
        case STYLE_BACK_AT_LEFT: {
            layoutId = R.string.config_navBarLayout;
            break;
        }
        case STYLE_BACK_AT_RIGHT: {
            layoutId = R.string.config_navBarLayout_back_at_right;
            break;
        }
        case STYLE_BACK_AT_LEFT_WITH_HIDE: {
            layoutId = R.string.config_navBarLayout_back_at_left_with_hide;
            break;
        }
        case STYLE_BACK_AT_RIGHT_WITH_HIDE: {
            layoutId = R.string.config_navBarLayout_back_at_right_with_hide;
            break;
        }
        default: {
        ...
        String layout = mContext.getString(layoutId);
        if (DEBUG_NAV_BAR) {
            Log.d(TAG, "getLayout() layout=" + layout);
        }
        return layout;
    }

    public void setNewLayout(String layout) {
        setNewLayout(layout, false);
    }
    
    public void setNewLayout(String layout, boolean force) {
        if (DEBUG_NAV_BAR) {
            Log.d(TAG, "setNewLayout() layout=" + layout + ", force=" + force
                        + ", currentLayout=" + mCurrentLayout);
        }

        if (Objects.equals(mCurrentLayout, layout) && !force) {
            return;
        }
    ...
    protected void onAttachedToWindow() {
        ...
        /* Reposition nav bar back key feature & Dynamically hiding nav bar feature. prize-linkh-20161115 */
        // When invoking addTunable method, TunerService willl invoke onTunningChanged method and reset layout
        // to custom layout. It issn't our propose. We must prevent it. En, if external has changed layout through
        // TunnerService after attached to win, ok, let it do.
        mIsAttachingToWin = true;
        //END...
        TunerService.get(getContext()).addTunable(this, NAV_BAR_VIEWS);
        /* Reposition nav bar back key feature & Dynamically hiding nav bar feature. prize-linkh-20161115 */
        mIsAttachingToWin = false;
        //END...        
    }
    ...
    public void onTuningChanged(String key, String newValue) {
        /* Reposition nav bar back key feature & Dynamically hiding nav bar feature. prize-linkh-20161115 */
        if (PrizeOption.PRIZE_REPOSITION_BACK_KEY || PrizeOption.PRIZE_DYNAMICALLY_HIDE_NAVBAR) {
            if (mIsAttachingToWin) {
                Log.w(TAG, "onTuningChanged() ignore this event because of in attaching to win!");
                return;
            }
        } //END...    
    ...
    protected View inflateButton(String buttonSpec, ViewGroup parent, boolean landscape,
        ...
        } else if (PrizeOption.PRIZE_DYNAMICALLY_HIDE_NAVBAR && HIDE.equals(button)) {
