...
/*PRIZE-import package- liufan-2015-04-15-start*/
import com.android.systemui.statusbar.phone.FeatureOption;
import android.text.TextUtils.TruncateAt;
/*PRIZE-import package- liufan-2015-04-15-end*/
...
public class QSDualTileLabel extends LinearLayout {
    ...
    public void setFirstLineCaret(Drawable d) {
        /*PRIZE-cancel the arrow on the qs- liufan-2015-05-06-start*/
        if(FeatureOption.PRIZE_QS_SORT){
            return;
        }
        /*PRIZE-cancel the arrow on the qs- liufan-2015-05-06-end*/
    ...
    private TextView initTextView() {
        /*PRIZE-change qs text view to QSMarqueeTextView- liufan-2015-05-06-start*/
        if(FeatureOption.PRIZE_QS_SORT){
            final TextView tv = new QSMarqueeTextView(mContext);
        ...
        /*PRIZE-change qs text view to QSMarqueeTextView- liufan-2015-05-06-end*/
    }
    ...
    private void updateText() {
        ...
        /*PRIZE-set QSMarqueeTextView text- liufan-2015-05-06-start*/
        if(FeatureOption.PRIZE_QS_SORT){
            mFirstLine.setPadding(0, 0, 0, 0);
            mFirstLine.setSingleLine();
            mFirstLine.setEllipsize(TruncateAt.MARQUEE);
            mFirstLine.setFocusable(true);
            mFirstLine.setFocusableInTouchMode(true);
            mFirstLine.setGravity(Gravity.CENTER);
            
            final float maxWidth = getWidth() - mFirstLineCaret.getWidth() - mHorizontalPaddingPx
                - getPaddingLeft() - getPaddingRight();
            final LayoutParams lp = new LayoutParams((int)maxWidth, LayoutParams.WRAP_CONTENT);
            mFirstLine.setLayoutParams(lp);
            
            //mFirstLine.setMinLines(2);
            mFirstLine.setText(mText);
            mSecondLine.setText(null);
            mSecondLine.setVisibility(GONE);
            return;
        }
        /*PRIZE-set QSMarqueeTextView text- liufan-2015-05-06-end*/
        final float maxWidth = getWidth() - mFirstLineCaret.getWidth() - mHorizontalPaddingPx
                - getPaddingLeft() - getPaddingRight();
