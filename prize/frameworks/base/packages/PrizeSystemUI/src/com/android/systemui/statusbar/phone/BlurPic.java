...
public class BlurPic {
    ...
    */
    ...
    /**
     * add by xiarui 2017-11-15
     * 缩放图片，模糊处理
     * @param bm
     * @return
     */
    public static Bitmap blur(Bitmap bm) {
        float scaleFactor = 12;//图片缩放比例
        float radius = 10;//模糊程度

        Bitmap overlay = Bitmap.createBitmap(
                (int) Math.ceil(bm.getWidth() / scaleFactor),
                (int) Math.ceil(bm.getHeight() / scaleFactor), Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(overlay);
        canvas.scale(1 / scaleFactor, 1 / scaleFactor);
        Paint paint = new Paint();
        paint.setFlags(Paint.FILTER_BITMAP_FLAG);
        ...
        doBlur(overlay, (int) radius, true);
    ...
    public static Bitmap blurScaleOtherRadius(Bitmap bmp,float radius){
        ...
        int targetW = (int) (bmp.getWidth() / scaleFactor);
	 int targetH = (int) (bmp.getHeight() / scaleFactor);
	 if(targetW <= 0)targetW = 1;
	 if(targetH <= 0)targetH = 1;
	 /*prize-add by lihuangyuan,for fingerapplock-2017-07-04-end*/
        Bitmap overlay = Bitmap.createBitmap(
                targetW,
                targetH, Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(overlay);
        canvas.scale(1 / scaleFactor, 1 / scaleFactor);
        Paint paint = new Paint();
        paint.setFlags(Paint.FILTER_BITMAP_FLAG);
