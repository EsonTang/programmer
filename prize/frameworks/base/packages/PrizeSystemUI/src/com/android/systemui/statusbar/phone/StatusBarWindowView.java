...
public class StatusBarWindowView extends FrameLayout {
    ...
    public static boolean IS_HIDE_KEYGUARD_WHEN_SLIP =  false;
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        /*PRIZE-for bug:42005-liufan-2017-11-10-start*/
        if (ev.getActionMasked() == MotionEvent.ACTION_DOWN) {
            if(IS_HIDE_KEYGUARD_WHEN_SLIP) {
                IS_HIDE_KEYGUARD_WHEN_SLIP = false;
                android.util.Log.d(TAG,"42005 set IS_HIDE_KEYGUARD_WHEN_SLIP fasle at ACTION_DOWN ");
            }
        }
        /*PRIZE-for bug:42005-liufan-2017-11-10-end*/
        ...
        /*PRIZE-for bug:42005-liufan-2017-11-10-start*/
        if(IS_HIDE_KEYGUARD_WHEN_SLIP) {
            android.util.Log.d(TAG,"42005 happen hide keyguard when slip on keyguard");
            intercept = true;
            if(mService != null) mService.collapsePanels(false);
        }
        /*PRIZE-for bug:42005-liufan-2017-11-10-end*/
        if (!intercept) {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (mService.getBarState() == StatusBarState.KEYGUARD) {
            /*PRIZE-Don't allow to swipe when lockscreen-liufan-2015-09-03-start*/
            if(VersionControl.isAllowDropDown(getContext())){
                handled = mDragDownHelper.onTouchEvent(ev);
            }
            /*PRIZE-Don't allow to swipe when lockscreen-liufan-2015-09-03-end*/
        }
        if (!handled) {
        ...
        if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) {
            if(IS_HIDE_KEYGUARD_WHEN_SLIP){
                IS_HIDE_KEYGUARD_WHEN_SLIP = false;
                android.util.Log.d(TAG,"42005 set IS_HIDE_KEYGUARD_WHEN_SLIP fasle at ACTION_UP/ACTION_CANCEL " + action);
            }
        }
        /*PRIZE-for bug:42005-liufan-2017-11-10-start*/
        return handled;
