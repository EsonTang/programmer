...
import com.android.systemui.recents.DisplayUtils;
...
public class TaskStackLayoutAlgorithm {
    ...
    public void initialize(Rect displayRect, Rect windowRect, Rect taskStackBounds,
        ...
        /*prize add by xiarui 2017-11-08 for TaskView Rect start*/
        if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
            float scale = RecentsConfiguration.taskViewScalePortrait;//0.6f;
            int tempClipH = 0;
            int tempClipW = 0;
            int taskBarHeight = mContext.getResources().getDimensionPixelSize(
                    R.dimen.recents_task_view_header_height);

            /**modify by xiarui { for ORIENTATION_LANDSCAPE && hasDockedTask Bug#43277} 2017-11-25 start**/
            if (Recents.getSystemServices().hasDockedTask() &&
                    Utilities.getAppConfiguration(mContext).orientation == Configuration.ORIENTATION_LANDSCAPE) {
                tempClipW = displayRect.height();
                if (DisplayUtils.checkDeviceHasNavigationBar(mContext)) {
                    height -= DisplayUtils.getNavigationBarHeight(mContext);
                }
                tempClipH = (int) (height / scale);
            } else
            /**modify by xiarui { for ORIENTATION_LANDSCAPE && hasDockedTask Bug#43277} 2017-11-25 end**/

            if (Utilities.getAppConfiguration(mContext).orientation == Configuration.ORIENTATION_LANDSCAPE) {
                scale = RecentsConfiguration.taskViewScaleLand;//0.53f;//0.56f
                tempClipW = displayRect.width();
                if (DisplayUtils.checkDeviceHasNavigationBar(mContext)) {
                    tempClipW -= DisplayUtils.getNavigationBarHeight(mContext);
                }
                if (PrizeOption.PRIZE_NOTCH_SCREEN) {//for bug57230
                    tempClipW -= DisplayUtils.getStatusBarHeight(mContext);
                }
                //height
                tempClipH = displayRect.height()/* - DisplayUtils.getStatusBarHeight(mContext)*/;
            } else {
                tempClipW = displayRect.width();
                tempClipH = displayRect.height()/* - DisplayUtils.getStatusBarHeight(mContext)*/;
                if (PrizeOption.PRIZE_NOTCH_SCREEN) { //for bug57230
                    tempClipH -= DisplayUtils.getStatusBarHeight(mContext);
                }
                if (DisplayUtils.checkDeviceHasNavigationBar(mContext)) {
                    tempClipH -= DisplayUtils.getNavigationBarHeight(mContext);
                }
                // docked reset height
                boolean isDocked = Recents.getSystemServices().hasDockedTask();
                if (isDocked) {
                    tempClipH = (int) (height / scale);
                }
            }
            int tempW = (int) (tempClipW * scale);
            int tempH = (int) (tempClipH * scale) + taskBarHeight;//112; //112 is the TaskViewHeader height
            int left = 0;//mStackRect.left + (mStackRect.width() - tempW) / 2;
            int top = mStackRect.top;
            //prize add by xiarui 2018-04-26 for bug56473 @{
            ...
            //@}
            int right = left + tempW;
            int bottom = top + tempH;
            mTaskRect.set(left, top, right, bottom);
        } else {
            mTaskRect.set(mStackRect.left, mStackRect.top, mStackRect.right, mStackRect.top + height);
        }
        /*prize add by xiarui 2017-11-08 for TaskView Rect end*/
    ...
    void update(TaskStack stack, ArraySet<Task.TaskKey> ignoreTasksSet) {
        ...
        if (getInitialFocusState() == STATE_FOCUSED) {
            ...
            /*prize add by xiarui 2017-11-27 start*/
            if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
                maxBottomOffset = mStackBottomOffset + mTaskRect.width();
            }
            /*prize add by xiarui 2017-11-27 end*/
    ...
    public void setTaskOverridesForInitialState(TaskStack stack, boolean ignoreScrollToFront) {
        ...
        if (getInitialFocusState() == STATE_UNFOCUSED && mNumStackTasks > 1) {
            if (ignoreScrollToFront || (!launchState.launchedWithAltTab && !scrollToFront)) {
                ...
                /*prize add by xiarui 2017-11-27 start*/
                if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
                    maxBottomTaskNormX = getNormalizedXFromUnfocusedY(mFocusedTopPeekHeight +
                            mTaskRect.width() - mMinMargin, FROM_TOP);
                }
                /*prize add by xiarui 2017-11-27 end*/
    ...
    public void getStackTransform(float taskProgress, float nonOverrideTaskProgress,
        ...
        if (!ssp.hasFreeformWorkspaceSupport() && mNumStackTasks == 1 && !ignoreSingleTaskCase) {
            ...
            /*prize add for TaskStack x xiarui 2017-11-04 start*/
            if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
               int centerXOffset = (mStackRect.left - mTaskRect.left) +
                            (mStackRect.width() - mSystemInsets.right - mTaskRect.width()) / 2;
                x = centerXOffset + getXForDeltaP(tmpP, 0);
            } else {
            ...
            /*prize add for TaskStack x xiarui 2017-11-04 end*/
			...
			/*prize modify for TaskStack start-y xiarui 2017-11-04 start*/
			int unfocusedY = 0;
			int focusedY = 0;
			
            if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
	            unfocusedY = (int) ((1f - mUnfocusedCurveInterpolator.getInterpolation(
	                    unfocusedRangeX)) * mStackRect.width());
	            focusedY = (int) ((1f - mFocusedCurveInterpolator.getInterpolation(
			...
			/*prize modify for TaskStack start-y xiarui 2017-11-04 end*/
        ...
        /*prize add for TaskStack start-y xiarui 2017-11-04 start*/
        if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
            if (Utilities.getAppConfiguration(mContext).orientation == Configuration.ORIENTATION_LANDSCAPE) {
                if (ssp.hasDockedTask()) {
                    y = (mStackRect.height() - mTaskRect.height()) / 4;
                } else {
                    y = 0;//DisplayUtils.getStatusBarHeight(mContext) + 20;
                }
            } else {
                y = (mStackRect.height() - mTaskRect.height()) / 4;
            }
        }
        /*prize add for TaskStack start-y xiarui 2017-11-04 end*/

        // Fill out the transform
        /**prize add for scale by xiarui 2017-11-09 start**/
        //float pTaskRelative = taskProgress - stackScroll;
        //float pBounded = Math.max(0, Math.min(pTaskRelative, 1f));
        // float scale = getTaskViewScale(mStackRect, x);//Utilities.curveProgressToScale(pBounded);
        /**prize add for scale by xiarui 2017-11-09 end**/
        ...
        if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
            transformOut.titleAlpha = getHeaderAlpha(ssp, x);
        }
        /*prize add by xiarui 2017-11-09 end*/

        Utilities.scaleRectAboutCenter(transformOut.rect, transformOut.scale);

        /*prize modify for visible xiarui 2017-11-04 start*/
        if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
            //set transformOut.visible = true directly by xiarui , for BUG #43102
        ...
        /*prize modify for visible xiarui 2017-11-04 end*/
    }
     ...
     * @return
     */
    public float getHeaderAlpha(SystemServicesProxy ssp, int x) {
        float headerAlpha = 1f;
        int hidePoint = 20;
        int alphaPoint = 100;
        LogUtils.d("x", "-------x = " + x);
        if (x < hidePoint) {
            headerAlpha = 0f;
        } else if (x < alphaPoint) {
            float scale = Math.abs(hidePoint - x) / (float) Math.abs(hidePoint - alphaPoint);
            headerAlpha = Math.max(0, scale);
        } else {
            headerAlpha = 1f;
        }
        return headerAlpha;
     ...
     * @return
     */
    public float getTaskViewScale(Rect stackRect, int x) {
        int MIN_LEFT = -120;
        float MIN_SCALE = 0.98f;
        float scale = MIN_SCALE;
        if (x < MIN_LEFT) {
            scale = MIN_SCALE;
        } else {
            scale = MIN_SCALE + (1 - MIN_SCALE) * Math.abs(x - MIN_LEFT) / stackRect.width();
        }
        return scale;
    }
    /**prize add by xiarui 2017-11-09 end**/
    ...
    float getStackScrollForTask(Task t) {
        /*prize modify by xiarui {fix : The difference between LANDSCAPE and PORTRAIT x value of the last taskView is too big} 2017-11-27 start*/
        if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
            return  (float) mTaskIndexMap.get(t.key.id, 0);
        }
        /*prize modify by xiarui {fix : The difference between LANDSCAPE and PORTRAIT x value of the last taskView is too big} 2017-11-27 end*/
    ...
    /**prize add xiarui-2017-11-06-start**/

    /**
     * Maps a movement in screen y, relative to {@param downX}, to a movement in along the arc
     * length of the curve.  We know the curve is mostly flat, so we just map the length of the
     * screen along the arc-length proportionally (1/arclength).
     */
    public float getDeltaPForX(int downX, int x) {
        float deltaP = (float) (x - downX) / mStackRect.width() *
                mUnfocusedCurveInterpolator.getArcLength();
        return -deltaP;
    }

    /**
     * This is the inverse of {@link #getDeltaPForX}.  Given a movement along the arc length
     * of the curve, map back to the screen x.
     */
    public int getXForDeltaP(float downScrollP, float p) {
        int x = (int) ((p - downScrollP) * mStackRect.width() *
                (1f / mUnfocusedCurveInterpolator.getArcLength()));
        return -x;
    }

	/**prize add xiarui-2017-11-04-end**/
    ...
    public int getYForDeltaP(float downScrollP, float p) {
        ...
        /*prize add by xiarui 2017-11-27 start*/
        if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
            return getXForDeltaP(downScrollP, p);
        }
        /*prize add by xiarui 2017-11-27 end*/
    ...
    public void getTaskStackBounds(Rect displayRect, Rect windowRect, int topInset, int leftInset,
        ...
        /* delete rightInset, when ORIENTATION_LANDSCAPE, fix : quick pull down TaskView,the right side flicker*/
        if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
            taskStackBounds.set(windowRect.left, windowRect.top/* + topInset*/, //modify by xiarui 2017-12-01 for topInset = StatusBarHeight, hide StatusBar in FULLSCREEN
    ...
    private float getNormalizedXFromUnfocusedY(float y, @AnchorSide int fromSide) {
        ...
        /*prize add by xiarui 2017-11-27 start*/
        if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
            offset = (fromSide == FROM_TOP)
                    ? mStackRect.width() - y
                    : y;
            offsetPct = offset / mStackRect.width();
        }
        /*prize add by xiarui 2017-11-27 end*/
    ...
    private Path constructFocusedCurve() {
        ...
        /*prize add by xiarui 2017-11-27 start*/
        if (PrizeOption.PRIZE_SYSTEMUI_RECENTS) {
            topPeekHeightPct = (float) mFocusedTopPeekHeight / mStackRect.width();
            bottomPeekHeightPct = (float) (mStackBottomOffset + mFocusedBottomPeekHeight) /
                    mStackRect.width();
            minBottomPeekHeightPct = (float) (mFocusedTopPeekHeight + mTaskRect.width() -
                    mMinMargin) / mStackRect.width();
        }
        /*prize add by xiarui 2017-11-27 end*/
