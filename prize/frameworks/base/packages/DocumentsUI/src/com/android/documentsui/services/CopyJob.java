...
class CopyJob extends Job {
        ...
        for (int i = 0; i < mSrcs.size() && !isCanceled(); ++i) {
            ...
            try {
                if (dstInfo.equals(srcInfo) || isDescendentOf(srcInfo, dstInfo)) {
                    if (DEBUG) Log.e(TAG, "Skipping recursive copy of " + srcInfo.derivedUri);
                    onFileFailed(srcInfo);
                } else {
                    processDocument(srcInfo, null, dstInfo);
                }
            } catch (ResourceException e) {
                if (DEBUG) Log.e(TAG, "Failed to copy " + srcInfo.derivedUri, e);
                onFileFailed(srcInfo);
            }
        }
        ...
        if (src.authority.equals(dstDirInfo.authority)) {
            if ((src.flags & Document.FLAG_SUPPORTS_COPY) != 0) {
                try {
                    ...
                    if (DEBUG) Log.e(TAG, "Provider side copy failed for: " + src.derivedUri
    ...
    private void copyDirectoryHelper(DocumentInfo srcDir, DocumentInfo destDir)
        ...
        try {
            ...
            if (DEBUG) Log.e(TAG, "Failed to copy a file %s to %s. "
        ...
        try {
            ...
            if (!success) {
                ...
                try {
                    ...
                    if (DEBUG) Log.w(TAG, "Failed to cleanup after copy error: " +
                        src.derivedUri, e);
                }
            }
