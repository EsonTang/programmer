...
import com.android.documentsui.PrizeUtils;
...
public class DirectoryFragment extends Fragment
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        ///M: SelectionManager impacts only UI, hence a null check has no impact
    ...
    private void updateLayout(@ViewMode int mode) {
        ///M: Padding should be set before the column count is calculated
        ...
        ///M: Padding calculation is already done
        /*int pad = getDirectoryPadding(mode);
            mRecView.setPadding(pad, pad, pad, pad);*/
    ...
    private void showEmptyDirectory() {
        showEmptyView(R.string.empty, R.drawable.prize_empty_icon);
    }

    private void showNoResults(RootInfo root) {
        CharSequence msg = getContext().getResources().getText(R.string.no_results);
        showEmptyView(String.format(String.valueOf(msg), root.title), R.drawable.prize_empty_icon);
    }
    ...
    public void onLoadFinished(Loader<DirectoryResult> loader, DirectoryResult result) {
        ...
        mAdapter.notifyDataSetChanged();
        /*if (isServiceRunning() && isDeleteInProgress) {
            if (DEBUG) Log.d(TAG, "Don't update Model");
            mModel.update(result, false);
        }
        else if (!isServiceRunning()) {
            if (DEBUG) Log.d(TAG, "update Model and reset isDeleteInProgress");
            isDeleteInProgress = false;
            mModel.update(result, true);
        }
        else {
            if (DEBUG) Log.d(TAG, "update Model");
            mModel.update(result, true);
        }*/
  ...
  /// @}
    ...
    //prize-public-bug:11980 --refresh file list --20160219-pengcancan- start
    private void verifyFiles() {
        for(int i=0;i<mAdapter.getItemCount();i++){
            Cursor cursor = getModel().getItem(mAdapter.getModelId(i));
            DocumentInfo doc = DocumentInfo.fromDirectoryCursor(cursor);
            File tempFile = new File(PrizeUtils.getPath(getActivity(), doc.derivedUri));
            if (!(tempFile != null && tempFile.exists())) {
                DocumentsContract.deleteDocument(getActivity().getContentResolver(), doc.derivedUri);
            }
        }
        mAdapter.notifyDataSetChanged();
    }
    //prize-public-bug:11980 --refresh file list --20160219-pengcancan- end
