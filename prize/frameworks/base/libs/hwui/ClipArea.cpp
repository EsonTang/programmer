...
const ClipBase* ClipArea::serializeIntersectedClip(LinearAllocator& allocator,
    ...
    if (!mLastResolutionResult
        ...
        if (CC_LIKELY(mMode == ClipMode::Rectangle
            ...
            /// M: Dump op's clip which is transformed to global space.
            OP_STATE_LOGD("Op's global clip in case1: mode %d [l=%.2f t=%.2f r=%.2f b=%.2f]",
                recordedClip->mode, rectClip->rect.left, rectClip->rect.top, rectClip->rect.right,
                rectClip->rect.bottom);
            ...
            /// M: Dump op's clip which is transformed to global space.
            SkIRect otherBounds = SkIRect();
            otherBounds = other.getBounds();
            OP_STATE_LOGD("Op's global clip in case2: mode %d [l=%d t=%d r=%d b=%d]",
                recordedClip->mode, otherBounds.left(), otherBounds.top(), otherBounds.right(),
                otherBounds.bottom());
            ...
            /// M: Dump op's clip which is transformed to global space.
            OP_STATE_LOGD("Op's global clip in case3 need to dump with additional steps; mode %d.",
                    recordedClip->mode);
        }
    }
