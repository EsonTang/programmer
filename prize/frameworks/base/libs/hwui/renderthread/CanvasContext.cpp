...
bool CanvasContext::isSwapChainStuffed() {
    /// M: make threshold longer to reduce frame drop @{
    /*
    static const auto SLOW_THRESHOLD = 6_ms;
    */
    static const auto SLOW_THRESHOLD = 20_ms;
    /// @}
...
void CanvasContext::prepareTree(TreeInfo& info, int64_t* uiFrameInfo,
    ...
    // M:Set prepareTextures default value as true @{
...
//    }
    ...
    /// M: Skip triggering damage frame if surface is invalid.
    if (frame.isSurfaceValid()) mEglManager.damageFrame(frame, dirty);
...
void CanvasContext::dumpFrames(int fd) {
    ...
    int dfd = dup(fd);
    if (dfd < 0) {
        ALOGE("Error when dup fd");
        return;
    }
    FILE* file = fdopen(dfd, "a");
    /// @}
