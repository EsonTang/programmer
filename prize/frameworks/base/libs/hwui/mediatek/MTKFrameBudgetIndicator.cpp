...
//#define MTK_PERFSERVICE_SUPPORT 1

#if defined(MTK_PERFSERVICE_SUPPORT)
static int inited = false;

static int (*fbcNotifyFrameComplete)(int) = NULL;
static int (*fbcNotifySwapBuffers)(void) = NULL;
static int (*fbcNotifyNoRender)(void) = NULL;
#endif

typedef int (*notify_frame_complete)(int);
typedef int (*notify_swap_buffers)(void);
typedef int (*notify_no_render)(void);
...
int notifyFrameComplete(int duration)
{
#if defined(MTK_PERFSERVICE_SUPPORT)
    if (!inited)
        init();
    ...
    if (fbcNotifyFrameComplete) {
        return fbcNotifyFrameComplete(duration);
    }
