...
ResolvedRenderState::ResolvedRenderState(LinearAllocator& allocator, Snapshot& snapshot,
    ...
    /// M: Dump draw bounds in global space.
    OP_STATE_LOGD("Op(%p id=%d) global draw bounds: [l=%.2f t=%.2f r=%.2f b=%.2f]",
            &recordedOp, recordedOp.opId, clippedBounds.left, clippedBounds.top,
    ...
    /// M: Dump current clip which is same as parent clip.
    Rect parentClip = Rect();
    parentClip = snapshot.getClipArea().getClipRect();
    OP_STATE_LOGD("Op(%p id=%d) current parent clip [l=%.2f t=%.2f r=%.2f b=%.2f]",
        &recordedOp, recordedOp.opId, parentClip.left, parentClip.top, parentClip.right,
        parentClip.bottom);
    ...
    /// M: Dump final clip which is op clip intersect with current clip.
    OP_STATE_LOGD("Op(%p id=%d) clip intersect with global clip: [l=%.2f t=%.2f r=%.2f b=%.2f]",
            &recordedOp, recordedOp.opId, clipRect.left, clipRect.top,
    ...
    if (CC_UNLIKELY(clipRect.isEmpty() || !clippedBounds.intersects(clipRect))) {
        ...
        OP_STATE_LOGD("Op(%p id=%d): rejected due to empty clip(%d) or bounds not intersect with clip(%d)",
                &recordedOp, recordedOp.opId, clipRect.isEmpty(),
...
BakedOpState* BakedOpState::tryStrokeableOpConstruct(LinearAllocator& allocator,
    ...
    if (bakedState->computedState.clippedBounds.isEmpty()) {
        /// M: Warn when op is rejected due to empty clipped bounds.
        OP_STATE_LOGD("Op(%p id=%d) is rejected due to empty clippedBounds!", &recordedOp, recordedOp.opId);
        // bounds are empty, so op is rejected
