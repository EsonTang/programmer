...
#define SYS_PRIZE_PACKAGE_ID  0x03 /// M: 0x03 is prize-framework-res.apk resource ID
        ...
        if (off < (mStringPoolSize-1)) {
            ...
            *outLen = decodeLength(&str);
            ...
            if ((uint32_t)(str+encLen-strings) < mStringPoolSize) {
                // Reject malformed (non null-terminated) strings
                /*prize-removed by lihuangyuan,for some apk crash -2018-04-12-start*/
                /*if (str[encLen] != 0x00) {
                    ALOGW("Bad string block: string #%d is not null-terminated",
                          (int)idx);
                    return NULL;
                }*/
                /*prize-removed by lihuangyuan,for some apk crash -2018-04-12-end*/
    ...
    if (*s == '@') {
        ...
        if (len == 5 && s[1]=='n' && s[2]=='u' && s[3]=='l' && s[4]=='l') {
            ...
            if (rid != 0) {
                ...
                if (packageId != APP_PACKAGE_ID && packageId != SYS_PACKAGE_ID && packageId != SYS_MTK_PACKAGE_ID && packageId != SYS_PRIZE_PACKAGE_ID) {
            ...
            if (accessor) {
                ...
                if (rid != 0) {
                    ...
                    if (packageId == 0x00) {
                        ...
                        outValue->dataType = Res_value::TYPE_DYNAMIC_REFERENCE;
                        return true;
                    } else if (packageId == APP_PACKAGE_ID || packageId == SYS_PACKAGE_ID || packageId == SYS_MTK_PACKAGE_ID || packageId == SYS_PRIZE_PACKAGE_ID) {
    ...
    , mAppAsLib(appAsLib)
{
    ...
    mLookupTable[SYS_PRIZE_PACKAGE_ID] = SYS_PRIZE_PACKAGE_ID;
}
