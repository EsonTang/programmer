...
extern int register_android_view_PerfFrameInfo(JNIEnv* env);
/// M: App-based AAL @{
...
void AndroidRuntime::start(const char* className, const Vector<String8>& options, bool zygote)
{
    ...
    if (startClass == NULL) {
        ...
        if (startMeth == NULL) {
            ...
            /*prize add for debug some process exit from main thread liuwei 2018-05-30 begin */
            if (env->ExceptionCheck()){
                env->ExceptionDescribe();
                ALOGE("print error log before Shutting down VM\n");
            }
            /*prize add for debug some process exit from main thread liuwei 2018-05-30 end */
...
static const RegJNIRec gRegJNI[] = {
    ...
    REG_JNI(register_android_view_PerfFrameInfo),
/// M: App-based AAL @{
