...
import static android.util.PrizeNavBarColorHintUtil.NB_COLOR_HINT_NONE;
import static android.util.PrizeNavBarColorHintUtil.NB_COLOR_HINT_DISABLE;
import static android.util.PrizeNavBarColorHintUtil.NB_COLOR_HINT_DISABLE_FOR_STARTING_WIN;
import static android.util.PrizeNavBarColorHintUtil.NB_COLOR_HINT_ENABLE_SYS_UI_ITSELF;
import static android.util.PrizeNavBarColorHintUtil.NB_COLOR_HINT_IGNORE_NON_STANDARD_LAYOUT;
import static android.util.PrizeNavBarColorHintUtil.NB_COLOR_HINT_IGNORE_NON_MATERIAL_THEME;
import static android.util.PrizeNavBarColorHintUtil.NB_COLOR_HINT_IGNORE_NAV_BAR_COLOR_FROM_APP;
...
public class DecorView extends FrameLayout implements RootViewSurfaceTaker, WindowCallbacks {
    ...
    private static final String Screenshot_ACTION = "org.android.broadcastreceiverregister.SLIDESCREENSHOT";

    // rewrite codes for three-fingers-screenshot. prize-linkh-20160804
    private boolean mDbgSlidingForScreenShot = false;
    ...
    private long mPoint3DownTime;
    private int mPointContinuanceTime = 230;
    //END...
    /*PRIZE-three-fingers-shotscreen-huangdianjun-2015-04-06-end*/
    ...
    /* prize-add-3 finger move switch, B2T-open split screen, T2B screen shot -liyongli-20180423-start*/
    private boolean m3FingerScreenshotOpened = false;
    private boolean m3FingerSplitScreenOpened = false;
    /* prize-add-3 finger move switch, B2T-open split screen, T2B screen shot -liyongli-20180423-end*/
        ...
        if (DBG_MOTION) {
            ...
            switch (ev.getAction() & ev.ACTION_MASK) {
                case MotionEvent.ACTION_DOWN: {
                    ...
                    /* prize-add-3 finger move switch, B2T-open split screen, T2B screen shot -liyongli-20180423-start
                    *   replace the up lines -- mAllowThreeFingersSlidingForScreenShot =
                    */
                    m3FingerScreenshotOpened = Settings.System.getInt(
                            getContext().getContentResolver(),Settings.System.PRIZE_SLIDE_SCREENSHOT, 0) == 1;
                    m3FingerSplitScreenOpened = Settings.System.getInt(
                            getContext().getContentResolver(),Settings.System.PRIZE_SLIDE_SPLIT_SCREEN, 0) == 1;
                    mAllowThreeFingersSlidingForScreenShot = (m3FingerScreenshotOpened || m3FingerSplitScreenOpened);                            
                    /* prize-add-3 finger move switch, B2T-open split screen, T2B screen shot-liyongli-20180423-end*/
                    ...
                    if (ev.getPointerCount() == 1){
                        mPoint1DownTime = System.currentTimeMillis();
                    }
                    break;
                }
                case MotionEvent.ACTION_POINTER_DOWN: {
                    if (mAllowThreeFingersSlidingForScreenShot) {
                        ...
                        if (ev.getPointerCount() == 2){
                            mPoint2DownTime = System.currentTimeMillis();
                        }
                        if (ev.getPointerCount() == 3) {
                            if (mDbgSlidingForScreenShot) {
                                Log.d(TAG, "ACTION_POINTER_DOWN: detect 3 fingers.");
                            }
                            mPoint3DownTime = System.currentTimeMillis();
                            ...
                            mWindow.getWindowManager().getDefaultDisplay().getMetrics(
                                    mDisplayMetrics);
                        ...
                        if (mPoint1DownTime != 0 && mPoint2DownTime != 0 && mPoint3DownTime != 0){
                            int amongP1P2 = (int) (mPoint2DownTime - mPoint1DownTime);
                            int amongP2P3 = (int) (mPoint3DownTime - mPoint2DownTime);
                            if (amongP1P2 <= mPointContinuanceTime && amongP2P3 <= mPointContinuanceTime){
                                mHasEatThreeFingers = true;
                                MotionEvent eventClone = MotionEvent.obtain(ev);
                                eventClone.setAction(MotionEvent.ACTION_CANCEL);
                                boolean handled = cb != null && !mWindow.isDestroyed() && mFeatureId < 0 ? cb.dispatchTouchEvent(eventClone)
                    ...
                    if (mAllowThreeFingersSlidingForScreenShot) {
                        if (mAreThreeFingerssSliding && mHasEatThreeFingers) {
                            if (mAreThreeFingerssSliding && !mHasStartedScreenShot && ev.getPointerCount() == 3) {                            
                                ...
                                float dyf1 = ev.getY(0) - mFirstPointStartY;
                                float dyf2 = ev.getY(1) - mSecondPointStartY;
                                float dyf3 = ev.getY(2) - mThirdPointStartY;
                                
                                /* prize-add-3 finger move, rewrite for add open split screen-liyongli-20180423-start*/
                                boolean sendbroadcaset = false;
                                String actionStr="x";
                                if (m3FingerScreenshotOpened
                                        && dyf1 > mSlidingDistanceThreshold
                                        && dyf2 > mSlidingDistanceThreshold
                                        && dyf3> mSlidingDistanceThreshold) {
                                    ...
                                    sendbroadcaset = true;
                                    actionStr = Screenshot_ACTION;
                                    //Log.d(TAG, "3lyl ----  screen shot  dyf1= "+dyf1+", "+dyf2+", "+dyf3);                                    
                                }else if (m3FingerSplitScreenOpened
                                        && dyf1 < -mSlidingDistanceThreshold
                                        && dyf2 < -mSlidingDistanceThreshold
                                        && dyf3 < -mSlidingDistanceThreshold) {
                                    mHasStartedScreenShot = true;
                                    sendbroadcaset = true;
                                    actionStr = "org.android.prize.OPENSPLITSCREEN";
                                    //Log.d(TAG, "3lyl ----  splitscreen  dyf1= "+dyf1+", "+dyf2+", "+dyf3);
                                }
                                if(sendbroadcaset){
                                    Context context = getContext();
                                    final Intent intent = new Intent(actionStr);
    ...
    WindowInsets updateColorViews(WindowInsets insets, boolean animate) {
        ...
        if (PrizeOption.PRIZE_NAVBAR_COLOR_CUST) {
            if (DBG_NAV_BAR_COLOR) {
                Log.d(TAG, "updateColorViews() consumingNavBar=" + consumingNavBar
                     + ", mLastShouldAlwaysConsumeNavBar=" + mLastShouldAlwaysConsumeNavBar
                     + ", sysUI=0x" + Integer.toHexString(sysUiVisibility) + ", w=" + mWindow);
            }
        } // @}
     ...
     * Nav bar color customized feature. prize-linkh-2017.08.01 @{
     */
    private static final boolean DBG_NAV_BAR_COLOR = PhoneWindow.DBG_NAV_BAR_COLOR;
    private boolean mIsStandardContentRoot = true;
    private boolean mWindowChanged = false;
    ...
    public void onViewRemoved(View child) {
        if (PrizeOption.PRIZE_NAVBAR_COLOR_CUST) {
            if (mFeatureId >= 0) {
                if (DBG_NAV_BAR_COLOR) {
                    Log.d(TAG, "onViewRemoved() mFeatureId >= 0! Ignore!");
                }
                return;
            }
            
            if (mContentRoot == child) {
                mIsStandardContentRoot = false;
            }
            if (DBG_NAV_BAR_COLOR) {
                Log.d(TAG, "onViewRemoved() child=" + child
                        + ", contentRoot=" + mContentRoot
                        + ", mIsStandardContentRoot=" + mIsStandardContentRoot
                        + ", w=" + mWindow);
            }
        }           
    }
    ...
    public void onViewAdded(View child) {
        if (PrizeOption.PRIZE_NAVBAR_COLOR_CUST) {
            if (mFeatureId >= 0) {
                if (DBG_NAV_BAR_COLOR) {
                    Log.d(TAG, "onViewAdded() mFeatureId >= 0! Ignore!");
                }
                return;
            }
            
            if (mContentRoot == child) {
                mIsStandardContentRoot = true;
            }
    ...
    @Override
    public void onNavBarColorChanged(int color) {
        if (mFeatureId >= 0) {
            if (DBG_NAV_BAR_COLOR) {
                Log.d(TAG, "onNavBarColorChanged() mFeatureId >= 0! Ignore!");
            }
            return;
        }        
    
        WindowManager.LayoutParams lp = mWindow.getAttributes();
        final boolean isInputMethod = lp.type == WindowManager.LayoutParams.TYPE_INPUT_METHOD;
        if (DBG_NAV_BAR_COLOR) {
            Log.d(TAG, "onNavBarColorChanged() color=0x" + Integer.toHexString(color)
                    + ", mIsFloating=" + mWindow.mIsFloating + ", isInputMethod=" + isInputMethod
                    + ", mIsNavBarColorPickerActivity=" + mWindow.mIsNavBarColorPickerActivity
                    + ", mShouldUseSystemCustNavBarColor=" + mWindow.mShouldUseSystemCustNavBarColor 
                    + ", w=" + mWindow
                    + ", this=" + Integer.toHexString(System.identityHashCode(this)));
        }
        if ((mWindow.mSystemCustNavBarColor == color && !mWindowChanged) 
            || (((lp.dimAmount != 1.0f && lp.dimAmount != 0.0f) && !mWindow.getIgnoreDimAmountAdjustment()) 
                && !isInputMethod && !mWindow.mIsNavBarColorPickerActivity)) {
            if (DBG_NAV_BAR_COLOR) {
                Log.d(TAG, "onNavBarColorChanged() Nothing to do!");
            }

            return;
        }
        
        mWindow.mSystemCustNavBarColor = color;
        mWindowChanged = false;

        if (mNavigationGuard != null) {
            // An input method window. So always use cust nav bar color
            mNavigationGuard.setBackgroundColor(mWindow.mSystemCustNavBarColor);
        }

        // force to fire a relayout event to notify wms
        if (mLayoutParams != null && mLayoutParams != lp) {
            // Maybe the layout param from this window is different from that from ViewRootImpl.
            // Eg. Dialog.
            lp = mLayoutParams;
            if (DBG_NAV_BAR_COLOR) {
                Log.d(TAG, "onNavBarColorChanged() Dispatch this layout params { " + Integer.toHexString(System.identityHashCode(lp))
                + "} " + lp);            
            }
        }
        mWindow.dispatchWindowAttributesChanged(lp);
    }
    
    @Override
    public boolean applyNavBarColorParams(WindowManager.LayoutParams attrs) {
        if (mFeatureId >= 0) {
            if (DBG_NAV_BAR_COLOR) {
                Log.d(TAG, "applyNavBarColorParams() mFeatureId >= 0! Ignore!");
            }
            return false;
        }
    
        final boolean hasLayoutNoLimitsFlag = (attrs.flags & WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS) != 0;
        final boolean disableCustNavBarColor = mWindow.getDisableCustNavBarColor();
        final boolean isAppWin = attrs.type >= WindowManager.LayoutParams.FIRST_APPLICATION_WINDOW
                && attrs.type <= WindowManager.LayoutParams.LAST_APPLICATION_WINDOW;
        final boolean isInputMethodWin = attrs.type == WindowManager.LayoutParams.TYPE_INPUT_METHOD
                || attrs.type == WindowManager.LayoutParams.TYPE_INPUT_METHOD_DIALOG;;
        final boolean hasAppRequestedDrawsSysBarBgFlag = !mWindow.mHasAddedSystemBarBackgroudsFlag && (attrs.flags & FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS) != 0;
    
        boolean updated = false;
        boolean disableCustNavBarForSpecialApp = false;
        //boolean hasAddedDrawSysBarBkFlag = false;
        int navBarColorHint = NB_COLOR_HINT_NONE;
        mLayoutParams = attrs;
        
        if (DBG_NAV_BAR_COLOR) {
            Log.d(TAG, "applyNavBarColorParams() old attrs{ " + Integer.toHexString(System.identityHashCode(attrs))
                + "} " + attrs
                + ", mHasAddedSystemBarBackgroudsFlag=" + mWindow.mHasAddedSystemBarBackgroudsFlag
                + ", w=" + mWindow + ", this=" + Integer.toHexString(System.identityHashCode(this)));
        }
    
        // force resetting.        
        mWindowChanged = false;
        attrs.alwaysDisableNavbarColorCust = false;
        attrs.navBarColor = 0;
        attrs.enableSystemUINavBarColorBackground = false;
        mWindow.mShouldUseSystemCustNavBarColor = false;
        mWindow.mForceUseNavBarColorFromApp = false;
        
        ComponentName cn = null;
        final Window.Callback callback = mWindow.getCallback();
        if (DBG_NAV_BAR_COLOR) {
            Log.d(TAG, "applyNavBarColorParams() callback=" + callback);
        }
    
        if (callback != null) {
            if (callback instanceof Activity) {
                cn = ((Activity)callback).getComponentName();
            } else if (mContext instanceof Activity) {
                // This win(Eg: Wechat) has its custom callback. So we try 
                // to get component through Context.
                cn = ((Activity)mContext).getComponentName();
            }
            if (DBG_NAV_BAR_COLOR) {
                Log.d(TAG, "applyNavBarColorParams() cn=" + cn);
            }

            /*
            if (!mWindow.mIsNavBarColorPickerActivity && cn != null && 
                cn.getClassName().contains("com.android.settings.PrizeNavBarColorPickerActivity")) {
                mWindow.mIsNavBarColorPickerActivity = true;
                if (DBG_NAV_BAR_COLOR) {
                    Log.d(TAG, "applyNavBarColorParams() it's NavBarColorPickerActivity. Should enable cust nav bar color.");
                }
           } */
        }
    
        if (cn != null) {
            navBarColorHint = getCustNavBarColorHint(cn, null);
        } else {
            navBarColorHint = getCustNavBarColorHint(null, mContext.getPackageName());
        }
        if (DBG_NAV_BAR_COLOR) {
            Log.d(TAG, "applyNavBarColorParams() navBarColorHint=" + getNavBarColorHintDescr(navBarColorHint));
        }
                
        if (disableCustNavBarColor || (navBarColorHint & (NB_COLOR_HINT_DISABLE | NB_COLOR_HINT_DISABLE_FOR_STARTING_WIN)) != 0) {
            attrs.alwaysDisableNavbarColorCust = true;
            if (DBG_NAV_BAR_COLOR) {
                Log.d(TAG, "applyNavBarColorParams() Disable cust nav bar color!");
            }                
        } else if ((isAppWin || isInputMethodWin) && 
            ((attrs.dimAmount == 0.0f || attrs.dimAmount == 1.0f) || mWindow.getIgnoreDimAmountAdjustment())) {
            if (hasAppRequestedDrawsSysBarBgFlag) {
                // app asks for it explictly.
                final boolean isNotAllowedNavBarColor = (mWindow.mNavigationBarColor == 0xFF000000);
                if (DBG_NAV_BAR_COLOR) {
                    Log.d(TAG, "applyNavBarColorParams() App has requested FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS flag itself!"
                        + ", mNavigationBarColor=0x" + Integer.toHexString(mWindow.mNavigationBarColor)
                        + ", isNotAllowedNavBarColor=" + isNotAllowedNavBarColor);
                }
    
                if (isNotAllowedNavBarColor || 
                    ((navBarColorHint & NB_COLOR_HINT_IGNORE_NAV_BAR_COLOR_FROM_APP) != 0)) {
                    // use system cust nav bar color.
                    mWindow.mShouldUseSystemCustNavBarColor = true;
                    if (DBG_NAV_BAR_COLOR) {
                        Log.d(TAG, "applyNavBarColorParams() Force use cust nav bar color-0x" 
                                + Integer.toHexString(mWindow.mSystemCustNavBarColor));
                    }
                } else {
                    if (DBG_NAV_BAR_COLOR) {
                        Log.d(TAG, "applyNavBarColorParams() Retain nav bar color from app. color: 0x"  
                                + Integer.toHexString(mWindow.mNavigationBarColor));
                    }
    
                    //attrs.alwaysDisableNavbarColorCust = true;
                    mWindow.mForceUseNavBarColorFromApp = true;
                    // Use it to make that the icons of nav bar can fit this color.
                    mWindow.mShouldUseSystemCustNavBarColor = true;
                }
            } else {
                if (DBG_NAV_BAR_COLOR) {
                    Log.d(TAG, "applyNavBarColorParams() add FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS.");
                }
                mWindow.mShouldUseSystemCustNavBarColor = true;
                mWindow.mHasAddedSystemBarBackgroudsFlag = true;
                attrs.flags |= FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS;
            }
        }/* else if (mWindow.mIsNavBarColorPickerActivity && (attrs.flags & FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS) != 0) {
            // Our nav bar color picker activity with setting dim amount.
            // This win needs show user nav bar color.
            mWindow.mShouldUseSystemCustNavBarColor = true;
            if (DBG_NAV_BAR_COLOR) {
                Log.d(TAG, "applyNavBarColorParams() Nav Bar Color Picker Activity! It should use sys cust nav bar color!");
            }
        } */
    
        if (DBG_NAV_BAR_COLOR) {
            Log.d(TAG, "applyNavBarColorParams() mShouldUseSystemCustNavBarColor=" + mWindow.mShouldUseSystemCustNavBarColor);
        }

        attrs.hasSetNavBarColor = mWindow.mShouldUseSystemCustNavBarColor;
        if (mWindow.mShouldUseSystemCustNavBarColor) {
            if (hasLayoutNoLimitsFlag || 
                (!hasAppRequestedDrawsSysBarBgFlag && !mIsStandardContentRoot && (navBarColorHint & NB_COLOR_HINT_IGNORE_NON_STANDARD_LAYOUT) == 0)
                || (navBarColorHint & NB_COLOR_HINT_ENABLE_SYS_UI_ITSELF) != 0
                || mWindow.getEnableSystemUINavBarColorBackground()) {
                // Not compatible with this flag. Progate nav bar cust color to System UI itself.
                if (mWindow.mHasAddedSystemBarBackgroudsFlag) {
                    mWindow.mHasAddedSystemBarBackgroudsFlag = false;
                    attrs.flags &= ~FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS;
                }
    
                if (DBG_NAV_BAR_COLOR) {
                    Log.d(TAG, "applyNavBarColorParams() Enable nav bar color in system ui! hasLayoutNoLimitsFlag=" 
                        + hasLayoutNoLimitsFlag
                        + ", mIsStandardContentRoot=" + mIsStandardContentRoot
                        + ", navBarColorHint=" + navBarColorHint);
                }
    
                attrs.enableSystemUINavBarColorBackground = true;
            } else if (!hasAppRequestedDrawsSysBarBgFlag /* && !mWindow.mIsMaterialTheme 
                && (navBarColorHint & NB_COLOR_HINT_IGNORE_NON_MATERIAL_THEME) == 0 */) {
                if (mWindow.mHasAddedSystemBarBackgroudsFlag) {
                    mWindow.mHasAddedSystemBarBackgroudsFlag = false;
                    attrs.flags &= ~FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS;
                }
                ...
                if (DBG_NAV_BAR_COLOR) {
                    //Log.d(TAG, "applyNavBarColorParams() Enable nav bar color in system ui! Because this win" 
                      //  + " doesn't use Material Theme!");
                     Log.d(TAG, "applyNavBarColorParams() Enable nav bar color in system ui! Because this win" 
                        + " doesn't request FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS!");                          
                }
            }/* else if (mWindow.getIsStartingWindow() && (getBackground() != null || mBackgroundFallback.hasFallback())
                    && !hasAppRequestedDrawsSysBarBgFlag) {
                if (mWindow.mHasAddedSystemBarBackgroudsFlag) {
                    mWindow.mHasAddedSystemBarBackgroudsFlag = false;
                    attrs.flags &= ~FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS;
                }
                ...
                if (DBG_NAV_BAR_COLOR) {
                    Log.d(TAG, "applyNavBarColorParams() Enable nav bar color in system ui! Because this starting win" 
                        + " has set background!");
                }
            } */
    
            if (attrs.type == WindowManager.LayoutParams.TYPE_INPUT_METHOD) {
                // Solved bug-39862. Input method window can't use the latest color.
                // Ok, we read from settings again.
                final int newColor = mWindow.getSystemCustNavBarColor();
                if (mWindow.mSystemCustNavBarColor != newColor) {
                    mWindow.mSystemCustNavBarColor = newColor;
                    if (mNavigationGuard != null) {
                        mNavigationGuard.setBackgroundColor(mWindow.mSystemCustNavBarColor);
                    }
                }
            ...
            if (mWindow.mForceUseNavBarColorFromApp) {
                attrs.navBarColor = mWindow.mNavigationBarColor;
            } else {
                attrs.navBarColor = mWindow.mSystemCustNavBarColor;
            }
        }
    
        if (DBG_NAV_BAR_COLOR) {
            Log.d(TAG, "applyNavBarColorParams() new attrs: " + attrs);
        }
    ...
    private String getNavBarColorHintDescr(int hint) {
        return mWindow.mPrizeNavBarColorHintUtil.getNavBarColorHintDescription(hint);
    }
    
    private int getCustNavBarColorHint(ComponentName cn, String pkg) {
        int hint = NB_COLOR_HINT_NONE;
        
        if (mWindow.getIsStartingWindow()) {
            if (mWindow.mDisableNavbarColorCustInStartingWinFromTheme) {
                if (DBG_NAV_BAR_COLOR) {
                    Log.d(TAG, "getCustNavBarColorHint() Starting win requests disable cust nav bar color.");
                }
                hint = NB_COLOR_HINT_DISABLE;
            } /* else if (mContentLayoutResIdForStartingWin == R.layout.screen_simple) {
                // eg. MiShiTaoTuo game starting win. Not exactly right. See bug-39765
                if (false) { //(mBackgroundDrawable == null && mBackgroundResource < 0x7f000000) {
                    if (DBG_NAV_BAR_COLOR) {
                        Log.d(TAG, "getCustNavBarColorHint() Starting win seems that it doesn't need cust nav bar color. So disable it!");
                    }
                    hint = NB_COLOR_HINT_DISABLE;
                }
            } */
            else if (cn == null && pkg != null && !pkg.equals("android")) {
                // Starting win can't have component and only has package name.
                hint = mWindow.mPrizeNavBarColorHintUtil.getNavBarColorHint(pkg);
                // Just consider NB_COLOR_HINT_DISABLE_FOR_STARTING_WIN hint.
                hint &= NB_COLOR_HINT_DISABLE_FOR_STARTING_WIN;
            }
        } else if (cn != null) {
            if (mWindow.mLastNavBarColorHint < 0) {
                hint = mWindow.mPrizeNavBarColorHintUtil.getNavBarColorHint(cn);
                mWindow.mLastNavBarColorHint = hint;
            } else {
                if (DBG_NAV_BAR_COLOR) {
                    Log.d(TAG, "getCustNavBarColorHint() use last hint. mLastNavBarColorHint=" + getNavBarColorHintDescr(mWindow.mLastNavBarColorHint));
                }
                hint = mWindow.mLastNavBarColorHint;
            }
        ...
        if (DBG_NAV_BAR_COLOR) {
            Log.d(TAG, "getCustNavBarColorHint() cn=" + cn 
                + ", pkg=" + pkg
                + ", isStartingWin=" + mWindow.getIsStartingWindow()
                + ", hint=" + getNavBarColorHintDescr(hint));
        }
        return hint;
    }
    // @}  
    ...
    private void updateColorViewInt(final ColorViewState state, int sysUiVis, int color,
        ...
        // Nav bar color customized feature. prize-linkh-2017.08.01 @{
        if (PrizeOption.PRIZE_NAVBAR_COLOR_CUST) {
            if (mFeatureId < 0 && state == mNavigationColorViewState && !mWindow.getDisableCustNavBarColor()
                && mWindow.mShouldUseSystemCustNavBarColor) {
                if (DBG_NAV_BAR_COLOR) {
                    Log.d(TAG, "updateColorViewInt() mNavigationBarColor=0x" + Integer.toHexString(mWindow.mNavigationBarColor));
                }
               
                if (((sysUiVis & View.SYSTEM_UI_FLAG_LOW_PROFILE) == 0) && 
                    ((sysUiVis & SYSTEM_UI_FLAG_HIDE_NAVIGATION) == 0)) {
                    // If nav bar sysUi is in low_profile / hide mode, then 
                    // we can't set cust nav bar color.
                    if (mWindow.mForceUseNavBarColorFromApp) {
                        if (DBG_NAV_BAR_COLOR) {
                            Log.d(TAG, "updateColorViewInt() use nav bar color from app. color-0x"
                                    + Integer.toHexString(mWindow.mNavigationBarColor));
                        }
                        color = mWindow.mNavigationBarColor;
                    } else {
                        if (DBG_NAV_BAR_COLOR) {
                            Log.d(TAG, "updateColorViewInt() force using system cust nav bar color--0x"
                                    + Integer.toHexString(mWindow.mSystemCustNavBarColor));
                        }
                        color = mWindow.mSystemCustNavBarColor;
                    }
                } else if (DBG_NAV_BAR_COLOR) {
                    Log.d(TAG, "updateColorViewInt() Nav Bar is in low_profile/hide mode! Ignore cust nav bar color!");
                }
            }
        
            if (DBG_NAV_BAR_COLOR) {
                if (state == mNavigationColorViewState) {
                    Log.d(TAG, "updateColorViewInt() size=" + size + ", hasSystemUiHideFlag="
                        + ((sysUiVis & state.systemUiHideFlag) != 0) + ", hasHideWindowFlag="
                        + ((mWindow.getAttributes().flags & state.hideWindowFlag) != 0) + ", hasDrawsBG="
                        + ((mWindow.getAttributes().flags & FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS) != 0) + ", hasTranslucentFlag="
                        + ((mWindow.getAttributes().flags & state.translucentFlag) != 0) + ", sysUiVis=0x" + Integer.toHexString(sysUiVis)
                        + ", mShouldUseSystemCustNavBarColor=" + mWindow.mShouldUseSystemCustNavBarColor
                        + ", disableCustNavBarColor=" + mWindow.getDisableCustNavBarColor()
                        + ", mFeatureId=" + mFeatureId
                        + ", view=" + state.view + ", w=" + mWindow
                        + ", this=" + Integer.toHexString(System.identityHashCode(this)));
                }
            }
        }
    ...
    void updateNavigationGuardColor() {
        if (mNavigationGuard != null) {
            ...
            // Nav bar color customized feature. prize-linkh-20171018 @{
            if (PrizeOption.PRIZE_NAVBAR_COLOR_CUST) {
                // Always Show nav bar to show nav bar color.
                mNavigationGuard.setVisibility(View.VISIBLE);
            } //@}            
        }
    }
    ...
    void setWindow(PhoneWindow phoneWindow) {
        // Nav bar color customized feature. prize-linkh-20171020 @{
        if (PrizeOption.PRIZE_NAVBAR_COLOR_CUST) {
            mWindowChanged = phoneWindow != mWindow;

            if (DBG_NAV_BAR_COLOR) {
                Log.d(TAG, "setWindow() oldW=" + mWindow 
                    + ", newW=" + phoneWindow
                    + ", this=" + Integer.toHexString(System.identityHashCode(this)));
            }
        } // @}     
