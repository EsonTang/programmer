...
public class PhoneWindow extends Window implements MenuBuilder.Callback {
    ...
    // Nav bar color customized feature. prize-linkh-2017.08.01 @{
    static final boolean DBG_NAV_BAR_COLOR = "eng".equals(Build.TYPE);
    boolean mIsMaterialTheme = false;
    boolean mHasNavBarColorFromTheme = false;
    boolean mShouldUseSystemCustNavBarColor = false;
    boolean mDisableNavbarColorCustInStartingWinFromTheme = false;
    boolean mIsNavBarColorPickerActivity = false;
    boolean mHasAddedSystemBarBackgroudsFlag = false;
    boolean mForceUseNavBarColorFromApp = false;
    int mLastNavBarColorHint = -1;
    int mSystemCustNavBarColor;
    int mContentLayoutResIdForStartingWin;
    PrizeNavBarColorHintUtil mPrizeNavBarColorHintUtil;
    // @}
    ...
    public PhoneWindow(Context context) {
        ...
        // Nav bar color customized feature. prize-linkh-20171019 @{
        if (PrizeOption.PRIZE_NAVBAR_COLOR_CUST) {
            mPrizeNavBarColorHintUtil = PrizeNavBarColorHintUtil.getInstance(context);
        }
        // @}        
    }
    ...
    public PhoneWindow(Context context, Window preservedWindow) {
        ...
        if (preservedWindow != null) {
            ...
            // Nav bar color customized feature. prize-linkh-20171019 @{
            if (PrizeOption.PRIZE_NAVBAR_COLOR_CUST) {
                adjustDataFromWindow(preservedWindow);
            }
            // @}
        }
    ...
    // Nav bar color customized feature. prize-linkh-20179018 @{    
    @Override
    public void setFlags(int flags, int mask) {
        if (PrizeOption.PRIZE_NAVBAR_COLOR_CUST) {
            if ((mask & FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS) != 0) {
                if ((flags & FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS) != 0) {
                    // Add operation. This win requests its FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS flag.
                    if (DBG_NAV_BAR_COLOR) {
                        if (mHasAddedSystemBarBackgroudsFlag) {
                            Log.d(TAG, "setFlags() this win requests FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS flag."
                                + " Reset mHasAddedSystemBarBackgroudsFlag!");
                        } else {
                            Log.d(TAG, "setFlags() this win requests FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS flag!");
                        }
                    }
                    ...
                    if (DBG_NAV_BAR_COLOR) {
                        if (mHasAddedSystemBarBackgroudsFlag) {
                            Log.d(TAG, "setFlags() this win wants to clear FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS flag."
                                + " Reset mHasAddedSystemBarBackgroudsFlag!");
                        } else {
                            Log.d(TAG, "setFlags() this win wants to clear FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS flag!");
                        }
                ...
                // If app invokes getAttributes(), and modifies FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS flag, then invokes setAttributes(), 
                // ooooh, we can't determine if we should reset mHasAddedSystemBarBackgroudsFlag!
                mHasAddedSystemBarBackgroudsFlag = false;
            }
        }
    ...
    boolean getIsStartingWindow() {
        return mIsStartingWindow;
    }

    int getSystemCustNavBarColor() {
        return Settings.System.getInt(
                getContext().getContentResolver(),Settings.System.PRIZE_NAV_BAR_BG_COLOR, 
                StatusBarManager.DEFAULT_NAV_BAR_COLOR);
    }

    private void adjustDataFromWindow(Window win) {
        if (DBG_NAV_BAR_COLOR) {
            Log.d(TAG, "adjustDataFromWindow() w=" + win
                + ", host=" + this);
        }

        if (win == null || !(win instanceof PhoneWindow)) {
            return;
        }

        PhoneWindow w = (PhoneWindow)win;
        mIsMaterialTheme = w.mIsMaterialTheme;
        mHasNavBarColorFromTheme = w.mHasNavBarColorFromTheme;
        mShouldUseSystemCustNavBarColor = w.mShouldUseSystemCustNavBarColor;
        mDisableNavbarColorCustInStartingWinFromTheme = w.mDisableNavbarColorCustInStartingWinFromTheme;
        mIsNavBarColorPickerActivity = w.mIsNavBarColorPickerActivity;
        mHasAddedSystemBarBackgroudsFlag = w.mHasAddedSystemBarBackgroudsFlag;
        mForceUseNavBarColorFromApp = w.mForceUseNavBarColorFromApp;
        mLastNavBarColorHint = w.mLastNavBarColorHint;
        // Retrieve the newest state.
        mSystemCustNavBarColor = w.mSystemCustNavBarColor = getSystemCustNavBarColor();
        mContentLayoutResIdForStartingWin = w.mContentLayoutResIdForStartingWin;  
    }
    // @}
    ...
    protected ViewGroup generateLayout(DecorView decor) {
        ...
        // Nav bar color customized feature. prize-linkh-2017.08.01 @{
        if (PrizeOption.PRIZE_NAVBAR_COLOR_CUST) {
            mHasNavBarColorFromTheme = a.hasValue(R.styleable.Window_navigationBarColor);
            mSystemCustNavBarColor = getSystemCustNavBarColor();

            if (mIsStartingWindow) {
                TypedArray ta = context.obtainStyledAttributes(
                            new int[] {android.R.attr.disableNavbarColorCustInStartingWin});
                mDisableNavbarColorCustInStartingWinFromTheme = ta.getBoolean(0, false);
                ta.recycle();
            } else {
                mDisableNavbarColorCustInStartingWinFromTheme = false;
            }

            TypedArray ta = context.obtainStyledAttributes(
                        new int[] {android.R.attr.isMaterialTheme});
            mIsMaterialTheme = ta.getBoolean(0, false);
            ta.recycle();

            if (DBG_NAV_BAR_COLOR) {
                Log.d(TAG, "mHasNavBarColorFromTheme=" + mHasNavBarColorFromTheme
                        + ", mIsMaterialTheme=" + mIsMaterialTheme
                        + ", mDisableNavbarColorCustInStartingWinFromTheme=" + mDisableNavbarColorCustInStartingWinFromTheme
                        + ", mSystemCustNavBarColor=0x" + Integer.toHexString(mSystemCustNavBarColor)
                        + ", mIsStartingWindow=" + mIsStartingWindow);
            }
        } //@}        
        ...
        if (mIsStartingWindow) {
            ...
            if (mIsFloating) {
				...
				if(PrizeOption.PRIZE_NOTCH_SCREEN){
					layoutResource = R.layout.prize_screen_toolbar;
				}else{
        ...
        if (PrizeOption.PRIZE_NAVBAR_COLOR_CUST) {
            // Note: Don't use mIsStaringWindow variable in this method!
            if (getAttributes().type == WindowManager.LayoutParams.TYPE_APPLICATION_STARTING) {
                mContentLayoutResIdForStartingWin = layoutResource;            
                if (DBG_NAV_BAR_COLOR) {
                    Log.d(TAG, "generateLayout() mContentLayoutResIdForStartingWin=0x" + Integer.toHexString(mContentLayoutResIdForStartingWin)
                        + ", mBackgroundResource=0x" + Integer.toHexString(mBackgroundResource)
    ...
    public void setNavigationBarColor(int color) {
        ...
        // Nav bar color customized feature. prize-linkh-20171013 @{
        if (PrizeOption.PRIZE_NAVBAR_COLOR_CUST) {
            if (DBG_NAV_BAR_COLOR) {
                Log.d(TAG, "setNavigationBarColor() color=0x" + Integer.toHexString(color));
            }
            // This call can make applyNavBarColorParams method to be called 
            // to update cust nav bar color.
            dispatchWindowAttributesChanged(getAttributes());
        } // @}
