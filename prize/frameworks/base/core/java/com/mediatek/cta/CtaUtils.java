...
public class CtaUtils {
    ...
    public static boolean enforceCheckPermission(final String pkgName, final String permission,
        ...
        if (!isPackageRequestPermission(pkgName, permission, UserHandle.getUserId(uid))) {
            Slog.d(TAG, "enforceCheckPermission(): pkg = " + pkgName + "does not request "
    ...
    private static boolean isPackageRequestPermission(final String pkgName, final String perm,
        ...
        try {
            ...
            if (pkgInfo == null) {
                Slog.w(TAG, "isPackageRequestPermission pkgInfo is null for pkg: " + pkgName);
                return false;
            }
            return Arrays.asList(pkgInfo.requestedPermissions).contains(perm);
        } catch (RemoteException e) {
            Slog.w(TAG, "isPackageRequestPermission RemoteException", e);
            return false;
        }
    }
