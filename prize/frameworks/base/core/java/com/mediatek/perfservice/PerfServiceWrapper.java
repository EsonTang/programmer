...
public class PerfServiceWrapper implements IPerfServiceWrapper {
    ...
    private static final int RENDER_THREAD_UPDATE_DURATION = 200;
    ...
    public void notifyFrameUpdate(int level) {
        ...
        try {
            ...
            if (mPreviousTime == 0 || (nowTime - mPreviousTime) > RENDER_THREAD_UPDATE_DURATION) {
                ...
                if (not_gl == 0)
                    boostEnableTimeoutMs(IPerfServiceWrapper.SCN_GLSURFACE, GLSURFACE_BOOST_TIME);
    ...
    public void amsBoostProcessCreate(AMEventHookData.StartProcessForActivity data) {
        ...
        if(reason.compareTo("activity") == 0) {
            String nextPkg = data.getString(StartProcessForActivity.Index.packageName);
