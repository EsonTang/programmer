...
/* performance debug
    private static long latency[] = new long[50];
    private static long count[] = new long[50];
 */
    ...
    static final class SensorEventQueue extends BaseEventQueue {
        ...
        protected void dispatchSensorEvent(int handle, float[] values, int inAccuracy,
            ...
            /* performance debug
            if(1 == t.accuracy ) {
                latency[handle] += SystemClock.elapsedRealtimeNanos() - t.timestamp;
                count[handle] ++;
            }
            if(1000 == count[handle]) {
                Log.d("SensorPerformance",
                    "sensor["+handle+"] latency=" + latency[handle]/count[handle] );
                count[handle] = 0;
                latency[handle] = 0;
            }
            */
