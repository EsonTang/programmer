    ...
    private final IBluetoothStateChangeCallback mBluetoothStateChangeCallback =
        ...
        public void onBluetoothStateChange(boolean up) {
            ...
            synchronized (mConnection) {
                if (!mBindDunService) {
                    Log.d(TAG,
                        "DUN is not enabled in project configuration. Ignore BT state change.");
                    return;
                }
                if (!up) {
    ...
    public BluetoothDun(Context context, ServiceListener l) {
        ...
        synchronized (mConnection) {
            if (dunEnabled.isEmpty() || (Integer.parseInt(dunEnabled) == 0)) {
                ...
                // DUN is not enabled in project configuration. Do not bind DunService
                mBindDunService = false;
            } else {
                mBindDunService = doBind();
            }
        }
    }
