...
//END...
/*PRIZE-add for anti brushing machine-wangxianzhen-2016-03-04-start*/
import com.mediatek.common.prizeoption.PrizeOption;
/*PRIZE-add for anti brushing machine-wangxianzhen-2016-03-04-end*/
...
public class PackageParser {
    ...
    // add for conveniently managing third apps that are prebuilt in system. prize-linkh-20160525
    public final static int PARSE_IS_PREBUILT_THIRD_APPS = 1 << 14;
    ...
    private boolean parseBaseApplication(Package owner, Resources res,
        ...
        if ((flags&PARSE_IS_SYSTEM) != 0) {
            if (sa.getBoolean(
                    ...
                    false)) {                
                // add for conveniently managing third apps that are prebuilt in system. prize-linkh-20160525
                // If it's a prebuilt third app, we don't allow to give it this property.
                if(PrizeOption.PRIZE_REMOVE_PERSISTENT_PROP_FOR_THIRD_APP && (flags & PARSE_IS_PREBUILT_THIRD_APPS) != 0) {
                    Slog.i(TAG, "Remove Persistent prop for " + pkgName);
                } else { //end..
    ...
    /** @} 2014-12-02 */
        ...
        // If we have a pre-defined IME which hasn't boot aware flag, system can't 
        // choose this IME. So we need to add this flag for this IME. 
        final String preInstalledIME = SystemProperties.get("ro.prize_default_ime");
        if (preInstalledIME != null && (ai.privateFlags & ApplicationInfo.PRIVATE_FLAG_DIRECT_BOOT_AWARE) == 0
                && preInstalledIME.contains(pkgName)) {
            Slog.w(TAG, "Add boot aware flag for " + pkgName + ", because it's a pre-defined ime!");
            ai.privateFlags |= ApplicationInfo.PRIVATE_FLAG_DIRECT_BOOT_AWARE;
        } // END...
        ...
        /* prize-modify-split screen-liyongli-20170802-start */
        if (PrizeOption.PRIZE_SPLIT_SCREEN_ALLAPP
                && (ai.privateFlags & PRIVATE_FLAG_RESIZEABLE_ACTIVITIES) == 0) {
            //Slog.w(TAG, "------  resize parseBaseApplication : " + pkgName +" "+ai.versionCode  );
            if ((flags&PARSE_IS_SYSTEM) == 0|| (flags & PARSE_IS_PREBUILT_THIRD_APPS) != 0) {
                if( PrizeSplitScreenApk.apkCanSplitScreen(pkgName) ){
                    Slog.w(TAG, "---  apkCanSplitScreen parseBaseApplication : " + pkgName +" "+ai.versionCode  );
                    ai.privateFlags |= PRIVATE_FLAG_RESIZEABLE_ACTIVITIES;
                }
            }
        }
        /* prize-modify-split screen-liyongli-20170802-end */
        
        /* prize-modify-split screen-liyongli-20170830-start */
        boolean prizeChangeForceResize = false;
        if (PrizeOption.PRIZE_SPLIT_SCREEN_ALLAPP) {
            //Slog.w(TAG, "---  parseBaseApplication : " + pkgName +" "+(flags&PARSE_IS_SYSTEM)  );
            if ( (flags&PARSE_IS_SYSTEM) == 0 ) {
                if( PrizeSplitScreenApk.apkNotSplitScreen(pkgName) ){
                    Slog.w(TAG, "---  apkNotSplitScreen parseBaseApplication : " + pkgName +" "+ai.versionCode  );
                    prizeChangeForceResize = true;
                }
            }else{
                if( PrizeSplitScreenApk.sysApkNotSplitScreen(pkgName) ){
                    Slog.w(TAG, "---  sysApkNotSplitScreen  parseBaseApplication : " + pkgName +" "+ai.versionCode  );
                    prizeChangeForceResize = true;
                }
            }
        }
        /* prize-modify-split screen-liyongli-20170830-end */
        ...
        while ((type = parser.next()) != XmlPullParser.END_DOCUMENT
            ...
            if (tagName.equals("activity")) {
                ...
                /* prize-modify-split screen-liyongli-20170830-start */
                if( prizeChangeForceResize
                     //&& a.info.resizeMode == RESIZE_MODE_FORCE_RESIZEABLE  //com.sohu.sohuvideo NOT to split screen  2017/10/24  modify
                   ){
                    a.info.resizeMode = RESIZE_MODE_UNRESIZEABLE;
                    //Slog.w(TAG, "---  RESIZE_MODE_UNRESIZEABLE : " +a );
                }
                /* prize-modify-split screen-liyongli-20170830-end */
    ...
    private Activity parseActivity(Package owner, Resources res,
        ...
        if (!receiver) {
            ...
            /* prize-modify-split screen  com.UCMobile  FORCE to split screen-liyongli-20171024-start */
...
// liyongli use bellow , replace  the line above (original code)
            boolean prize_resizeable = sa.getBoolean(
                    R.styleable.AndroidManifestActivity_resizeableActivity, appDefault);
            if( PrizeOption.PRIZE_SPLIT_SCREEN_ALLAPP && appDefault && !prize_resizeable
                  && owner.applicationInfo.targetSdkVersion < Build.VERSION_CODES.N
            ){
                //Slog.w(TAG,"lyl  resize  resizeable FORCE TO true ");
                prize_resizeable = true;
            }
            final boolean resizeable = prize_resizeable;
            /* prize-modify-split screen-liyongli-20171024-end */
