...
import com.mediatek.common.prizeoption.PrizeOption; // prize-add-LiuHai fix iqiyi YouKu ROTATION_90 paly movie, not in MID -liyongli-20180711
...
public class View implements Drawable.Callback, KeyEvent.Callback,
    ...
    */
    ...
    void dispatchDetachedFromWindow() {
        ...
        mAttachInfo = null;
        if (DBG_MOTION) {
            Log.d(VIEW_LOG_TAG, "After dispatchDetachedFromWindow: this = " + this
                    + ", mAttachInfo = null ");
        }
        if (mOverlay != null) {
    ...
    public final void measure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        /* prize-add-LiuHai, YouKu ROTATION_90 paly movie, button cover with LiuHai rect-liyongli-20180711-start */
        if(false && PrizeOption.PRIZE_NOTCH_SCREEN ){//prize-deleted by lihuangyuan-for lihai,2018-08-08-see moved to DisplayInfo.java:DisplayInfo(Parcel source)
            if(MeasureSpec.getSize(widthMeasureSpec)==2246){ //newInfo.logicalWidth
                widthMeasureSpec -= 87;//com.android.internal.R.dimen.status_bar_height
            }
 /* bellow code will cause SoHuShiPin BoBoShiPin 0 to 90 Block -- liyongli
            DisplayInfo newInfo = DisplayManagerGlobal.getInstance().getDisplayInfo(Display.DEFAULT_DISPLAY);
           if(newInfo!=null && (newInfo.rotation == Surface.ROTATION_90 || newInfo.rotation == Surface.ROTATION_270) ){
                if( MeasureSpec.getSize(widthMeasureSpec)==newInfo.logicalWidth ){
                    int h = mResources.getDimensionPixelSize(com.android.internal.R.dimen.status_bar_height);
                    widthMeasureSpec -= h;
                    //Log.d("viewmeasure", "---w-- "+ newInfo.logicalWidth  +" "+ getClass().getName() );//, new Throwable()  );
                }
            }*/
        }
        /* prize-add-LiuHai, YouKu ROTATION_90 paly movie, button cover with LiuHai rect-liyongli-20180711-end */
    ...
    /** {@hide} */
    public boolean applyNavBarColorParams(WindowManager.LayoutParams a) {
        return false;
    }
    // @}  
    ...
    /** {@hide} */
    public void clearAllMeasureCache() {
        clearMeasureCache();
    }
    // @}
}
