...
import com.mediatek.common.prizeoption.PrizeOption; // prize-add-LiuHai fix iqiyi YouKu ROTATION_90 paly movie, not in MID -liyongli-20180706
                                                                                                                  // fix iqiyi ReDian 90 paly, not in mid
...
public final class DisplayInfo implements Parcelable {
    ...
    private DisplayInfo(Parcel source) {
        ...
        //prize-added by lihuangyuan,notchscreen,when rotation-90/rotation-270 width reduce liuhai hight-2018-08-07-start
        //e.g. wodeshijie donot show full content
        if(PrizeOption.PRIZE_NOTCH_SCREEN && (rotation == Surface.ROTATION_90 || rotation == Surface.ROTATION_270))
        {
            Display.Mode defaultMode = getDefaultMode();
            int lcdHi = defaultMode.getPhysicalHeight();
            if(lcdHi==logicalWidth){
                logicalWidth -= 87;
            }
        }
        //prize-added by lihuangyuan,notchscreen,when rotation-90/rotation-270 width reduce liuhai hight-2018-08-07-end
    }
    ...
    private void getMetricsWithSize(DisplayMetrics outMetrics, CompatibilityInfo compatInfo,
        ...
        /* prize-add-LiuHai, YouKu ROTATION_90 paly movie, button cover with LiuHai rect-liyongli-20180706-start
         *  iqiyi-->ReDian 
         */
         //prize-deleted by lihuangyuan,for liuhai,moved to DisplayInfo(Parcel source)-2018-08-08
        if(false && PrizeOption.PRIZE_NOTCH_SCREEN && (rotation == Surface.ROTATION_90 || rotation == Surface.ROTATION_270) ){
            if(outMetrics.widthPixels==logicalWidth){
                //Log.d("lyldisp", "--getMetricsWithSize-- logicalWidth="+logicalWidth + "  "+rotation  + " "+overscanLeft );  
                outMetrics.widthPixels -=87; // == getDimensionPixelSize(com.android.internal.R.dimen.status_bar_height);
            }
        }/* prize-add-LiuHai, YouKu ROTATION_90 paly movie, button cover with LiuHai rect-liyongli-20180706-end */
    }
