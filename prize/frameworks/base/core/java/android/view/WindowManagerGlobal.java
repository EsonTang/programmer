...
public final class WindowManagerGlobal {
	...
	private String parsePackageName(String packageNameSetting){
		String[] splitNames = packageNameSetting.split("\\.");
	...
	public  void setFloatPackageEnable(String packageNameSetting, boolean enable){
		String packageName = packageNameSetting;
		try {
			IWindowManager windowManager = getWindowManagerService();
			windowManager.setFloatPackageEnable(packageName,enable);
		} catch (RemoteException e) {
	...
	public  boolean getFloatPackageEnable(String packageNameSetting){
		...
		try {
			IWindowManager windowManager = getWindowManagerService();
			enabled = windowManager.getFloatPackageEnable(packageName);
		} catch (RemoteException e) {
			Log.e(TAG, "Failed to open window session", e);
		}
		return enabled;
	...
	public  int getFloatPackageEnableForWM(String packageNameSetting){
		...
		try {
			IWindowManager windowManager = getWindowManagerService();
			enabled = windowManager.getFloatPackageEnableForWM(packageName);
		} catch (RemoteException e) {
			Log.e(TAG, "Failed to open window session", e);
		}
		return enabled;
	...
	private int parseStackTrace(){
		int checkDisableFloat = -1;
		StackTraceElement[] elements = Thread.currentThread().getStackTrace();
		for(int i = 3; i < 7; i++) {
			checkDisableFloat = getFloatPackageEnableForWM(elements[i].getClassName());
			if (-1 != checkDisableFloat) {
				break;
			}
		}
		return checkDisableFloat;
	}
    ...
    public boolean getFullscreenMode(String packageName) {
        boolean result = true;
        try {
            result = getWindowManagerService().getFullscreenMode(packageName);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
        return result;
    ...
    public boolean isFullScreenMode()
    {
        ...
        try {
            result = getWindowManagerService().isFullScreenMode();
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
        return result;
