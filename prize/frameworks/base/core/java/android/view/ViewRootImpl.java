...
/*-prize-add by lihuangyuan,for notchscreen cut systemui-2018-05-05-start*/
import android.content.pm.ApplicationInfo;
/*-prize-add by lihuangyuan,for notchscreen cut systemui-2018-05-05-end*/
...
public final class ViewRootImpl implements ViewParent,
    ...
    /* Dynamically hiding nav bar feature. prize-linkh-20161115 */
    private static final boolean PRIZE_DYNAMICALLY_HIDE_NAVBAR = PrizeOption.PRIZE_DYNAMICALLY_HIDE_NAVBAR;
    private boolean mShouldAdjustScroll = false;
    private boolean mForceNextWindowRelayoutForIME;
    //END...
	...
	public void disableFloatWindow(String packageName){
		int enabled = WindowManagerGlobal.getInstance().getFloatPackageEnableForWM(packageName);
		if (0 == enabled) {
			mAttachInfo.mWindowVisibility = View.GONE;
			mAppVisible = false;
		} else if (1 == enabled) {
        ...
        synchronized (this) {
            if (mView == null) {
                ...
                if (PrizeOption.PRIZE_NAVBAR_COLOR_CUST) {
                    mView.applyNavBarColorParams(attrs);
                } //@}
                ...
                /*-prize-add by lihuangyuan,for notchscreen cut systemui-2018-05-05-start*/
                attrs.applicationInfoFlags = 0;
                if(mBasePackageName != null)
                {
                    ApplicationInfo appinfo = null;
                    try
                    {
                        appinfo = mContext.getPackageManager().getApplicationInfo(mBasePackageName,0);
                    }
                    catch(Exception e){}
                    if(appinfo != null)
                    {
                        attrs.applicationInfoFlags = appinfo.flags;
                    }                    
                }                    
                /*-prize-add by lihuangyuan,for notchscreen cut systemui-2018-05-05-end*/
                ...
                //solve bug-5980. prize-linkh-20150915
                // If the win wants to use status-bar gray color style, and has dim effect at the same time,
                // The status bar will look ugly. So make it beauty, we foce to decrease the dim value.
                if(PrizeOption.PRIZE_STATUSBAR_INVERSE_COLOR) {
                    if(attrs.statusBarInverse == StatusBarManager.STATUS_BAR_INVERSE_GRAY &&
                        (attrs.dimAmount > 0.25f && attrs.dimAmount < 1f)) {
                        Log.d(TAG, "StatusBarInverse: view=" + view + ", dimAmount=" + attrs.dimAmount);
                        Log.d(TAG, "StatusBarInverse: This view enables status bar gray color feature., the dimAmout is larger than 0.25f. Force to set to 0.25f.");
                        attrs.dimAmount = 0.25f;
                    }
                    
                    //add for debugging the feature.
                    if(IS_ENG_BUILD) {
                        String value = SystemProperties.get("sys.dbg.dim_amount");
                        if(value != null) {
                            try {
                                attrs.dimAmount = Float.parseFloat(value);
                                Log.d(TAG, "override dimAmount from system property. attr.dimAmount=" + attrs.dimAmount);
                            } catch(NumberFormatException e) {}
                        }
                        int inverse = SystemProperties.getInt("sys.dbg.sb_inverse",  -1);
                        if(inverse >= 0 && inverse < StatusBarManager.STATUS_BAR_INVERSE_TOTAL) {
                            attrs.statusBarInverse = inverse;
                            Log.d(TAG, "override statusBarInverse from system property. attrs.statusBarInverse=" + inverse);
                        }
                    }                    
                } //end...                
			...
			/* PRIZE- Disalbe FloatWindow - liuweiquan - 2016-5-31 -start */
            if(PrizeOption.PRIZE_FLOAT_WINDOW_CONTROL){
                /* PRIZE- Disalbe FloatWindow - chenlong - 20150-6-18 -start */
    			Log.d("morgan","setView packageName=" + mWindowAttributes.packageName + ", type=" + attrs.type);
    			if(mWindowAttributes.type == WindowManager.LayoutParams.TYPE_PHONE 
    				|| mWindowAttributes.type == WindowManager.LayoutParams.TYPE_SYSTEM_ALERT
    				|| mWindowAttributes.type == WindowManager.LayoutParams.TYPE_PRIORITY_PHONE
					|| mWindowAttributes.type == WindowManager.LayoutParams.TYPE_TOAST
    				|| mWindowAttributes.type == WindowManager.LayoutParams.TYPE_SYSTEM_ERROR) {
                                    //add. prize-linkh-20150703
                                    //Bug: When AM Service shows an app crash dialog,  and the process that holding the blacklist provider isn't launched,
                                    //invoking disableFloatWindow function will cause dead-lock problem and WatchDog will kill system_server process.
                                    //So i added these codes to avoid this issue temporarily.
                                    //For details, see android logs.
                                    if("android".equals(mWindowAttributes.packageName) 
                                                && (mWindowAttributes.type == WindowManager.LayoutParams.TYPE_SYSTEM_ALERT ||
                                                        mWindowAttributes.type == WindowManager.LayoutParams.TYPE_SYSTEM_ERROR) ||
										"com.tencent.mm".equals(mWindowAttributes.packageName) && (mWindowAttributes.type == WindowManager.LayoutParams.TYPE_PHONE) ||
										"Toast".equals(mWindowAttributes.getTitle()) && (mWindowAttributes.type == WindowManager.LayoutParams.TYPE_TOAST)
                                                /* @remove it for AppWaitingForDebuggerDialog@ && (mWindowAttributes.privateFlags & WindowManager.LayoutParams.PRIVATE_FLAG_SYSTEM_ERROR) != 0*/) {

                                    } else {
                                        Log.d("morgan","disableFloatWindow()....");
                                        disableFloatWindow(mWindowAttributes.packageName);
                                    }
    			}
				
				if("com.tencent.wifimanager".equals(mWindowAttributes.packageName) && (mWindowAttributes.type == WindowManager.LayoutParams.TYPE_SYSTEM_OVERLAY)) {
					disableFloatWindow(mWindowAttributes.packageName);
				}
				
    			/* PRIZE- Disalbe FloatWindow - chenlong - 20150-6-18 -start */
			}
            /* PRIZE- Disalbe FloatWindow - liuweiquan - 2016-5-31 -end */	
        }
    }
    ...
    void setLayoutParams(WindowManager.LayoutParams attrs, boolean newView) {
        synchronized (this) {
			...
			if(PrizeOption.PRIZE_FLOAT_WINDOW_CONTROL){
                /* PRIZE- Disalbe FloatWindow - chenlong - 20150-6-18 -start */
    			Log.d("morgan","setLayoutParams packageName=" + mWindowAttributes.packageName + ", type=" + attrs.type);
    			if(mWindowAttributes.type == WindowManager.LayoutParams.TYPE_PHONE 
    					|| mWindowAttributes.type == WindowManager.LayoutParams.TYPE_SYSTEM_ALERT
    					|| mWindowAttributes.type == WindowManager.LayoutParams.TYPE_PRIORITY_PHONE
						|| mWindowAttributes.type == WindowManager.LayoutParams.TYPE_TOAST
    ...
    /* Dynamically hiding nav bar feature. prize-linkh-20161115 */
    void dispatchApplyInsetsFromPendingInsets(View host) {
        host.dispatchApplyWindowInsets(new WindowInsets(
                mPendingContentInsets, null /* windowDecorInsets */,
                mPendingStableInsets, false, mAttachInfo.mAlwaysConsumeNavBar));
    }
    //END...
    ...
    private void performTraversals() {
        ...
        if (mApplyInsetsRequested) {
            ...
            /* Dynamically hiding nav bar feature. prize-linkh-20161115 */
            //When the nav bar was hidden and the system insets changed, 
            // the WMS reports this resized event to us.
            //We must recalucate the insets of input method for correctly layouting before invoking relayoutWindow().
            // See WMS.WindowState.reportResized(), PhoneWindow.onApplyWindowInsets() for details.
            if(PRIZE_DYNAMICALLY_HIDE_NAVBAR && 
                (lp.type == WindowManager.LayoutParams.TYPE_INPUT_METHOD) && (
                    !mPendingContentInsets.equals(mAttachInfo.mContentInsets) || 
                        !mPendingVisibleInsets.equals(mAttachInfo.mVisibleInsets))) {
                Log.v(mTag, "Inputmethod view: dispatchApplyInsetsFromPendingInsets().");                
                dispatchApplyInsetsFromPendingInsets(host);
                if (mLayoutRequested) {
                    windowSizeMayChange |= measureHierarchy(host, lp,
                            mView.getContext().getResources(),
                            desiredWindowWidth, desiredWindowHeight);
        ...
        if (mFirst || windowShouldResize || insetsChanged ||
            ...
            if(PRIZE_DYNAMICALLY_HIDE_NAVBAR &&
                ...
                // When the visibility of nav bar became gone, we
                // must readjust the scroll for correctly layouting.
                mShouldAdjustScroll = true;
            } //end...
            ...
            if (focus == lastScrolledFocus && !mScrollMayChange && rectangle == null) {
                ...
                if (focus.getGlobalVisibleRect(mVisRect, null)) {
                    ...
                    if (mTempRect.intersect(mVisRect)) {
                        ...
                        } else if(mShouldAdjustScroll && scrollY != 0) { //add for hiding nav bar. prize-linkh-20150811
                            // A case: when the visibilty of nav bar becomes gone, 
                            // this will cause layout action, and the scroll may be incorrect.
                            // So we must re-adjust it again.
                            Log.v(mTag, "Re-adjust Scroll. before adjusting, scrollY=" + scrollY);
                            mShouldAdjustScroll = false;
                            if(scrollY > 0) {
                                //Bottom covered.
                                int newScrollY = mTempRect.bottom  - (mView.getHeight() - vi.bottom);
                                Log.v(mTag, "Re-adjust Scroll. Bottom covered. new scrollY=" + newScrollY);
                                ...
                                //Top convered.
                                int newScrollY = vi.top - mTempRect.top;
                                Log.v(mTag, "Re-adjust Scroll. Top covered. new scrollY=" + newScrollY);
                                if(newScrollY <= 0 && newScrollY >= scrollY) {
                                    scrollY = newScrollY;
                                    immediate = true;
                                }                                
                            }
                            Log.v(mTag, "Re-adjust Scroll. After adjusting, scrollY=" + scrollY);                            
                        } else { //END...
                            scrollY = 0;
                        }
                        handled = true;
     ...
     * be careful this message id.
     */
    private final static int MSG_DISPATCH_NAV_BAR_COLOR_CHANGED = 28;
    // @}

    /* Dynamically hiding nav bar feature. prize-linkh-20180505 @{ */
    private final static int MSG_DISPATCH_NAV_BAR_VISIBILITY_CHANGED_TO_IME = 29;
    // @}
    ...
    final class ViewRootHandler extends Handler {
        ...
        public String getMessageName(Message message) {
            switch (message.what) {
                 ...
                 * Nav bar color customized feature. prize-linkh-2017.08.01 @{
                 */
                case MSG_DISPATCH_NAV_BAR_COLOR_CHANGED:
                    return "MSG_DISPATCH_NAV_BAR_COLOR_CHANGED";
                // @}
                /* Dynamically hiding nav bar feature. prize-linkh-20180505 @{ */
                case MSG_DISPATCH_NAV_BAR_VISIBILITY_CHANGED_TO_IME:
                    return "MSG_DISPATCH_NAV_BAR_VISIBILITY_CHANGED_TO_IME";
                // @}
            }
        ...
        public void handleMessage(Message msg) {
            ...
            case MSG_DISPATCH_NAV_BAR_COLOR_CHANGED: {
                if (mView != null) {
                    mView.onNavBarColorChanged(msg.arg1);
                }
            } break; // @}         
            ...
            case MSG_DISPATCH_NAV_BAR_VISIBILITY_CHANGED_TO_IME: {
                if (mView != null && 
                    mWindowAttributes.type == WindowManager.LayoutParams.TYPE_INPUT_METHOD) {
                    Log.i(mTag, "Clear all measure cache because the visibility of nav bar has changed.");
                    mView.clearAllMeasureCache();
                }
            } break; 
            // @}
            }
    ...
    private void finishInputEvent(QueuedInputEvent q) {
        ...
        if ((DEBUG_DEFAULT || DEBUG_INPUT_STAGES) && inputElapseTime >= INPUT_TIMEOUT) {
    ...
    public void dispatchNavBarColorChanged(int color) {
        Message msg = Message.obtain();
        msg.what = MSG_DISPATCH_NAV_BAR_COLOR_CHANGED;
        msg.arg1 = color;
        mHandler.sendMessage(msg);
    } // @}
    ...
    public void dispatchNavBarVisibilityChangedToIME() {
        Message msg = Message.obtain();
        msg.what = MSG_DISPATCH_NAV_BAR_VISIBILITY_CHANGED_TO_IME;
        mHandler.sendMessage(msg);
    } // @}
    ...
    static class W extends IWindow.Stub {
        ...
        public void dispatchNavBarColorChanged(int color) {
            final ViewRootImpl viewAncestor = mViewAncestor.get();
            if (true) {
                Log.d(TAG, "dispatchNavBarColorChanged() color=0x" + Integer.toHexString(color)
                        + ", viewAncestor=" + viewAncestor);
            }
            if (viewAncestor != null) {
                viewAncestor.dispatchNavBarColorChanged(color);
            }
        } // @}
        ...
        public void dispatchNavBarVisibilityChangedToIME() {
            final ViewRootImpl viewAncestor = mViewAncestor.get();
            if (viewAncestor != null) {
                viewAncestor.dispatchNavBarVisibilityChangedToIME();
            }
        } // @}        
    }
