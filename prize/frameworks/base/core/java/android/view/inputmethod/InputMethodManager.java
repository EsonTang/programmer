...
public final class InputMethodManager {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_TIMEOUT_INPUT_EVENT: {
                    if (DEBUG) {
                    Log.i(TAG, "MSG_TIMEOUT_INPUT_EVENT recieved with seq =" + msg.arg1);
                    }
                ...
                case MSG_FLUSH_INPUT_EVENT: {
                    if (DEBUG) {
                      Log.i(TAG, "MSG_FLUSH_INPUT_EVENT recieved with seq =" + msg.arg1);
                    }
    ...
    /*-prize add by lihuangyuan,for add interface hidesoft-2017-11-24-start*/    
    /** @hide */
    public boolean prizeHideSoftInput(int flags,ResultReceiver resultReceiver)
    {
    	 try {
                if (DEBUG) Log.d(TAG, "Soft input will be hidden");
                return mService.prizeHideSoftInput(flags, resultReceiver);
            } catch (RemoteException e) {
                throw e.rethrowFromSystemServer();
            }    	 
    }
    /*-prize add by lihuangyuan,for add interface hidesoft-2017-11-24-end*/
    ...
    public int dispatchInputEvent(InputEvent event, Object token,
        ...
        synchronized (mH) {
            if (DEBUG) {
        Log.i(TAG, "dispatchInputEvent: event =  " + event
        + ",token= " + token + ",handler=" + handler);
        Log.i(TAG, "dispatchInputEvent: mCurMethod =  " + mCurMethod);
      }
            if (mCurMethod != null) {
        ...
        if (DEBUG) {
        Log.i(TAG, "dispatchInputEvent:DISPATCH_NOT_HANDLED");
      }
    ...
    int sendInputEventOnMainLooperLocked(PendingEvent p) {
        if (DEBUG) {
          Log.i(TAG, "sendInputEventOnMainLooperLocked: mCurChannel= " + mCurChannel);
        }
        if (mCurChannel != null) {
            if (mCurSender == null) {
            ...
            if (DEBUG) {
            Log.i(TAG, "sendInputEventOnMainLooperLocked: mCurSender= " + mCurSender);
          }

            final InputEvent event = p.mEvent;
            final int seq = event.getSequenceNumber();
            if (DEBUG) {
            Log.i(TAG, "sendInputEventOnMainLooperLocked: sending event= " + event
            + "seq = " + seq + "to sender =" + mCurSender);
          }
            if (mCurSender.sendInputEvent(seq, event)) {
                if (DEBUG) {
                Log.i(TAG, "sendInputEventOnMainLooperLocked: sendInputEvent of event="
                + event + " to " + mCurSender + "SUCCESSFUL");
              }
                ...
                if (DEBUG) {
                Log.i(TAG, "sendInputEventOnMainLooperLocked: msg for MSG_TIMEOUT_INPUT_EVENT = "
                + msg + "send delayed with value = " + INPUT_METHOD_NOT_RESPONDING_TIMEOUT);
            ...
            if (DEBUG) {
            Log.i(TAG, "sendInputEventOnMainLooperLocked: sendInputEvent of event= "
            + event + " to " + mCurSender + "FAILED");
            Log.i(TAG, "Unable to send input event to IME: "
    ...
    void finishedInputEvent(int seq, boolean handled, boolean timeout) {
        ...
        synchronized (mH) {
            ...
            if (timeout) {
                if (DEBUG) {
                Log.i(TAG, "Timeout waiting for IME to handle input event after "
        ...
        if (DEBUG) {
        Log.i(TAG, "finishedInputEvent:FinishedInputEventCallback CALLED");
      }
        invokeFinishedInputEventCallback(p, handled);
    }
    ...
    public void notifyUserAction() {
        synchronized (mH) {
            if (mLastSentUserActionNotificationSequenceNumber ==
                    mNextUserActionNotificationSequenceNumber) {
                if (DEBUG) {
                    Log.i(TAG, "Ignoring notifyUserAction as it has already been sent."
    ...
    private final class PendingEvent implements Runnable {
        ...
        public void recycle() {
            ...
            mHandled = false;
            if (DEBUG) {
            Log.i(TAG, "PendingEvent:recycle()");
          }
        }
        ...
        public void run() {
            if (DEBUG) {
            Log.i(TAG, "PendingEvent:onFinishedInputEvent called on " + mCallback);
          }
