...
public abstract class ViewGroup extends View implements ViewParent, ViewManager {
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        if (onFilterTouchEventForSecurity(ev)) {
			...
			//add liup 20171019 start
			if(getHeight() > getWidth()){
				if(actionMasked == MotionEvent.ACTION_POINTER_DOWN && ev.getPointerCount() >= 2){
					if(ev.getX(0) < getWidth()/12 || ev.getX(0) > (getWidth() -getWidth()/12)){
						actionMasked = MotionEvent.ACTION_DOWN;
					}
				}
			}
			//add liup 20171019 end
    ...
    protected void dispatchRestoreInstanceState(SparseArray<Parcelable> container) {
        ...
        for (int i = 0; i < count; i++) {
            View c = children[i];
            if (DBG_MOTION) {
                Log.d(TAG, "dispatchRestoreInstanceState count = " + count + ",mChildrenCount = "
                        + mChildrenCount + ",i = " + i + ",child = " + c + ",children = "
                        + children + ",mChildren = " + mChildren + ",this = " + this);
            }
    ...
    void dispatchViewRemoved(View child) {
        if (DBG_MOTION) {
            Log.e(TAG, "dispatchViewRemoved view from parent " + this
                     + " view == " + child, new Throwable());
        }
        onViewRemoved(child);
        if (mOnHierarchyChangeListener != null) {
    ...
    public void clearAllMeasureCache() {
        ...
        for (int i = 0; i < count; i++) {
            final View child = getChildAt(i);
            child.clearMeasureCache();
        }
        clearMeasureCache(); // clear myself.
    } // @}
