...
oneway interface IDockedStackListener {
    ...
    /**
     * Called when window manager notify the  focused stack changed.
     */
    void prizeOnFocusStackChanged(int focusedStackId, int lastFocusedStackId);
    /* prize-add-split screen-liyongli-20170708-end */
    
    /* prize-add-split screen-liyongli-20170722-start */
    /**
     * Called when window manager notify Return Home , exit split mode.
     * btnType : HOME  or  Return
     */
    void prizeOnRequestExitSplitScreen(int btnType, int focusedStackId);
