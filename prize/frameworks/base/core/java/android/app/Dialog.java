...
/* prize add for dialog display exception liuwei 2018-05-02 begin*/
import android.util.Log;
import android.content.Intent;
import android.text.TextUtils;
import android.content.IntentFilter;
import android.content.BroadcastReceiver;
/* prize add for dialog display exception liuwei 2018-05-02 end*/
...
public class Dialog implements DialogInterface, Window.Callback,
    ...
    /* prize add for dialog display exception liuwei 2018-05-02 begin*/
    private int prize_windowAnimations = 0;
    private boolean prize_hideFromHome = false;
    private boolean prize_registerBroadcast = false;
    private Thread prize_thread;
    /* prize add for dialog display exception liuwei 2018-05-02 end*/
    ...
    public void show() {
        ...
        /* prize add for dialog display exception liuwei 2018-05-02 begin*/
        prize_thread = Thread.currentThread();
        /* prize add for dialog display exception liuwei 2018-05-02 end*/
    ...
    public void onWindowAttributesChanged(WindowManager.LayoutParams params) {
        ...
        if(params.windowAnimations > 0){
            prize_windowAnimations = params.windowAnimations;
        }
        /* prize add for dialog display exception liuwei 2018-05-02 end*/
    ...
    /* prize add for dialog display exception liuwei 2018-05-02 begin*/
    private class HomeAndRecentsReceiver extends BroadcastReceiver {

        private static final String SYSTEM_DIALOG_REASON_KEY = "reason";
        private static final String SYSTEM_DIALOG_REASON_HOME_KEY = "homekey";
        private static final String SYSTEM_DIALOG_REASON_RECENTS_KEY = "recentapps";
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (TextUtils.equals(intentAction, Intent.ACTION_CLOSE_SYSTEM_DIALOGS)) {
                ...
                Log.i("Dialog","action: "+intentAction+ " reason =" + reason);
                boolean needFresh = true;
                if (Dialog.this.prize_thread != Thread.currentThread()) {
                    needFresh = false;
                }

                if(needFresh && mShowing && TextUtils.equals(reason, SYSTEM_DIALOG_REASON_HOME_KEY) || TextUtils.equals(reason, SYSTEM_DIALOG_REASON_RECENTS_KEY)){
                    if(prize_windowAnimations > 0){
                        prize_hideFromHome = true;
                        WindowManager.LayoutParams lp = Dialog.this.getWindow().getAttributes();
                        lp.alpha=0.0f;
                        Dialog.this.getWindow().setAttributes(lp);

                    }
                }

                prize_registerBroadcast = false;
                if(mContext != null)
    ...
    HomeAndRecentsReceiver mHomeAndRecentsReceiver = new HomeAndRecentsReceiver();
    /* prize add for dialog display exception liuwei 2018-05-02 end*/
    ...
    public void onWindowFocusChanged(boolean hasFocus) {
        ...
        if(lp != null){
            ...
            if(mContext instanceof ContextThemeWrapper){
                className = ((ContextThemeWrapper) mContext).getBaseContext().getClass().getName();
            }

            if(isAppWindow && className != null && className.contains("Activity")){
                //needRegister = true;
            }
        }

        if(needRegister && hasFocus && mContext != null){
            //IntentFilter filter = new IntentFilter(Intent.ACTION_CLOSE_SYSTEM_DIALOGS);
            //mContext.registerReceiver(mHomeAndRecentsReceiver,filter);
            //prize_registerBroadcast = true;
            if(prize_windowAnimations > 0){
                mHandler.postDelayed(new Runnable(){
                    @Override public void run() {
                        getWindow().setWindowAnimations(prize_windowAnimations);
                    }
                },10);

            }

            if(prize_hideFromHome){
                Log.i("Dialog", "onWindowFocusChanged +++");
                mHandler.postDelayed(new Runnable(){
                    @Override public void run() {
            ...
            prize_hideFromHome = false;
        }

        if(needRegister && !hasFocus && mContext != null && prize_windowAnimations >0){
    ...
    public void onDetachedFromWindow() {
        ...
        /* prize add for dialog display exception liuwei 2018-05-02 begin*/
        if(prize_registerBroadcast && mContext != null){
            mContext.unregisterReceiver(mHomeAndRecentsReceiver);
        }
        /* prize add for dialog display exception liuwei 2018-05-02 end*/
