    ...
    public boolean rename(Context context, long id, String displayName) {
        ...
        try {
            ...
            if (cursor.moveToFirst()) {
                ...
                oldMediaProviderUri =
                    cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_MEDIAPROVIDER_URI));
        ...
        Log.v(XLOGTAG, "rename Delete from MediaDB ret = " + i);
        ...
        updateRet = mResolver.update(mBaseUri, values, getWhereClauseForIds(ids),
                getWhereArgsForIds(ids));

        Log.v(XLOGTAG, "rename update DownloadDB ret = " + updateRet);
    if (!before.renameTo(after)) {
        ContentValues newValues = new ContentValues();
        newValues.put(Downloads.Impl.COLUMN_TITLE, oldDisplayName);
        newValues.put(Downloads.Impl._DATA, before.toString());
        newValues.put(Downloads.Impl.COLUMN_MEDIAPROVIDER_URI, oldMediaProviderUri);
            updateRet = mResolver.update(mBaseUri, newValues, getWhereClauseForIds(ids),
                        getWhereArgsForIds(ids));
            Log.v(XLOGTAG, "rename revert update DownloadDB ret = " + updateRet);
        throw new IllegalStateException("Failed to rename to " + after);
    }
