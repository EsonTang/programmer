...
/// @}
/*add by liuweiquan actionbar 20160802 start*/
import android.widget.TextView;
import android.util.Log;
/*add by liuweiquan actionbar 20160802 end*/
...
public class Activity extends ContextThemeWrapper
	...
	/* prize-add-by-lijimeng-for font size-20180614-start*/
	private String mPackageName = null;
	private boolean isPrizeFontSize = false;
	/* prize-add-by-lijimeng-for font size-20180614-end*/
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            /*add by liuweiquan actionbar 20160802 start*/
            if(savedInstanceState.containsKey("prize:title")){               
                final boolean titleVisibility = savedInstanceState.getBoolean("prize:title");
                TextView actionbarTitle = getActionBarTextView();
                if(null!=actionbarTitle){
                    actionbarTitle.setVisibility(titleVisibility?View.VISIBLE:View.INVISIBLE);
                } 
            }
            /*add by liuweiquan actionbar 20160802 end*/
        }
		...
		isPrizeFontSize = SystemProperties.getBoolean("persist.sys.prize.fontsize",false);
    }
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        TextView actionbarTitle = getActionBarTextView();
        if(null!=actionbarTitle){
            outState.putBoolean("prize:title", actionbarTitle.getVisibility()==View.VISIBLE?true:false);
        }
        /*add by liuweiquan actionbar 20160802 end*/
    }
    /*add by liuweiquan actionbar 20160802 start*/
    ...
    private TextView getActionBarTextView() {
        ...
        int resId = getResources().getIdentifier("actionbar_title_prize", "id", "android");
    ...
    public boolean startActivityIfNeeded(@RequiresPermission @NonNull Intent intent,
        ...
        if (mParent == null) {
            ...
            try {
                ...
                {
                    intent.setIntentFromSource(Intent.FROM_ACTIVITY);
                    //Log.d("lhy","startActivityIfNeeded set fromsource "+Intent.FROM_ACTIVITY);
                }
    ...
    public boolean startNextMatchingActivity(@RequiresPermission @NonNull Intent intent,
        ...
        if (mParent == null) {
            try {
                ...
                {
                    intent.setIntentFromSource(Intent.FROM_ACTIVITY);
                    //Log.d("lhy","startNextMatchingActivity set fromsource "+Intent.FROM_ACTIVITY);
                }
                /*prize-add by lihuangyuan,for fingerapplock-2017-07-04-end*/
    ...
    /// M: add for fullscreen switch feature @{
    private static boolean mSupportFullscreenSwitch =
            "1".equals(SystemProperties.get("ro.mtk_fullscreen_switch"));
    private boolean hasGetFullScreanMode = false;
    private boolean isFullOn = true;
    private static boolean hasGetMultiWindow = false;
    private static boolean isInMultiWindow = false;
    private int cropScreenLayout;
    @Override
    public Resources getResources() {
        Resources r = super.getResources();
        if (mSupportFullscreenSwitch) {
            if (!hasGetMultiWindow) {
                isInMultiWindow = isInMultiWindowMode();
                hasGetMultiWindow = true;
            }
            if (hasGetFullScreanMode) {
                if (!isFullOn && !isInMultiWindow) {
                    Configuration c = r.getConfiguration();
                    c.screenLayout = cropScreenLayout;
                }
                return r;
            }
            ...
            if (baseContext != null) {
                ...
                if (mWindowManager == null) {
                    windowManager = (WindowManager) baseContext
                            .getSystemService(Context.WINDOW_SERVICE);
                } else {
                    windowManager = mWindowManager;
                }
                if (!hasGetFullScreanMode) {
                    isFullOn = windowManager.getFullscreenMode(getPackageName());
                    if (!isFullOn) {
                        Configuration c = r.getConfiguration();
                        ...
                        final int sl = Configuration.resetScreenLayout(c.screenLayout);
                        cropScreenLayout =
                                Configuration.reduceScreenLayout(sl, longSize, shortSize);
                        c.screenLayout = cropScreenLayout;
                    }
                    hasGetFullScreanMode = true;
                }
            }
        }
		/* prize-add-by-lijimeng-for fontsize-20180522-start*/
		if(mPackageName != null && isPrizeFontSize){
			r.setPackageName(mPackageName); 
		}
		/* prize-add-by-lijimeng-for fontsize-20180522-start*/
        return r;
    }
