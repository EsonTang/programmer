...
public final class PowerManager {
    ...
    * boolean isSuperSaverMode = PowerManager.isSuperSaverMode();
    * @return whether the device is super saver mode
    */
    public static boolean isSuperSaverMode() {
        final String REBOOT_POWER_EXTEND_PROPERTY = "persist.sys.power_extend_mode";
        boolean lRet = SystemProperties.getBoolean(REBOOT_POWER_EXTEND_PROPERTY, false);
        //boolean lRet = Settings.System.getInt(getContentResolver(),Settings.System.PRIZE_POWER_EXTEND_MODE,0) == 1;
        return lRet;
    }
    /**
    * Switch power saver mode and reboot the device.
    * PowerManager pm = (PowerManager) getSystemService(Context.POWER_SERVICE);
    * boolean isScreenOn = pm.switchSuperSaverMode(true);
    */
    public void switchSuperSaverMode(boolean intoSuperSaver) {
        Log.i(TAG,"PowerExtendMode switchSuperSaverMode is " + intoSuperSaver);
        try {
            mService.switchSuperSaverMode(intoSuperSaver);
        } catch (RemoteException e) {
        }
    }
    /**
    * Return whether the device is booted for the first time.
    */
    public static boolean isFirstBoot(){
        final String REBOOT_POWER_EXTEND_PROPERTY = "persist.sys.power_extend_mode";
        String bootProperty = SystemProperties.get(REBOOT_POWER_EXTEND_PROPERTY);
        if (bootProperty != null && !bootProperty.equals("")) {
            return false;
        }
        return true;
    }
