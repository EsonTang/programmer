...
public abstract class AbsSeekBarPrize extends ProgressBar {
    ...
    public AbsSeekBarPrize(Context context) {
        super(context);
    }

    public AbsSeekBarPrize(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public AbsSeekBarPrize(Context context, AttributeSet attrs, int defStyleAttr) {
        this(context, attrs, defStyleAttr, 0);
    }

    public AbsSeekBarPrize(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private boolean isDownInThumb = false;
    @Override
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (thumbDrawable != null) {
                    ...
                    int scrollX = mScrollX - mThumbOffset;
                    int scrollY = mScrollY + mPaddingTop;
                    int left = rect.left + scrollX - 20;
                    int top = rect.top + scrollY - 10;
                    int right = rect.right + scrollX + 15;
                    int bottom = rect.bottom + scrollY + 10;
                    if(rect != null){
                        int dx = (int)event.getX() - mPaddingLeft;
                        int dy = (int)event.getY();
                        Log.d("AbsSeekBarPrize","dx = " + dx + ",dy = " + dy
                            + ", rect(" + left + ", " + top + ", " + right + ", " + bottom + ")");
                        if(dx >= left && dx <= right
                            && dy >= top && dy <= bottom){
                            Log.d("AbsSeekBarPrize","down in thumb isDownInThumb turn true ");
                            isDownInThumb = true;
                        }
                    }
                }
            ...
            case MotionEvent.ACTION_UP:
                if (mIsDragging) {
                    ...
                    if(isDownInThumb){
                        ...
                        Log.d("AbsSeekBarPrize","ACTION_UP isDownInThumb is false");
                    }
                }
                isDownInThumb = false;
                Log.d("AbsSeekBarPrize","ACTION_UP isDownInThumb turn false");
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                isDownInThumb = false;
                Log.d("AbsSeekBarPrize","ACTION_CANCEL isDownInThumb turn false");
    ...
    public CharSequence getAccessibilityClassName() {
        return AbsSeekBarPrize.class.getName();
    }
