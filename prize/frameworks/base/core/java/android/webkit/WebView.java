...
public class WebView extends AbsoluteLayout
    ...
    private int boostFor3Cluster() {
        ...
        if (handle != -1) {
            int boostValue = SystemProperties.getInt("chromium.boostvalue", 80);
            // setup boost value
            sPerfService.userRegScnConfig(handle, IPerfServiceWrapper.CMD_SET_FG_BOOST_VALUE,
                                          boostValue, 0, 0, 0);
        }
        return handle;
    }
    ...
    private void setupPerfMode() {
        ...
        sPerfHandle = (Runtime.getRuntime().availableProcessors() <= 8) ?
                        boostNormal() : boostFor3Cluster();
     ...
     * or below, all public methods (including the inherited ones) can be accessed, see the
