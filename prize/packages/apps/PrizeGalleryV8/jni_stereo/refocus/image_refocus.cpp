...
bool ImageRefocus::initRefocusNoDepthMap(const char *sourceFilePath, int outImgWidth, int outImgHeight,
    ...
    // decode jpeg file
    initRefocusIMGSource(sourceFilePath, outImgWidth, outImgHeight, IMAGE_SAMPLE_SIZE);
...
bool ImageRefocus::initRefocusNoDepthMap(uint8_t* jpegBuf, int jpegbufferSize, int outImgWidth,
    ...
    // decode jpeg buffer
    if (!initRefocusIMGSource(jpegBuf, jpegbufferSize, outImgWidth, outImgHeight, IMAGE_SAMPLE_SIZE)) {
...
bool ImageRefocus::initRefocusWithDepthMap(const char *sourceFilePath, int outImgWidth, int outImgHeight,
    ...
    mRefocusImageInfo.DepthBufferAddr = new MUINT8[depthBufferSize];
    memcpy(mRefocusImageInfo.DepthBufferAddr, depthBuffer, depthBufferSize);
...
void ImageRefocus::initRefocusIMGSource(const char *sourceFilePath, int outImgWidth, int outImgHeight,
    ...
    ALOGD("<initRefocusIMGSource>start, filePath:%s,sampleSize:%d", sourceFilePath, sampleSize);
    m_pJpegFactory->jpgDecode(sourceFilePath, &imageInfo, sampleSize);
    ALOGD("<initRefocusIMGSource> decode image resource end, destBuffer:%p,width:%d,height:%d",
...
bool ImageRefocus::initRefocusIMGSource(uint8_t* jpegBuf, int bufSize, int outImgWidth,
        int outImgHeight, int sampleSize) {
    if (jpegBuf == NULL) {
    ...
    if (!m_pJpegFactory->jpgToYV12(align128_file_buffer, bufSize, &imageInfo, sampleSize)) {
...
bool ImageRefocus::initJPSBuffer(MUINT8* jpsBuffer, int jpsBufferSize, int jpsWidth, int jpsHeight) {
    ...
    if (!m_pJpegFactory->jpgToYV12(align128_file_buffer, jpsBufferSize, &imageInfo, JPS_SAMPLE_SIZE)) {
        ALOGI("ERROR: decode failed!!");
        free(pJpsBuffer);
        pJpsBuffer = NULL;
        return false;
...
bool ImageRefocus::generate() {
    ...
    if (mRefocusImageInfo.DepthBufferAddr == NULL) {
        ...
        ALOGD("<generate>copy depthBuffer from %p to %p", mRefocusResultInfo.DepthBufferAddr, depthBuffer);
    }
    return SUCCESS;
}
...
ImageBuffer ImageRefocus::generateRefocusImage(int touchCoordX, int touchCoordY, int depthOfField) {
    ...
    mRefocusImageInfo.TouchCoordX = touchCoordX / IMAGE_SAMPLE_SIZE;
    mRefocusImageInfo.TouchCoordY = touchCoordY / IMAGE_SAMPLE_SIZE;
    ...
    if (generate()) {
        ...
        ALOGD("generate success, width:%d,height:%d", image.width, image.height);
    }

    if (NULL != m_DebugUtils) {
        m_DebugUtils->dumpBufferToFile("generate.raw", image.buffer, image.width * image.height * 4);
    }
    return image;
}

void ImageRefocus::deinit() {
    ALOGD("<deinit>start release memory!");
...
void ImageRefocus::saveRefocusImage(const char *saveFileName, int inSampleSize) {
    ...
    mRefocusImageInfo.TouchCoordX = mRefocusImageInfo.TouchCoordX * IMAGE_SAMPLE_SIZE;
    mRefocusImageInfo.TouchCoordY = mRefocusImageInfo.TouchCoordY * IMAGE_SAMPLE_SIZE;
    mRefocusImageInfo.DepthOfField = mRefocusImageInfo.DepthOfField;
    initRefocusIMGSource(mSourceFileName, mRefocusImageInfo.TargetWidth * IMAGE_SAMPLE_SIZE,
            mRefocusImageInfo.TargetHeight * IMAGE_SAMPLE_SIZE, 1);
    if (createRefocusInstance() && setBufferAddr() && generate()) {
        ALOGD("<saveRefocusImage>image refocus init end, success");
    } else {
        ALOGD("ERROR: image refocus init fail!!!");
        return;
    }

    sprintf(file, "%s", saveFileName);
    ALOGD("<saveRefocusImage>test file:%s", file);
    fp = fopen(file, "w");
    if (fp == NULL) {
        ALOGD("ERROR: Open file %s failed!!!", file);
        return;
    }
    ...
    if (jpgBuf == NULL) {
        ALOGD("ERROR: Can not allocate memory!!!");
        fclose(fp);
        return;
    }
    if (NULL != m_DebugUtils) {
    ...
    ALOGD("<saveRefocusImage>RefocusedYUVImageAddr:%p", (size_t)mRefocusResultInfo.RefocusedYUVImageAddr);
