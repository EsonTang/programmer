...
// count the time of initialize segment,
// add one when initialize, reduce one when release
// if sCount is not zero, will not delete m_segment when release
// if sCount is zero, will delete m_segment when release
static int sCount = 0;
...
jboolean initSegmentMask(JNIEnv *env, jobject thiz, jbyteArray mask, jobject rect, jobject point) {
    ALOGD("<initSegmentMask>");

    jclass rect_class = env->FindClass("android/graphics/Rect");
    if (rect_class == NULL) {
        ALOGD("<initSegmentMask> fail!!!");
        return false;
    }
    jfieldID left_field = env->GetFieldID(rect_class, "left", "I");
    jfieldID right_filed = env->GetFieldID(rect_class, "right", "I");
...
jboolean setNewBitmap(JNIEnv *env, jobject thiz, jobject bitmap, jint bitmapWidth,
    ...
    if (NULL == mask && NULL == m_segment) {
        ALOGD("<setNewBitmap> null mask and m_segment object,fail!!!");
        assert(false);
        return false;
    }
    char* destination = 0;
