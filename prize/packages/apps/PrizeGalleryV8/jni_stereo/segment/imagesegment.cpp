...
bool ImageSegment::setNewBitmap(char* img, int imgWidth, int imgHeight, unsigned char* mask,
    ...
    ALOGI("<setNewBitmap> output_mask_addr=%p,center.x=%d,center.y=%d,bbox.left=%d,bbox.top=%d,"
            "bbox.right=%d,bbox.bottom=%d", gResult.output_mask_addr, gResult.center.x,
            gResult.center.y, gResult.bbox.left, gResult.bbox.top,
