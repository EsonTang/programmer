...
public class FreeView3DPlayer extends Player implements GyroPositionListener,
    ...
    private MBitmapTexture mOriginalTexture;
    ...
    public static final boolean DYNAMIC_DEF = SystemProperties.get(
            "freeview_mode").equals("1");
    /**
     * Constructor.
     * @param context
    ...
    /**
     * Prepare for Freeview3D view.
     */
    public void prepareForFreeview3D() {
        if (!mOnFreeViewMode) {
            return;
        }
        synchronized (this) {
    ...
    protected boolean onPrepare() {
        ...
        /// @}
        mBitmap = FreeView3DUtils.decodeBitmap(mMediaData.filePath);
        /// M: [DEBUG.ADD] @{
        TraceHelper.traceEnd();
        /// @}
        if (mBitmap != null) {
            mOriginalTexture = new MBitmapTexture(mBitmap);
            mCurrentTexture = mOriginalTexture;
    ...
    protected void onRelease() {
        ...
        if (mOriginalTexture != null) {
            mOriginalTexture.recycle();
            mOriginalTexture = null;
        }
    ...
    private boolean init() {
        if (mBitmapBuf != null && (!mIsInitFreeView) && mOriginalTexture != null) {
