...
public class AnimatedContentSharer {
    ...
    private void tryToShareAsVideo(Intent intent) {
		...
		if (item.mediaType.equals(MediaData.MediaType.CONTAINER)
            ...
            if ((currentData != null && !(currentData.mediaType
                    .equals(MediaData.MediaType.CONTAINER)))
                    || !(isShareTypeChangeSupported(intent, MIME_TYPE_VIDEO))) {
                MtkLog.i(TAG, "<tryToShareAsVideo> currentData not container,"
                        + " startActivity directly");
                startActivity(mActivity, intent);
                return;
            }
            MtkLog.i(TAG, "<tryToShareAsVideo> currentData is conshot,"
                    + " call onShareAsVideoRequested");
