...
public class FreeView3DLayer extends BottomControlLayer {
    ...
    private boolean mHasCheckGyroSensor;
    ...
    public void setPlayer(Player player) {
        ...
        if (player == null) {
            ...
            if (onStereoDynamicPreviewView()) {
                mPlayer.prepareForFreeview3D();
            }
        } else {
    ...
    public boolean supportGyroSensor() {
        if (!mHasCheckGyroSensor) {
            SensorManager sensorManager = (SensorManager) mActivity
                    .getSystemService(Context.SENSOR_SERVICE);
            mHasGyroSensor = sensorManager
                    .getDefaultSensor(Sensor.TYPE_GYROSCOPE) != null;
            mHasCheckGyroSensor = true;
        }
        return mHasGyroSensor;
    }

    private boolean onFreeviewMode() {
        if (FreeView3DPlayer.DYNAMIC_DEF) {
            return onStereoStartView();
        } else {
            return onStereoDynamicPreviewView();
        }

    }

    @Override
    public boolean supportDynamicPreview() {
        return (!FreeView3DPlayer.DYNAMIC_DEF) || (!supportGyroSensor());
    }
