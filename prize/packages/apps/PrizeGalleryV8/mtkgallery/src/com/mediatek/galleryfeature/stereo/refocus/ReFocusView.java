...
public class ReFocusView extends StageView {
     ...
     *            view Height
     */
    public synchronized void setImageActor(Object pic, final float viewWidht,
     ...
     * @param pic bitmap or GraphicBuffer
     * @param recycle need to do bitmap recycle if previous bitmap existed
     */
    public synchronized void setImageActorNew(Object pic, boolean recycle) {
        ...
        if (pic != null) {
            ...
            mBitmapNew = pic;
            setTextureNew(mImageActor, mBitmapNew);
        }
    }
     ...
     * @param pic bitmap or GraphicBuffer
     */
    public synchronized void setImageActorNew(Object pic) {
    ...
    private void onDrawFrameImageReset(boolean reloadBitmap) {
        ...
        if (reloadBitmap) {
            setTextureBase(mImageActor, mBitmap);
            mImageActor.setSize(getDimension(mBitmap));
            setTextureNew(mImageActor, mBitmapNew);
        }
    ...
    private synchronized void setImageActor(Object pic, boolean recycle) {
    ...
    private void setImageActorViewSize(Image img, Object bmp,
            final float viewWidth, final float viewHeight) {

        if (bmp == null) {
            return;
        }
        final float bitmapWidth = getWidth(bmp);
        final float bitmapHeight = getHeight(bmp);
    ...
    private static void recycle(Object pic) {
        if (pic instanceof Bitmap) {
            ((Bitmap)pic).recycle();
        }
        if (pic instanceof GraphicBuffer) {
            ((GraphicBuffer)pic).destroy();
        }
    }

    private static float getWidth(Object pic) {
        if (pic instanceof Bitmap) {
            return ((Bitmap)pic).getWidth();
        }
        if (pic instanceof GraphicBuffer) {
            return ((GraphicBuffer)pic).getWidth();
        }
        return 0;
    }

    private static float getHeight(Object pic) {
        if (pic instanceof Bitmap) {
            return ((Bitmap)pic).getHeight();
        }
        if (pic instanceof GraphicBuffer) {
            return ((GraphicBuffer)pic).getHeight();
        }
        return 0;
    }

    private static Dimension getDimension(Object pic) {
        if (pic instanceof Bitmap) {
            return new Dimension(((Bitmap)pic).getWidth(), ((Bitmap)pic).getHeight());
        }
        if (pic instanceof GraphicBuffer) {
            return new Dimension(((GraphicBuffer)pic).getWidth(), ((GraphicBuffer)pic).getHeight());
        }
        return new Dimension();
    }

    private boolean mIsBaseImgOESTexture = false;
    private boolean mIsNewImgOESTexture = false;

    private void applyShader(Image img) {
        if (!mIsBaseImgOESTexture && !mIsNewImgOESTexture) {
            img.setMaterial("speed.mat");
        } else if (!mIsBaseImgOESTexture && mIsNewImgOESTexture) {
            img.setMaterial("speed_2d_oes.mat");
        } else if (mIsBaseImgOESTexture && !mIsNewImgOESTexture) {
            img.setMaterial("speed_oes_2d.mat");
        } else if (mIsBaseImgOESTexture && mIsNewImgOESTexture) {
            img.setMaterial("speed_oes_oes.mat");
        }
    }

    private synchronized void setTextureBase(Image img, Object pic) {
        logD("setTextureBase() " + pic);
        if (pic instanceof Bitmap) {
            img.setImageFromBitmap((Bitmap)pic);
            mIsBaseImgOESTexture = false;
        } else if (pic instanceof GraphicBuffer) {
            img.setGraphicBufferImage((GraphicBuffer)pic);
            mIsBaseImgOESTexture = true;
        }
        applyShader(img);
    }

    private synchronized void setTextureNew(Image img, Object pic) {
        logD("setTextureNew_() " + pic);
        if (pic instanceof Bitmap) {
            img.setMaterialProperty("", "M_NEW_TEXTURE", (Bitmap)pic);
            mIsNewImgOESTexture = false;
        } else if (pic instanceof GraphicBuffer) {
            img.setMaterialProperty("", "M_NEW_TEXTURE", (GraphicBuffer)pic);
            mIsNewImgOESTexture = true;
        }
        applyShader(img);
    }
