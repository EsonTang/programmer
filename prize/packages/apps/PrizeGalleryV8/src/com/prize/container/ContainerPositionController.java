...
import com.prize.container.ContainerPhotoView.Size;
 ...
 */
public class ContainerPositionController {
/// @}
    ...
    private static final float FILM_MODE_PORTRAIT_HEIGHT = 0.65f;//0.48f;
    private static final float FILM_MODE_PORTRAIT_WIDTH = 0.9f;//0.7f;
    ...
    private static final int BOX_MAX = ContainerPhotoView.SCREEN_NAIL_MAX;
    private static final int[] CENTER_OUT_INDEX = new int[2 * BOX_MAX + 1];

    public static final int IMAGE_GAP = GalleryUtils.dpToPixel(13);
    ...
    public ContainerPositionController(Context context, Listener listener) {
        mListener = listener;
...
//        LogUtil.i(TAG, "setConstrainedFrame cFrame=" + cFrame);
    ...
    /// @}
//        LogUtil.i(TAG, "setImageSize cFrame=" + cFrame);
        if (s.width == 0 || s.height == 0) return;
        /// M: [BUG.ADD] @{
    ...
    public void endScale(float focusX, float focusY) {
        ...
        Box b = mBoxes.get(0);
        float s = getMinimalScale(b);
        if (b.mCurrentScale > s) {
        	zoomIn(focusX, focusY, s);
		}
    ...
    public void scrollFilmX(int dx) {
        ...
        LogUtil.i(TAG, "scrollFilmX dx=" + dx + " p.mCurrentX=" + p.mCurrentX + " mPlatform.mDefaultX=" + mPlatform.mDefaultX + " x=" + x);
        if (!mHasPrev && x > 0) {
    ...
    public boolean flingFilmX(int velocityX) {
        LogUtil.i(TAG, "flingFilmX velocityX=" + velocityX);
        ...
        mFilmScroller.fling(p.mCurrentX, 0, velocityX / 3, 0,
                Integer.MIN_VALUE, Integer.MAX_VALUE, 0, 0);
        int targetX = mFilmScroller.getFinalX();
        LogUtil.i(TAG, "flingFilmX targetX=" + targetX + " p.mCurrentX=" + p.mCurrentX + " mViewW=" + mViewW);
        float sign = Math.signum(targetX);
        int drawW= getPosition(0).right - getPosition(0).left + IMAGE_GAP;
        targetX = (int) (targetX + sign * mViewW) / mViewW * drawW;
        mFilmScroller.setFinalX(targetX);
        LogUtil.i(TAG, "flingFilmX end targetX=" + targetX);
    ...
    public int flingFilmY(int boxIndex, int velocityY) {
        LogUtil.i(TAG, "flingFilmY boxIndex=" + boxIndex + " velocityY=" + velocityY);
        Box b = mBoxes.get(boxIndex);
    ...
    public synchronized void advanceAnimation() {
        ...
        for (int i = -BOX_MAX; i <= BOX_MAX; i++) {
            ...
            if (box != null) {
                changed |= box.advanceAnimation();
            }
        }
        for (int i = -BOX_MAX; i < BOX_MAX; i++) {
            Gap gap = mGaps.get(i);
            if (gap != null) {
                changed |= gap.advanceAnimation();
            }
        }
    ...
    */
     ...
     */
    public synchronized void updateBox(int fromIndex[], boolean hasPrev, boolean hasNext,
            boolean constrained, Size[] sizes) {
    /// @}
...
//        if (mFilmMode) return getMinimalScale(b);
    ...
    private class Platform extends Animatable {
        ...
        public boolean startSnapback() {
            ...
            LogUtil.i(TAG, "startSnapback x=" + x + " mCurrentX=" + mCurrentX);
        ...
        private boolean doAnimation(int targetX, int targetY, int kind) {
            LogUtil.i(TAG, "doAnimation targetX=" + targetX + " mCurrentX=" + mCurrentX);
        ...
        private boolean interpolateFlingFilm(float progress) {
            ...
            if (dir != EdgeView.INVALID_DIRECTION) {
                ...
                mCurrentX = mDefaultX;
                LogUtil.i(TAG, "interpolateFlingFilm INVALID_DIRECTION mCurrentX=" + mCurrentX + " mFlingOffset=" + mFlingOffset);
            }
