...
public class ContainerPhotoDataAdapter implements ContainerSavePage.Model {
    @SuppressWarnings("unused")
    private static final String TAG = "Gallery2/ContainerPhotoDataAdapter";
    ...
    /// @}
    private static final int SCREEN_NAIL_MAX = ContainerPhotoView.SCREEN_NAIL_MAX;
    ...
    public ContainerPhotoDataAdapter(AbstractGalleryActivity activity, ContainerPhotoView view,
    ...
    private void fireDataChange(int index) {
        // First check if data actually changed.
        boolean changed = false;
        for (int i = -SCREEN_NAIL_MAX; i <= SCREEN_NAIL_MAX; ++i) {
            long newVersion = getVersion(mCurrentIndex + i);
        ...
        mPhotoView.updateDataChange(fromIndex, -mCurrentIndex,
                mSize - 1 - mCurrentIndex);
    }
    ...
    private void updateScreenNail(Path path, Future<ScreenNail> future) {
        LogUtil.i(TAG, "<updateScreenNail> path " + path);
        ...
        if (path != null) {
            ...
            if (item instanceof LocalMediaItem) {
                ...
                if (!file.exists()) {
                    LogUtil.i(TAG, "<updateScreenNail> filePath" + filePath + " not exists!!!");
                    return;
                }
            }
        }
        ...
        LogUtil.i(TAG, "updateScreenNail path=" + path + " screen=" + screenNail);
        ...
        mPhotoView.notifyTipChange(path, screenNail.getBitmap());
        LogUtil.i(TAG, "<updateScreenNail> path end" + path);
        updateImageRequests();
    ...
    private void updateFullImage(Path path, Future<BitmapRegionDecoder> future) {
        ...
        mPhotoView.notifyTipChange(path, entry.screenNail.getBitmap());
        updateImageRequests();
    }
    ...
    private void updateCurrentIndex(int index, boolean isScroll) {
        ...
        LogUtil.i(TAG, "updateCurrentIndex index=" + index + " item=" + item + " mItemPath=" + mItemPath);
    ...
    private void uploadScreenNail(int offset) {
        ...
        if (e.highQualityScreenNail != null) {
            ...
            LogUtil.d(TAG, "<uploadScreenNail> highQualityScreenNail " + s.getWidth()
                    + "  " + s.getHeight());
        } else {
            s = e.screenNail;
        }
    ...
    public ScreenNail getScreenNail(int offset) {
        ...
        LogUtil.i(TAG, "getScreenNail offset=" + offset + " path=" + item.getPath());
        ...
        if (entry.screenNail == null && !isCamera(offset)) {
            if (offset == 0) {
                ...
                if (item instanceof LocalMediaItem) {
                    ...
                    found = cacheService.getImageData(item.getPath(), ((LocalMediaItem) item).dateModifiedInSec, MediaItem.TYPE_THUMBNAIL, buffer);
                }
                Bitmap bitmap = null;
                if (!found) {
                    if (item.getMediaType() == MediaItem.MEDIA_TYPE_VIDEO) {
                        bitmap = ThumbnailUtils.createVideoThumbnail(item.getFilePath(), MediaStore.Images.Thumbnails.MINI_KIND);
                    } else {
                        bitmap = ThumbnailUtils.createImageThumbnail(item.getFilePath(), MediaStore.Images.Thumbnails.MINI_KIND);
                    }
                }
                LogUtil.i(TAG, "getScreenNail bitmap=" + bitmap);
                if (bitmap == null) {
                    entry.screenNail = newPlaceholderScreenNail(item);
                } else {
                    LogUtil.i(TAG, "*******************************getScreenNail bitmapW=" + bitmap.getWidth() + " h=" + bitmap.getHeight());
                    entry.screenNail = new BitmapScreenNail(bitmap);
                }
            } else {
                entry.screenNail = newPlaceholderScreenNail(item);
            }
    ...
    @Override
    public void getImageSize(int offset, ContainerPhotoView.Size size) {
    ...
    private void updateTileProvider(ImageEntry entry) {
        ...
        if (entry.highQualityScreenNail != null) {
            ...
            LogUtil.d(TAG, "<updateTileProvider> highQualityScreenNail  " + screenNail.getWidth()
    ...
    private class ScreenNailJob implements Job<ScreenNail> {
        ...
        public ScreenNail run(JobContext jc) {
            ...
            if (isTemporaryItem(mItem)) {
                /// M: [DEBUG.ADD] @{
                LogUtil.d(TAG, "<ScreenNailJob.run> this is temporary item");
                /// @}
                return newPlaceholderScreenNail(mItem);
            }
            /// M: [DEBUG.ADD] @{
            LogUtil.d(TAG, "<ScreenNailJob.run> ScreenNail requestImage");
            /// @}
            ...
            LogUtil.d(TAG, "<ScreenNailJob.run> ScreenNail requestImage end");
    ...
    private class FullImageJob implements Job<BitmapRegionDecoder> {
        ...
        public BitmapRegionDecoder run(JobContext jc) {
            ...
            if (entry != null && entry.failToLoad == true) {
                LogUtil.i(TAG, "<FullImageJob.run> decode thumbnail fail,"
    ...
    private ScreenNail newPlaceholderScreenNail(MediaItem item) {
        ...
        if ((mediaData != null) && mediaData.isVideo) {
            ...
            LogUtil.v(TAG, "<newPlaceholderScreenNail> width=" + width
                    + ", height=" + height);
        }
    ...
    private Future<?> startTaskIfNeeded(int index, int which) {
        ...
        if (which == BIT_FULL_IMAGE && entry.requestedFullImage != version
            ...
            LogUtil.d(TAG, "<startTaskIfNeeded> fullImageTask!");
            entry.requestedFullImage = version;
        ...
        //should no use ReginDecoder to decode tile . @{
                ...
                && ((item.getSupportedOperations() & MediaItem.SUPPORT_FULL_IMAGE) == 0)) {
            LogUtil.d(TAG, "<startTaskIfNeeded> highQualityImageTask!");
    ...
    private class FullImageListener
        ...
        @Override
        public void run() {
            LogUtil.i(TAG, "FullImageListener mPath=" + mPath);
    ...
    private class ScreenNailListener
        ...
        @Override
        public void run() {
            LogUtil.i(TAG, "ScreenNailListener mPath=" + mPath);
    ...
    private class ReloadTask extends Thread {
        ...
        public void run() {
            /// M: [DEBUG.ADD] @{
            LogUtil.i(TAG, "<ReloadTask.run> begin, tid = " + this.getId());
            /// @}
            while (mActive) {
                synchronized (this) {
                    if (!mDirty && mActive) {
                ...
                if (info.version != version) {
                    ...
                    /// M: [DEBUG.ADD] @{
                    LogUtil.i(TAG, "<ReloadTask.run> set=" + mSource + ", name="
                ...
                */
                ...
                if (mItemPath == null) {
                    mItemPath = info.items.get(0).getPath();
                }
                /// @}
            ...
            /// M: [DEBUG.ADD] @{
            LogUtil.i(TAG, "<ReloadTask.run> exit, tid = " + this.getId());
            /// @}
        }
        ...
        public synchronized void terminate() {
            /// M: [DEBUG.ADD] @{
            LogUtil.i(TAG, "<ReloadTask.terminate>");
            /// @}
            mActive = false;
        ...
        private int getIndexOfItemQuickStop(Path path, int hint) {
            ...
            LogUtil.i(TAG, "<ReloadTask.getIndexOfItemQuickStop> mActive = false, "
                    + "return MediaSet.INDEX_NOT_FOUND");
            return MediaSet.INDEX_NOT_FOUND;
        }
        /// @}
    ...
    private class HighQualityScreenNailListener implements Runnable,
        ...
        @Override
        public void run() {
            LogUtil.i(TAG, "HighQualityScreenNailListener mPath=" + mPath);
