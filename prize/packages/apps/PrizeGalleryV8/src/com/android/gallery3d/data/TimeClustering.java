...
public class TimeClustering extends Clustering {
	...
	private static final long ONE_DAY_TIME_IN_MS = 24 * 3600000L;
    ...
    public static String getTimeTitleName(Context context, int datetaken) {
        long minTimestamp = datetaken * 24 * 3600000L;
        long maxTimestamp = System.currentTimeMillis();
        String YYMMDD_FORMAT = "yyyyMMdd";
        String caption;
        String minDay = DateFormat.format(YYMMDD_FORMAT, minTimestamp)
                .toString();
        String maxDay = DateFormat.format(YYMMDD_FORMAT, maxTimestamp)
                .toString();
        ...
        if (minDay.substring(4).equals(maxDay.substring(4))) {
            ...
            caption = DateUtils.formatDateRange(context, minTimestamp,
                    minTimestamp, DateUtils.FORMAT_SHOW_DATE);
        } else {
            int flags = DateUtils.FORMAT_ABBREV_MONTH | DateUtils.FORMAT_SHOW_DATE;
            caption = DateUtils.formatDateRange(context, minTimestamp,
                    minTimestamp, flags);
        }
        Log.i(TAG, "prizeGenerateCaption minDay=" + minDay + " maxDay=" + maxDay + " caption=" + caption + " minTimestamp=" + minTimestamp);
        return caption;
    }
    ...
    public void run(MediaSet baseSet) {
        ...
        mNames = new String[m];
        for (int i = 0; i < m; i++) {
            mNames[i] = mClusters.get(i).prizeGenerateCaption(mContext);
        }
    }

    public void computeNew(ArrayList<SmallItem> items) {
        ...
        long lastDate = 0;
        for (int i = 0; i < n; i++) {
            SmallItem currentItem = items.get(i);
            long t = currentItem.dateInMs;
            if (t == 0) continue;
            long date = (t + 28800000) / (24 * 3600000L);
            Log.i(TAG, "computeNew date= " + date);
            if (lastDate == 0 || equestCluster(date, lastDate)) {
    ...
    private static boolean equestCluster(long a, long b) {
    ...
    @Override
    public int getClusterCount(int index) {
        // TODO Auto-generated method stub
        return mClusters.get(index).mCount;
...
class Cluster {
    ...
    /// @prize liudong 2015-05-20 {
    private static final String YYMMDD_FORMAT = "yyyyMMdd";
    
    public String prizeGenerateCaption(Context context) {
        int n = mItems.size();
        long minTimestamp = 0;
        long maxTimestamp = System.currentTimeMillis();

        for (int i = 0; i < n; i++) {
        	minTimestamp = mItems.get(i).dateInMs;
            if (minTimestamp != 0) break;
        }
        if (minTimestamp == 0) return "";
