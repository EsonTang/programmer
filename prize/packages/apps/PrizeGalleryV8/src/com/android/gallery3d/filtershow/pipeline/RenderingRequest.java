...
public class RenderingRequest {
    ...
    public static final int ICON_STICKER_RENDERING = 8;
	...
	public static void postStickerIconRequest(Context context, int w, int h,
			ImagePreset preset, RenderingRequestCaller caller) {
		if (preset == null || caller == null) {
			Log.v(LOGTAG, "something null, preset: " + preset + " or caller: "
					+ caller);
			return;
		}
		RenderingRequest request = new RenderingRequest();
		...
		request.setScaleFactor(MasterImage.getImage().getScaleFactor());
		request.setImagePreset(passedPreset);
		request.setType(RenderingRequest.ICON_STICKER_RENDERING);
		request.setCaller(caller);
		request.setIconBounds(new Rect(0, 0, w, h));
		request.post(context);
	}
