...
public class TimeLineSlidingWindow implements TimeLineDataLoader.DataListener {
    ...
    public TimeLineSlidingWindow(AbstractGalleryActivity activity, TimeLineDataLoader source,
        ...
        this(activity, source, cacheSize, labelSpec, slotView, null);
    }

    public TimeLineSlidingWindow(AbstractGalleryActivity activity, TimeLineDataLoader source,
                                 int cacheSize, TimeLineSlotRenderer.LabelSpec labelSpec, TimeLineSlotView slotView, SelectionManager selectionManager) {
        ...
        mTitleMaker = new TimeLineTitleMaker(activity, labelSpec, slotView);
    }
...
//        if (item.thumbnailTarget != null) Glide.clear(item.thumbnailTarget); // run mainthread
    }
    ...
    public void updateAllTimelineTitle(boolean switchTitle, int titleIndex, int slotIndex) {
        LogUtil.i(TAG, "updateAllTimelineTitle switchTitle=" + switchTitle + " slotIndex=" + slotIndex);
        if (mSelectionManager.inSelectionMode()) {
            if (switchTitle) {
                AlbumEntry entry = mData[titleIndex % mData.length];
                AlbumEntry firstEntry = mData[(titleIndex + 1) % mData.length];
                if (entry == null || firstEntry == null) {
                    LogUtil.i(TAG, "updateAllTimelineTitle entry == null || firstEntry == null entry=" + entry);
                    return;
                }
                Path path = firstEntry.path;
    ...
    private void loadSelect() {
        if (mSelectTask == null) {
            ...
            mSelectTask.notifyDirty();
        }
    }

    private void recycleSelect() {
        if (mSelectTask != null) {
            mSelectTask.terminate();
            mSelectTask = null;
        }
    }

    public void updateAllTimelineTitleContent(boolean needRequest) {
        LogUtil.i(TAG, "updateAllTimelineTitleContent needRequest=" + needRequest);
    ...
    public void updateAllTimelineSelect() {
        for (int slotIndex = mContentStart, n = mContentEnd; slotIndex < n; ++slotIndex) {
            ...
            if (entry.mediaType == MediaItem.MEDIA_TYPE_TIMELINE_TITLE) {
                entry.isAllSelect = mSelectionManager.isTimeLineSetAllSelect(entry.path);
            }
        }
    }

    private boolean mActiveRequestEnd = false;
    private void updateAllImageRequests() {
        LogUtil.i(TAG, "updateAllImageRequests start");
        if (GalleryUtils.isOnMainThread()) {
...
//            Glide.clear(this);
            ...
            if (isActiveSlot(mSlotIndex)) {
                mTileUploader.addBgTexture(entry.bitmapTexture);
                --mActiveRequestCount;
                if (mActiveRequestCount == 0 && mActiveRequestEnd) requestNonactiveImages();
            ...
            if (mItem.getMediaType() != MediaObject.MEDIA_TYPE_TIMELINE_TITLE) {
                ...
                return mThreadPool.submit(
                        ((TimeLineTitleMediaItem) mItem).requestTitle(
                                MediaItem.TYPE_MICROTHUMBNAIL, mTitleMaker, mSelectionManager), this);
            }
            return null;
        }
    ...
    private class SelectTask extends Thread implements EntryUpdater {
    ...
    public void onContentChanged(int index) {
        LogUtil.i(TAG, "onContentChanged index=" + index + " mIsActive=" + mIsActive + " mContentStart=" + mContentStart + " mContentEnd=" + mContentEnd);
    ...
    public void resume() {
        LogUtil.i(TAG, "resume mContentStart=" + mContentStart + " mContentEnd=" + mContentEnd);
        mIsActive = true;
    ...
    public void onSlotSizeChanged(int width, int height) {
        LogUtil.i(TAG, "onSlotSizeChanged mSlotWidth=" + mSlotWidth + " width=" + width + " height=" + height);
        if (mSlotWidth == width) {
