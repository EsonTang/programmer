...
/// @}
        ...
        while (noBitmap) {
            try {
                ...
                if (putExifData(mDestinationFile, exif, bitmap, quality)) {
                    ...
                    uri = SaveImage.linkNewFileGetUri(mContext, mSelectedImageUri,
                                    mDestinationFile, time, !flatten);
    ...
    /** Photos after editing, save the original cut after disappear update by liudong 2015-08-26 start*/
    public static Uri makeSaveInsertUri(Context context, Uri sourceUri){
        long time = System.currentTimeMillis();
        String filename = new SimpleDateFormat(TIME_STAMP_NAME).format(new Date(time));
        File saveDirectory = getFinalSaveDirectory(context, sourceUri);
        File file = new File(saveDirectory, PREFIX_IMG + filename  + ".JPG");
      
        return linkNewFileGetUri(context, sourceUri, file, time, false);
    }
    /** Photos after editing, save the original cut after disappear update by liudong 2015-08-26 end*/
    ...
    /** Photos after editing, save the original cut after disappear update by liudong 2015-08-26 start*/
    public static Uri linkNewFileGetUri(Context context, Uri sourceUri,
