...
public class GalleryUtils {
    ...
    public static int sSetBitmapWidth;
    public static int sSetBitmapHeight;
    ...
    public static void initialize(Context context) {
		...
		/*prize Camera take pictures into the album load slow, display thumbnails first wzj 2017-6-26 start*/
        //initializeThumbnailSizes(metrics, r);
		/*prize Camera take pictures into the album load slow, display thumbnails first wzj 2017-6-26 end*/
        /// M: [PERF.ADD] @{
        ...
        sBitmapWidth = reMetrics.widthPixels / 4;
        sBitmapWidth &= ~0x01;
        
        sSetBitmapWidth = (int) (reMetrics.widthPixels * 0.428f);
        sSetBitmapHeight = (int) (sSetBitmapWidth * 0.67f);
        // @}
        /// M: [BEHAVIOR.ADD] @{
        MediaItem.sHighQualityThumbnailSize = sRealResolutionMaxSize;
        /// @}
		/*prize Camera take pictures into the album load slow, display thumbnails first wzj 2017-6-26 start*/
        initializeThumbnailSizes(reMetrics, r);
		/*prize Camera take pictures into the album load slow, display thumbnails first wzj 2017-6-26 end*/
    ...
    public static boolean isShowVideoThumbnail() {
        /*if (!FeatureConfig.SUPPORT_VIDEO_THUMBNAIL_PLAY || FeatureConfig.sIsLowRamDevice) {
            return  false;
        }*/
        return  false;
    }
	...
	//4:3 7776*10368; 16:9 5840*10368
    public static final int HD_QUALITY_PIXEL_LARGE = 8320;
    public static final int HD_QUALITY_PIXEL_LARGE_39N = 5760;
    public static final int HD_QUALITY_PIXEL_LARGE_5701 = 9360;
    public static final int HD_QUALITY_PIXEL_LARGE_5701N = 9216;

    public static final int HD_QUALITY_PIXEL_SMALL_3 = 6240;
    public static final int HD_QUALITY_PIXEL_SMALL_9 = 4686;
    public static final int HD_QUALITY_PIXEL_SMALL_39N = 4320;
    public static final int HD_QUALITY_PIXEL_SMALL_39N_9 = 3248;
    public static final int HD_QUALITY_PIXEL_SMALL_5701 = 7024;
    public static final int HD_QUALITY_PIXEL_SMALL_5701N = 5184;
	public static final int HD_NINETY_MILLION_PIXEL = 90000000;
	public static final int HD_SIXTY_MILLION_PIXEL  = 60000000;

    public static boolean isHd90MillionPixel(int width, int height) {
        int big = Math.max(width, height);
        int small = Math.min(width, height);
		return (width*height > HD_NINETY_MILLION_PIXEL)&&(small != 0)&&(big/small < 2);
    }

    public static boolean isHd60MillionPixel(int width, int height) {
        int big = Math.max(width, height);
        int small = Math.min(width, height);
		return (width*height > HD_SIXTY_MILLION_PIXEL)&&(small != 0)&&(big/small < 2);
    }

    public static boolean isHdQualityPixel(int width, int height) {
        int big = Math.max(width, height);
        int small = Math.min(width, height);
        return ( big == HD_QUALITY_PIXEL_LARGE || big == HD_QUALITY_PIXEL_LARGE_39N || big == HD_QUALITY_PIXEL_LARGE_5701 || big == HD_QUALITY_PIXEL_LARGE_5701N) && 
			   (small == HD_QUALITY_PIXEL_SMALL_3 || small == HD_QUALITY_PIXEL_SMALL_9 || small == HD_QUALITY_PIXEL_SMALL_39N || small == HD_QUALITY_PIXEL_SMALL_39N_9
			    || small == HD_QUALITY_PIXEL_SMALL_5701 || small == HD_QUALITY_PIXEL_SMALL_5701N );
    }

    public static boolean isPanoramaPicture(MediaItem item) {
        if (item == null) return false;
        MediaData md = item.getMediaData();
		if(md == null){
			return false;
		}else if (md.isVideo || md.height <= 0 || md.width <= 0) {
            return false;
        }
        float ratio = 0.f;
        ...
        boolean match = (ratio >= 2.5f
                //&& ratio <= 13.f
                && !md.mimeType.equals("image/gif")
                && !md.mimeType.equals("image/mpo")
                && !md.mimeType.equals("image/x-jps"));
        return match;
    }
}
