...
//        mFRep.fillStrokeParameters(mTmpStrokData);
    ...
    // As a solution, we tempararily set HighResRenderingTask to be not delayed
    // when enter ImageDraw, to ensure the key clearing-stroke message to be
    // handled. HighResRenderingTask is a delayed task as google designed in
    // other cases.
    @Override
    protected void onVisibilityChanged(View changedView, int visibility) {
        if (visibility == VISIBLE) {
            ...
            if (master != null) {
                // set HighresRenderingTask to be not "delayed" task tempararily.
                master.setHighresRenderingDelayed(false);
            }
        }
    }
}
