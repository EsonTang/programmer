...
public class AlbumSetSlotRenderer extends AbstractSlotRenderer {
    ...
    private final NinePatchTexture mContentWaitLoadingTexture;
	private final ResourceTexture mCameraOverlay;
	private final ColorTexture mOverBgTexture;
    ...
    public AlbumSetSlotRenderer(AbstractGalleryActivity activity,
        ...
        mContentWaitLoadingTexture = new NinePatchTexture(activity, R.drawable.ic_set_loading);
        
        mOverBgTexture = new ColorTexture(Color.argb(77, 0, 0, 0));
    ...
    protected int getPressedIconId() {
        return R.drawable.ic_set_pressed;
    }
    ...
    public int renderSlot(GLCanvas canvas, int index, int pass, int width, int height) {
        ...
        try {
            ...
            LogUtil.i(TAG, "<renderSlot> AssertionError when mDataWindow.get, " + e.getMessage());
        }
        /// @}
        /// M: [BUG.ADD] avoid nullPointer JE @{
        if (entry == null) {
            LogUtil.i(TAG, "<renderSlot> entry is null, so return");
            return 0;
        }
        /// @}
    ...
    protected int renderLabel(
		...
		content.draw(canvas, -b, height + DensityUtil.dip2px(mActivity, 1), width + b + b, h);
	...
	private void drawContent(GLCanvas canvas, Texture content, int width,
    ...
    protected int renderContent(GLCanvas canvas, int index, AlbumSetEntry entry, int width,
		...
		Texture content = /*checkTexture*/(entry.content);
		if (content == null) {
			content = mContentWaitLoadingTexture;
			/*if (enableFancy)*/ {
//                mContentWaitLoadingTexture.setSize(width, height);
			}
			entry.isWaitLoadingDisplayed = true;
		} else if (entry.isWaitLoadingDisplayed) {
			entry.isWaitLoadingDisplayed = false;
			// content = new FadeInTexture(mPlaceholderColor,
			// entry.bitmapTexture);
			content = entry.bitmapTexture;
			entry.content = content;
		}
		...
		if (!hasDraw) {
            if (enableFancy) {
                ...
                /*drawContent(canvas, content, width - width / 10, width - width / 10,
                        width / 20, 1 * width / 10 + 1 * width / 60 + 1 * width / 70, 0.5f);
                drawContent(canvas, content, width - width / 20, width - width / 20,
                        width / 40, 1 * width / 20 + 1 * width / 80, 0.7f);*/

                if (content instanceof NinePatchTexture) {
