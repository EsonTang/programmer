    ...
    public void run(MediaSet baseSet) {
        ...
        baseSet.enumerateTotalMediaItems(new MediaSet.ItemConsumer() {
            ...
            public void consume(int index, MediaItem item) {
                ...
                s.lat = latLong[0];
                s.lng = latLong[1];
                /** PRIZE- Baidu location  2015-6-13 - start*/
                if(item instanceof LocalImage){
                    s.addr = ((LocalImage)item).getAddr();
                }else if(item instanceof LocalVideo){
                    s.addr = ((LocalVideo)item).getAddr();
                    /** PRIZE-Baidu location-2015-6-12-end*/
                }
                /** PRIZE- Baidu location 2015-6-15 - start*/
                buf[index] = s;
            }
        ...
        for (ArrayList<SmallItem> cluster : clusters) {
        	...
        	/** PRIZE-Baidu location-2015-6-12-start*/
        	String name = cluster.get(0).getAddr();
        ...
        if (hasUnresolvedAddress) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                	/** Prize -- 2015-06-24  Baidu location*/
...
//                            Toast.LENGTH_LONG).show();
   ...
   /// @}
    ...
    public String getClusterName(int index) {
    	if (mNames == null || mNames.size() <= index) {
    		return "";
    	}
    	Log.i(TAG, "getClusterName mNames=" + mNames);
