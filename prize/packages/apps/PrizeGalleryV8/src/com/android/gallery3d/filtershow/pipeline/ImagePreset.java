...
import com.prize.sticker.ISticker;
import com.prize.sticker.Sticker;
...
public class ImagePreset {
    ...
    public boolean hasModifications() {
    	...
    	if (StickerManager.getStickerManager().hasStickerModify()) {
    		return true;
    	}
    ...
    public boolean isPanoramaSafe() {
        for (FilterRepresentation representation : mFilters) {
			...
			/** PRIZE-watermark-wanzhijuan-2016-1-21-start **/
            if (representation.getFilterType() == FilterRepresentation.TYPE_STICKER
                    && !representation.isNil()) {
                return false;
            }
			/** PRIZE-watermark-wanzhijuan-2016-1-21-end **/
    ...
    public void removeFilter(FilterRepresentation filterRepresentation) {
        if (filterRepresentation.getFilterType() == FilterRepresentation.TYPE_BORDER || filterRepresentation.getFilterType() == FilterRepresentation.TYPE_STICKER) {
            for (int i = 0; i < mFilters.size(); i++) {
    ...
    public void addFilter(FilterRepresentation representation) {
		...
		/** PRIZE-watermark-wanzhijuan-2016-1-21-start **/
		else if (representation.getFilterType() == FilterRepresentation.TYPE_STICKER) {
            removeFilter(representation);
            if (!isNoneStickerFilter(representation)) {
                mFilters.add(representation);
            }
        } 
		/** PRIZE-watermark-wanzhijuan-2016-1-21-end **/
	...
	/** PRIZE-watermark-wanzhijuan-2016-1-21-start **/
    private boolean isNoneStickerFilter(FilterRepresentation representation) {
        return representation instanceof FilterImageStickerRepresentation &&
                ((FilterImageStickerRepresentation) representation).getWatermarkBean().getThumbId() == 0;
    }
	/** PRIZE-watermark-wanzhijuan-2016-1-21-end **/
	...
	/** PRIZE-watermark-wanzhijuan-2016-1-21-start **/
    public Bitmap applySticker(Bitmap bitmap, FilterEnvironment environment) {
    	FilterRepresentation border = getFilterRepresentationForType(FilterRepresentation.TYPE_STICKER);
    	Log.i("ImagePreset", "applySticker border=" + border);
    	if (border != null) {
    		bitmap = environment.applyRepresentation(border, bitmap);
    	}
        return bitmap;
	...
	/** PRIZE-watermark-wanzhijuan-2016-1-21-start **/
    public void applySticker(Allocation in, Allocation out,
            boolean copyOut, FilterEnvironment environment) {
        FilterRepresentation border = getFilterRepresentationForType(
                FilterRepresentation.TYPE_STICKER);
        if (border != null) {
            // TODO: should keep the bitmap around
            Allocation bitmapIn = in;
            if (copyOut) {
        ...
        if (mDoApplyFilters) {
            ...
            for (int i = from; i < to; i++) {
                ...
                if (representation.getFilterType() == FilterRepresentation.TYPE_GEOMETRY
                        ...
                        || representation.getFilterType() == FilterRepresentation.TYPE_STICKER) {
                    continue;
                }
                if (i > from) {
                    in.copyFrom(out);
	...
	/** PRIZE-watermark-wanzhijuan-2016-1-21-start **/
    public ArrayList<ISticker> saveSticker() {
    	return StickerManager.getStickerManager().saveStickers();
    }
	
	public Bitmap applyStickers(Bitmap bitmap, FilterEnvironment mEnvironment) {
		LogTools.i(LOGTAG, "applyStickers Bitmap width=" + bitmap.getWidth() + " height=" + bitmap.getHeight());
		ArrayList<ISticker> stickers = saveSticker();
		Bitmap newb = bitmap;
		if (stickers.size() > 0) {
			Point sizePoint = MasterImage.getImage().getImageShowSize();
			int bmWidth = bitmap.getWidth();
			int bmHeight = bitmap.getHeight();
			LogTools.i(LOGTAG, "applyStickers createBitmap sizePoint=" + sizePoint);
			newb = Bitmap.createBitmap(sizePoint.x + 30, sizePoint.y+30, Config.ARGB_8888);
			Canvas cv = new Canvas(newb);
			PaintFlagsDrawFilter mSetfil = new PaintFlagsDrawFilter(0, Paint.FILTER_BITMAP_FLAG);
			cv.setDrawFilter(mSetfil);
			float scale;
			float dx, dy;
			cv.save();
			if (bmWidth * sizePoint.y > bmHeight * sizePoint.x) {
				scale = (float) sizePoint.x / bmWidth;
				dx = 15;
				dy = (sizePoint.y - scale * bmHeight) / 2 + 15;
			} else {
				scale = (float) sizePoint.y / bmHeight;
				dy = 15;
				dx = (sizePoint.x - scale * bmWidth) / 2 + 15;
			}
			Matrix matrix = new Matrix();
			matrix.setScale(scale, scale);
			matrix.postTranslate(dx, dy);
			cv.drawBitmap(bitmap, matrix, null);
			cv.restore();
			for (ISticker sticker : stickers) {
				cv.drawBitmap(sticker.getBitmap(true), sticker.getMatrix(), null);
			}
//			cv.clipRect(new Rect((int) dx, (int) dy, (int) (sizePoint.x + 30 - dx), (int) (sizePoint.y + 30 - dy)));  
			LogTools.i(LOGTAG, "applyStickers createBitmap dx=" + dx + " dy=" + dy);
			return Bitmap.createBitmap(newb, (int) dx, (int) dy, (int) (sizePoint.x + 30 - dx - dx), (int) (sizePoint.y + 30 - dy - dy));
		}
		return newb;
	}
	/** PRIZE-watermark-wanzhijuan-2016-1-21-end **/
