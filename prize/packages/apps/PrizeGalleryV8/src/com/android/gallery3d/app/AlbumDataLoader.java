...
public class AlbumDataLoader {
    ...
    public AlbumDataLoader(AbstractGalleryActivity context, MediaSet mediaSet) {
        this(context, mediaSet, false, false);
        if (mIsCameraSource) {
            preLoadItems();
        }
    }

    public AlbumDataLoader(AbstractGalleryActivity context, MediaSet mediaSet, boolean isContainer, boolean isAlbum) {
        mActivity = context;
        mSource = mediaSet;
        mIsAlbum = isAlbum;
        if (mIsAlbum) {
            mIsCameraSource = MediaSetUtils.isCameraSource(mSource.getPath());
        }
    ...
    private void preLoadItems() {
        GalleryApp app = (GalleryApp) mActivity.getApplicationContext();
        ArrayList<MediaItem> items = app.getPreTimeItem();
        int loadIndex = 0;
        for (int i = 0, count = items.size(); i < count; i++) {
            MediaItem item = items.get(i);
            if (item.isSelectable()) {
                int index = loadIndex % DATA_CACHE_SIZE;
                mData[index] = item;
    ...
    private class GetUpdateInfo implements Callable<UpdateInfo> {
        ...
        public UpdateInfo call() throws Exception {
            ...
            info.size = mSize;
            long setVersion[] = mSetVersion;
            LogUtil.i(TAG, "GetUpdateInfo mVersion=" + mVersion + " mSize=" + mSize + " mSourceVersion=" + mSourceVersion);
    ...
    private class UpdateContent implements Callable<Void> {
        ...
        public Void call() throws Exception {
            ...
            if (mSize != info.size) {
                ...
                if (mDataListener != null && mSize >= 0 && !mIsContainer) mDataListener.onSizeChanged(mSize);
                /// @}
            ...
            LogUtil.i(TAG, "UpdateContent start=" + start + " mSetVersion=" + mSetVersion + " end=" + end);
            ...
            for (int i = start; i < end; ++i) {
                ...
                if (mItemVersion[index] != itemVersion) {
                    LogUtil.i(TAG, "UpdateContent itemVersion=" + itemVersion + " mItemVersion[index]=" + mItemVersion[index]);
    ...
    private class ReloadTask extends Thread {
        ...
        private void updateLoading(boolean loading) {
            if (mIsLoading == loading && !mIsContainer) return;
            mIsLoading = loading;
        ...
        public void run() {
            ...
            while (mActive) {
                LogUtil.i(TAG, "ReloadTask mActive=" + mActive + " mDirty=" + mDirty + " updateComplete=" + updateComplete);
                synchronized (this) {
                ...
                LogUtil.d(TAG, "ReloadTask mSource = " + mSource);
                ...
                if (info.version != version) {
                    ...
                    LogUtil.d(TAG, "ReloadTask run info.size=" + info.size);
                }
                if (info.reloadCount > 0) {
