...
public class CachingPipeline implements PipelineInterface {
    ...
    public synchronized void renderSticker(RenderingRequest request) {
        // TODO: cleanup/remove GEOMETRY / FILTERS paths
        synchronized (CachingPipeline.class) {
            if (getRenderScriptContext() == null) {
                return;
            ...
            if (DEBUG) {
                Log.v(LOGTAG, "renderSticker render image of type 1 " + getType(request));
            }
            ...
            if (DEBUG) {
                Log.v(LOGTAG, "renderSticker render image of type " + getType(request));
            }
            ...
            if (request.getType() == RenderingRequest.PARTIAL_RENDERING) {
                ...
                /// @}
                if (bitmap == null) {
                    Log.w(LOGTAG, "renderSticker could not get bitmap for: " + getType(request));
                    return;
                }
            }

            if (request.getType() == RenderingRequest.STYLE_ICON_RENDERING
                    || request.getType() == RenderingRequest.ICON_STICKER_RENDERING) {

                if (request.getType() == RenderingRequest.ICON_RENDERING || request.getType() == RenderingRequest.ICON_STICKER_RENDERING) {
                    mEnvironment.setQuality(FilterEnvironment.QUALITY_ICON);
                } else {
                    mEnvironment.setQuality(FilterEnvironment.QUALITY_PREVIEW);
                }
                ...
                Bitmap bmp = preset.applySticker(bitmap, mEnvironment);
                Log.w(LOGTAG, "renderSticker mEnvironment.needsStop(): " + mEnvironment.needsStop());
                if (!mEnvironment.needsStop()) {
                    request.setBitmap(bmp);
                }
                mFiltersManager.freeFilterResources(preset);
    ...
    public synchronized void renderImage(ImagePreset preset, Allocation in, Allocation out) {
        synchronized (CachingPipeline.class) {
			...
			/** PRIZE-watermark-wanzhijuan-2016-1-21-start **/
            preset.applySticker(in, out, copyOut, mEnvironment);
			/** PRIZE-watermark-wanzhijuan-2016-1-21-end **/
        }
    }
    ...
    public synchronized Bitmap renderFinalImage(Bitmap bitmap, ImagePreset preset) {
        synchronized (CachingPipeline.class) {
            ...
            bitmap = preset.apply(bitmap, mEnvironment);
			/** PRIZE-watermark-wanzhijuan-2016-1-21-start **/
            bitmap = preset.applyStickers(bitmap, mEnvironment);
			/** PRIZE-watermark-wanzhijuan-2016-1-21-end **/
            return bitmap;
        }
    }
