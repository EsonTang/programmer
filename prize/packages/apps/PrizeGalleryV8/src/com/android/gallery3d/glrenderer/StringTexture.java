...
public class StringTexture extends CanvasTexture {
    ...
    private int mBackgroundColor = -2;
    ...
    private StringTexture(String text, TextPaint paint,
                          FontMetricsInt metrics, int width, int height, int backgroundColor) {
        super(width, height);
//		LogUtil.i("StringTexture", "StringTexture width=" + width + " height=" + height);
        mText = text;
        mPaint = paint;
        mMetrics = metrics;
    ...
    public static StringTexture newInstance(String text, TextPaint paint, int backgroundColor) {
        FontMetricsInt metrics = paint.getFontMetricsInt();
        int width = (int) Math.ceil(paint.measureText(text));
        int height = metrics.bottom - metrics.top;
        // The texture size needs to be at least 1x1.
        if (width <= 0) width = 1;
        if (height <= 0) height = 1;
        return new StringTexture(text, paint, metrics, width, height, backgroundColor);
    }
    ...
    protected void onDraw(Canvas canvas, Bitmap backing) {
		...
		//LogUtil.i("StringTexture", "onDraw mMetrics.ascent=" + mMetrics.ascent);
