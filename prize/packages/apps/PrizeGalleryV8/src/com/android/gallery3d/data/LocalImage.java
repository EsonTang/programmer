...
public class LocalImage extends LocalMediaItem {
    ...
    /// @}
            ...
            /** PRIZE-Baidu location-2015-4-6-start*/
            Images.Media.DESCRIPTION // 21
            /** PRIZE-Baidu location-2015-4-6-end*/
            /// @}
    };
    ...
    public LocalImage(Path path, GalleryApp application, int rotation, String filePath) {
        super(path);
        mApplication = application;
    ...
    private void parseAddr() {
        if (!TextUtils.isEmpty(dbAddr)) {
            String[] addrDetails = dbAddr.split(":");
            if (addrDetails != null && addrDetails.length > 1) {
                dbAddr = addrDetails[0];
            }
        }
    }
    ...
    private void loadFromCursor(Cursor cursor) {
        ...
        dbAddr = cursor.getString(cursor.getColumnIndex(Images.Media.DESCRIPTION));
        parseAddr();
    ...
    public MediaDetails getDetails() {
        ...
        /*PRIZE-1473 Camera: the details of the photo inside the browser mode and the focus is the default zero value-fuqiang-2015-6-23-start*/
        int rotation = 0;
        ExifInterface exif = new ExifInterface();
        try {
        ...
        /*PRIZE-1473 Camera: the details of the photo inside the browser mode and the focus is the default zero value-fuqiang-2015-6-23-end*/
    ...
    private boolean updateMediaData(Cursor cursor) {
        ...
        if (mIsConshot) {
            if (cursor == null) {
                Log.i(TAG, "<updateMediaData> mIsConshot, cursor is null, return");
                return false;
            }
            ...
            if (cursor == null) {
                Log.i(TAG, "<updateMediaData> other, cursor is null, return");
                return false;
            }
            synchronized (mMediaDataLock) {
                mMediaData = MediaDataParser.parseLocalImageMediaData(cursor);
