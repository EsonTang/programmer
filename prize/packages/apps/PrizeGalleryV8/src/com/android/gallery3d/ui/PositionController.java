...
/// @}
    ...
    //ADD by liangchangwei 2018-4-13
    public static final int CLAMP_SCALE_LIMIT = 6;
    public static final int CLAMP_END_SCALE_LIMIT = 3;
    ...
    public int scaleBy(float s, float focusX, float focusY) {
        ...
        //
        s = b.clampLimitScale(s * getTargetScale(b));
        //prize-public-bug:scale problem-pengcancan-20161217-start
        /*if (s > b.mScaleMin*3.0f){
            s = b.mScaleMin*3.0f;
        }*/
    ...
    public void endScale(float focusX, float focusY) {
    	//prize-public-bug:21875-fit zoom size-20160913-pengcancan-start
        Box b = mBoxes.get(0);
        float scale = getTargetScale(b);
        stopAnimation();
        float s = b.clampEndLimitScale(b.mCurrentScale);
        if (scale > s) {
        	zoomIn(mLastFocusX, mLastFocusY, s);
		}
        //prize-public-bug:21875-fit zoom size-20160913-pengcancan-end
        mInScale = false;
        snapAndRedraw();
    }
    ...
    public boolean flingPage(int velocityX, int velocityY) {
        ...
        if ((viewWiderThanScaledImage(b.mCurrentScale) &&
            viewTallerThanScaledImage(b.mCurrentScale)) || b.mAnimationStartTime != NO_ANIMATION) {
            return false;
        }
        //prize-public-bug:21875-fit zoom size-20160913-pengcancan-end
    ...
    */
    ...
    public boolean isPanoramaScale() {
		boolean isPanorama = mModel == null ? false : GalleryUtils.isPanoramaPicture(mModel.getMediaItem(0));
        return isPanorama;
    }

	public float getPanoramaScale(){
		boolean isPanorama = mModel == null ? false : GalleryUtils.isPanoramaPicture(mModel.getMediaItem(0));
        Box b = mBoxes.get(0);
        return isPanorama? Math.max(mViewH * 1.f /b.mImageH, mViewW * 1.f /b.mImageW) : 4;
	}
    ...
    private class Box extends Animatable {
        ...
        public float clampScale(float s) {
            return Utils.clamp(s,
                    SCALE_MIN_EXTRA * mScaleMin, SCALE_MAX_EXTRA * mScaleMax);
        }

        public float clampLimitScale(float s) {
            //prize
            boolean isHdQuality = GalleryUtils.isHdQualityPixel(mImageW, mImageH);
            boolean isPanorama = mModel == null ? false : GalleryUtils.isPanoramaPicture(mModel.getMediaItem(0));
            float maxScale = (float)CLAMP_SCALE_LIMIT * mScaleMin;
			if(GalleryUtils.isHd90MillionPixel(mImageW,mImageH)){
                maxScale = 36.f * mScaleMin;
			}else if(GalleryUtils.isHd60MillionPixel(mImageW,mImageH)){
                maxScale = 26.f * mScaleMin;
            } else if (isPanorama) {
                maxScale = Math.max(mViewH * 1.f /mImageH, mViewW * 1.f /mImageW) + 2.f * mScaleMin;
            }
            return Utils.clamp(s,
                    SCALE_MIN_EXTRA * mScaleMin,
                    /*SCALE_MAX_EXTRA * mScaleMax*/maxScale);
        }

        public float clampEndLimitScale(float s) {
            //prize
            boolean isHdQuality = GalleryUtils.isHdQualityPixel(mImageW, mImageH);
            boolean isPanorama = mModel == null ? false : GalleryUtils.isPanoramaPicture(mModel.getMediaItem(0));
            float maxScale = (float)CLAMP_END_SCALE_LIMIT * mScaleMin;
			if(GalleryUtils.isHd90MillionPixel(mImageW,mImageH)){
                maxScale = 30.f * mScaleMin;
			}else if(GalleryUtils.isHd60MillionPixel(mImageW,mImageH)){
                maxScale = 18.f * mScaleMin;
            } else if (isPanorama) {
                maxScale = Math.max(mViewH * 1.f /mImageH, mViewW * 1.f /mImageW);
            }
            return maxScale;
        }
        ...
        private void noticeZoom() {
            boolean isHdQuality = isHdPicture();
            if (isHdQuality && (mAnimationKind == ANIM_KIND_SCALE || mAnimationKind == ANIM_KIND_ZOOM)) {
                mListener.onShowZoom(mCurrentScale / getMinimalScale(this));
            }
        }
    ...
    public boolean isHd90MillionPicture() {
        if (mModel != null) {
            MediaItem item = mModel.getMediaItem(0);
            if (item != null) {
                return GalleryUtils.isHd90MillionPixel(item.getWidth(), item.getHeight());
            }
        }
        return false;
    }
    ...
    public boolean isHdPicture() {
        if (mModel != null) {
            MediaItem item = mModel.getMediaItem(0);
            if (item != null) {
                return GalleryUtils.isHd60MillionPixel(item.getWidth(), item.getHeight());
            }
        }
        return false;
    }
    ...
    public boolean loadOk() {
        if (mModel != null) {
            MediaItem item = mModel.getMediaItem(0);
            if (item != null) {
                if(GalleryUtils.isHd60MillionPixel(item.getWidth(), item.getHeight())){
					Box b = mBoxes.get(0);
					if (item != null) {
						return (b.mImageW == item.getWidth() && b.mImageH == item.getHeight()) || (b.mImageW == item.getHeight() && b.mImageH == item.getWidth());
					}
                }else{
                    return true;
                }
