...
public class StateManager {
    ...
    public void onConfigurationChange(Configuration config) {
    	if (!mStack.isEmpty()) {
            getTopState().onConfigurationChanged(config);
        }
    }
...
/// @}
        finishState(state, true, false);
    }
    
    public void finishState(ActivityState state, boolean isPhotoPage) {
    	finishState(state, true, isPhotoPage);
    }
    ...
    void finishState(ActivityState state, boolean fireOnPause, boolean isPhotoPage) {
        ...
        if (mStack.size() == 1) {
            ...
            if (activity instanceof GalleryActivity) {
            	((GalleryActivity) activity).setShowMask(isPhotoPage);
            }
            activity.finish();
            ...
            // running, call state.pause() maybe cause blurred screen. so first
            // call
            // GLSurfaceView.onPause() to stop GL-thread, then state.pause().
            return;
    ...
    public void restoreFromState(Bundle inState) {
        ...
        for (Parcelable parcelable : list) {
            ...
            if (activityState instanceof AlbumSetPage && state != null && data != null) {
                ...
                if (isReused) {
                    data.putInt(AlbumSetPage.KEY_PAGE_ID, state.getInt(AlbumSetPage.KEY_PAGE_ID));
                }
    ...
    public void setAlbumSetPage(int page){
    	 ActivityState state = getTopState();
    	 if (state instanceof AlbumSetPage){
    		((AlbumSetPage) state).setPage(page);
    	 }
    }
