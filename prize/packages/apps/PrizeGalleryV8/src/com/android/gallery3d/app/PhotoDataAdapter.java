...
public class PhotoDataAdapter implements PhotoPage.Model {
    ...
    private void updateScreenNail(Path path, Future<ScreenNail> future) {
        LogUtil.d(TAG, "<updateScreenNail> path " + path);
        ...
        LogUtil.d(TAG, "<updateScreenNail> path end" + path);
        updateImageRequests();
    ...
    private boolean isGif(MediaData data) {
        if (data != null && data.mediaType == MediaData.MediaType.GIF) {
            return true;
        }
        return false;
    }
    ...
    public ScreenNail getScreenNail(int offset) {
        ...
        if (entry.screenNail == null && !isCamera(offset)) {
            ...
            if (offset == 0 && !isGif(item.getMediaData())) {
                ...
                LogUtil.i(TAG, "getScreenNail found=" + found);
                Bitmap bitmap = null;
                if (!found) {
