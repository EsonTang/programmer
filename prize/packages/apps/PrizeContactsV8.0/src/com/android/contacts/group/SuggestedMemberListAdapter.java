...
public class SuggestedMemberListAdapter extends ArrayAdapter<SuggestedMember> {
    ...
    private static final String[] PROJECTION_FILTERED_MEMBERS_INTERNAL = new String[] {
        Contacts._ID,                        // 0
        Contacts.DISPLAY_NAME_PRIMARY        // 1
    };
    private static final String[] PROJECTION_FILTERED_MEMBERS;

    static {
        List<String> projectionList = Lists.newArrayList(PROJECTION_FILTERED_MEMBERS_INTERNAL);

        if (ContactsPortableUtils.MTK_PHONE_BOOK_SUPPORT) {
            /// M: Add some columns for Contacts extensions. @{
            projectionList.add(Contacts.INDICATE_PHONE_SIM);   //2
            projectionList.add(Contacts.IS_SDN_CONTACT);       //3
            /// @}
        }

        PROJECTION_FILTERED_MEMBERS = projectionList.toArray(new String[projectionList.size()]);
    }
    ...
    public class SuggestedMemberFilter extends Filter {
        ...
        protected FilterResults performFiltering(CharSequence prefix) {
            ...
            + " FROM view_raw_contacts WHERE view_raw_contacts.contact_id=view_contacts._id AND "
                    + accountFilter + ")";
            Log.d(TAG, "[performFiltering] begin the first query");
            ...
            Log.d(TAG, "[performFiltering] End the first query");
            /// @}
            ...
            Log.d(TAG, "[performFiltering] Begin the second query");
            ...
            Log.d(TAG, "[performFiltering] End the second query");
            /// M: Bug fix ALPS00280807.
