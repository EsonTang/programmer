...
public class ContactSaveService extends IntentService {
        ...
        if (listener instanceof ContactEditorBaseActivity) {
            for (Listener currentlistener : sListeners) {
                if (currentlistener instanceof ContactEditorBaseActivity) {
                    Log.w(TAG,
                            "[registerListener] only one ContactEditorBaseActivity instance " +
                                    "allowed,finish old one: " + currentlistener);
    ...
    private void updateGroup(Intent intent) {
        ...
        if (groupId > 0 && label != null && !ContactSaveServiceEx.checkGroupNameExist(
                        this, label, accountName, accountType, true)) {
        ...
        /// M: ALPS02829476.when udpate group,it should delete first then add member to group @{
    ...
    private void joinSeveralContacts(Intent intent) {
        ...
        closeintent.setAction("prize.close.dialog");
