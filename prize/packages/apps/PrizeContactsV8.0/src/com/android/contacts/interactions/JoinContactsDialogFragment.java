 ...
 */
public class JoinContactsDialogFragment extends BaseEventHandlerFragment
...
*/
    ...
    public void setContactIds(TreeSet<Long> contactIds) {
        ...
        if (isStarted()) {
            ...
            getLoaderManager().restartLoader(
                    R.id.dialog_link_multiple_contact_loader_id, args, this);
        }
    }
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
        ...
        int messageId = R.string.batch_link_single_contact_warning;
        int index_in_sim = -1;
        ...
        if (cursor.getCount() <= 1 || index_in_sim > 0) {
            ...
            messageId = R.string.batch_link_confirmation;
            showDialog(messageId, true);
        }

        // We don't want onLoadFinished() calls any more, which may come when the database is
        // updating.
        getLoaderManager().destroyLoader(R.id.dialog_link_multiple_contact_loader_id);
    }
    ...
    private void showDialog(int messageId, boolean showNegativeButton) {
        Log.d(TAG, "[showDialog] showNegativeButton: " + showNegativeButton);
    ...
    /// M: refactor phb state change.when merge sim contacts,popup up error dialog,plugout/in sim
    /// dialog will dismiss.@{
    @Override
    public void onReceiveEvent(String eventType, Intent extraData) {
        Log.d(TAG, "[onReceiveEvent] eventType: " + eventType + ",mIsMergeSimContacts: " +
                mIsMergeSimContacts);
        if ((GeneralEventHandler.EventType.PHB_STATE_CHANGE_EVENT.equals(eventType))
                && mIsMergeSimContacts && (mDialog != null) && mDialog.isShowing()) {
            Log.i(TAG, "[onReceiveEvent] Phb event, merge sim error dialog dismiss!");
            mDialog.dismiss();
        }
    }
    /// @}
