...
public class PhoneticNameEditorView extends TextFieldsEditorView {
    ...
    public void setValues(DataKind kind, ValuesDelta entry, RawContactDelta state, boolean readOnly,
            ViewIdGenerator vig) {
        /// M: Bug fix ALPS02006512, need null check firstly before using it.
        if (entry != null && !(entry instanceof PhoneticValuesDelta)) {
            entry = new PhoneticValuesDelta(entry);
        }
        /// M: Bug fix ALPS00101852, need null pointer check.
        if (kind != null) {
