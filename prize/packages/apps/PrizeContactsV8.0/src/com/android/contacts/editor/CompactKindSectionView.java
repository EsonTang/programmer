...
public class CompactKindSectionView extends LinearLayout {
    private static final String TAG = "CompactKindSectionView";
    ...
    private class NonNameEditorListener implements Editor.EditorListener {
        ...
        @Override
        public void onDeleteRequested(Editor editor) {
            ///Add for AAS, when do delete AAS, we just clear it. @{
            if ((mShowOneEmptyEditor && mEditors.getChildCount() == 1)
                    || GlobalEnv.getAasExtension().updateView(
        ...
        if (isNameKindSection) {
            ...
            for (int i = 0; i < mEditors.getChildCount(); i++) {
                ...
                if (view instanceof GroupMembershipView) {
                    ...
                    /// M: Fix CR ALPS02813651. add/edit profile shouldn't contain group view. @{
    ...
    private void updateEmptyNonNameEditors(boolean shouldAnimate) {
        ...
        int max = 1;
        if (mKindSectionDataList.getMimeType() != null) {
            max = GlobalEnv.getAasExtension().getMaxEmptyEditors(
                    mKindSectionDataList.get(0).getRawContactDelta(),
                    mKindSectionDataList.getMimeType());
            String accountType = mKindSectionDataList.get(0).getRawContactDelta().getAccountType();
            if (SimAasSneUtils.isUsimOrCsim(accountType)
                    && SimAasSneUtils.isPhoneNumType(mKindSectionDataList.getMimeType())) {
                mKindSectionDataList.get(0).getDataKind().typeOverallMax = max;
        ...
        if (mShowOneEmptyEditor) {
            ...
            if (values != null && values.containsKey("data2")) {
                if (values.getAsInteger("data2", -1) == Aas.PHONE_TYPE_AAS) {
                    ...
                    values.put(Data.IS_ADDITIONAL_NUMBER, 1);
                }
            }
            /// @}
