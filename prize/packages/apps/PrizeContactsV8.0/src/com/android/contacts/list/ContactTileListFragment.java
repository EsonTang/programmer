...
public class ContactTileListFragment extends Fragment {
    ...
    private final LoaderManager.LoaderCallbacks<Cursor> mContactTileLoaderListener =
        ...
        public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
            ...
            /**
             * M: add for ALPS01464088. check whether the fragment still in
             * Activity @{
             */
    ...
    public void onResume() {
        ...
        if (loadContactsPreferences()) {
            ...
            for (int i = 0; i < loaderTypes.length; i++) {
                if (loaderTypes[i] == mDisplayType) {
                    getLoaderManager().restartLoader(mDisplayType.ordinal(), null,
                            mContactTileLoaderListener);
                }
            }
        }
    ...
    public void onStop() {
        if (mContactsPreferences != null) {
            Log.d(TAG, "[onDestroy] unregisterChangeListener...");
    ...
    private ChangeListener mPreferencesChangeListener = new ChangeListener() {
        ...
        public void onChange() {
            Log.d(TAG, "[onChange] ContactsPreferences has changed,reload contacts...");
            if (loadContactsPreferences()) {
                final DisplayType[] loaderTypes = mDisplayType.values();
                for (int i = 0; i < loaderTypes.length; i++) {
                    if (loaderTypes[i] == mDisplayType) {
