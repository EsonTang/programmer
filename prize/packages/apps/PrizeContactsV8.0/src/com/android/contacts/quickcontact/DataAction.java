...
public class DataAction implements Action {
    ...
    public DataAction(Context context, DataItem item, DataKind kind) {
        ...
        /// M: add for AAS
        mSubtitle = (String) GlobalEnv.getAasExtension().getTypeLabel(
                    item.getContentValues().getAsInteger(Data.DATA2),
                    (CharSequence) item.getContentValues().getAsString(Data.DATA3),
                    (String) mSubtitle, SimAasEditor.SIM_ID_DONT_KNOW_CURR);
