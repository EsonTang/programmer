...
import com.mediatek.contacts.list.service.MultiChoiceHandlerListener.OnJobCancelListener;//prize-add for custom progressbar -hpf-2017-12-27
...
public class MultiChoiceService extends Service {
    ...
    /*private*/public static final Map<Integer, ProcessorBase> RUNNINGJOBMAP =
            new HashMap<Integer, ProcessorBase>();   /*prize modify public for bug58893 by zhaojian 20180525 start*/
    ...
    public synchronized void handleDeleteRequest(List<MultiChoiceRequest> requests,
        ...
        if (tryExecute(new DeleteProcessor(this, listener, requests, sCurrentJobId))) {
            if (listener != null) {
                ...
                listener.setOnJobCancelListener(new OnJobCancelListener() {
					
					@Override
					public void onJobCancel(int jobId) {
						prizeHandleCancelRequest(jobId);
					}
				});
    ...
    /*prize-add for custom progressbar -hpf-2017-12-27-start*/
    /*private*/public synchronized void prizeHandleCancelRequest(int jobId) {  /*prize modify public for bug58893 by zhaojian 20180525*/
        Log.i(TAG, "[prizeHandleCancelRequest]jobId:" + jobId);
        final ProcessorBase processor = RUNNINGJOBMAP.remove(jobId);
