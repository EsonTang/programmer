...
/*prize add for bug 56665 by zhaojian 20180502 start*/
import android.content.res.ColorStateList;
/*prize add for bug 56665 by zhaojian 20180502 end*/
/**
...
public abstract class AbstractPickerFragment extends
     ...
     */
    // prize modify for bug 45001 by zhaojian 20171212 start
    //public static final int DEFAULT_MULTI_CHOICE_MAX_COUNT = 3500;
    public static final int DEFAULT_MULTI_CHOICE_MAX_COUNT = 5000;
    // prize modify for bug 45001 by zhaojian 20171212 end
    ...
    protected void onCreateView(LayoutInflater inflater, ViewGroup container) {
        ...
        if (mEmptyView != null) {
            mEmptyView.setText(R.string.prize_no_contacts);//prize-change-hpf-2018-3-15
        }
    }
   ...
   @Override
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        if (data == null || (data != null && !data.isClosed() && data.getCount() == 0)) {  //prize add !data.isClosed() for bug 54802 by zhaojian 20180411
            Log.w(TAG, "[onLoadFinished]nothing loaded, data:" + data + ",empty view: "
                    + mEmptyView);
            if (mEmptyView != null) {
            	/*prize-change-hpf-2018-3-15-start*/
                /*if (getAdapter().isSearchMode()) {
                    mEmptyView.setText(R.string.listFoundAllContactsZero);
                } else {
                    mEmptyView.setText(R.string.noContacts);
                }*/
            	mEmptyView.setText(R.string.prize_no_contacts);
            	/*prize-change-hpf-2018-3-15-end*/
                mEmptyView.setVisibility(View.VISIBLE);
            }
            // Disable fast scroll bar
            getListView().setFastScrollEnabled(false);
            getListView().setFastScrollAlwaysVisible(false);
            getListView().setVerticalScrollBarEnabled(false);//prize-add-huangliemin-2016-7-29
        } else {
            if (mEmptyView != null) {
            	/*prize-change-hpf-2018-3-15-start*/
                /*if (getAdapter().isSearchMode()) {
            ...
            if (!getAdapter().isSearchMode()) {
			/*PRIZE-remove-fastscroll-shiyicheng-2015-9-23-start*/
                getListView().setFastScrollEnabled(false);
                getListView().setFastScrollAlwaysVisible(false);
                getListView().setVerticalScrollBarEnabled(false);//prize-add-huangliemin-2016-7-29
			/*PRIZE-remove-fastscroll-shiyicheng-2015-9-23-end*/
            }
        }
        ...
        if (data != null && !data.isClosed()) { //prize add for bug 54802 by zhaojian 20180412
        ...
        // prize delete for bug 45208 by zhaojian 20171214 start
//        updateSelectedItemsView(getAdapter().getSelectedContactIds().size());
        // prize delete for bug 45208 by zhaojian 20171214 end
        ...
        // prize add for bug 45208 by zhaojian 20171214 start
        updateSelectedItemsView(getAdapter().getSelectedContactIds().size());
        // prize add for bug 45208 by zhaojian 20171214 end
    }
    ...
    protected void updateSelectedItemsView(int checkedItemsCount) {
        ...
        //this method can update delete contacts button status
        mSelectedContactsCheckBoxChangeListener.onSelectedContactsCheckBoxChange();//prize-add-huangpengfei-2016-9-3
        ...
        // prize add zhaojian for bug 52455 20180314 start
        TextView selectAllButton = (TextView)getActivity().findViewById(R.id.select_all);
        android.util.Log.d(TAG,"selectAllButton = " + selectAllButton);
        if(count == 0){
            if(selectAllButton != null){
                selectAllButton.setEnabled(false);
                selectAllButton.setTextColor(getActivity().getResources().getColor(R.color.prize_button_text_dark_color));
            }
        }else {
            if(selectAllButton != null){
                selectAllButton.setEnabled(true);
                selectAllButton.setTextColor(getActivity().getResources().getColorStateList(R.drawable.prize_selector_actionbar_text_btn));
            }
        }
        // prize add zhaojian for bug 52455 20180314 end
    ...
    private void showSelectedCount(int checkedItemsCount) {
        ...
        View view = hostActivity.getActionBar().getCustomView();
        /// @}
        if (view != null) {
    ...
    private void setOkButtonStatus(boolean enable) {
        ...
        View view = hostActivity.getActionBar().getCustomView();
        /// @}
        /* PRIZE-set ok button status -zhangzhonghao-2015-5-20 -start */
        TextView prizeOK = ((ContactListMultiChoiceActivity) getActivity()).getPrizeOKButton();
        /* PRIZE-set ok button status -zhangzhonghao-2015-5-20 -end */
        if (view != null) {
        ...
        if (optionView != null) {
            if (enable) {
                ...
                optionView.setTextColor(/*getResources().getColor(R.color.background_primary)*/Color.BLACK); //zhangzhonghao
            } else {
        ...
        if (prizeOK != null){
            if (enable) {
                prizeOK.setEnabled(true);
                //prizeOK.setTextColor(getResources().getColor(R.color.prize_button_text_default_color));
            } else {
                prizeOK.setEnabled(false);
                //prizeOK.setTextColor(getResources().getColor(R.color.prize_button_text_dark_color));
            }
        }
    ...
    protected OnSelectedContactsCheckBoxChangeListener mSelectedContactsCheckBoxChangeListener;
    public void setSelectedContactsCheckBoxChangeListener(OnSelectedContactsCheckBoxChangeListener listener){
    	mSelectedContactsCheckBoxChangeListener = listener;
    }
    interface OnSelectedContactsCheckBoxChangeListener{
    	void onSelectedContactsCheckBoxChange();
    }
