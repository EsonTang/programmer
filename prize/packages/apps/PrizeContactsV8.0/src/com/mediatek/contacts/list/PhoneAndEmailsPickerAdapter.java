...
public class PhoneAndEmailsPickerAdapter extends DataKindBasePickerAdapter {
    ...
    static {
        ...
        if (ContactsPortableUtils.MTK_PHONE_BOOK_SUPPORT) {
            ...
            projectionList.add(Contacts.INDICATE_PHONE_SIM);   //11
    ...
    protected void bindData(ContactListItemView view, Cursor cursor) {
        ...
        if (!cursor.isNull(getDataTypeColumnIndex())) {
            ...
            label = GlobalEnv.getAasExtension().getLabelForBindData(mContext.getResources(),
                    type, customLabel, mimeType, cursor, label);
