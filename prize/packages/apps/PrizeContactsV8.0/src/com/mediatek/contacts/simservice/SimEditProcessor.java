...
public class SimEditProcessor extends SimProcessorBase {
    ...
    public void doWork() {
        ...
        boolean hasImported = SimServiceUtils.isServiceRunning(mContext, mSubId);
    ...
    private void getRawContactDataFromIntent(int kindCount, String[] additionalNumberBuffer,
        ...
        for (int countIndex = 0; countIndex < kindCount; countIndex++) {
            ...
            if (StructuredName.CONTENT_ITEM_TYPE.equals(mimeType)) {
                ...
                {
                    // TBD: 7?
                    if ("7".equals(mSimData.get(0).getContentValues().get(countIndex)
                            .getAsString(Data.DATA2))
                            || ((mAccountType.equals(AccountTypeUtils.ACCOUNT_TYPE_USIM) ||
                                    mAccountType.equals(AccountTypeUtils.ACCOUNT_TYPE_CSIM))
                             && GlobalEnv.getAasExtension().checkAasEntry(
    ...
    private boolean checkIfSaveFail(Uri checkUri) {
        Log.d(TAG, "[checkIfSaveFail] checkUri: " + checkUri);
        if (checkUri == null) {
            return true;
        }
        String msg = checkUri.getPathSegments().get(0);
        Log.d(TAG, "[checkIfSaveFail] msg: " + msg);
        if ("error".equals(msg)) {
            ...
            if (StructuredName.CONTENT_ITEM_TYPE.equals(mimeType)) {
            ...
            } else if (Phone.CONTENT_ITEM_TYPE.equals(mimeType)) {
                {
                    if (mSimOldData.get(0).getContentValues().get(oldIndex).getAsString(
                            Data.DATA2).equals("7") || GlobalEnv.getAasExtension().checkAasEntry(
    ...
    private void insertSIMContact(final ContentResolver resolver, ContentValues values) {
        ...
        GlobalEnv.getAasExtension().updateValues(mIntent, mSubId, values);
        GlobalEnv.getSneExtension().updateValues(mIntent, mSubId, values);
        ...
        if (rawContactUri != null) {
            GlobalEnv.getSneExtension().editSimSne(mIntent, indexFromUri,
    ...
    private void editSIMContact(final ContentResolver resolver) {
        ...
        // AAS, Set all anr into usim and db. here we assume anr as
        // "anr","anr2","anr3".., and so as aas.[COMMD_FOR_AAS]
        GlobalEnv.getAasExtension().updateValues(mIntent, mSubId, updatevalues);
        GlobalEnv.getSneExtension().updateValues(mIntent, mSubId, updatevalues);
        ...
        if (SimCardUtils.SimType.SIM_TYPE_USIM_TAG.equals(mSimType)) {
            ...
            GlobalEnv.getSneExtension().editSimSne(mIntent, mIndexInSim, mSubId, mRawContactId);
        }
        showResultToastText(errorType);
    ...
    private boolean isUSIMUpdateValuesInvalid() {
        String nickName = GlobalEnv.getSneExtension().getNickName(mIntent, mSubId);
        ...
        if (!TextUtils.isEmpty(nickName) &&
                !GlobalEnv.getSneExtension().isSneNicknameValid(nickName, mSubId)) {
    ...
    private void updateAdditionalNumberToDB(final ContentResolver resolver) {
        ...
        if (GlobalEnv.getAasExtension().updateAdditionalNumberToDB(mIntent, mRawContactId)) {
    ...
    private boolean compareData() {
        ...
        Log.i(TAG, "[compareData]compareName : " + compareName + " | comparePhone : "
                ...
                + compareEmail + " | compareGroup : " + compareGroup +",sName : "
                + sName + " | mOldName : " + mOldName
