...
public class HotKnotImportVCardActivity extends Activity implements
    ...
    private static final String HOTKNOT_EXTRA_DATA =
            "com.mediatek.hotknot.extra.DATA";
    ...
    protected void onCreate(Bundle bundle) {
        ...
        mData = intent.getByteArrayExtra(HOTKNOT_EXTRA_DATA);
    ...
    public void onImportFinished(ImportRequest request, int jobId, Uri uri) {
        if (isFinishing()) {
            Log.i(TAG, "[onImportFinished] Late import -- ignoring");
            return;
        }
    ...
    public void onImportFailed(ImportRequest request) {
        if (isFinishing()) {
            Log.i(TAG, "[onImportFailed] Late import failure -- ignoring");
            return;
        }
    }
