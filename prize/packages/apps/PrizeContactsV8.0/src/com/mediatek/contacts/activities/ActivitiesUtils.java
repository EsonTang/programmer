...
public class ActivitiesUtils {
    ...
    public static boolean isDeleteingContact(final Activity activity) {
        if (MultiChoiceService.isProcessing(MultiChoiceService.TYPE_DELETE)
            ...
            Log.d(TAG, "[isDeleteingContact] delete or copy is processing ");
            activity.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(activity.getApplicationContext(), R.string.phone_book_busy,
    ...
    public static Handler initGroupHandler(final Activity activity) {
        SimGroupProcessor.Listener l = (SimGroupProcessor.Listener) activity;
        Handler handler = new Handler() {
            public void handleMessage(Message msg) {
                String content = null;
                int contentId = msg.arg1;
        ...
        SimGroupProcessor.registerListener(l, handler);
    ...
    public static void setAllFramgmentShow(View contactsUnavailableView,
        ...
        Log.i(TAG, "[setAllFramgmentShow]isNeedShow = " + isNeedShow);
        if (isNeedShow) {
            if (null != allFragment) {
                if (tabPager != null) {
    ...
    public static void customAccountsList(List<AccountWithDataSet> accountList, Activity activity) {
        if (accountList != null) {
            ...
            //M: ALPS03621760, add NONE_SIM_TYPE_ACCOUNT
            case ContactsSettingsUtils.NONE_SIM_TYPE_ACCOUNT:
