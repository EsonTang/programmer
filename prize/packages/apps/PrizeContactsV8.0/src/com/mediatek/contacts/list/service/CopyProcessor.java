...
public class CopyProcessor extends ProcessorBase {
    ...
    private void copyContactsToSim() {
        ...
        Log.d(TAG, "[copyContactsToSim] AccountName:" + account.name + ",accountType:"
                + account.type +",dstSubId:" + dstSubId);
        ...
        for (MultiChoiceRequest request : this.mRequests) {
            ...
            for (int i = 0; i < maxCount; i++) {
                ...
                Log.d(TAG, "[copyContactsToSim] simTag: " + simTag);
                simNum = null;
                simAnrNum = null;
                simEmail = null;
                ...
                if (isTargetUsim) {
                    ...
                    GlobalEnv.getAasExtension().updateValuesforCopy(dataUri,
                            dstSubId, mAccountDst.type, values);
                    GlobalEnv.getSneExtension().updateValuesforCopy(dataUri,
                            dstSubId, mAccountDst.type, values);
                    ...
                    if ("error".equals(checkUriPathSegs.get(0))) {
                        ...
                        if ("-3".equals(checkUriPathSegs.get(1))) {
                            ...
                            if (retUri != null
                                ...
                                GlobalEnv.getSneExtension().copySimSneToAccount(operationList,
    ...
    private void generateDataBuilder(Cursor dataCursor, Builder builder, String[] columnNames,
        ...
        for (int i = 1; i < columnNames.length; i++) {
            /** M:AAS [COMMD_FOR_AAS]@ { */
            if (GlobalEnv.getAasExtension().cursorColumnToBuilder(dataCursor, builder,
