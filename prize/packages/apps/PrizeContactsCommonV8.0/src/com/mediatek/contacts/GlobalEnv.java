...
public class GlobalEnv {
    ...
    private static SimAasEditor sAasExtension = null;
    private static SimSneEditor sSneExtension = null;
    ...
    public static void setApplicationContext(Context context) {
        if (sContext == null) {
            if (context != null && context instanceof Application) {
                sContext = context;
                Log.i(TAG, "[setApplicationContext]sContext: " + sContext);
            } else {
                throw new IllegalArgumentException("Only Application context can be set: "
                        + context);
            }
        } else {
        	/*prize-change for dido os8.0 -hpf-2017-9-13-start*/
            //throw new IllegalStateException("application context could be set only once");
        	 Log.i(TAG, "[setApplicationContext] sContext has been set up");
        	return;
        	/*prize-change for dido os8.0 -hpf-2017-9-13-end*/
        }
    }
    ...
    public static SimAasEditor getAasExtension() {
        return sAasExtension;
    }

    public static SimAasEditor setAasExtension() {
        if (sAasExtension == null) {
            sAasExtension = new SimAasEditor(sContext);
        } else {
    ...
    public static SimSneEditor getSneExtension() {
        return sSneExtension;
    }

    public static SimSneEditor setSneExtension() {
        if (sSneExtension == null) {
            sSneExtension = new SimSneEditor(sContext);
        } else {
