...
public class SimSneEditor {
    ...
    private boolean updateNicknameToDB(String accountType, ContentResolver resolver,
        ...
        if (SimAasSneUtils.isUsimOrCsim(accountType)) {
            ...
            Log.d(TAG, "[updateNicknameToDB]whereNickname is:" + whereNickname
            ...
            if (!TextUtils.isEmpty(updateNickname) && !TextUtils.isEmpty(oldNickname)) {
                ...
                Log.d(TAG, "[updateNickname] upNickname is " + upNickname);
                ...
                Log.d(TAG, "[updateNickname] upNicknameUri is " + upNicknameUri);
                ...
                Log.d(TAG, "[updateNickname] deleteNickname is " + deleteNickname);
            }
            return true;
        }
        return false;
    ...
    private boolean buildSneOperation(String accountType,
        ...
        if (SimAasSneUtils.isUsimOrCsim(accountType)) {
            ...
            if (!TextUtils.isEmpty(nickname)) {
                ...
                Log.d(TAG, "[buildSneOperation] nickname:" + nickname);
                ...
                Log.d(TAG, "[buildSneOperation] builder nickname:" + nickname);
                operationList.add(builder.build());
                Log.d(TAG, "[buildSneOperation] operationList:" + operationList);
                return true;
            }
        }
        return true;
    ...
    public String buildSimNickname(String accountType, ArrayList<String> nicknameArray, int subId,
        ...
        if (SimAasSneUtils.isUsimOrCsim(accountType)) {
            ...
            boolean hasSne = SlotUtils.usimHasSne(subId);
            ...
            if (!nicknameArray.isEmpty() && hasSne) {
                ...
                Log.d(TAG, "[buildSimNickname] simNickname" + simNickname);
                int len = SlotUtils.getUsimSneMaxNameLength(subId);

                Log.d(TAG, "[buildSimNickname]before Endode simNickname=" + simNickname);
                ...
                Log.d(TAG, "[buildSimNickname]after Endode simNickname=" + simNickname);
            }
            return simNickname;
        }
        return defValue;
    ...
    public void onEditorBindEditors(RawContactDelta entity, AccountType type, int subId) {
        ...
        boolean hasSne = SlotUtils.usimHasSne(subId);
        if (SimAasSneUtils.isUsim(type.accountType)) {
        ...
        if (c != null && c.moveToFirst()) {
            do {
                ...
                if (isNickname(mimeType)) {
                    ...
                    Log.d(TAG, "[fillNickNameArray] nickname is:" + nickName);
    ...
    public void copySimSneToAccount(ArrayList<ContentProviderOperation> operationList,
        ...
        Log.d(TAG, "[copySimSneToAccount] after buildSimNickname simNickname is:" + simNickname);
    ...
    public void editSimSne(Intent intent, long indexInSim, int subId, long rawContactId) {
        ...
        Log.d(TAG, "[editSimSne] Accountype from newSimData:" + accountType);
        String data = null;
        String mimeType = null;
        if (!SlotUtils.usimHasSne(subId) || !SimAasSneUtils.isUsimOrCsim(accountType)) {
        ...
        if (SimAasSneUtils.isUsimOrCsim(accountType)) {
            for (int countIndex = 0; countIndex < kindCount; countIndex++) {
                ...
                if (Nickname.CONTENT_ITEM_TYPE.equals(mimeType)) {
                    ...
                    Log.d(TAG, "[editSimSne] updated nickname is" + sNickname);
                }
            }
        }
        ...
        if (oldSimData != null) {
            ...
            for (int oldIndex = 0; oldIndex < oldCount; oldIndex++) {
                ...
                Log.d(TAG, "[getOldRawContactData]Data.MIMETYPE: " + mimeType + ",data:" + data);
                if (Nickname.CONTENT_ITEM_TYPE.equals(mimeType)) {
                    sOldNickname = data;
                    Log.d(TAG, "[editSimSne]sOldNickname=" + sOldNickname);
                }
            }
        }
        ContentResolver resolver = mContext.getContentResolver();
    ...
    public boolean isSneNicknameValid(String nickName, int subId) {
        ...
        final int maxLength = SlotUtils.getUsimSneMaxNameLength(subId);
        ...
        if (SimAasSneUtils.isUsimOrCsim(accountType)) {
            for (int countIndex = 0; countIndex < kindCount; countIndex++) {
                ...
                if (Nickname.CONTENT_ITEM_TYPE.equals(mimeType)) {
                    ...
                    Log.d(TAG, "[checkNickName] updated nickname is" + nickname);
                    break;
                }
            }
        }
        ...
        if (SimAasSneUtils.isUsimOrCsim(accountType)) {
            for (int countIndex = 0; countIndex < kindCount; countIndex++) {
                ...
                if (Nickname.CONTENT_ITEM_TYPE.equals(mimeType)) {
                    ...
                    sNickname = data;
                    sNickname = TextUtils.isEmpty(sNickname) ? "" : sNickname;
                    Log.d(TAG, "[updateValues] updated nickname is" + sNickname);
                }
            }
        }

        Log.d(TAG, "[updateValues] hasSne and sne is:" + sNickname);
    ...
    public void updateValuesforCopy(Uri sourceUri, int subId, String accountType,
        ...
        Log.d(TAG, "[updateValuesforCopy] Entry sourceUri is :"
                + sourceUri + " Entry subId is : " + subId);
