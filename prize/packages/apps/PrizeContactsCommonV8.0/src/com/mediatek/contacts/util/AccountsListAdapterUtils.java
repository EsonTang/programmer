...
public final class AccountsListAdapterUtils {
     ...
     *            Account type manager.
     */
    public static ArrayList<AccountWithDataSet> getWritableNoneSimAccount(
            AccountTypeManager accountTypeManager) {
        List<AccountWithDataSet> accountList = accountTypeManager
                .getAccounts(true);
        ArrayList<AccountWithDataSet> newAccountList = new ArrayList<AccountWithDataSet>();
        Log.i(TAG,
                "[getWritableNoneSimAccount]accountList size:"
                        + accountList.size());
        for (AccountWithDataSet account : accountList) {
            if (!(account instanceof AccountWithDataSetEx)) {
                newAccountList.add(account);
        ...
        Log.i(TAG,
                "[getWritableNoneSimAccount] return: " + newAccountList);
        return newAccountList;
    }
    
    /*prize-add-huangliemin-for-menu-on-bottom-2016-7-9-start*/
    public static void getViewForName(Context context, AccountWithDataSet account,
            AccountType accountType, TextView text2) {
        int subId = -1;
        if (account instanceof AccountWithDataSetEx) {
            subId = ((AccountWithDataSetEx) account).getSubId();
            String displayName = ((AccountWithDataSetEx) account).getDisplayName();
            ...
            if(activtedSubInfoCount >1 ) {
            	...
            	if(simindex == 1) {
            		displayName = accountType.getDisplayLabel(context)+"1 ("+displayName+")";
            	} else if(simindex == 2) {
            		displayName = accountType.getDisplayLabel(context)+"2 ("+displayName+")";
            	}
            } else {
            	displayName = ""+accountType.getDisplayLabel(context);
            }
            text2.setText(displayName);
        } else {
    ...
    /*prize-add-huangliemin-for-menu-on-bottom-2016-7-9-end*/
