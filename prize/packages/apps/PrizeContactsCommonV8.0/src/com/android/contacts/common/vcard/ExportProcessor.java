...
public class ExportProcessor extends ProcessorBase {
    ...
    private NotificationImportExportListener mListener;//prize-add for custom progressbar -hpf-2017-12-27
    public ExportProcessor(VCardService service, ExportRequest exportRequest, int jobId,
            String callingActivity,/*Activity activity,*/VCardImportExportListener listener) {//prize-change for custom progressbar -hpf-2017-12-27
        Log.i(LOG_TAG, "[ExportProcessor]new,jobId = " + jobId
                + ",callingActivity = " + callingActivity);
        mListener = (NotificationImportExportListener)listener;//prize-add for custom progressbar -hpf-2017-12-27
        mService = service;
        ...
        try {
           ...
           // composer = ExtensionManager.getInstance().getImportExportExtension()
            ...
            while (!composer.isAfterLast()) {
                ...
                mListener.onExportParsed(current, total);//prize-change for custom progressbar -hpf-2017-12-27
                current++;
            }
            ...
            if (isLocalFile(uri)) {
                ...
                /// M: ALPS02689253. export Notification show vcf file name.@{
                /* mtk no use
            ...
            mListener.onComplete();//prize-add for custom progressbar -hpf-2017-12-27
    ...
    public synchronized boolean cancel(boolean mayInterruptIfRunning) {
        ...
        mListener.doCancelExport();//prize-add for custom progressbar -hpf-2017-12-27
        if (mDone || mCanceled) {
            return false;
        }
        mCanceled = true;
