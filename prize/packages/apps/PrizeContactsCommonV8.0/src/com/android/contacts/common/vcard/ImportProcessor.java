...
public class ImportProcessor extends ProcessorBase implements VCardEntryHandler {
    ...
    public void onEntryCreated(VCardEntry entry) {
        //M:OP01 RCS load rich call screen from server when import vcard. @{
        ExtensionManager.getInstance().getRcsRichUiExtension()
                .loadRichScrnByVcardEntry(mCurrentCount == 0, entry, mService);
        /** @} */
        mCurrentCount++;
        if (mListener != null) {
    ...
    public void run() {
        ...
        try {
            ...
            if (isCancelled() && mListener != null) {
                Log.d(LOG_TAG, "ImportProcessor, run() onImportCanceled jobId is:" + mJobId);
    ...
    private void runInternal() {
        ...
        //final VCardEntryConstructor constructor =
           //     ExtensionManager.getInstance().getImportExportExtension()
          //      .getVCardEntryConstructorExt(estimatedVCardType, account, estimatedCharset);
        /// @}

        final VCardEntryConstructor constructor = new VCardEntryConstructor(estimatedVCardType,
    ...
    public synchronized boolean cancel(boolean mayInterruptIfRunning) {
        ...
        if (DEBUG) {
            Log.d(LOG_TAG, "[cancel] ImportProcessor send notification mIsRunning:" + mIsRunning);
        }
        /** @} */
        synchronized (this) {
