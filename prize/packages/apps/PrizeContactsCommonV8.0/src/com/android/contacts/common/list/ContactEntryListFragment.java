...
/**PRIZE-add-match char-shiyicheng-20150905-start*/
import com.prize.contacts.common.widget.BladeView;
...
import com.prize.contacts.common.util.PrizeAnimationHelper;//prize-add for dido os8.0-hpf-2017-8-11
...
public abstract class ContactEntryListFragment<T extends ContactEntryListAdapter>
 ...
 /**PRIZE-add-match char-shiyicheng-20150908-start*/
	public boolean mDialtactsActivityEnabled = false;
 /**PRIZE-add-match char-shiyicheng-20150908-end*/
...
//        if(isDeletionProcessorRun()) {
//        	Log.d(TAG, "[startLoading] return...");
//        	return;
//        }
//        /*prize-add-fix bug [55320] -hpf-2018-05-17-end*/
        ...
        if (mAdapter == null) {
            ...
            Log.w(TAG, "[statLoading] mAdapter is null");
            return;
        }
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        if (!mEnabled) {
            Log.i(TAG, "[onLoadFinished] finish,mEnabled:" + mEnabled);
            return;
        }
        ...
        if (ContactsPortableUtils.MTK_PHONE_BOOK_SUPPORT) {
            //Just check if the USIM PHB info map need refresh after SIM contacts loaded.
            if (mDirectoryListStatus == STATUS_NOT_LOADED) {
                SlotUtils.getActiveUsimPhbInfoMap();
            }
        }
        /** @}*/
    }
...
//        /*prize-add-fix bug [55320] -hpf-2018-05-17-end*/
    ...
    private void configureVerticalScrollbar() {
		...
		/*PRIZE-remove-fastscroll-shiyicheng-2015-9-23-start*/
            ...
            mListView.setScrollBarSize(10);
            /*PRIZE-change scrollbar style -zhangzhonghao-2015-5-20 -start*/
//            mListView.setScrollBarStyle(ListView.SCROLLBARS_OUTSIDE_OVERLAY);
            mListView.setScrollBarStyle(ListView.SCROLLBARS_OUTSIDE_INSET);
            mListView.setVerticalScrollBarEnabled(false);//prize-add-huangliemin-2016-7-29
            /*PRIZE-change scrollbar style -zhangzhonghao-2015-5-20 -end*/
	...
	/**PRIZE-add-match char-shiyicheng-20150915-start*/
		...
		SharedPreferences.Editor editor = sp.edit();
	     ...
	     String mclassname = getActivity().getLocalClassName();

	     if(mclassname.equals("DialtactsActivity")){

		mDialtactsActivityEnabled = true;
		editor.putBoolean("mprize_boolean", true);
		 editor.commit();
	      }else{

		mDialtactsActivityEnabled = false;
		editor.putBoolean("mprize_boolean", false);
		 editor.commit();
		  }
	/**PRIZE-add-match char-shiyicheng-20150908-end*/
		...
		int currentSectionItem = 0;
		 ...
		 if(getAdapter() != null && mBladeView != null){
	        mBladeView.configCharacterColorToDefault();
		    ContactsSectionIndexer indexer = (ContactsSectionIndexer)getAdapter().getIndexer();
		    if(indexer != null){		
		   	Object[] sections=indexer.getSections();
			if(mDialtactsActivityEnabled){
				currentSectionItem=indexer.getSectionForPosition(firstVisibleItem);
			}else{
				 currentSectionItem=indexer.getSectionForPosition(firstVisibleItem - 2);	
			}
		   			
			if(mSectionItem != currentSectionItem){
				mSectionItem =currentSectionItem;
				if(currentSectionItem < 0){
					mBladeView.configCharacterColor("A");
					mBladeView.reDraw();
					return ;
			    }
		    	String s=(String)sections[currentSectionItem];
				mBladeView.configCharacterColor(s);	
				mBladeView.reDraw();	    	
			}
		   }
	      }
	...
	private void configureBladeView() {
        ...
        if (mListView != null) {
            ...
            if (!hasBladeView || !UniverseUtils.BLADEVIEW_SUPPORT) {
		...
		/**PRIZE-add-match char-shiyicheng-20150915-start*/
			onScroll(mListView,  mfirstVisibleItem,  mvisibleItemCount,  mtotalItemCount);
		/**PRIZE-add-match char-shiyicheng-20150915-end*/
			
                    BladeView bladeView = (BladeView)view;
                    String[] alphabet = bladeView.getFullAlphabet(bladeView.isTw(mContext));
                    String currentSection = alphabet[item];
					
		/**PRIZE-add-match char-shiyicheng-20150906-start*/
			//if(currentSection.equals("xx")){
				//Log.i("www", "------all");
    ...
    public void setBladeViewVisibility(boolean flag) {
    	Log.d(TAG,"[setBladeViewVisibility]   flag = "+flag);
      if(mBladeView!=null) {
        if(!flag) {
          //mBladeView.setVisibility(View.GONE);
          PrizeAnimationHelper.bladeViewAnimation(mBladeView,PrizeAnimationHelper.HIDE);
          
        } else {
          //mBladeView.setVisibility(View.VISIBLE); 
          PrizeAnimationHelper.bladeViewAnimation(mBladeView,PrizeAnimationHelper.SHOW);
...
//    /*prize-add-fix bug [55320] -hpf-2018-05-17-start*/
//    //It was overrode in MultiDeletionPickerFragment;
