...
public abstract class ContactListAdapter extends ContactEntryListAdapter {
    ...
    public static class ContactQuery {
        private static final String[] CONTACT_PROJECTION_PRIMARY_INTERNAL = new String[] {
            Contacts._ID,                           // 0
        ...
        static {
            List<String> projectionList = Lists.newArrayList(CONTACT_PROJECTION_PRIMARY_INTERNAL);

            if (ContactsPortableUtils.MTK_PHONE_BOOK_SUPPORT) {
                /// M: Add some columns for Contacts extensions. @{
                projectionList.add(Contacts.INDICATE_PHONE_SIM);   //9
                projectionList.add(Contacts.INDEX_IN_SIM);         //10
                projectionList.add(Contacts.IS_SDN_CONTACT);       //11
                projectionList.add(Contacts.IS_SDN_CONTACT);       //12
                /// @}
            }

            CONTACT_PROJECTION_PRIMARY = projectionList.toArray(new String[projectionList.size()]);
        }

        private static final String[] CONTACT_PROJECTION_ALTERNATIVE_INTERNAL = new String[] {
            Contacts._ID,                           // 0
        ...
        static {
            List<String> projectionList = Lists
                    .newArrayList(CONTACT_PROJECTION_ALTERNATIVE_INTERNAL);
            ...
            CONTACT_PROJECTION_ALTERNATIVE = projectionList.toArray(new String[projectionList
                    .size()]);
        }

        private static final String[] FILTER_PROJECTION_PRIMARY_INTERNAL = new String[] {
            Contacts._ID,                           // 0
            Contacts.DISPLAY_NAME_PRIMARY,          // 1
            Contacts.CONTACT_PRESENCE,              // 2
            Contacts.CONTACT_STATUS,                // 3
        ...
        static {
            List<String> projectionList = Lists.newArrayList(FILTER_PROJECTION_PRIMARY_INTERNAL);

            if (ContactsPortableUtils.MTK_PHONE_BOOK_SUPPORT) {
                /// M: Add some columns for Contacts extensions. @{
                projectionList.add(Contacts.INDICATE_PHONE_SIM);   //12
                projectionList.add(Contacts.INDEX_IN_SIM);         //13
                projectionList.add(Contacts.IS_SDN_CONTACT);       //14
                /// @}
            }

            FILTER_PROJECTION_PRIMARY = projectionList.toArray(new String[projectionList.size()]);
        }

        private static final String[] FILTER_PROJECTION_ALTERNATIVE_INTERNAL = new String[] {
            Contacts._ID,                           // 0
            Contacts.DISPLAY_NAME_ALTERNATIVE,      // 1
            Contacts.CONTACT_PRESENCE,              // 2
            Contacts.CONTACT_STATUS,                // 3
        ...
        static {
            List<String> projectionList = Lists
                    .newArrayList(FILTER_PROJECTION_ALTERNATIVE_INTERNAL);
            ...
            FILTER_PROJECTION_ALTERNATIVE = projectionList
        ...
        /** M: New Feature Gemini @{ */
        protected static final int CONTACT_INDICATE_PHONE_SIM = 12;
        protected static final int CONTACT_INDEX_IN_SIM = 13;
        public static final int IS_SDN_CONTACT           = 14;
        /** @} */
    }
    ...
    protected void bindSectionHeaderAndDivider(ContactListItemView view, int position,
        ...
        /* prize change for dido os8.0-hpf-2017-8-21-start */
        Placement placement = getItemPlacementInSection(position);
        String HeaderText = placement.sectionHeader;
        view.setDividerVisible(mIsPeopleActivity?!placement.lastInSection:true);
        view.setIsSectionHeaderEnabled(false);
        if (/*isSectionHeaderDisplayEnabled()*/!TextUtils.isEmpty(HeaderText)) {
            //Placement placement = getItemPlacementInSection(position);
            view.setSectionHeader(/*placement.sectionHeader*/HeaderText);
            view.setIsSectionHeaderEnabled(true);
        } else {
            view.setSectionHeader(null);
            view.setIsSectionHeaderEnabled(false);
        }
        /* prize change for dido os8.0-hpf-2017-8-21-end */
    }
    
    /* prize add for dido os8.0-hpf-2018-1-2-start */
    private boolean mIsPeopleActivity = false;
    public void setIsPeopleActivity(boolean isPeopleActivity){
    ...
    public void changeCursor(int partitionIndex, Cursor cursor) {
        ...
        if (cursor != null && !cursor.isClosed() && cursor.moveToFirst()) {//prize-add " && !cursor.isClosed()"fix bug[52385]-hpf-2018-3-15
