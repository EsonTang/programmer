...
public class CallLogMultipleDeleteAdapter extends CallLogAdapter {
    ...
    public CallLogMultipleDeleteAdapter(Context context, CallFetcher callFetcher,
        ...
        super(context, callFetcher, contactInfoHelper, null, ACTIVITY_TYPE_CALL_LOG);
        /*PRIZE-Add-Prevent_Bad_Contact-wangzhong-2017_10_31-start*/
    ...
    protected void bindCallLogListViewHolder(ViewHolder viewHolder, int position) {
        ...
        log("bindCallLogListViewHolder, cursor: " + c + " count: " + getGroupSize(position));
        ...
        if (mSelectedCallLogIdList.size() > 0) {
            ...
            for (int i = 0; i < getGroupSize(position); i++) {
        ...
        views.callDetailLayout.setFocusable(false);
        views.callDetailLayout.setClickable(false);

        /*PRIZE-Change-DialerV8-wangzhong-2017_7_19-start*/
        //views.callDetailLayout.setVisibility(View.INVISIBLE);//PRIZE-add -yuandailin-2016-7-18
        views.callDetailLayout.setVisibility(View.GONE);
        /*PRIZE-Change-DialerV8-wangzhong-2017_7_19-end*/

        // hide the action views
        //views.showActions(false);//PRIZE-remove-yuandailin-2016-3-19
        ...
        if (checkBox != null) {
            ...
            /*PRIZE-Add-Prevent_Bad_Contact-wangzhong-2017_10_31-start*/
            //checkBox.setOnClickListener(mItemClickListener);
            final int pst = position;
            checkBox.setOnCheckedChangeListener(
                    new android.widget.CompoundButton.OnCheckedChangeListener() {
                        @Override
                        public void onCheckedChanged(android.widget.CompoundButton buttonView, boolean isChecked) {
                            ((CallLogMultipleDeleteActivity) mContext).updateSelectedItemsView(
                                    changeSelectedStatusToMap(pst));
                        }
                    }
            );
            /*PRIZE-Add-Prevent_Bad_Contact-wangzhong-2017_10_31-end*/
    ...
    public int selectAllItems() {
        log("selectAllItems() " + mContext);
        /** M: Fix CR ALPS3369897.
        //exit muti-window and cllick select all imediately, happen JE because mCursor
        //is not set value yet. @{ */
        if (mCursor == null) {
            log("selectAllItems() mCursor is null, return");
    ...
    public int changeSelectedStatusToMap(final int listPosition) {
        ...
        int count = getGroupSize(listPosition);
