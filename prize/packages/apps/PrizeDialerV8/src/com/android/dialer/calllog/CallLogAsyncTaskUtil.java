...
public class CallLogAsyncTaskUtil {
   ...
   /** The enumeration of {@link AsyncTask} objects used in this class. */
    ...
    private static final class CallDetailQuery {
        ...
        static final int CONFERENCE_CALL_ID_INDEX = 14;
        /// @}
        /// M: [IP Dial] For MTK IP prefix. @{
        static int IP_PREFIX_INDEX;
        /// @}
        static {
            ...
            if (DialerFeatureOptions.isVolteConfCallLogSupport()) {
                projectionList.add(CallLog.Calls.CONFERENCE_CALL_ID);
            }
            /// @}
            /// M: [IP Dial] For MTK IP prefix. @{
            if (DialerFeatureOptions.isIpPrefixSupport()) {
                projectionList.add(CallLog.Calls.IP_PREFIX );
                IP_PREFIX_INDEX = projectionList.size() -1;
            }
            /// @}
    ...
    private static PhoneCallDetails getPhoneCallDetailsForUri(Context context, Uri callUri) {
        ...
        try {
            ...
            /// M: [VoLTE ConfCallLog] For volte conference callLog @{
            if (DialerFeatureOptions.isVolteConfCallLogSupport()) {
                details.conferenceId = cursor.getLong(CallDetailQuery.CONFERENCE_CALL_ID_INDEX);
            }
            /// @}
            /** M: [IP Dial] For MTK IP prefix. @{ */
            if (DialerFeatureOptions.isIpPrefixSupport()) {
                details.ipPrefix = cursor.getString(CallDetailQuery.IP_PREFIX_INDEX);
    ...
    public static void getConferenceCallDetails(
        ...
        sAsyncTaskExecutor.submit(Tasks.GET_CALL_DETAILS,
                new ConferenceDetailsAsyncTask(context, selection.toString(),
                        confCallLogAsyncTaskListener));
    }
            ...
            try {
                ...
                if (cursor != null && cursor.moveToFirst()) {
                    ...
                    for (int i = 0; i < cursor.getCount(); i++) {
                        mDetails[i] = CallLogAsyncTaskUtil.createConferenceCallDetails(
                                mContext, cursor);
                        cursor.moveToNext();
                    }
                }
