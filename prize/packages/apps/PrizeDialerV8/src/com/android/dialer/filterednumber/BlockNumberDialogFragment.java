...
public class BlockNumberDialogFragment extends DialogFragment {
    ...
    private static final String TAG = "BlockNumberDialogFragment";
    /** M: use to do view debug-only be enabled in eng.*/
    private static final boolean VIEW_DEBUG = DialtactsActivity.DEBUG;
    ...
    public static void show(
            ...
            Callback callback) {
        /** M:if the dialog is already showing, do not show again.@{ */
        if (fragmentManager.findFragmentByTag(BLOCK_DIALOG_FRAGMENT) != null) {
            Log.i(TAG, "BLOCK_DIALOG_FRAGMENT already exists.");
            return;
        }
        /**@}*/
        ...
        // M: avoid JE due to called after onSaveInstanceState
        newFragment.showAllowingStateLoss(fragmentManager,
                BlockNumberDialogFragment.BLOCK_DIALOG_FRAGMENT);
    }
    ...
    private static BlockNumberDialogFragment newInstance(
        ...
        /** M:add some log. */
        if (VIEW_DEBUG) {
            Log.i(TAG, "newInstance, parentViewId=" + parentViewId);
        }
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        /** M:add some log. */
        if (VIEW_DEBUG) {
            Log.i(TAG, "onCreateDialog, mParentView=" + mParentView);
        }
        CharSequence title;
        String okText;
        String message;
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(getActivity())
                ...
                .setPositiveButton(okText, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int id) {
                        /** M:add some log. */
                        if (VIEW_DEBUG) {
                            Log.i(TAG, "onClick, mParentView=" + mParentView + ",isBlocked="
                                    + isBlocked);
                        }
                        if (isBlocked) {
                            unblockNumber();
    ...
    private void blockNumber() {
        ...
        final CharSequence unblockFailedMsg = getUnblockedFailedMessage();
        ...
        final OnUnblockNumberListener onUndoListener = new OnUnblockNumberListener() {
            ...
            public void onUnblockComplete(int rows, ContentValues values) {
                /** M:handle unblock failed.@{ */
                if (rows < 1) {
                    // show failed massage.
                    Snackbar.make(mParentView, unblockFailedMsg, Snackbar.LENGTH_LONG).show();
                    return;
                }
                /**@}*/
        ...
        final OnBlockNumberListener onBlockNumberListener = new OnBlockNumberListener() {
            ...
            public void onBlockComplete(final Uri uri) {
                ...
                /** M:add some log. */
                if (VIEW_DEBUG) {
                    Log.i(TAG, "blockNumber, onBlockComplete, mParentView=" + mParentView);
                }
    ...
    private void unblockNumber() {
        ...
        mHandler.unblock(new OnUnblockNumberListener() {
            ...
            public void onUnblockComplete(int rows, final ContentValues values) {
                ...
                /** M:add some log.@{ */
                if (VIEW_DEBUG) {
                    Log.i(TAG, "unblockNumber onUnblockComplete, mParentView=" + mParentView);
                }
                Snackbar.make(mParentView, message, Snackbar.LENGTH_LONG)
                        .setAction(R.string.block_number_undo, undoListener)
                        .setActionTextColor(actionTextColor)
    ...
    /**
     * M:get unblocked failed tip message
     */
    private CharSequence getUnblockedFailedMessage() {
        return ContactDisplayUtils.getTtsSpannedPhoneNumber(getResources(),
                R.string.snackbar_number_unblocked_failed, mDisplayNumber);
    }
}
