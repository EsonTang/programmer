...
public class ContactInfoHelper {
    ...
    private ContactInfo createPhoneLookupContactInfo(Cursor phoneLookupCursor, String lookupKey) {
        ...
        int indicateSimIndex = phoneLookupCursor.getColumnIndex(PhoneLookup.INDICATE_PHONE_SIM);
        int sdnContactIndex = phoneLookupCursor.getColumnIndex(PhoneLookup.IS_SDN_CONTACT);
    ...
    public static ContactInfo getContactInfo(Cursor c) {
        ContactInfo info = new ContactInfo();
        if (null == c) {
            Log.w(TAG, "ContactInfo.fromCursor(c) - c is null");
            return info;
        }
        ...
        int indicateSimIndex = c.getColumnIndex(PhoneLookup.INDICATE_PHONE_SIM);
        int sdnContactIndex = c.getColumnIndex(PhoneLookup.IS_SDN_CONTACT);
        if (indicateSimIndex != -1) {
        ...
        /// @}
        /// M: if uri is not a temp uri (temp uri is create as an encoded uri), determine UserType
        /// by contact id, or set as default USER_TYPE_CURRENT
        info.userType = (info.lookupUri != null && !UriUtils.isEncodedContactUri(info.lookupUri)) ?
                ContactsUtils.determineUserType(null, ContentUris.parseId(info.lookupUri))
