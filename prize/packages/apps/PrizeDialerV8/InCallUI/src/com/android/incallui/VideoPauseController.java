...
class VideoPauseController implements InCallStateListener, IncomingCallListener {
    ...
    private void sendRequest(Call call, boolean resume) {
        /**
         * M: [Video call]PAUSE_VIDEO capability depends on operator, but we always need
         * a flag telling video service that the video ui is in foreground or background.
         * So we introduce this setUIMode API for this.
         */
        call.getVideoCall().setUIMode(resume ? 0 : 1);

        /**
         * M: [Video call]
         * TODO: We do not support PAUSE currently.
         * We have no conception about google's expectation when send PAUSE to video provider.
         * The sip protocol doesn't support the PAUSE who won't modify the session. So the PAUSE
         * can't be transferred to the remote side.
         * We would mark this part until we were able to handle this ux behavior.
