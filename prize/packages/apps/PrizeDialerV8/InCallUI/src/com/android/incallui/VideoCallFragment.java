...
public class VideoCallFragment extends BaseFragment<VideoCallPresenter,
    ...
    private boolean mPrizeIsAnswerFragmentShow = false;//prize-add-huangpengfei-2017-3-3
    ...
    private static class VideoCallSurface implements TextureView.SurfaceTextureListener,
        ...
        public void onSurfaceTextureSizeChanged(SurfaceTexture surfaceTexture, int width,
            ...
            // Not handled
            /**
             * M: ALPS03756202, fix preview video cropped issue.
             * It maybe invoked by setPreviewSize because of TextureView's size changed.
             * TextureView will modify the SurfaceTexture buffer size and notify AP,
             * so we check it here. @{
             */
            Log.d(this, "onSurfaceTextureSizeChanged mSurfaceId=" + mSurfaceId +
                    ", w:" + width + ", h:" + height);
...
//------------------------------------------Mediatek-------------------------------------
    ...
    public void onResume() {
        ...
        //prize-change-fix bug[29269]-huangpengfei-2017-3-3
        InCallActivity activity = (InCallActivity) getActivity();
        AnswerFragment answerFragment = activity.getAnswerFragment();
        if (answerFragment != null) {
            mPrizeIsAnswerFragmentShow = answerFragment.isVisible();
        } else {
            mPrizeIsAnswerFragmentShow = false;
        }
        if (!mPrizeIsAnswerFragmentShow) {
            updateVideoUIPrize(isVideoCall);
        }
        //prize-change-fix bug[29269]-huangpengfei-2017-3-3
    }
    ...
    /*PRIZE-Add-InCallUI_VideoCall-wangzhong-2016_12_26-start*/
    @Override
    public void updateVideoUIPrize(boolean isUpdateForPrize) {
        InCallActivity activity = (InCallActivity) getActivity();
        if (null == activity) {
            return;
        }
        isVideoCall = isUpdateForPrize;
        if (isUpdateForPrize) {
            //activity.getCallCardFragment().updatePrimaryCallInfo(true);
            activity.getCallCardFragment().updateBottomButtons(true);
            activity.displayCallButtons(false);
        } else {
            //activity.getCallCardFragment().updatePrimaryCallInfo(false);
            activity.getCallCardFragment().updateBottomButtons(false);
            if (activity.isDialpadFragmentVisible()) {
                activity.displayCallButtons(false);
            } else {
