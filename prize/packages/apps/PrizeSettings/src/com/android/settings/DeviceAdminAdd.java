...
public class DeviceAdminAdd extends Activity {
    ...
    void continueRemoveAction(CharSequence msg) {
        ...
        if (msg == null) {
			...
			if(!isFinishing()){//add liup 20180103 add isFinishing
    ...
    protected void onResume() {
		...
		try{
			...
			mCurToastAppOpMode = mAppOps.checkOp(AppOpsManager.OP_TOAST_WINDOW, uid, pkg);
			 ...
			 Log.w(TAG, "not allowed to perform SYSTEM_ALERT_WINDOW");
        }
    ...
    private void addDeviceAdminPolicies(boolean showDescription) {
        if (!mAdminPoliciesInitialized) {
            ...
            for (DeviceAdminInfo.PolicyInfo pi : mDeviceAdmin.getUsedPolicies()) {
				...
				ImageView mImageView = (ImageView)view.findViewById(com.android.internal.R.id.perm_icon);
				TextView mTextViewGroup = (TextView)view.findViewById(com.android.internal.R.id.permission_group);
				TextView mTextViewList = (TextView)view.findViewById(com.android.internal.R.id.permission_list);
				if(mImageView != null){
					mImageView.setVisibility(View.GONE);
				}
				if(mTextViewGroup != null){
					mTextViewGroup.setTextSize(15);
					mTextViewGroup.setTextColor(getResources().getColor(R.color.prize_settings_textcolor));
				}
				if(mTextViewList != null){
					mTextViewList.setTextSize(13);
					mTextViewList.setTextColor(getResources().getColor(R.color.prize_settings_enabled_false_textcolor));
				}
