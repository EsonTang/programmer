...
public abstract class SettingsPreferenceFragment extends InstrumentedPreferenceFragment
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		Log.e("liup","listView11 = " + listView);
        listView.setOverScrollMode(View.OVER_SCROLL_NEVER);
		listView.addItemDecoration(new RecyclerViewDivider(getContext(), LinearLayoutManager.HORIZONTAL, 1,getContext().getResources().getColor(R.color.preference_divider,null)));
		//listView.addItemDecoration(new RecyclerViewDivider(getContext(), LinearLayoutManager.HORIZONTAL));
    ...
    public void addPreferencesFromResource(@XmlRes int preferencesResId) {
		...
		Log.e("liup","addPreferencesFromResource");
    ...
    public void setLoading(boolean loading, boolean animate) {
		/* prize-modify-by-lijimeng-for NullPointerException-20171014-start*/
		if(getView() != null){
		...
		/* prize-modify-by-lijimeng-for NullPointerException-20171014-end*/
    }
    ...
    public void registerObserverIfNeeded() {
        if (!mIsDataSetObserverRegistered) {
			...
			Log.e("liup","listView22 = " + getListView());
    ...
    public void highlightPreferenceIfNeeded() {
		Log.e("liup","isAdded() = " + isAdded());
		Log.e("liup","mPreferenceHighlighted = " + mPreferenceHighlighted);
		Log.e("liup","TextUtils.isEmpty(mPreferenceKey) = " + TextUtils.isEmpty(mPreferenceKey));
    ...
    private int canUseListViewForHighLighting(String key) {
		Log.e("liup","canUseListViewForHighLighting getListView = " + getListView());
        if (getListView() == null) {
            return -1;
        }
    ...
    public RecyclerView.LayoutManager onCreateLayoutManager() {
		Log.e("liup","onCreateLayoutManager");
    ...
    protected RecyclerView.Adapter onCreateAdapter(PreferenceScreen preferenceScreen) {
		Log.e("liup","onCreateAdapter");
        mAdapter = new HighlightablePreferenceGroupAdapter(preferenceScreen,getContext());
		Log.e("liup","mAdapter = " + mAdapter);
        return mAdapter;
    }
    ...
    private void highlightPreference(String key) {
        ...
        if (position >= 0) {
            ...
            getView().postDelayed(new Runnable() {
                ...
                public void run() {
					RecyclerView listView = getListView();
					/* prize-modify-by-lijimeng-for bugid 39800-20171016-start*/
					if (listView != null && (!(listView.getScrollState() == RecyclerView.SCROLL_STATE_IDLE) || (listView.isComputingLayout() == true))) {
						 Log.e(TAG, "Prohibit updating data!");
					}else{
						mAdapter.highlight(position);
					}
					/* prize-modify-by-lijimeng-for bugid 39800-20171016-end*/
                }
    ...
    protected void showDialog(int dialogId) {
        if (mDialogFragment != null) {
            ...
            ///M:  do not new a dialogfragment if current dialogfragment is not null
            ///else removed old dialog. @{
            if (mDialogFragment.getDialogId() == dialogId) {
                return;
            } else {
                removeDialog(mDialogFragment.getDialogId());
            }
            ///@}
        }
    ...
    protected void removeDialog(int dialogId) {
        ...
        if (mDialogFragment != null && mDialogFragment.getDialogId() == dialogId) {
            /// M: for ALPS02345076, avoid exception when fragment state check
            // mDialogFragment.dismiss();
    ...
    public static class HighlightablePreferenceGroupAdapter extends PreferenceGroupAdapter {
		private ArrayMap<CharSequence, PreferenceViewHolder> mPreferenceViewHolder;
        private int mHighlightPosition = -1;
		private Context mContext = null;
        public HighlightablePreferenceGroupAdapter(PreferenceGroup preferenceGroup,Context context) {
            super(preferenceGroup);
			mContext = context;
			mPreferenceViewHolder = new ArrayMap<CharSequence, PreferenceViewHolder>();
        }
        ...
        public void onBindViewHolder(PreferenceViewHolder holder, int position) {
			...
			if(position == 0 || preference instanceof PreferenceCategory){
				holder.itemView.setBackgroundResource(R.drawable.preferencecategory_selector);
				int top = mContext.getResources().getDimensionPixelSize(R.dimen.prize_preferencefragment_card_magintop);
				setMargins(holder,top,0);
			}else if(!(preference instanceof PreferenceCategory)){
				holder.itemView.setBackgroundResource(R.drawable.npreferencecategory_selector);
				holder.setDividerAllowedBelow(true);
				setMargins(holder,0,0);
			}
			if(!(preference instanceof PreferenceCategory)){
				if(position < getItemCount()-1){
					if(getItem(position+1) instanceof PreferenceCategory && position == 0){
						holder.itemView.setBackgroundResource(R.drawable.toponepreferencecategory_selector);
						int top = mContext.getResources().getDimensionPixelSize(R.dimen.prize_preferencefragment_card_magintop);
						holder.setDividerAllowedBelow(false);
						setMargins(holder,top,0);
					}else if(getItem(position+1) instanceof PreferenceCategory){
						holder.itemView.setBackgroundResource(R.drawable.topmorepreferencecategory_selector);
						holder.setDividerAllowedBelow(false);
						setMargins(holder,0,0);
					}else if((getItem(position+1) instanceof SpacePreference) && position == 0){
                        holder.itemView.setBackgroundResource(R.drawable.toponepreferencecategory_selector);
                        int top = mContext.getResources().getDimensionPixelSize(R.dimen.prize_preferencefragment_card_magintop);
                        holder.setDividerAllowedBelow(false);
                        setMargins(holder,top,0);
                    }else if(position >0 && getItem(position-1) instanceof SpacePreference){
                        holder.itemView.setBackgroundResource(R.drawable.preferencecategory_selector);
				...
				}else if(position == getItemCount()-1 && (preference instanceof PreferenceCategory)){
					holder.itemView.setBackgroundResource(R.drawable.toponepreferencecategory_selector);
					int top = mContext.getResources().getDimensionPixelSize(R.dimen.prize_preferencefragment_card_magintop);
					holder.setDividerAllowedBelow(false);
					setMargins(holder,top,0);
			...
			if(position == getItemCount()-1 && position == 0 && !(preference instanceof PreferenceCategory)){
					holder.setDividerAllowedBelow(false);
					holder.itemView.setBackgroundResource(R.drawable.toponepreferencecategory_selector);
					int top = mContext.getResources().getDimensionPixelSize(R.dimen.prize_preferencefragment_card_magintop);
					setMargins(holder,top,0);
				...
				holder.setDividerAllowedBelow(false);
				holder.itemView.setBackgroundResource(R.drawable.topmorepreferencecategory_selector);
				int bottom = mContext.getResources().getDimensionPixelSize(R.dimen.prize_preferencefragment_card_maginbottom);
				setMargins(holder,0,bottom);
			}
		...
		public void setMargins(PreferenceViewHolder holder,int top,int bottom){
