...
public class ConfigDialogFragment extends DialogFragment implements
    ...
    /// M: Move API to ConfigDialogFragment and change it to private
    ...
    public void onClick(DialogInterface dialogInterface, int button) {
        ...
        if (button == DialogInterface.BUTTON_POSITIVE) {
            ...
            /// M: Do not disconnect when always-on setting
            final KeyStore keyStore = KeyStore.getInstance();
            final String lockdownKey = getStringOrNull(
                    keyStore, Credentials.LOCKDOWN_VPN);
            if (lockdownKey == null) {
                disconnect(profile);
            }
            ...
            if (!dialog.isEditing() && !VpnUtils.isVpnLockdown(profile.key)) {
                try {
                    /// M: Do not connect when always-on setting
                    if (lockdownKey == null) {
                        connect(profile);
                    } else {
                        Toast.makeText(getActivity(),
                                R.string.lockdown_vpn_already_connected,
                ...
                /// M: After edit VPN, need valid it if it is always-on selection
                String selectedKey = profile.key;
                if (TextUtils.equals(selectedKey, lockdownKey)) {
                    if (!profile.isValidLockdownProfile()) {
                        keyStore.delete(Credentials.LOCKDOWN_VPN);
                        ConnectivityManager.from(getActivity()).updateLockdownVpn();
                        Toast.makeText(getActivity(), R.string.vpn_lockdown_config_error,
                                Toast.LENGTH_LONG).show();
                    }
                }
            }
