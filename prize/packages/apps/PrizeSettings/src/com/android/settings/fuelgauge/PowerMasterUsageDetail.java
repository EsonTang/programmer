...
public class PowerMasterUsageDetail extends Fragment implements Button.OnClickListener {
    ...
    public static void startBatteryDetailPage(
        ...
        args.putString(PowerMasterUsageDetail.EXTRA_TITLE, entry.name);
        args.putInt(PowerMasterUsageDetail.EXTRA_PERCENT, (int)
                ((entry.sipper.totalPowerMah * dischargeAmount / helper.getTotalPower()) + .5));
        args.putInt(PowerMasterUsageDetail.EXTRA_GAUGE, (int)
                Math.ceil(entry.sipper.totalPowerMah * 100 / helper.getMaxPower()));
        args.putLong(PowerMasterUsageDetail.EXTRA_USAGE_DURATION, helper.getStatsPeriod());
        args.putString(PowerMasterUsageDetail.EXTRA_ICON_PACKAGE, entry.defaultPackageName);
        args.putInt(PowerMasterUsageDetail.EXTRA_ICON_ID, entry.iconId);
        args.putDouble(PowerMasterUsageDetail.EXTRA_NO_COVERAGE, entry.sipper.noCoveragePercent);
        if (entry.sipper.uidObj != null) {
            args.putInt(PowerMasterUsageDetail.EXTRA_UID, entry.sipper.uidObj.getUid());
        }
        args.putSerializable(PowerMasterUsageDetail.EXTRA_DRAIN_TYPE, entry.sipper.drainType);
        args.putBoolean(PowerMasterUsageDetail.EXTRA_SHOW_LOCATION_BUTTON, showLocationButton);
            ...
            case USER:
            {
                ...
                if (entry.sipper.drainType == BatterySipper.DrainType.APP) {
                    ...
                    helper.getStats().dumpLocked(caller, printWriter, "", helper.getStatsType(),
                            uid.getUid());
                    printWriter.flush();
                    args.putString(PowerMasterUsageDetail.EXTRA_REPORT_DETAILS, result.toString());
                    ...
                    helper.getStats().dumpCheckinLocked(caller, printWriter, helper.getStatsType(),
                            uid.getUid());
                    printWriter.flush();
                    args.putString(PowerMasterUsageDetail.EXTRA_REPORT_CHECKIN_DETAILS,
                            result.toString());
        ...
        args.putIntArray(PowerMasterUsageDetail.EXTRA_DETAIL_TYPES, types);
        args.putDoubleArray(PowerMasterUsageDetail.EXTRA_DETAIL_VALUES, values);

        // This is a workaround, see b/17523189
        if (userId == UserHandle.myUserId()) {
            caller.startPreferencePanel(PowerMasterUsageDetail.class.getName(), args,
                    R.string.details_title, null, null, 0);
        } else {
            caller.startPreferencePanelAsUser(PowerMasterUsageDetail.class.getName(), args,
    ...
    private static final String TAG = "PowerMasterUsageDetail";
