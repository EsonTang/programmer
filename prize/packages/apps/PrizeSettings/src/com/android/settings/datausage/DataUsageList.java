...
public class DataUsageList extends DataUsageBase {
    ...
    private void updatePolicy(boolean refreshCycle) {
        ...
        //SUB SELECT
        if (ENG_LOAD && LOGD) {
            Log.d(TAG, "updatePolicy, mTemplate = " + mTemplate + ", polocy = " + policy);
        }
    ...
    public void bindStats(NetworkStats stats, int[] restrictedUids) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            } else if (uid == UID_REMOVED || uid == UID_TETHERING || uid == UID_VILTE) {
                collapseKey = uid;
                category = AppItem.CATEGORY_APP;
            } else {
    ...
    private static long accumulate(int collapseKey, final SparseArray<AppItem> knownItems,
        ...
        if (LOGD && collapseKey == UID_VILTE) {
            Log.d(TAG, "VILTE total = " + item.total);
        }
