...
public final class SmsDefaultDialog extends AlertActivity implements
    ...
    /// M: Showing popup prior to setting default sms app OP03 @ {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        /// M: Showing popup prior to setting default sms app OP03 @ {
        mSmsDialogExt = UtilsExt.getSMSApDialogPlugin(this);
        /// @ }
        ...
        mExt = UtilsExt.getRcsSettingsPlugin(this);
    }
    ...
    public void onClick(DialogInterface dialog, int which) {
        switch (which) {
            case BUTTON_POSITIVE:
                ...
                /// M: Set the new app for positive @ {
                mSmsDialogExt.onClick(
                  mNewSmsApplicationData != null ? mNewSmsApplicationData.mPackageName : null,
                                     SmsDefaultDialog.this, getApplicationContext(), which);
                /// @}
                setResult(RESULT_OK);
                break;
            case BUTTON_NEGATIVE:
                /// M: Reset on button cancel@ {
                mSmsDialogExt.onClick(
                  mNewSmsApplicationData != null ? mNewSmsApplicationData.mPackageName : null,
                                     SmsDefaultDialog.this, getApplicationContext(), which);
                /// @}
                ...
                if (which >= 0) {
                    ...
                    if (!adapter.isSelected(which)) {
                        ...
                        if (!TextUtils.isEmpty(packageName)) {
                            /// M: check if need user further confirm @{
                            //SmsApplication.setDefaultApplication(packageName, this);
                           if (mSmsDialogExt.onClick(packageName, SmsDefaultDialog.this,
                                           getApplicationContext(), which) == true) {
                            mExt.setDefaultSmsApplication(packageName, this);
                            /// @}
                           }
                            setResult(RESULT_OK);
                        }
    ...
    private boolean buildDialog(String packageName) {
        ...
        if (mNewSmsApplicationData != null) {
            ...
            if (oldSmsApplicationData != null) {
                ...
                /// M: Building confirmation message for OP03 @ {
                mSmsDialogExt.buildMessage(p, packageName, getIntent(),
                                  mNewSmsApplicationData.getApplicationName(this),
                                  oldSmsApplicationData.getApplicationName(this));
                /// @ }
    ...
    /// M: add for plugin OP03 @ {
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        mSmsDialogExt.onKeyDown(keyCode, event, SmsDefaultDialog.this);
    ...
    private class AppListAdapter extends BaseAdapter {
        ...
        /// M: setting different view for th SMS app in the list @ {
        ...
        public AppListAdapter() {
            ...
            /// M: setting different view for th SMS app in the list @ {
            mAppListExt = UtilsExt.getAppListPlugin(SmsDefaultDialog.this);
            /// @ }
        }
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            /// M: Different view for plugin OP03 @ {
            mAppListExt.setAppListItem(mItems.get(position).packgeName, position);
            return mAppListExt.addLayoutAppView(view, textView,
                (TextView) view.findViewById(R.id.default_label), position, item.icon, parent);
            /// @}
        }
