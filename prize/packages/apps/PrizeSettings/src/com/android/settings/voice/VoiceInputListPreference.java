...
public class VoiceInputListPreference extends AppListPreferenceWithSettings {
    ...
    public void refreshVoiceInputs() {
        ...
        for (int i = 0; i < mHelper.mAvailableRecognizerInfos.size(); ++i) {
            VoiceInputHelper.RecognizerInfo info = mHelper.mAvailableRecognizerInfos.get(i);
            /// M: Vow recognition service @ {
            // Don't show VOW Recognition Service, beacuse it is no need for VOW voice detect.
            if (!InputMethodExts.isVoiceRecognitionServiceSupport(getContext(), info.key)) {
		...
		if(entries.size() == 0){
			  entries.add(getContext().getResources().getString(R.string.prize_empty_prompt));
		}
