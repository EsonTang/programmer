...
public class ChooseAccountActivity extends SettingsPreferenceFragment {
    ...
    private boolean isAuthoritiesContainCalendar = false;
    ...
    public void onCreate(Bundle icicle) {
		...
		/*prize-modify-bugid:30261 In the calendar application click on the account can not jump-xiaoping-2017-03-16-start*/
        if (mAuthorities != null && mAuthorities.length > 0){
            for (int i = 0; i < mAuthorities.length; i++) {
                if ("com.android.calendar".equals(mAuthorities[i])){
                    isAuthoritiesContainCalendar = true;
                }
            }
        }
		/*prize-modify-bugid:30261 In the calendar application click on the account can not jump-xiaoping-2017-03-16-end*/
    ...
    private void updateAuthDescriptions() {
		...
		/*prize-modify-bugid:30261 In the calendar application click on the account can not jump-xiaoping-2017-03-16-start*/
        if (isAuthoritiesContainCalendar){
            for (int i = 0; i < mAuthDescs.length; i++) {
                mTypeToAuthDescription.put(mAuthDescs[i].type, mAuthDescs[i]);
            }
        } else {
            for (int i = 0; i < mAuthDescs.length; i++) {
            /* Modify by zhudaopeng at 2017-01-11 Start*/
            // mTypeToAuthDescription.put(mAuthDescs[i].type, mAuthDescs[i]);
            String accountType = mAuthDescs[i].type;
            if("com.tencent.mobileqq.account".equals(accountType)|| "com.tencent.mm.account".equals(accountType)){
                mTypeToAuthDescription.put(mAuthDescs[i].type, mAuthDescs[i]);
            }
            /* Modify by zhudaopeng at 2017-01-11 End*/
            }
        }
		/*prize-modify-bugid:30261 In the calendar application click on the account can not jump-xiaoping-2017-03-16-end*/
    ...
    private void onAuthDescriptionsUpdated() {
        ...
        for (int i = 0; i < mAuthDescs.length; i++) {
            ...
            /* Modify by zhudaopeng at 2017-01-11 Start*/
            /*
	        //solve bug-1226. prize-linkh-20150522
	        //The account serivce of QQ will return an error msg 'Manually add account unsupported!'
	        // I don't know the mechanism of the account serivce of QQ, so I filter it out here.
	        ...
	        */
            ...
            if (!isAuthoritiesContainCalendar){
                if("com.tencent.mobileqq.account".equals(accountType)||
                        "com.tencent.mm.account".equals(accountType)){
                    addAccountPref = true;
                } else {
                    addAccountPref = false;
                }
            }else{
				if(mAuthDescs[i].type.equals("com.android.email") || mAuthDescs[i].type.equals("com.android.exchange")
                        || mAuthDescs[i].type.equals("com.android.email.pop3") || mAuthDescs[i].type.equals("com.youan.wifipassword")
                        || mAuthDescs[i].type.equals("cn.richinfo.android.calendar.account_pe")){
					addAccountPref = false;
				}
			}
            /* Modify by zhudaopeng at 2017-01-11 End*/
        ...
        /* prize-modify-by-lijimeng-for bugid 47367-20180110-start*/
        ...
        /* prize-modify-by-lijimeng-for bugid 47367-20180110-end*/
        if (mProviderList.size() > 0) {
            ...
            Toast.makeText(getPrefContext(),R.string.prize_no_account,Toast.LENGTH_SHORT).show();
