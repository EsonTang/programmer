...
public class RunningState {
    ...
    private boolean update(Context context, ActivityManager am) {
        ...
        try {
            ...
            /// M: get swap usage @{
            long[] pswap = new long[pids.length];
            for (int i = pids.length - 1; i >= 0; i--) {
                pswap[i] = Debug.getPswap(pids[i]);
            }
            ...
            for (int i=0; i<pids.length; i++) {
                ...
                /// M: get swap usage @{
                long procSize = (long) (pss[i] + (float) (pswap[i] / zramCompressRatio));
                ...
                if (proc.mCurSeq == mSequence) {
                    ...
                    /// M: Record pswap size in bytes @{
                    backgroundProcessSwapMemory += (pswap[i] * 1024);
                    /// @}
        ...
        synchronized (mLock) {
            ...
            mBackgroundProcessSwapMemory = backgroundProcessSwapMemory;
