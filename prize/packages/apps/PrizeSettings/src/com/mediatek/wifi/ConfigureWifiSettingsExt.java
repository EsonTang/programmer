...
public class ConfigureWifiSettingsExt {
    ...
    private static final String TAG = "ConfigureWifiSettingsExt";
    ...
    private void initPreferences() {
        ...
        if (sleepPolicyPref != null) {
            mExt.setSleepPolicyPreference(sleepPolicyPref, mFragment.getResources().getStringArray(
                    (Utils.isWifiOnly(mActivity) ? R.array.wifi_sleep_policy_entries_wifi_only
                            : R.array.wifi_sleep_policy_entries)), mFragment.getResources()
                    .getStringArray(R.array.wifi_sleep_policy_values));
        }
    }
            ...
            if (ipAddress != null) {
                ...
                for (int i = 0; i < ipAddressesLength; i++) {
                    if (ipAddresses[i].indexOf(":") == NOT_FOUND_STRING) {
                        ...
                        mIpAddressPref.setSummary(ipAddresses[i] == null ? mActivity
                                .getString(R.string.status_unavailable) : ipAddresses[i]);
                        ...
                        // mActivity.getString(R.string.status_unavailable) :
                        // ipAddresses[i]);
                        if (ipAddressesLength == ONLY_ONE_IP_ADDRESS) {
