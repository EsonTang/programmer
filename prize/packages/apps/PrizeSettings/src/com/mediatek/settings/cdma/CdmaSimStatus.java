...
public class CdmaSimStatus {
    ...
    public CdmaSimStatus(SettingsPreferenceFragment fragment, SubscriptionInfo subInfo) {
        ...
        mTelephonyManager = (TelephonyManager)fragment.getContext().getSystemService(Context.TELEPHONY_SERVICE);
        mDefaultText = fragment.getString(R.string.device_info_default);
    }
    ...
    public void updateNetworkType(String key, String networktype) {
        ...
        if (CdmaUtils.isSupportCdma(mSubInfo.getSubscriptionId())
                && ("LTE".equals(networktype) || "4G".equals(networktype))
                && mServiceState != null
    ...
    private void setCdmaSignalStrength(SignalStrength signalStrength, Preference preference) {
        ...
        if ("CDMA 1x".equals(getNetworkType())) {
            ...
            preference.setSummary(mFragment.getString(R.string.sim_signal_strength,
                    signalDbm, signalAsu));
        }
    }
    ...
    private ServiceState getServiceState() {
        ...
        Log.d(TAG,"serviceState = " + serviceState);
        return serviceState;
    }
    ...
    private void setCdmaSidNid() {
        if (mPhone != null && mPhone.getPhoneType() == PhoneConstants.PHONE_TYPE_CDMA) {
            int sid = mPhone.getServiceState().getSystemId();
            int nid = mPhone.getServiceState().getNetworkId();
    ...
    private void setCellId() {
        if (mPhone != null && mPhone.getPhoneType() == PhoneConstants.PHONE_TYPE_CDMA) {
            ...
            if (cellLocation instanceof CdmaCellLocation) {
                String cellId = Integer.toString(((CdmaCellLocation) cellLocation)
                        .getBaseStationId());
    ...
    private void setSummaryText(String key, String text) {
        ...
        final Preference preference = mFragment.findPreference(key);
        if (preference != null) {
    ...
    private void removePreferenceFromScreen(String key) {
        Preference pref = mFragment.findPreference(key);
        if (pref != null) {
