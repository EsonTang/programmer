...
public class DisplaySettingsExt implements OnPreferenceClickListener, OnPreferenceChangeListener {
    ...
    private void initPreference(PreferenceScreen screen) {
		...
		mMiraVision.setWidgetLayoutResource(R.layout.dynamic_preference_widget_right_arrow);
        ...
        if (FeatureOption.MTK_MIRAVISION_SETTING_SUPPORT && !(android.os.UserHandle.myUserId() != 0
            ...
            /* PRIZE-Modify for BluLight-zhudaopeng-2017-05-12-Start */
            //screen.addPreference(mMiraVision);
            /* PRIZE-Modify for BluLight-zhudaopeng-2017-05-12-End */
        }
        // @ }
        // add for HDMI Settings @ {
        ...
        displayExt = UtilsExt.getDisplaySettingsPlugin(mContext);
        displayExt.addPreference(mContext, screen);
        displayExt.removePreference(mContext, screen);
        Log.d(TAG, "removePreference ");
        /// @ }

        /// Add for AOD Settings @{
        if (FeatureOption.MTK_AOD_SUPPORT) {
            mAodSwitchPrf = new SwitchPreference(mContext);
            mAodSwitchPrf.setKey(KEY_AOD_SETTINGS);
            mAodSwitchPrf.setTitle(R.string.aod_title);
            mAodSwitchPrf.setSummary(R.string.aod_summary);
            mAodSwitchPrf.setChecked(Settings.Secure.getInt(mContext.getContentResolver(),
                    Settings.Secure.DOZE_ENABLED, 1) != 0);
            mAodSwitchPrf.setOnPreferenceChangeListener(this);
            screen.addPreference(mAodSwitchPrf);
            // Remove doze switch
            Preference dozePreference = screen.findPreference(KEY_DOZE);
    ...
    public boolean isCustomPrefPresent() {
        return displayExt.isCustomPrefPresent();
    }

    public String[] getFontEntries(String[] defaultStr) {
         return displayExt.getFontEntries(defaultStr);
    }

    public String[] getFontEntryValues(String[] defaultStr) {
         return displayExt.getFontEntryValues(defaultStr);
    }
    ...
    private RotationPolicyListener mRotationPolicyListener = new RotationPolicyListener() {
        ...
        public void onChange() {
            if (mRotatePreference != null) {
                mRotatePreference.setValueIndex(RotationPolicy.isRotationLocked(mContext) ?
                      1 : 0);
            }
        }
    };
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (preference == mAodSwitchPrf) {
            boolean enabled = (Boolean) newValue;
            Settings.Secure.putInt(mContext.getContentResolver(), Settings.Secure.DOZE_ENABLED,
                    enabled ? 1 : 0);
        }
        return true;
    }
