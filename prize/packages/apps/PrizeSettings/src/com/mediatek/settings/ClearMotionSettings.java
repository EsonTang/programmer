...
public class ClearMotionSettings extends Activity implements
    ...
    private static final int KEY_CLEARMOTION_ENABLE_DEMO = 1600;
    ...
    private BroadcastReceiver mUpdateClearMotionStatusReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context1, Intent intent) {
            Log.d("@M_" + TAG, "mUpdateClearMotionStatusReceiver");
    ...
    private void updateClearMotionStatus() {
        if (mActionBarSwitch != null) {
            Log.d("@M_" + TAG, "updateClearMotionStatus");
    ...
    private void updateClearMotionDemo(boolean status) {
        Log.d("@M_" + TAG, "updateClearMotionDemo status: " + status);
    ...
    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
        Log.d("@M_" + TAG, "onCheckedChanged " + isChecked);
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        Log.d("@M_" + TAG, "surfaceCreated " + holder);
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
        Log.d("@M_" + TAG, "surfaceChanged " + holder);
    }

    @Override
    public void surfaceDestroyed(SurfaceHolder holder) {
        Log.d("@M_" + TAG, "surfaceDestroyed " + holder);
        releaseMediaPlayer();
    }

    @Override
    public void onPrepared(MediaPlayer mp) {
        Log.d("@M_" + TAG, "onPrepared ");
        mMediaPlayer.start();
    }

    @Override
    public void onCompletion(MediaPlayer mp) {
        Log.d("@M_" + TAG, "onCompletion ");
        // cycle play
        mp.seekTo(0);
        mp.start();
    }
    ...
    private OnErrorListener mErrorListener = new OnErrorListener() {
        ...
        public boolean onError(MediaPlayer mp, int what, int extra) {
            Log.d("@M_" + TAG, "play error: " + what);
            Log.d("@M_" + TAG, "play error: " + extra);
            releaseMediaPlayer();
            return false;
        }
    };
    ...
    private void prepareVideo() {
        try {
            if (mMediaPlayer != null) {
                ...
                Log.d("@M_" + TAG, "video path = " + afd.getFileDescriptor());
                mMediaPlayer.setDataSource(afd.getFileDescriptor(), afd.getStartOffset(), afd
                        .getLength());
                afd.close();
                int isDemoModeEnabled = mActionBarSwitch.isChecked() ? 1 : 0;
                mMediaPlayer.setParameter(KEY_CLEARMOTION_ENABLE_DEMO, isDemoModeEnabled);
                mMediaPlayer.prepare();
                // resizeSurfaceView();
                Log.d("@M_" + TAG, "mMediaPlayer prepare()");
            }
        } catch (IOException e) {
            Log.e("@M_" + TAG, "unable to open file; error: " + e.getMessage(), e);
            releaseMediaPlayer();
        } catch (IllegalStateException e) {
            Log.e("@M_" + TAG, "media player is in illegal state; error: " + e.getMessage(), e);
            releaseMediaPlayer();
        }
    }

    private void releaseMediaPlayer() {
        Log.d("@M_" + TAG, "releaseMediaPlayer");
        if (mMediaPlayer != null) {
