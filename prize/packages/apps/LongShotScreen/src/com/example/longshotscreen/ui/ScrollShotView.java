...
public class ScrollShotView {
	...
	private boolean isInUnmoveableApp() {
		boolean isHome = SuperShotUtils.isHome(mContext);
		boolean isInUnmoveableApp = SuperShotUtils.isInUnmoveableApp(mContext);
		Log.d(TAG, "[isInUnmoveableApp]   isHome = "+isHome+"   isInUnmoveableApp = "+isInUnmoveableApp);
		if (isHome || isInUnmoveableApp) {
			return true;
		}
		return false;
	}
