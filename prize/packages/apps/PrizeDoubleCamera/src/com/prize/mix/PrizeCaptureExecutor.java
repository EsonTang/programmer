...
import com.mediatek.camera.mode.pip.pipwrapping.JpegHeaderWrapper;
import com.mediatek.camera.mode.pip.pipwrapping.AnimationRect;

public class PrizeCaptureExecutor {
    private static final String TAG = PrizeCaptureExecutor.class.getSimpleName();
    private static final int NUM_FOR_SINGLE_CAPTURE = 1;
    private static final int MAX_PENDING_CAPTURE_COUNT = 1;
    private final Context mContext;
    private final PrizeRendererManager mRendererManager;
    ...
    public interface ImageCallback {
        ...
        PrizeRendererManager getRendererManager();
        AnimationRect getPreviewAnimationRect();
    }

    public PrizeCaptureExecutor(Context context,
            PrizeRendererManager rendererManager, ImageCallback callback) {
        mContext = context;
        mRendererManager = rendererManager;
        mImageCallback = callback;
        mJpegHeaderWrapper = new JpegHeaderWrapper();
        mImageReceiveThread = new HandlerThread("wb-Image-Receive");
    ...
    public void setUpCapture(Size topJpegSize) {
        Log.i(TAG, "setUpCapture" + "released:" + mReleased);
        if (mReleased) {
            return;
        }
        CaptureInitRunnable r = new CaptureInitRunnable(topJpegSize);
        try {
    ...
    private class CaptureInitRunnable implements Runnable {
        ...
        public CaptureInitRunnable(Size topSz) {
            mTopSz = topSz;
        }
        ...
        public void run() {
            ...
            if (JpegEncoder.isHwEncoderSupported(mContext)) {
                Surface jpegOutputSurface = setUpImageReader(mTopSz.getWidth(),
                		mTopSz.getHeight(), ImageFormat.JPEG);
                ...
                jpegInputSurface = mJpegEncoder.configInputSurface(jpegOutputSurface,
                		mTopSz.getWidth(), mTopSz.getHeight(), pixelFormat);
            } else {
            ...
            mRendererManager.setCaptureSize(mTopSz);
    ...
    private class JpegProcessingRunnable implements Runnable {
        ...
        public void run() {
            ...
            mJpegDecoder = JpegDecoder.newInstance(mRendererManager.getTopCapSt());
            mRendererManager.setJpegRotation(Exif.getOrientation(mJpegData));
            if (mIsBottom) {
                try {
