...
public class RuleContainer {
    ...
    public void addRule() {
       ...
       //        SettingConstants.KEY_CAMERA_ZSD, hdrzsdRule);
        ...
        SDStorageRule sdStorageRule = new SDStorageRule();
        mISettingCtrl.addRule(SettingConstants.KEY_SD_MOUNT,
                SettingConstants.KEY_STORAGE_PATH, sdStorageRule);
	...
	/*prize add Storage path selection function wanzhijuan 2016-10-19 start*/
    private class SDStorageRule implements ISettingRule {

        public SDStorageRule() {

        }
        @Override
        public void execute() {
            SettingItem zsdSetting = mISettingCtrl.getSetting(SettingConstants.KEY_STORAGE_PATH);

            if (!Storage.isHaveExternalSDCard()) {
                ListPreference pref = zsdSetting.getListPreference();
                if (pref != null) {
                    Record record = zsdSetting.new Record(zsdSetting.getValue(),
                            zsdSetting.getValue());
                    zsdSetting.addOverrideRecord(SettingConstants.KEY_SD_MOUNT, record);
                    pref.setOverrideValue(pref.getDefaultValue());
                    pref.setEnabled(false);
                }
            } else {
                Record record  = zsdSetting.getOverrideRecord(SettingConstants.KEY_SD_MOUNT);
                if (record == null) {
                    return;
                }
                zsdSetting.removeOverrideRecord(SettingConstants.KEY_SD_MOUNT);
                int overrideCount = zsdSetting.getOverrideCount();
                if (overrideCount > 0) {
                    Record topRecord = zsdSetting.getTopOverrideRecord();
                    if (topRecord != null) {
