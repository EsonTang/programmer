...
import com.android.prize.PanoramaCameraPrieviewSizeRule;
...
public class PanoramaMode extends CameraMode {
    ...
    private PanoramaCameraPrieviewSizeRule mPanoramaCameraPrieviewSizeRule;
    ...
    public PanoramaMode(ICameraContext cameraContext) {
        ...
        setPanoramaCameraPreviewSizeRule();
    }
    ...
    public boolean execute(ActionType type, Object... arg) {
        ...
        case ACTION_ON_SINGLE_TAP_UP:
            ...
            break;
        case ACTION_STOP_PANORAMA_MODE_BY_VOLUME_KEY:	//prize-public-standard:add volume key stop capture function-20160624-pengcancan-start
        	if (getModeState() == ModeState.STATE_CAPTURING) {
				if (mCurrentNum > 1) {
					stopCapture(true);
				}else {
					if (!mIsInStopProcess) {
            ...
            break;//prize-public-standard:add volume key stop capture function-20160624-pengcancan-end
        default:
            returnValue = false;
        }
    ...
    private void stopPreview() {
        ...
        ((CameraActivity)mActivity).getCameraDevice().cancelAutoFocus();
        ((CameraActivity)mActivity).getCameraDevice().stopPreview();
    }
	...
	private boolean capture() {
        ...
        mICameraAppUi.setViewState(ViewState.VIEW_STATE_PANORAMA_CAPTURE);
    ...
    private class PanoramaHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            case MSG_SHOW_INFO:
                ...
                mICameraAppUi.showInfo(showInfoStr,5 * 1000,(int)mActivity.getResources().getDimension(R.dimen.info_bottom));
                break;
    ...
    private void onPictureTaken(byte[] jpegData) {
        ...
        if (mCurrentNum == NUM_AUTORAMA_CAPTURE || mIsMerging) {
        	...
        	/*prize Panorama mode mute wanzhijuan 2016-5-30 start*/
            if (mCameraSound != null && isCameraUnMute()) {
                mCameraSound.play(MediaActionSound.SHUTTER_CLICK);
            }
            /*prize Panorama mode mute wanzhijuan 2016-5-30 end*/
    ...
    private void showGuideString(int step) {
        ...
        // show current guide
        if (guideId != 0) {
            mICameraAppUi.showInfo(mActivity.getString(guideId), SHOW_INFO_LENGTH_LONG,(int)mActivity.getResources().getDimension(R.dimen.info_bottom));
        }
    }
    
    /*prize Front camera touch screen camera wanzhijuan 2016-5-30 start*/
    protected void checkFrontTouchShutter() {
    	if (SettingTool.isTouchShutter(mISettingCtrl)) {
    		ImageView shutterIm = mICameraAppUi.getPhotoShutter();
    		Log.d(TAG, "checkFrontTouchShutter() shutterIm=" + shutterIm);
    		if (shutterIm != null && shutterIm.isEnabled()) {
    			shutterIm.performClick();
    		}
    	}
    }
    /*prize Front camera touch screen camera wanzhijuan 2016-5-30 end*/
    
    private void setPanoramaCameraPreviewSizeRule() {
    	mPanoramaCameraPrieviewSizeRule = new PanoramaCameraPrieviewSizeRule(mICameraContext);
    	mISettingCtrl.addRule(SettingConstants.KEY_PANORAMA, SettingConstants.KEY_PICTURE_RATIO,mPanoramaCameraPrieviewSizeRule);
    	mISettingCtrl.addRule(SettingConstants.KEY_PANORAMA, SettingConstants.KEY_PICTURE_SIZE,null);
    	mPanoramaCameraPrieviewSizeRule.addLimitation("on", null, null);
    }
}
