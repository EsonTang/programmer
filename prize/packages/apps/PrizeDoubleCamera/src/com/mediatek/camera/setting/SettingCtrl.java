...
import com.android.camera.CamApplicaion;
...
public class SettingCtrl implements ISettingCtrl {
    ...
    @Override
    public void initializeSettings(int preferenceRes, int frontPreferenceRes, SharedPreferences globalPref,
        ...
        mSettingGenerator.setFrontPreferenceRes(frontPreferenceRes);
        ...
        if (!isExecutedByRule) {
            if (SettingConstants.KEY_PICTURE_RATIO.equals(key)) {
                ...
                case SettingConstants.BOTH_IN_PARAMETER_AND_PREFERENCE:
                    if (setting.isEnable()) {
                    	if (!(SettingConstants.KEY_FLASH.equals(key)
                    			&& CamApplicaion.getInstance()!= null
                                && !CamApplicaion.getInstance().isCameraSwitchingDone()
                                && CamApplicaion.getInstance().getSwitchToCamera() == 1
                                && currentCameraId == 0)){
	                        ParametersHelper
    ...
    public ISettingRule getRule(String conditionKey, String resultKey) {
        Log.i(TAG, "[getRule], conditionKey:" + conditionKey + ", " + "resultKey:" + resultKey);
        int condtionSettingId = SettingConstants.getSettingId(conditionKey);
        int resultSettingId = SettingConstants.getSettingId(resultKey);
        return mRuleMatrix[condtionSettingId][resultSettingId];
    }
    ...
    public String getSettingValue(String key) {
        ...
        /*prize-add-bugid:52670 camera crash on facebeautymode-xiaoping-20180409-start*/
        if (settingItem != null) {
        	value = settingItem.getValue();
		}
        /*prize-add-bugid:52670 camera crash on facebeautymode-xiaoping-20180409-end*/
        if (value == null) {
    ...
    public void updateSetting(SharedPreferences localPref) {
        ...
        for (int i = 0; i < SettingConstants.SETTING_COUNT; i++) {
            ...
            /*prize-add-bugid:56917 reset the value of face_beauty_key at camera exits before and after switching-xiaoping-20180509-start*/
            if (mContext instanceof CameraActivity && ((CameraActivity)mContext).getCurrentMode() == ModePicker.MODE_PHOTO && SettingConstants.KEY_FACE_BEAUTY.equals(key) && "on".equals(value)) {
                Log.i(TAG,"current mode  is photomode ,but face_beauty_key value is on ,we nedd reset the value off");
                value = "off";
            }
            /*prize-add-bugid:56917 reset the value of face_beauty_key at camera exits before and after switching-xiaoping-20180509-end*/
            Log.i(TAG, "[updateSetting], key:" + key + ", value:" + value + "" + ", lastValue:"
                    + lastValue);
            /*prize-xuchunming-20161102-bugid:23858-start*/
            if (settingItem.isEnable() && value != null && isNeedInitSettingItem(key)) {
            /*prize-xuchunming-20161102-bugid:23858-end*/
    ...
    private void createRuleFromResctrictionMatrix() {
        ...
        for (int column = 0; column < columnLen; column++) {
            ...
            for (int row = 0; row < rowLen; row++) {
                ...
                Log.d(TAG, "row:"+row+",column:"+column+",conditionKey:"+conditionKey);
    ...
    /*prize-xuchunming-20161102-bugid:23858-start*/
    private boolean isNeedInitSettingItem(String key){
    	if(key.equals(SettingConstants.KEY_COLOR_EFFECT) ||
    	   key.equals(SettingConstants.KEY_FN_FB_SMOOTHING_LEVEL) ||
    	   key.equals(SettingConstants.KEY_FN_FB_SLIMMIMG_LEVEL) ||
    	   key.equals(SettingConstants.KEY_FN_FB_TONING_LEVEL) ||
    	   key.equals(SettingConstants.KEY_FN_FB_EYECIRCLES_LEVEL) ||
    	   key.equals(SettingConstants.KEY_FN_FB_EYEENLARGE_LEVEL) ||
    	   key.equals(SettingConstants.KEY_FN_SFB_AUTO_CONFIG)){
    	   Log.d(TAG, "not NeedInitSettingItem key = " + key);
    	   return false;
    	}
    	return true;
    }
