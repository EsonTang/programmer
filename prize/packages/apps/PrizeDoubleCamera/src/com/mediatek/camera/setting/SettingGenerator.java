...
public class SettingGenerator {
    ...
    public static final int QUALITY_PRIZE_4K       = 119;
    private static final String VIDEO_QUALITY_4K = Integer
            .toString(QUALITY_PRIZE_4K);
    ...
    private int mFrongPreferenceRes = 0;
    ...
    public void createSettings(int preferenceRes) {
        ...
        if(currentCameraId == 0) {
        	group = (PreferenceGroup) mInflater.inflate(preferenceRes);
        }else {
        	group = (PreferenceGroup) mInflater.inflate(mFrongPreferenceRes);
        }
    ...
    public void updatePreferences() {
        ...
        if (preferences == null) {
        	...
        	/*prize-xuchunming-20180523-add shortcuts-end*/
        		...
        		group = (PreferenceGroup) mInflater.inflate(mPreferenceRes);
        	}
            mPreferencesGroupMap.put(currentCameraId, group);
            createPreferences(group, currentCameraId);
        } else {
            ...
            SettingItem zsdSetting = getSettingItem(SettingConstants.ROW_SETTING_ZSD, mCameraId);
            if (zsdSetting != null) {
            	zsdSetting.setLastValue(null);
            }
            
            SettingItem antiBandingSetting = getSettingItem(SettingConstants.ROW_SETTING_ANTI_FLICKER, mCameraId);
            ...
            SettingItem prizeFlipSetting = getSettingItem(SettingConstants.ROW_SETTING_PRIZE_FLIP, mCameraId);
            if (prizeFlipSetting != null) {
            	prizeFlipSetting.setLastValue(null);
            }
            /*prize-xuchunming-20170420-add frontcamera mirror switch-end*/
            
            SettingItem prizeKooBeeWaterSetting = getSettingItem(SettingConstants.ROW_SETTING_KOOBEE_WATERMARK, mCameraId);
            if (prizeKooBeeWaterSetting != null) {
            	prizeKooBeeWaterSetting.setLastValue(null);
            }
            /*prize-xuchunming-201804026-add spotlight-start*/
            SettingItem spotLightDirSetting = getSettingItem(SettingConstants.ROW_SETTING_FN_FB_SPOTLIGHT_DIR, mCameraId);
            if (spotLightDirSetting != null) {
            	spotLightDirSetting.setLastValue(null);
            }
            
            SettingItem spotLightLevelSetting = getSettingItem(SettingConstants.ROW_SETTING_FN_SFB_SPOTLIGHT_LEVEL, mCameraId);
            if (spotLightLevelSetting != null) {
            	spotLightLevelSetting.setLastValue(null);
            }
            
            SettingItem spot3DlookLevelSetting = getSettingItem(SettingConstants.ROW_SETTING_FN_SFB_3DLOOK_LEVEL, mCameraId);
            if (spot3DlookLevelSetting != null) {
            	spot3DlookLevelSetting.setLastValue(null);
            }
    ...
    public void restoreSetting(int cameraId) {
        ...
        /*prize-xuchunming-20160704-bugid:17366-start*/
        SettingItem pictureSizeSetting = getSettingItem(SettingConstants.KEY_PICTURE_SIZE);
        ListPreference pictureSizePref = pictureSizeSetting.getListPreference();
        if (pictureSizePref != null) {
            String pictureSize = pictureSizePref.getValue();
            Log.d(TAG,"pictureSize: "+pictureSize);
            pictureSizeSetting.setValue(pictureSize);
            pictureSizeSetting.setDefaultValue(pictureSize);
        }
        /*prize-xuchunming-20160704-bugid:17366-end*/
    ...
    private void createPreferences(PreferenceGroup group, int cameraId) {
        ...
        if (preferences == null) {
            ...
            for (int settingId = 0; settingId < SettingConstants.SETTING_COUNT; settingId++) {
                ...
                /*prize-add-bugid:52467 unexpected camera exit fails to clear user settings-xiaoping-20180326-start*/
                if (SettingConstants.KEY_SELF_TIMER.equals(key)) {
                	Log.i(TAG, "getValue: "+preference.getValue()+",getDefaultValue: "+preference.getDefaultValue());	
    ...
    private boolean filterPreference(final ListPreference preference) {
        ...
        case SettingConstants.ROW_SETTING_KOOBEE_WATERMARK:
        	if(FeatureSwitcher.isBrandWaterSupported() == false) {
        		removePreference = true;
        	}
            break;
        ...
        case SettingConstants.ROW_SETTING_CONTINUOUS_NUM:
        	if (ParametersHelper.getParametersSupportedValues(parameters,
                    SettingConstants.KEY_CAPTURE_MODE) == null) {
        		removePreference = true;
        		break;
        	}
            if (mICameraContext.getFeatureConfig().isLowRamOptSupport() || 
        ...
        case SettingConstants.ROW_SETTING_PICTURE_SIZE:// camera special case
            ...
            List<String> supportedForRatio = SettingTool.buildAllSupportedPictureSize(mContext,
                    parameters);
        ...
        case SettingConstants.ROW_SETTING_GRIDE:
            break;
            
        case SettingConstants.ROW_SETTING_TOUCHSHUTTER:
            break;
        /*PRIZE-modify Photo size function-wanzhijuan-2016-05-03-end*/
        ...
        /*prize-xuchunming-20171123-arcsoft switch-start*/
        case SettingConstants.ROW_SETTING_ARC_PICSELFIE_ENABLE:
    ...
    private ArrayList<String> getMTKSupportedVideoQuality() {
        ...
        if (CamcorderProfile.hasProfile(cameraId, QUALITY_PRIZE_4K)) {
            supported.add(VIDEO_QUALITY_4K);
        }
    ...
    public void setFrontPreferenceRes(int frontPreferenceRes) {
        mFrongPreferenceRes = frontPreferenceRes;
    }
