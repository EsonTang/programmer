...
//prize fengshangdong add for video continus AF function 
...
public class VideoMode extends CameraMode implements MediaRecorder.OnErrorListener,
    ...
    protected static final int SHORT_VIDEO = 16 ;
    ...
    private static final int SLOW_MOTION_VIDEO_TAG_FOR_240FPS = 8;
    private static final int SLOW_MOTION_240FPS = 240;
    ...
    /*prize-xuchunming-20160903-bugid:19643-start*/
    private String mFlashValue = "off";
    /*prize-xuchunming-20160903-bugid:19643-end*/
    private ISettingRule mFlashRule;
    private static final long SHORT_VIDEO_DURATION = 11000;
    ...
    public boolean execute(ActionType type, Object... arg) {
        ...
        case ACTION_ON_CAMERA_CLOSE:
            /*prize-modify-bugid:56981 Animation is not hidden on click menu key -xiaoping-start*/
            if (isFromShortPreVideoMode() && isAnimationShow()) {
                stopAnimation();
            }
            /*prize-modify-bugid:56981 Animation is not hidden on click menu key -xiaoping-end*/
            onCameraClose();
        ...
        case ACTION_ON_CONFIGURATION_CHANGED:
            ...
            break;
        case ACTION_NAVIGAT_CHANGE:
        	Assert.assertTrue(arg.length == 1);
            if(mRecordingView != null){
            	mRecordingView.onChangeNavigationBar(((Boolean)arg[0]));
            }
            break;
        default:
            return false;
    ...
    protected void doOnCameraParameterReady(boolean isNeedStartPreview) {
        ...
        keepScreenOnAwhile();
        /*prize-xuchunming-2018040417-bugid:53035-start*/
        if (isNeedStartPreview && mStoppingAction != STOP_SHOW_ALERT) {
        /*prize-xuchunming-2018040417-bugid:53035-end*/
            startPreview();
        } else {
    ...
    protected void doStartPreview() {
        ...
        if (mICameraDevice != null) {
			...
			mICameraDevice.setAutoFocusMoveCallback(mAutoFocusMoveCallback); //prize fengshangdong add for video continus AF function
        }
        if (mIFocusManager != null) {
    ...
    protected boolean onVideoShutterButtonClick() {
        ...
        /*prize-modify-bugid:58900 Focus state does not affect video save-xiaoping-20180523-start*/
/*        if(((CameraActivity)mActivity).isCameraIdle() == false) {
        	 Log.i(TAG, "[onShutterButtonClick], isCameraIdle == false , return");
             return false;
        }*/
        /*prize-modify-bugid:58900 Focus state does not affect video save-xiaoping-20180523-end*/
        if (mIsMediaRecorderRecording) {
            /*prize-add-10s short video -xiaoping-20180505-start*/
            if (isFromShortPreVideoMode() && isAnimationShow()) {
                Log.i(TAG,"hasMessages true");
                mHandler.removeMessages(SHORT_VIDEO);
                stopAnimation();
            }
            ...
            startVideoRecording();
            /*prize-add-10s short video -xiaoping-20180505-start*/
            if (isFromShortPreVideoMode()) {
                if (mHandler.hasMessages(SHORT_VIDEO)) {
                    mHandler.removeMessages(SHORT_VIDEO);
                }
                mHandler.sendEmptyMessageDelayed(SHORT_VIDEO,SHORT_VIDEO_DURATION);
                startAnimation();
            }
            /*prize-add-10s short video -xiaoping-20180505-end*/
        }
    ...
    protected boolean startVideoRecording() {
        ...
        /*prize-xuchunming-20180424-bugid:56370-start*/
        Settings.System.putInt(mActivity.getContentResolver(), "camera_capturing", 0); //for key tasklist
        /*prize-xuchunming-20180424-bugid:56370-end*/
        ...
        if (!startRecording()) {
            ...
            /*prize-xuchunming-20180408-bugid:54719-start*/
            mICameraAppUi.setVideoShutterMask(false); 
            /*prize-xuchunming-20180408-bugid:54719-end*/
            return false;
        }
        mIsMediaRecoderRecordingPaused = false;
    ...
    protected void initializeNormalRecorder() {
        ...
        /*prize-add-bugid:48363 continuous shooting video there is a probability of stuck-xiaoping-start*/
        mMediaRecorder = getInstance();
        /*prize-add-bugid:48363 continuous shooting video there is a probability of stuck-xiaoping-end*/
    ...
    protected void stopVideoRecordingAsync(boolean needShowSavingUi) {
        ...
        /*prize-modify-bugid:63802  animation does not stop automatically when sd card is almost full-xiaoping-20180630-start*/
        if (isFromShortPreVideoMode() && mIsMediaRecorderRecording && needShowSavingUi && isAnimationShow()) {
        	Log.i(TAG, "current mode is shortvideo , stop animation before saving video");
			mHandler.removeMessages(SHORT_VIDEO);
			stopAnimation();
		}
        /*prize-modify-bugid:63802  animation does not stop automatically when sd card is almost full-xiaoping-20180630-end*/
    ...
    protected void addVideoToMediaStore() {
        if (mVideoFileDescriptor == null) {
            ...
            if ("on".equals(mISettingCtrl.getSettingValue(SettingConstants.KEY_SLOW_MOTION))) {
                if (mProfile.videoFrameRate == SLOW_MOTION_240FPS) {
                    videoTag = SLOW_MOTION_VIDEO_TAG_FOR_240FPS;
                } else {
    ...
    protected Runnable mVideoSavedRunnable = new Runnable() {
        public void run() {
            Log.i(TAG, "[mVideoSavedRunnable.run()] begin " + ", mStoppingAction = "
                    + mStoppingAction);
            if(mIModuleCtrl.isVideoCaptureIntent() == false){
    ...
    protected void backToLastModeIfNeed() {
        ...
        if (mIModuleCtrl.isVideoCaptureIntent()) {
            ...
            if(mStoppingAction == STOP_FAIL){
            	mStoppingAction = STOP_NORMAL;
            	mICameraAppUi.showInfo(mActivity.getResources().getString(R.string.video_stop_error));
            	updateViewState(false);
            }else{
            	/*prize-xuchunming-20160903-bugid:19643-start*/
                mFlashValue = ParametersHelper.getParametersValue(mParameters,SettingConstants.KEY_FLASH);
                ((CameraActivity)mActivity).getISettingCtrl().onSettingChanged(SettingConstants.KEY_FLASH, "off");
    ...
    protected boolean takeASnapshot() {
        ...
        mHandler.sendEmptyMessageDelayed(UPDATE_SNAP_UI, 20);
	...
	/*prize  fengshangdong for video continus AF
	*/
    ...
    private void startPreview() {
        ...
        /*prize-xuchunming-20180427-bugid:53035-start*/
        mStoppingAction = STOP_NORMAL;
        /*prize-xuchunming-20180427-bugid:53035-end*/
        ...
        public void onClick(View v) {
            ...
            if (mIsMediaRecoderRecordingPaused) {
                ...
                try {
                    ...
                    Log.i(TAG,"mRecordingStartTime: "+mRecordingStartTime+",mRecordingPausedDuration: "+mRecordingPausedDuration);
                    /*prize-add-10s short video -xiaoping-20180505-start*/
                    if (isFromShortPreVideoMode()) {
                        mHandler.sendEmptyMessageDelayed(SHORT_VIDEO,SHORT_VIDEO_DURATION-mRecordingPausedDuration-700);
                        startAnimation();
                    }
                    /*prize-add-10s short video -xiaoping-20180505-end*/
                    mRecordingPausedDuration = 0;
                ...
                pauseVideoRecording();
                /*prize-add-10s short video -xiaoping-20180505-start*/
                if (isFromShortPreVideoMode() && mHandler.hasMessages(SHORT_VIDEO)) {
                    pauseAnimation();
                    mHandler.removeMessages(SHORT_VIDEO);
                }
                /*prize-add-10s short video -xiaoping-20180505-end*/
    ...
    private OnClickListener mRetakeListener = new OnClickListener() {
        public void onClick(View v) {
            ...
            /*prize-xuchunming-20160220-bugid:24529-start*/
            /*prize-for bugid: 33676-liufan-2017-05-15-start*/
            ListPreference mListPreference = 
                ((CameraActivity)mActivity).getISettingCtrl().getListPreference(SettingConstants.KEY_FLASH);
            if(mListPreference != null){
                mListPreference.setOverrideValue(mFlashValue);
                mListPreference.setValue(mFlashValue);
                mListPreference.setEnabled(true);
            }
            /*prize-for bugid: 33676-liufan-2017-05-15-end*/
            /*prize-xuchunming-20160220-bugid:24529-end*/
            deleteCurrentVideo();
            mICameraAppUi.hideReview();
            mICameraAppUi.setVideoShutterEnabled(true);
            startPreview();
            /*prize-xuchunming-20160903-bugid:19643-start*/
            ((CameraActivity)mActivity).getISettingCtrl().onSettingChanged(SettingConstants.KEY_FLASH, mFlashValue);
            ((CameraActivity)mActivity).applyParametersToServer();
            /*prize-xuchunming-20160903-bugid:19643-end*/
            mICameraAppUi.switchShutterType(ShutterButtonType.SHUTTER_TYPE_VIDEO);
	...
	@Override
	public void resume() {
		 if (mICameraAppUi.getCameraView(CommonUiType.REVIEW).isShowing() == true && ((CameraActivity)mActivity).getISettingCtrl().getListPreference(SettingConstants.KEY_FLASH) != null) {
			 ((CameraActivity)mActivity).getISettingCtrl().getListPreference(SettingConstants.KEY_FLASH).setValue(mParameters.FLASH_MODE_OFF);
         }
	...
	/*prize-add-10s short video -xiaoping-20180505-start*/
	public boolean isFromShortPreVideoMode() {
        if (mActivity != null && mActivity instanceof CameraActivity) {
            return ((CameraActivity)mActivity).getPrevMode() == ModePicker.MODE_SHORT_PREVIDEO;
        }
        return false;
    }
    ...
    public void startAnimation() {
        if (((CameraAppUiImpl)((CameraActivity)mActivity).getCameraAppUI()) != null) {
            ((CameraAppUiImpl)((CameraActivity)mActivity).getCameraAppUI()).getShutterManager().loadShortVideoAnimation();
        }
    }

    public void stopAnimation() {
        if (((CameraAppUiImpl)((CameraActivity)mActivity).getCameraAppUI()) != null) {
            ((CameraAppUiImpl)((CameraActivity)mActivity).getCameraAppUI()).getShutterManager().stopShortVideoAnimation();
        }
    }

    public void pauseAnimation() {
        if (((CameraAppUiImpl)((CameraActivity)mActivity).getCameraAppUI()) != null) {
            ((CameraAppUiImpl)((CameraActivity)mActivity).getCameraAppUI()).getShutterManager().pauseShortVideoAnimation();
        }
    }
    
    public boolean isAnimationShow() {
        if (((CameraAppUiImpl)((CameraActivity)mActivity).getCameraAppUI()) != null) {
            return  ((CameraAppUiImpl)((CameraActivity)mActivity).getCameraAppUI()).getShutterManager().isShortVideoAnimationVisible();
        }
        return false;
    }
    /*prize-add-10s short video -xiaoping-20180505-end*/
