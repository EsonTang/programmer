...
public class Thumbnail {
  ...
  /**
     ...
     * @param orientation the image orientation, get from Image info.
     * @param inSampleSize the image size, get from the Image info.
     * @param uri the save image uri.
     * @param filePath the image path.
     * @return the thumbnail the image thumbnail.
     */
    public static Thumbnail createThumbnail(byte[] jpeg, int orientation,
            int inSampleSize, Uri uri, String filePath) {
        ...
        return createThumbnail(uri, bitmap, orientation, filePath);
    }
	
    public static Thumbnail createThumbnail(byte[] jpeg, int orientation, int size, Uri uri) {
        // Create the thumbnail.
    	if(jpeg == null) {
    		Log.d(TAG, "Thumbnail createThumbnail jpeg == null");
    		return null;
    	}
    	BitmapFactory.Options options = new BitmapFactory.Options();
    ...
    public static Thumbnail createThumbnail(Bitmap jpeg, int orientation, float size, Uri uri) {
        // Create the thumbnail.
    	if(jpeg == null) {
    		Log.d(TAG, "Thumbnail createThumbnail  Bitmap jpeg == null");
    		return null;
    	}
    	 int width = jpeg.getWidth();
         int height = jpeg.getHeight();
         Log.d(TAG, "Thumbnail createThumbnail source width = "+width+", height = "+height);
         Matrix matrix = new Matrix();
         matrix.preScale(size, size);
         Bitmap bitmap = Bitmap.createBitmap(jpeg, 0, 0, width, height, matrix, false);
         jpeg.recycle();
         Log.d(TAG, "Thumbnail createThumbnail dest width = "+bitmap.getWidth()+", height = "+bitmap.getHeight());
        return createThumbnail(uri, bitmap, orientation);
    }
     ...
     * @param bitmap the bitmap data for create thumbnail.
    ...
    public static Thumbnail createThumbnail(Uri uri, Bitmap bitmap,
        ...
        return new Thumbnail(uri, bitmap, orientation, UNKNOWNID, UNKNOWNID, filePath);
    }
