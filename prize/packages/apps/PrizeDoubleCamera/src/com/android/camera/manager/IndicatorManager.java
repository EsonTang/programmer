...
public class IndicatorManager extends ViewManager implements
    ...
    public void onRefresh() {
        ...
        /*prize-add-BUG:40970 Dynamically adjust the position of the toastmessae-xiaoping-20171020-start*/
        refreshindicatorPosition();
        /*prize-add-BUG:40970 Dynamically adjust the position of the toastmessae-xiaoping-20171020-end*/
        ...
        for (int i = 0; i < INDICATOR_COUNT; i++) {
            ...
            if (mPrefs[i] instanceof IconListPreference) {
                /*prize-modify-bugid:40046 do not show delay taking pictures on prevideomode-xiaoping-20171019-start*/
                if (!mVisibles[i] || value == null
                        || (mDefaults[i] != null && mDefaults[i].equals(value))
                        || ((getContext().getCurrentMode() == ModePicker.MODE_PREVIDEO || getContext().getCurrentMode() == ModePicker.MODE_SHORT_PREVIDEO)
                        && mPrefs[i].getKey().equals(SettingConstants.KEY_SELF_TIMER))) {
                    mViews[i].setVisibility(View.GONE);
                    /*prize-modify-bugid:40046 do not show delay taking pictures on prevideomode-xiaoping-20171019-end*/
                } else {
    ...
    private void refreshindicatorPosition() {
        int previewTopMargin = getPreviewTopmargin();
        Log.d(TAG,"gettop: "+previewTopMargin+",mLastTopmargin: "+mLastTopmargin);
        if (mLastTopmargin == previewTopMargin) {
            return;
        }
        RelativeLayout.LayoutParams layoutParams = (RelativeLayout.LayoutParams) mIndicatorGroup.getLayoutParams();
        layoutParams.topMargin = previewTopMargin;
        mIndicatorGroup.setLayoutParams(layoutParams);
        mLastTopmargin = previewTopMargin;
    }

    private int getPreviewTopmargin() {
    	 if(getContext().getCameraActor() != null  && getContext().getCameraActor() instanceof PhotoActor && ((PhotoActor)getContext().getCameraActor()).getICameraMode() instanceof GyBokehMode && ((GyBokehMode)((PhotoActor)getContext().getCameraActor()).getICameraMode()).getGlSurfaceView()!= null){
            return ((GyBokehMode)((PhotoActor)getContext().getCameraActor()).getICameraMode()).getGlSurfaceView().getTop()+(int)getContext().getResources().getDimension(R.dimen.indicator_view_additional_margintop);
       }else {
    	   return getContext().getPreviewSurfaceView().getTop()+(int)getContext().getResources().getDimension(R.dimen.indicator_view_additional_margintop);
       }
