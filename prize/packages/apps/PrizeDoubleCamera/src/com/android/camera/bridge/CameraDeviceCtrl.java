...
public class CameraDeviceCtrl implements SurfaceHolder.Callback {
    ...
    private static final int MSG_CAMERA_GRID_PARAMETERS_READY = 20;
    /*PRIZE-12404-wanzhijuan-2016-03-01-end*/
    
    /*prize-xuchunming-20180523-add shortcuts-start*/
    private static final int MSG_CAMERA_OPNE_AND_PREVIEW_DONE = 21;
    ...
    public interface PreviewCallbackListen{
    	public void onPreviewFrame(byte[] data, final Camera camera);
    }
    
    private HashSet<PreviewCallbackListen> mPreviewCallbackListen;
    public CameraDeviceCtrl(CameraActivity activity, ComboPreferences preferences) {
        ...
        mPreviewCallbackListen = new HashSet<CameraDeviceCtrl.PreviewCallbackListen>();
    ...
    public void onResume() {
        ...
        //mCameraStartUpThread.readCameraId();
        ...
        openCamera();
        /*prize-xuchunming-20180225-bugid:50430-start*/
        if(mMainHandler.hasMessages(MSG_PREPARE_SURFACE_VIEW) == false) {
        	mCameraStartUpThread.resumeThread();
        }
		return;
    }
    ...
    public void closeCamera() {
        ...
        if (mCurCameraDevice.getParameters() != null && Parameters.FLASH_MODE_TORCH.equals(mCurCameraDevice.getParameters().getFlashMode())) {
			Log.i(TAG, "onPause close flash" );
			mCurCameraDevice.getParameters().setFlashMode(Parameters.FLASH_MODE_OFF);	
    ...
    public void onModeChanged(boolean isNeedRestart) {
        ...
        if (isCameraOpened()) {
            ...
            applyModeParameters(isNeedRestart);
        }
        mModuleManager.onModeChangeDone();
    }
    ...
    public void applyParameters(boolean isNeedRestart) {
        ...
        /*prize-modify-bugid:53493 set the mfb and zsd status by camera-xiaoping-20180411-start*/
        if (mCurCameraDevice.getParametersExt() != null && mCameraActivity.isNonePickIntent() == true) {
				mCurCameraDevice.getParametersExt().set("mfb", "mfll");
		}
        /*prize-modify-bugid:53493 set the mfb and zsd status by camera-xiaoping-20180411-end*/
    ...
    public void applyModeParameters(boolean isNeedRestart) {
        Log.d(TAG, "[applyParameters] isNeedRestart:" + isNeedRestart);
        ...
        boolean needRestart = isZsdChanged || isPreviewSizeRestartCase || isNeedRestart || isHdrChanged;
        if(mCameraActivity.isSwitchingCamera()){
        	needRestart = false; // after modechang , switch camera do preview. 
        }
        if (needRestart) {
            mCameraActor.stopPreview();
            if (getCameraState() != CameraState.STATE_OPENING_CAMERA) {
        ...
        if (getCameraState() == CameraState.STATE_OPENING_CAMERA) {
            if(isPreviewRatioChanged && !mIsFirstOpenCamera  && !mIsWaitForStartUpThread) {
                //switch surface view in Ui thread
                //delay 5ms, be sure pause thread before resume thread
                /*prize-xuchunming-20180413-bugid:55292-start*/
                if(Looper.getMainLooper() != Looper.myLooper()) {
                	Message msg = mMainHandler.obtainMessage(MSG_PREPARE_SURFACE_VIEW, true);
                    mMainHandler.sendMessageDelayed(msg, 5);
                	mCameraStartUpThread.pauseThread();
                }else {
                	prepareSurfaceView(true);
                }
                /*prize-xuchunming-20180413-bugid:55292-end*/
    ...
    public void updateGridParameter() {
    	mMainHandler.sendEmptyMessage(MSG_CAMERA_GRID_PARAMETERS_READY);
    }
    /*PRIZE-12404-wanzhijuan-2016-03-01-end*/
    ...
    public void onSizeChanged(int width, int height) {
        if (mCurCameraDevice != null && mCurCameraDevice.getPreviewSize() != null) {
            ...
            if (width > height) {
                ...
                mUnCropWidth = Math.max(width, (int) width);
                mUnCropHeight = Math.max(height, (int) (width * mAspectRatio));
            }
        } else {
    ...
    private void initializeSettingController() {
        ...
        if (!mISettingCtrl.isSettingsInitialized()) {
        	/*if(getCameraId() == 1){
        		mISettingCtrl.initializeSettings(R.xml.camera_preferences_front, mPreferences.getGlobal(),
                        mPreferences.getLocal());
        	}else{*/
        		mISettingCtrl.initializeSettings(R.xml.camera_preferences, R.xml.camera_preferences_front, mPreferences.getGlobal(),
                        mPreferences.getLocal());
        	//}
    ...
    private class MainHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            case MSG_CAMERA_PARAMETERS_READY:
                ...
                break;
                /*PRIZE-12404-wanzhijuan-2016-03-01-start*/
            case MSG_CAMERA_GRID_PARAMETERS_READY:
            	mCameraActivity.updateGridParameter();
            	break;
            	/*PRIZE-12404-wanzhijuan-2016-03-01-end*/
            ...
            case MSG_CAMERA_PREVIEW_DONE:
                ...
                mCameraActivity.getCameraActor().onRestoreSettings();
                break;
            ...
            case MSG_PREPARE_SURFACE_VIEW:
                ...
                break;
            /*prize-xuchunming-20180523-add shortcuts-start*/
            case MSG_CAMERA_OPNE_AND_PREVIEW_DONE:
            	mModuleManager.onCameraFirstOpenDone();
            	break;
            /*prize-xuchunming-20180523-add shortcuts-end*/
            default:
                break;
    ...
    private void removeAllMessage() {
        ...
        /*prize-xuchunming-20180523-add shortcuts-start*/
        mMainHandler.removeMessages(MSG_CAMERA_OPNE_AND_PREVIEW_DONE);
        /*prize-xuchunming-20180523-add shortcuts-end*/
    }

    /*prize-for bugid:35822-liufan-2017-07-06-start*/
    public void removeOpenCameraFailMessage(){
        mMainHandler.removeMessages(MSG_OPEN_CAMERA_FAIL);
        mMainHandler.removeMessages(MSG_OPEN_CAMERA_DISABLED);
    }
    ...
    private class CameraStartUpThread extends Thread {
        ...
        /*prize-update for bugid: 34099-liufan-2017-05-25-start*/
        public void readCameraId(){
            int cameraId = getPreferredCameraId(mPreferences);
            Log.d(TAG,"readCameraId cameraId = "+cameraId+", mCameraId = "+mCameraId);
            ...
            while (mIsActive) {
                ...
                if (mIsFirstStartUp) {
                    ...
                    if (mCameraActivity.getCurrentMode() == ModePicker.MODE_PHOTO || 
                    		mCameraActivity.getCurrentMode() == ModePicker.MODE_PREVIDEO ||
                            mCameraActivity.getCurrentMode() == ModePicker.MODE_SHORT_PREVIDEO ||
                    		mCameraActivity.getCurrentMode() == ModePicker.MODE_DOUBLECAMERA || 
                    		mCameraActivity.getCurrentMode() == ModePicker.MODE_PORTRAIT) {
                    ...
                    mModuleManager.onCameraOpenDone();
                    ...
                    CameraPerformanceTracker.onEvent(TAG,
                            ...
                            CameraPerformanceTracker.ISEND);
                    /*prize-xuchunming-20180523-add shortcuts-start*/
                    mMainHandler.sendEmptyMessage(MSG_CAMERA_OPNE_AND_PREVIEW_DONE);
                    /*prize-xuchunming-20180523-add shortcuts-end*/
                    continue;
                }
    ...
    private void applyFirstParameters () {
        ...
        if (mCameraActivity.isNonePickIntent() == true && mCameraActivity.getCurrentMode() == ModePicker.MODE_PHOTO) {
        	/*mCurCameraDevice.getParametersExt()
            .setZSDMode(SettingUtils.getPreferenceValue(mCameraActivity,
                    mPreferences,SettingConstants.ROW_SETTING_ZSD, mCameraActivity.getResources().getString(R.string.pref_camera_zsd_default)));*/
        	/*prize-modify-bugid:53493 set the mfb adn zsd status by camera-xiaoping-20180411-start*/
        	mCurCameraDevice.getParametersExt().set("mfb", "mfll");
        }
        if (mCameraActivity.isNonePickIntent() == false ) {
        	mCurCameraDevice.getParametersExt()
            .setZSDMode(SettingUtils.getPreferenceValue(mCameraActivity,
                    mPreferences,SettingConstants.ROW_SETTING_ZSD, mCameraActivity.getResources().getString(R.string.pref_camera_nonepickintent_zsd_default)));
		}
        /*prize-modify-bugid:53493 set the mfb adn zsd status by camera-xiaoping-20180411-end*/
    ...
    private PreviewCallback mOneShotPreviewCallback = new PreviewCallback() {
        ...
        public void onPreviewFrame(byte[] data, Camera camera) {
            ...
            if(mCameraActivity.getModePicker().isWaitCameraSwitch() == true && mCameraActivity.getModePicker().isShowCoverAnimat() == false) {
            	/*prize-xuchunming-20180403-bugid:54203-end*/
            	mCameraActivity.getModePicker().hideSurfaceCover();
	           	mCameraActivity.getModePicker().setWaitCameraSwitch(false);
            }
            /*prize-xuchunming-20180307-bugid:50956-end*/
            mModuleManager.oneShotPreviewCallback();
            /*remoteAllPreviewCallbackListen();
            initPreviewCallbackListen();
            if(mCameraActivity.getCurrentMode() != ModePicker.MODE_BOKEH){
    ...
    private PreviewCallback mPreviewCallback = new PreviewCallback() {
        ...
        @Override
        public void onPreviewFrame(byte[] data, Camera camera) {
        	Log.d(TAG, "++ mPreviewCallback ++");
            /*for(PreviewCallbackListen listen:mPreviewCallbackListen){
            	listen.onPreviewFrame(data, camera);
            }*/
        }
        
    };
    
    public void addPreviewCallbackListen(PreviewCallbackListen listen){
    	/*if(listen != null){
    		mPreviewCallbackListen.add(listen);
    		Log.d(TAG, "mPreviewCallbackListen size:"+mPreviewCallbackListen.size());
    	}*/
    	
    }
    
    public void initPreviewCallbackListen() {
		// TODO Auto-generated method stub
    	/*if(mPreviewCallbackListen != null){
    		addPreviewCallbackListen(mCameraActivity.getModePicker().getPreviewCallbackListen());
    		addPreviewCallbackListen(mCameraActor.getPreviewCallbackListen());
    	}*/
    	
	}
    
    public void remoteAllPreviewCallbackListen() {
		// TODO Auto-generated method stub
    	/*if(mPreviewCallbackListen != null){
    		mPreviewCallbackListen.clear();
    	}*/
	}
    
    public void removePreviewCallbackListen(PreviewCallbackListen listen){
    	/*if(listen != null && mPreviewCallbackListen.contains(listen)){
    		mPreviewCallbackListen.remove(listen);
    	}*/
    }
    ...
    public void onSpotLightVisibleChange(boolean isVisible) {
    	mModuleManager.onSpotLightVisibleChange(isVisible);
    }
