...
public class Storage {
    ...
    public static final String Bokeh_PATH = "/" + Environment.DIRECTORY_DCIM + "/Camera/Bokeh";
    ...
    public static final int PICTURE_TYPE_BOKEH = 4;
    ...
    public static final int FILE_TYPE_BOKEH = 5;
    /*prize-xuchunming-20171018-change picturezoom UI interaction-start*/
    public static final int FILE_TYPE_PICTUREZOOM = 5;
    ...
    private static String getStoragePath(ISettingCtrl settingCtrl) {
    	String type = settingCtrl.getSettingValue(SettingConstants.KEY_STORAGE_PATH);
    	if (TextUtils.isEmpty(type)) {
    		type = "memory";
    	}
    	String path;
    	if (type.equals("external") && isHaveExternalSDCard()) {
    		path = StorageManagerEx.getExternalStoragePath();
    	} else {
    		path = getInternalVolumePath();
    	}
    	return path;
    }
    
    private static ISettingCtrl getISettingCtrl() {
    	return ((CameraActivity) mContext).getISettingCtrl();
    }
    ...
    public static boolean initializeStorageState() {
        StorageManager storageManager = getStorageManager();
		/*prize add Storage path selection function wanzhijuan 2016-10-19 start*/
        String defaultPath = /*getStoragePath(getISettingCtrl());*/StorageManagerEx.getDefaultPath();
		/*prize add Storage path selection function wanzhijuan 2016-10-19 end*/
        boolean diff = false;
    ...
    public static String getBokehFileDirectory() {
        String path = sMountPoint + Bokeh_PATH;
        return path;
    }
    ...
    public static String generateFileName(String title, int pictureType) {
        if (pictureType == PICTURE_TYPE_MPO || pictureType == PICTURE_TYPE_MPO_3D) {
            ...
            return title + ".jpg";
        } else if (pictureType == PICTURE_TYPE_BOKEH) {
            return title + ".bokeh";
        } else {
	...
	public static String generateBokehFilepath(String fileName) {
        // if filename have the folder name ,so need first create the folder
        File file = new File(getBokehFileDirectory());
        if (!file.exists()) {
            mkFileDir(getBokehFileDirectory());
        }
        return getBokehFileDirectory() + '/' + fileName;
    }

    public static String generateBokehFilepath(Context context, long dateTaken) {
        Util.ImageFileNamer fileNamer = new Util.ImageFileNamer(context.getString(R.string.bokeh_file_name_format));
        return getBokehFileDirectory() + '/' + fileNamer.generateName(dateTaken) + ".bokeh";
    }
