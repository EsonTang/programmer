...
/*prize-xuchunming-bokehmode touch event error-start*/
...
/*prize-xuchunming-bokehmode touch event error-end*/
...
public class GestureDispatcher implements GestureRecognizer.Listener,
    ...
    public boolean onScroll(float dx, float dy, float totalX, float totalY, MotionEvent e2) {
        ...
        if (mGestureListener != null
                && mGestureListener.onScroll(dx, dy, totalX, totalY, e2)) {
            return false;
        }
        if (mGestureDispatcherListener != null) {
    ...
    private boolean onTouchScreen(int x, int y, View area, int index) {
        ...
        if(mCameraActivity.getCameraActor() != null && mCameraActivity.getCameraActor() instanceof PhotoActor && ((PhotoActor)mCameraActivity.getCameraActor()).getICameraMode() instanceof GyBokehMode){
        	if(((GyBokehMode)((PhotoActor)mCameraActivity.getCameraActor()).getICameraMode()).getGlSurfaceView() != null) {
        		relativeLocation = Util.getRelativeLocation(
                        (View) ((GyBokehMode)((PhotoActor)mCameraActivity.getCameraActor()).getICameraMode()).getGlSurfaceView(),
                        area);
        	}else {
        		return true;
        	}
        	...
        	if(mCameraActivity.getPreviewSurfaceView() != null) {
        		relativeLocation = Util.getRelativeLocation(
