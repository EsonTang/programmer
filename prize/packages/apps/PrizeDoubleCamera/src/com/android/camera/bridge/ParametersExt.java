...
public class ParametersExt implements Parameters {
    ...
    public ParametersExt(CameraManager.CameraProxy cameraDevice,
            android.hardware.Camera.Parameters parameters,Activity activity) {
        mCameraDevice = cameraDevice;
        mParameters = parameters;
        mActivity = activity;
    }
        ...
        lockRun(new Runnable() {
            ...
            public void run() {
				...
				if (mActivity instanceof CameraActivity) {
					if (((CameraActivity) mActivity).isNonePickIntent() == false) {
						mParameters.setZSDMode("off");
					} else {
     ...
     * @param enable  true un-mute false un-mute
     * @return
     * @see 
     */
    public boolean enableShutterSound(boolean enable) {
    	mParameters.enableRecordingSound(enable ? String.valueOf(0) : String.valueOf(1));
    	if (mCameraDevice != null) {
