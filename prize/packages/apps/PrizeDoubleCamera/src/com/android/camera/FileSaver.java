...
public class FileSaver {
    ...
    private List<FileSaverStartListener> mSaverStartListenerList = new CopyOnWriteArrayList<FileSaverStartListener>();
    ...
    public boolean isMakeBokehFile = true;
    
    public interface FileSaverListener {
        void onFileSaved(SaveRequest request);
    }

    public interface FileSaverStartListener {
        void onFileSaveStart(SaveRequest request);
    }
    ...
    //os-add-for bokeh-20170424-pengcancan-start
	public SaveRequest prepareBokehPhotoRequest(int fileType, int pictureType, String desc) {
        Log.i(TAG, "[prepareBokehPhotoRequest]fileType =" + fileType + ",pictureType = " + pictureType);
        SaveRequest request = null;
        request = new BokehPhotoOperator(pictureType, desc);
        request.prepareRequest();
        mContext.applyParameterForCapture(request);

        return request;
    }
	//os-add-for bokeh-20170424-pengcancan-end
	
	/*prize-xuchunming-20171018-change picturezoom UI interaction-start*/
	public SaveRequest preparePictureZoomRequest( int pictureType) {
        Log.i(TAG, "[preparePictureZoomRequest] pictureType = " + pictureType);
        SaveRequest request = new PictureZoomOperator(pictureType);
        request.prepareRequest();
        mContext.applyParameterForCapture(request);
        return request;
    }
	...
	public SaveRequest prepareLowLightShotRequest( int pictureType) {
        Log.i(TAG, "[prepareLowLightShotRequest] pictureType = " + pictureType);
        SaveRequest request = new LowLightShotOperator(pictureType);
        request.prepareRequest();
        mContext.applyParameterForCapture(request);
        return request;
    }
    ...
    public SaveRequest copyPhotoRequest(SaveRequest originRequest) {
        ...
        else if (originRequest instanceof PictureZoomOperator) {
            request = ((PictureZoomOperator) originRequest).copyRequest();
        }
        /*prize-xuchunming-20171018-change picturezoom UI interaction-end*/
        
        /*prize-xuchunming-20171030-change lowlightshot UI interaction-start*/
        else if (originRequest instanceof LowLightShotOperator) {
            request = ((LowLightShotOperator) originRequest).copyRequest();
        }
        /*prize-xuchunming-20171030-change lowlightshot UI interaction-end*/
        return request;
    }
    ...
    public boolean addSaveStartListener(FileSaverStartListener listener) {
        Log.d(TAG, "[addSaveStartListener]...");
        if (!mSaverStartListenerList.contains(listener)) {
            return mSaverStartListenerList.add(listener);
        }

        return false;
    }
    
    public boolean removeSaveStartListener(FileSaverStartListener l) {
        Log.d(TAG, "[removeSaveStartListener]...");
        return mSaverStartListenerList.remove(l);
    }
    ...
    private FileSaverService.FileSaverListener mFileSaverListener = new FileSaverService.FileSaverListener() {
        ...
        @Override
        public void onFileStart(SaveRequest request) {
        	for (FileSaverStartListener listener : mSaverStartListenerList) {
                listener.onFileSaveStart(request);
            }
        };
    };
    ...
    private synchronized String createName(int fileType, long dateTaken, int count) {
        Log.d(TAG,"[createName],fileType: "+fileType+",dateTaken: "+dateTaken+",count: "+count+",mFileNamer: "+mFileNamer);
        if (mFileNamer == null) {
            ...
            //os-add-for bokeh-20170424-pengcancan-start
            ImageFileNamer bokeh = new ImageFileNamer(
                    mContext.getString(R.string.bokeh_file_name_format));
            mFileNamer.put(Storage.FILE_TYPE_BOKEH, bokeh);
			//os-add-for bokeh-20170424-pengcancan-end
        ...
        Log.d(TAG,"[createName],mFileNamer.get(fileType): "+mFileNamer.get(fileType)+",count: "+count);
    ...
    private abstract class RequestOperator implements SaveRequest {
        ...
        public void updateDataTaken(long time) {
            Log.d(TAG,"[updateDataTaken],time: "+time);
            mDateTaken = time;
        }
        ...
        public void saveImageToDatabase(RequestOperator r) {
            ...
            if (r.mLocation != null) {
                ...
                values.put(Images.Media.DESCRIPTION, r.mLocation.getExtras().getString(BDLocationManager.ADDR_KEY));
    ...
    public class PhotoOperator extends RequestOperator {
    ...
    private class VideoOperator extends RequestOperator {
        ...
        public void prepareRequest() {
            ...
            mFilePath = Storage.generateFilepath(mFileName);
            //bd location Shooting video to add location information
            Location loc = mContext.getLocationManager().getCurrentLocation();
            if (loc != null) {
                mLocation = new Location(loc);
            }
        ...
        public void saveRequest() {
            ...
            try {
                ...
                if (mLocation != null) {
                    ...
                    //bd location Shooting video to add location information
                    values.put(Video.Media.DESCRIPTION, mLocation.getExtras().getString(BDLocationManager.ADDR_KEY));
                    //end
                }
    ...
    //os-add-for bokeh-20170424-pengcancan-start
    private class BokehPhotoOperator extends RequestOperator {
        private BokehPhotoOperator(int pictureType, String desc) {
            mTempPictureType = pictureType;
            mBokehDesc = desc;
        }
        ...
        public void prepareRequest() {
            Log.i(TAG, "[prepareRequest]PhotoOperator...");
            mFileType = Storage.FILE_TYPE_BOKEH;
            mDateTaken = System.currentTimeMillis();
            Log.d(TAG,"mDateTaken: "+mDateTaken);
            Location loc = mContext.getLocationManager().getCurrentLocation();
            if (loc != null) {
                mLocation = new Location(loc);
            }
        ...
        public BokehPhotoOperator copyRequest() {
            Log.i(TAG, "[copyRequest]PhotoOperator...");
            BokehPhotoOperator newRequest = new BokehPhotoOperator(mTempPictureType,null);
            newRequest.mFileType = Storage.FILE_TYPE_BOKEH;
            newRequest.mDateTaken = System.currentTimeMillis();
            newRequest.mLocation = this.mLocation;
            newRequest.mTempJpegRotation = this.mTempJpegRotation;
        ...
        @Override
        public synchronized void saveRequest() {
            Log.d(TAG,"[BokehPhotoOperator,saveRequest]");
            if (mData == null) {
                Log.w(TAG, "[saveRequest]mData is null,return!");
                return;
            }
            if (mBokehDesc != null) {
                mTempPictureType = Storage.FILE_TYPE_PHOTO;
                mFileType = Storage.FILE_TYPE_PHOTO;
                int orientation = Exif.getOrientation(mData);
                // M: ConShots
                mGroupId = Exif.getGroupId(mData);
            ...
            if (null != mFileName) {
                ...
                mTitle = createName(mFileType, mDateTaken, mGroupIndex);
                mFileName = Storage.generateFileName(mTitle, mTempPictureType);
                Log.i(TAG, "[saveRequest]PhotoOperator,mFileName = " + mFileName+"mDateTaken: "+mDateTaken);
            }
            mFilePath = mBokehDesc == null ? Storage.generateBokehFilepath(mFileName) : Storage.generateFilepath(mFileName);
            mTempFilePath = mFilePath + TEMP_SUFFIX;
            saveImageToSDCard(mTempFilePath, mFilePath, mData);
            if (mBokehDesc != null) {
                // camera decouple
        ...
        private void saveImageToSDCard(String tempFilePath, String filePath, byte[] data) {
            ...
            if (filePath.endsWith("bokeh")) {
                isMakeBokehFile = true;
            } else {
                isMakeBokehFile = false;
            }
    ...
    /*prize-xuchunming-20171018-change picturezoom UI interaction-start*/
    public class PictureZoomOperator extends PhotoOperator{
    	private int mScreenWidth;
    	private int mScreenheight;
    	private PictureZoomOperator(int pictureType){
    		super(pictureType);
    		initScreenWidth(mContext);
    	}
    
        public void addRequest() {
            Log.i(TAG, "[addRequest]PictureZoomOperator...");
            addSaveRequest(this);
        }

    	public PictureZoomOperator copyRequest() {
            Log.i(TAG, "[copyRequest]PhotoOperator...");
            PictureZoomOperator newRequest = new PictureZoomOperator(mTempPictureType);
            newRequest.mFileType = Storage.FILE_TYPE_PHOTO;
            newRequest.mDateTaken = System.currentTimeMillis();
            newRequest.mLocation = this.mLocation;
            newRequest.mTempJpegRotation = this.mTempJpegRotation;
    	...
    	public Thumbnail createThumbnail(int thumbnailWidth) {
    		// TODO Auto-generated method stub
    		 Thumbnail thumb = null;
    		 Bitmap sourceBitmap = SurfaceControl.screenshot(mScreenWidth, mScreenheight);
             if (sourceBitmap != null) {
                 // Create a thumbnail whose width is equal or bigger than
                 // that of the thumbnail view.
             
            	 float ratio = (float)thumbnailWidth  / sourceBitmap.getWidth();
                 thumb = Thumbnail.createThumbnail(sourceBitmap, mOrientation, ratio, mUri);
    ...
    /*prize-xuchunming-20171030-change lowlightshot UI interaction-start*/
    public class LowLightShotOperator extends PhotoOperator{
    	private int mScreenWidth;
    	private int mScreenheight;
    	private LowLightShotOperator(int pictureType){
    		super(pictureType);
    		initScreenWidth(mContext);
    	}
    
        public void addRequest() {
            Log.i(TAG, "[addRequest]LowLightShotOperator...");
            addSaveRequest(this);
        }

    	public LowLightShotOperator copyRequest() {
            Log.i(TAG, "[copyRequest]LowLightShotOperator...");
            LowLightShotOperator newRequest = new LowLightShotOperator(mTempPictureType);
            newRequest.mFileType = Storage.FILE_TYPE_PHOTO;
            newRequest.mDateTaken = System.currentTimeMillis();
            newRequest.mLocation = this.mLocation;
            newRequest.mTempJpegRotation = this.mTempJpegRotation;
    	...
    	public Thumbnail createThumbnail(int thumbnailWidth) {
    		// TODO Auto-generated method stub
    		 Log.i(TAG, "[createThumbnail]LowLightShotOperator...");
    		 Thumbnail thumb = null;
    		 Bitmap sourceBitmap = SurfaceControl.screenshot(mScreenWidth, mScreenheight);
             if (sourceBitmap != null) {
                 // Create a thumbnail whose width is equal or bigger than
