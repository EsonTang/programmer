...
public class PreVideoPreviewRule implements ISettingRule {
   private String TAG = "PreVideoPreviewRule";
   ...
   private String mConditionSettingKey = SettingConstants.KEY_PREVIDEO;
   ...
   public PreVideoPreviewRule(ICameraContext cameraContext, CameraModeType mode) {
       Log.i(TAG, "[VideoPreviewRule]constructor...,mode: "+mode);
       /*prize-modify-add shortvideo mode-xiaoping-20180424-start*/
       if (mode == CameraModeType.EXT_MODE_SHORT_PREVIDEO) {
           mConditionSettingKey = SettingConstants.KEY_SHORT_PREVIDEO;
       } else {
           mConditionSettingKey = SettingConstants.KEY_PREVIDEO;
       }
   ...
   public void execute() {
       ...
       if (index == -1) {
           ...
           /*prize-modify-reset setting appears flashing-xiaoping-20171009-start*/
           ...
           /*prize-modify-reset setting appears flashing-xiaoping-20171009-end*/
   ...
   private void setVideoPreviewSize() {
       ...
       /*prize-modify-bugid:53777 Video preview screen is inconsistent with the video screen-xiaoping-20180330-start*/
       ...
       //pip is not video mode for camera native so
       //don't set video sizeo
       /*if (mMode != CameraModeType.EXT_MODE_VIDEO_PIP) {
           setVideoSize(mProfile, mParameters);
       }*/
   ...
   private int getQuality() {
       ...
       if(mISettingCtrl.getListPreference(SettingConstants.KEY_VIDEO_QUALITY).findIndexOfValue(settingQuality) == -1){
    	   CharSequence[] mEntryValues = mISettingCtrl.getListPreference(SettingConstants.KEY_VIDEO_QUALITY).getEntryValues();
    	   if(mEntryValues != null && mEntryValues.length >= 1){
    		   settingQuality = mEntryValues[mEntryValues.length -1].toString();
    		   mISettingCtrl.getListPreference(SettingConstants.KEY_VIDEO_QUALITY).setValue(settingQuality);
    	   }
    	   Log.d(TAG, "settingQuality:"+settingQuality);
       }
   ...
   private CamcorderProfile fetchProfile(int quality, int cameraId) {
       ...
       if (mProfile != null) {
        ...
        /* Force the video format to MP4 format
        * 1.3gp format ; 2.mp4 format
        *  Force the video codec format
        * 1.H263; 2.H264; 3.MPEG_4_SP
        * prize-by-zhongweilin
        */
       mProfile.fileFormat = 2;
       mProfile.videoCodec = 2;
   ...
   private void setVideoSize(CamcorderProfile profile, Parameters parameters) {
       if (profile != null && parameters != null) {
           String videoSize = Integer.toString(mProfile.videoFrameWidth) +
                   "x" + Integer.toString(mProfile.videoFrameHeight);
           Log.i(TAG, "video-size = " + videoSize);
           parameters.set("video-size", videoSize);
       }
   }
