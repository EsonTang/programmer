...
public class PanoramaCameraPrieviewSizeRule implements ISettingRule {
   private static final String TAG = "PanoramaCameraPrieviewSizeRule";
   ...
   public PanoramaCameraPrieviewSizeRule(ICameraContext ct) {
       mICameraContext = ct;
       mActivity = mICameraContext.getActivity();
       mISettingCtrl = mICameraContext.getSettingController();
       Log.i(TAG, "[PanoramaCameraPrieviewSizeRule]");
   }

   @Override
   public void execute() {
       String value = mISettingCtrl.getSettingValue(SettingConstants.KEY_PANORAMA);
   ...
   private void setPictureSize(String pictureSize){
	   if(pictureSize != null ){
	       ...
	       Log.d(TAG, " will set picture width = " + width + ",height = " +height);
	       mICameraDevice.setPictureSize(width, height);
	   }
   }
   
   private void setPanoramaCameraPreviewSize() {
	   if(((CameraActivity)mActivity).getParameters() != null){
		   String previewRatio = SettingDataBase.PICTURE_RATIO_19_9;
	       String optiomPictureSize  = SettingUtils.getOptimalPictureSize(mICameraDevice.getParameters(), previewRatio);
	       Log.d(TAG, "will set picturesize  = " + optiomPictureSize);
	       ...
	       Log.d(TAG, "will set preview width = " + optiomSize.width+ ",height = " + optiomSize.height);
