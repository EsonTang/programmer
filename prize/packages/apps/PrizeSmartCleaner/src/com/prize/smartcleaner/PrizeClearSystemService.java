...
public class PrizeClearSystemService extends Service {
    ...
    **/
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null) {
            String action = intent.getAction();
            LogUtils.i(TAG, "PrizeClearSystemService onStartCommand action: " + action);
...
//            }).start();
            ...
            if (UPDATE_FILTER_LIST.equals(action)) {
                LogUtils.i(TAG, "start update filter list");
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                ...
                if (PrizeClearUtil.isUserAMonkey()) {
                    LogUtils.i(TAG, "monkey running, return");
                } else if (PrizeClearUtil.isMorningCleaning()) {
                    LogUtils.i(TAG, "morning clear running, return");
                } else {
                ...
                if (PrizeClearUtil.isUserAMonkey()) {
                    ...
                    if (PrizeClearUtil.isServiceRunning()) {
                        ...
                        if (!(isLearningClearing || mClearRunningMgr.mClearThreadRunning)) {
                            ...
                            new Handler(Looper.getMainLooper()).post(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    LogUtils.i(TAG, "clear thread has finished removing task, show message and return");
                                }
                            });
                        }
                        LogUtils.i(TAG, "clear is running, return");
                    } else {
                        int uid = 0;
                        int userId;
                        int appIndex;
                        ...
                        if (extras != null && extras.containsKey("isLearningClearing")) {
                            ...
                            LogUtils.d(TAG, "onStartCommand, isLearningClearing:" + isLearningClearing);
                            if (isLearningClearing && PrizeClearUtil.isScreenOn(mContext) && !PrizeClearUtil.isUpdateFilterListRunning()) {
