...
public class PrizeClearRunningManager {
    ...
    private void forceStopPackageAsUser(String pkg, int userId, int appInstanceIndex) {
        LogUtils.i(TAG, "F [" + pkg + " , "  + userId + " , "  + appInstanceIndex + "] for remove task");
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case FORCE_STOP_SELF:
                    ...
                    mIsLearningClearing = false;
                    if (!PrizeClearUtil.isServiceRunning() && !PrizeClearUtil.isUpdateFilterListRunning()) {
    ...
    class ClearRunningThread extends Thread {
        ...
        public void run() {
            ...
            ArrayList<String> forceStopFilterPkgList = clearFilterMgr.getFilterListFromSP(mContext, PrizeClearUtil.TYPE_FORCE_STOP);//PrizeClearUtil.getForceStopFilterPkgList(mContext);
            ...
            ArrayList<String> pkgFilterList = clearFilterMgr.getFilterListFromSP(mContext, PrizeClearUtil.TYPE_PACKAGE);
            ArrayList<String> processFilterList = clearFilterMgr.getFilterListFromSP(mContext, PrizeClearUtil.TYPE_PROCESS);
            ...
            if (mClearTask) {
                ...
                while (index < size) {
                    ...
                    if (!canBeRemove && !isRedundent) {
                        ...
                        LogUtils.i(TAG, "RemoveTask Pkg =" + packageName + ", UserId=" + userId + ", Redundent=" + isRedundent);
            ...
            */
            ...
            ArrayList<String> beKilledList = new ArrayList<>();
            beKilledList.clear();
            ...
            while (proIndex < proSize) {
                ...
                if ((runningAppProcessInfo.flags & ActivityManager.RunningAppProcessInfo.FLAG_PERSISTENT) != 0) {
                    LogUtils.i(TAG, "skip persist " + runProName);
                } else {
                    if (PrizeClearUtil.mLocalProcessFilterList.contains(runProName)) {
                        LogUtils.i(TAG, "skip localClearFilterProcess=" + runProName);
                        ...
                        for (int i = 0; i < length; i++) {
                            try {
                                ...
                                if (applicationInfo != null) {
                                    boolean isThirdApp = (!applicationInfo.isSystemApp() || applicationInfo.isPrebuiltThirdApp());
                                    ...
                                    if (userIdPkgList.contains(userIdPkg)) {
                                        LogUtils.i(TAG, "duplicate packagename = " + pkgName + ", userid=" + userIdPkg.userId);
                                    } else if (forceStopFilterPkgList.contains(pkgName)) {
                                        LogUtils.i(TAG, "skip force stop filter package : " + pkgName);
                                    } else if (isPersistent) {
                                        LogUtils.i(TAG, "skip appmanager persist " + pkgName);
                                        ...
                                        if (!pkgName.contains("org.simalliance.openmobileapi") || !mIsLearningClearing) {
                                            ...
                                            if (!isLockedApp && isKill) {
                                                ...
                                                if (killType == 2) {
                                                    ...
                                                    LogUtils.i(TAG, "isThirdApp = " + isThirdApp);
                                                    if (beKilledList != null && !beKilledList.contains(pkgName) && isThirdApp) {
                                                        beKilledList.add(pkgName);
                                                        LogUtils.i(TAG, "pkgName = " + pkgName);
                                                    }
                                                } else if (killType == 0) {
                                                    LogUtils.i(TAG, "skip process " + runProName);
                                                }
                                            }
                                        } else {
                                            LogUtils.i(TAG, "learning clear skip simalliance packagename = " + pkgName);
                                        }
                                    } else {
                                        LogUtils.i(TAG, "skip system pkg = " + pkgName);
                                    }
                                }
                            } catch (PackageManager.NameNotFoundException e) {
            ...
            PrizeClearUtil.sendBroadcastCancelAlarmAndJob(mContext, beKilledList);
    ...
    class DelayClearThread extends Thread {
        ...
        public void run() {
            ...
            LogUtils.i(TAG, "before clear the mem is: " + before);

            ArrayList<String> forceStopList = new ArrayList<>();

            PrizeClearFilterManager clearFilterMgr = PrizeClearFilterManager.getInstance();
            ArrayList<String> launcher = PrizeClearUtil.getLauncherList(mContext);
            ArrayList<String> inputMethod = PrizeClearUtil.getDefInputMethodList(mContext);
            ArrayList<String> pkgFilterList = clearFilterMgr.getFilterListFromSP(mContext, PrizeClearUtil.TYPE_PACKAGE);
            ArrayList<String> morningClearFilterList = clearFilterMgr.getFilterListFromSP(mContext, PrizeClearUtil.TYPE_MORNING);
            ArrayList<String> forceStopFilterPkgList = clearFilterMgr.getFilterListFromSP(mContext, PrizeClearUtil.TYPE_FORCE_STOP);
            ArrayList<String> dozeFilterList = clearFilterMgr.getFilterListFromSP(mContext, PrizeClearUtil.TYPE_DOZE);

            //get inaction location app
            ArrayList<String> locationActiveList = new ArrayList<>();
            ArrayList<String> needKillLocationList = CommonUtil.getNeedKillLocationList(mContext, locationActiveList);

            //kill inactive music app
            ArrayList<String> musicActiveList = new ArrayList<>();
            ArrayList<String> needKillMusicList = CommonUtil.getNeedKillMusicList(mContext, musicActiveList);
            LogUtils.d(TAG, "needKillMusicList size : " + needKillMusicList.size());
            if (!(PrizeClearUtil.isPhoneInCall(mContext) || PrizeClearUtil.isScreenOn(mContext))) {
                for (String pkg : needKillMusicList) {
                    if (!forceStopFilterPkgList.contains(pkg)
                            && !pkgFilterList.contains(pkg)
                            && !morningClearFilterList.contains(pkg)
                            && !dozeFilterList.contains(pkg)
                            && !launcher.contains(pkg)
                            && !inputMethod.contains(pkg)
                            && !locationActiveList.contains(pkg)) {
                        PrizeClearUtil.forceStopPackage(mContext, pkg);
                        forceStopList.add(pkg);
                    }
                }
            } else {
                LogUtils.i(TAG, "cancel kill inactive music list pkg , phone in call or screen is on");
            }
            ...
            if (!(PrizeClearUtil.isPhoneInCall(mContext) || PrizeClearUtil.isScreenOn(mContext))) {
                ...
                LogUtils.i(TAG, "cancel kill audio list pkg , phone in call or screen is on");
            }

            //kill all net & gps app ,not in dozewhitelist @{
            //ArrayList<String> locationActiveList = new ArrayList<>();
            //ArrayList<String> needKillLocationList = CommonUtil.getNeedKillLocationList(mContext, locationActiveList);//PrizeClearUtil.getInUseLocationPkgList(mContext);
            ...
            if (!(PrizeClearUtil.isPhoneInCall(mContext) || PrizeClearUtil.isScreenOn(mContext))) {
                if ((needKillNetList != null && !needKillNetList.isEmpty()) ||
                        (needKillLocationList != null && !needKillLocationList.isEmpty())) {
                    ...
                    if (dozeWhite != null) {
                        ...
                        if (needKillLocationList != null) {
                            for (int i = 0; i < needKillLocationList.size(); i++) {
                                LogUtils.i(TAG, "needKilLocationList[" + needKillLocationList.get(i) + "]");
                            }
                        }
                        if (needKillNetList != null) {
                            for (int i = 0; i < needKillNetList.size(); i++) {
                                LogUtils.i(TAG, "needKillNetList[" + needKillNetList.get(i) + "]");
                            }
                        }
                        if (dozeWhiteList != null) {
                            for (int i = 0; i < dozeWhiteList.size(); i++) {
                                LogUtils.i(TAG, "DozeWhiteList[" + dozeWhiteList.get(i) + "]");
                            }
                        }

                        //force stop use net pkg
                        if (needKillNetList != null) {
                            for (String pkg : needKillNetList) {
                                if (!forceStopFilterPkgList.contains(pkg)
                                        && !dozeWhiteList.contains(pkg)
                                        && !pkgFilterList.contains(pkg)
                                        && !morningClearFilterList.contains(pkg)
                                        && !dozeFilterList.contains(pkg)
                                        && !launcher.contains(pkg)
                        ...
                        //force stop use location pkg
                        if (needKillLocationList != null) {
                            for (String pkg : needKillLocationList) {
                                if (!forceStopFilterPkgList.contains(pkg)
                                        //&& !dozeWhiteList.contains(pkg) //delete 2018-06-13 for can't kill which in doze white list app
                                        && !pkgFilterList.contains(pkg)
                                        && !morningClearFilterList.contains(pkg)
                                        && !dozeFilterList.contains(pkg)
                                        && !launcher.contains(pkg)
                    ...
                    LogUtils.i(TAG, "no pkg use net & location");
                }

                //loop kill for 37m & 50m
                if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.M) {
                    //top N
                    int count = SystemProperties.getInt(PrizeClearUtil.PROP_LOOP_KILL_COUNT, 1);
                    if (count > 1) {
                        LogUtils.i(TAG, "start clean top N");
                        ArrayList<String> hasCheckedList = new ArrayList<>();
                        ArrayList<ActivityManager.RecentTaskInfo> recentTaskInfos = PrizeClearUtil.getRecentTasks(mContext);
                        for (int i = 0; i < recentTaskInfos.size(); i++) {
                            ActivityManager.RecentTaskInfo recentTaskInfo = recentTaskInfos.get(i);
                            String pkg = recentTaskInfo.baseIntent.getComponent().getPackageName();
                            if (!hasCheckedList.contains(pkg)) {
                                LogUtils.i(TAG, "packageName = " + pkg);
                                if (!forceStopFilterPkgList.contains(pkg)
                                        && !pkgFilterList.contains(pkg)
                                        && !morningClearFilterList.contains(pkg)
                                        && !dozeFilterList.contains(pkg)
                                        ...
                                        && !musicActiveList.contains(pkg)
                                        && !locationActiveList.contains(pkg)
                                        && !PrizeClearUtil.isPrizeSystemApp(mContext, pkg)) {
                                    mActivityMgr.removeTask(recentTaskInfo.persistentId);
                                    LogUtils.i(TAG, "remove task packageName = " + pkg + " , persistentId = " + recentTaskInfo.persistentId);
                                    if (!forceStopList.contains(pkg)) {
                                        PrizeClearUtil.forceStopPackage(mContext, pkg);
                                    }
                                }
                                hasCheckedList.add(pkg);
                            }
                        }
                    }

                    PrizeClearUtil.setLoopKillAlarm(mContext);
                } else {
                    LogUtils.i(TAG, "(Build.VERSION.SDK_INT = " + Build.VERSION.SDK_INT + ") > " + "(Build.VERSION_CODES.M = " + Build.VERSION_CODES.M + ") needn't loop kill");
                }

            } else {
                LogUtils.i(TAG, "cancel kill location net list pkg , phone in call or screen is on");
            }

            long after = PrizeClearUtil.getAvailMem(mContext);

            LogUtils.i(TAG, "after clear the mem is: " + after);
