...
public class QsbContainerView extends FrameLayout {
    ...
    public static class QsbFragment extends Fragment implements View.OnClickListener {
        ...
        public View onCreateView(
            ...
            /// M: Add for OP customization. disable QSB in op09 @{
        ...
        private View createQsb(LayoutInflater inflater, ViewGroup container) {
            ...
            if (!isWidgetBound) {
                ...
                if (!isWidgetBound) {
                ///M: ALPS03162783:- Added check for deleting valid allocated widget id.
                    if (widgetId > -1) {
                        widgetHost.deleteAppWidgetId(widgetId);
                        widgetId = -1;
                    }
            ...
            if (requestCode == REQUEST_BIND_QSB) {
                if (resultCode == Activity.RESULT_OK) {
                    int widgetId = data.getIntExtra(AppWidgetManager.EXTRA_APPWIDGET_ID,
                            sSavedWidgetId);
                    Utilities.getPrefs(getActivity()).edit().putInt(QSB_WIDGET_ID,
                            widgetId).apply();
                    sSavedWidgetId = -1;
                    rebindFragment();
