...
public class CameraBackYuvSub extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			catDevPath("/sys/kernel/spc/spc_r_open");
		...
		mhandle = new Handler() {
			public void handleMessage(Message msg) {
				if (msg.what == 0x123) {
					 alsps.setText(getString(R.string.camera_back_sub_tip) + " : " + catDevPath("/sys/kernel/spc/spc_r_value"));

					if (!catDevPath("/sys/kernel/spc/spc_r_value").contains("No")&&!catDevPath("/sys/kernel/spc/spc_r_value").contains("-1")) {
	...
	public void finish() {
		Log.e("tangan","1");
                try{
                       Log.e("tangan","11");
                        catDevPath("/sys/kernel/spc/spc_r_close");
                        }catch(Exception e){
