 ...
 */
public class CallNotifier extends Handler implements SubInfoUpdateListener
    /// M: TODO MR1 for Plug-in @{
    , ICfiAction {
    /// @}
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case PHONE_MWI_CHANGED:
                ...
                if (DBG) log("Received PHONE_MWI_CHANGED event, subId = " + subId);
                if (PhoneUtils.isValidSubId(subId)) {
                    Phone phone = PhoneUtils.getPhoneUsingSubId(subId);
                    //onMwiChanged(phone.getMessageWaitingIndicator(), subId);
                }
                break;
    ...
    public void updatePhoneStateListeners() {
        ...
        while (itr.hasNext()) {
            ...
            if (subInfos == null || !containsSubId(subInfos, subId)) {
                ...
                /// M: Add for plug-in @{
                ExtensionManager.getCallForwardExt().updateCfiIconEx(subId,
                        mApplication.getApplicationContext(), CallNotifier.this, false);
                /// }@
    ...
    private class CallNotifierPhoneStateListener extends PhoneStateListener {
        ...
        public void onCallForwardingIndicatorChanged(boolean visible) {
            ...
            log("onCallForwardingIndicatorChanged(): " + this.mSubId + "visible= " + visible);
            if (!ExtensionManager.getCallForwardExt().isEnableCFUInTimeSlot(this.mSubId)) {
                mApplication.notificationMgr.updateCfi(this.mSubId, visible);
            }
            /// M: Add for plug-in @{
            ExtensionManager.getCallForwardExt().updateCfiIconEx(this.mSubId,
    ...
    private void handlePlugOut(List<Integer> list) {
        for (int i = 0; i < list.size(); i++) {
            int subId = list.get(i);
            if (!PhoneUtils.isValidSubId(subId)) {
                log("handlePlugOut: save CFU subId:" + subId);
//                onCfiChanged(false, subId);
            }
        }
    }
    ...
    /// M: for Plug-in @{
    @Override
    public void updateCfiEx(int subId, boolean visible) {
        log("updateCfiEx(): " + subId + " " + visible);
        mApplication.notificationMgr.updateCfi(subId, visible);
    }
    /// @}
}
