...
public class EmergencyCallbackModeExitDialog extends Activity implements OnDismissListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        /// M: PROPERTY_INECM_MODE is set by phoneId @{
        ...
        Log.i(TAG, "ECMModeExitDialog launched - phone:" + mPhone);
        if (mPhone == null || !mPhone.isInEcm()) {
        /// @}
            /// M:fix CR:ALPS03027553,null object reference to invloke virtual method.@{
            EmergencyCallbackModeExitDialog.this.setResult(RESULT_OK, (new Intent()).putExtra(
    ...
    private void showEmergencyCallbackModeExitDialog() {
        ...
        Log.d(TAG, "showEmergencyCallbackModeExitDialog mInEmergencyCall = " + mInEmergencyCall);
        if(mInEmergencyCall) {
    ...
    /// M:fix CR:ALPS02814429,Click notifiction don't show ECBM for the first time when
    /// close multiwindow by onPause ECBM.@{
    protected void onStop() {
        Log.d(TAG, "onStop.");
        super.onStop();
        // M:fix CR:ALPS02851279,null object reference to invloke virtual method.
        EmergencyCallbackModeExitDialog.this.setResult(RESULT_OK, (new Intent()).putExtra(
                EXTRA_EXIT_ECM_RESULT, false));
        finish();
    }
