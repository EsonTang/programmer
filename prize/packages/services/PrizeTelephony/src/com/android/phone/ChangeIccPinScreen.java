 ...
 */
public class ChangeIccPinScreen extends Activity implements SubInfoUpdateListener {
    ...
    public void onCreate(Bundle icicle) {
        ...
        // [MTK_FDN] Init after get the subId, in resolveIntent.
    ...
    //-----------------------MTK-----------------------
    // [MTK_FDN] 1.Add retry left time behind tips
    private TextView mOldPinLabel;
    private TextView mPukCodeLabel;
    private int mSubId;

    private void initTelephony(Intent intent) {
        mSubId = intent.getIntExtra(PhoneConstants.SUBSCRIPTION_KEY, INVALID_SUBSCRIPTION_ID);
        if (PhoneUtils.isValidSubId(mSubId)) {
            mPhone = PhoneUtils.getPhoneUsingSubId(mSubId);
        } else {
            log("SubId is not valid !!! And ID = " + mSubId);
            mPhone = PhoneGlobals.getPhone();
        }
    }
    ...
    private void onCreateMTK() {
        ...
        mPukCodeLabel = (TextView) findViewById(R.id.puk_code_label);
        addRetryNumberTips();
    }

    private void addRetryNumberTips() {
        mOldPinLabel.append("\n" +
                TelephonyUtils.getPinPuk2RetryLeftNumTips(this, mSubId, true));
        mPukCodeLabel.append("\n" +
