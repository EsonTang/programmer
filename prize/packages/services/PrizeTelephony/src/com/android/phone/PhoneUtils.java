...
public class PhoneUtils {
    ...
    static Dialog displayMMIInitiate(Context context,
        ...
        if (!isCancelable) {
            ...
            ExtensionManager.getMmiCodeExt().onMmiDailogShow(buttonCallbackMessage);
    ...
    public static void displayMMIComplete(final Phone phone, Context context, final MmiCode mmiCode,
        ...
        if ((app.getPUKEntryActivity() != null) && (state == MmiCode.State.COMPLETE)) {
            ...
            if (state != MmiCode.State.PENDING) {
                ...
                //add for plug in.@{
                //to configure MMI Dialog.
                ExtensionManager.getMmiCodeExt().configBeforeMmiDialogShow(mmiCode);
                //add for plug in.@}
                playUSSDTone(context);
                //add for plug in.@{
                //to cancel USSD
                log("displayMMICompleteExt: mmiCode = " + mmiCode + " dismissCallbackMessage=" + dismissCallbackMessage);
                ExtensionManager.getMmiCodeExt().onMmiDailogShow(dismissCallbackMessage);
                //add for plug in.@}
            } else {
                        ...
                        public void onClick(DialogInterface dialog, int whichButton) {
                            switch (whichButton) {
                                case DialogInterface.BUTTON_POSITIVE:
                                    //add for plug in.@{
                                    if (ExtensionManager.getMmiCodeExt().showUssdInteractionDialog(phone, inputText)) {
                                        break;
                                    }
                                    //add for plug in.@}
                ...
                // build the dialog
                if (sNewDialog == null) {
                    sNewDialog = new AlertDialog.Builder(contextThemeWrapper)
                        .setMessage(text)
                        .setView(dialogView)
                ...
                sNewDialog.getWindow().setType(
                        WindowManager.LayoutParams.TYPE_SYSTEM_DIALOG);
                sNewDialog.getWindow().addFlags(
                        WindowManager.LayoutParams.FLAG_DIM_BEHIND);

                // now show the dialog!
                sNewDialog.show();

                sNewDialog.getButton(DialogInterface.BUTTON_POSITIVE)
                        .setTextColor(context.getResources().getColor(R.color.dialer_theme_color));
                sNewDialog.getButton(DialogInterface.BUTTON_NEGATIVE)
                        .setTextColor(context.getResources().getColor(R.color.dialer_theme_color));
                //add for plug in.@{
                //to show Ussd dialog.
                ExtensionManager.getMmiCodeExt().configBeforeMmiDialogShow(mmiCode);
    ...
    /* package */ static boolean isPhoneInEcm(Phone phone) {
        if ((phone != null) && TelephonyCapabilities.supportsEcm(phone)) {
            ...
            String ecmMode =
            /**
             * M: The TelephonyProperties.PROPERTY_INECM_MODE might return string
             * like: "true,false". In such case, the ecmMode checking would be wrong.
             * google original code:
                    SystemProperties.get(TelephonyProperties.PROPERTY_INECM_MODE);
             * @{
             */
                    TelephonyManager.getDefault().getTelephonyProperty(phone.getPhoneId(),
    ...
    public static PhoneAccountHandle makePstnPhoneAccountHandleWithPrefix(
        ...
        // M: fix CR:ALPS03217604,IccId by subscription info to get to ensure right iccId. @{
        // Original code:
        // String id = isEmergency ? EMERGENCY_ACCOUNT_HANDLE_ID : prefix +
        // String.valueOf(phone.getFullIccSerialNumber());
        /// @}
        String id = isEmergency ? EMERGENCY_ACCOUNT_HANDLE_ID : prefix
                + String.valueOf(getIccIdForPhone(phone));
        /// @}
    ...
    private static Phone getPhoneFromIccId(String iccId) {
        if (!TextUtils.isEmpty(iccId)) {
            for (Phone phone : PhoneFactory.getPhones()) {
                ...
                Log.d(LOG_TAG, "iccId: " + iccId + " compare to IccId: " + phoneIccId +
                        " from phone" + slotId);
                /// @}
     ...
     * @return subInfoList in turn
     */
    public static List<SubscriptionInfo> getActiveSubInfoList() {
        List<SubscriptionInfo> tmp = SubscriptionManager.
                 from(PhoneGlobals.getInstance()).getActiveSubscriptionInfoList();
        if (tmp == null) {
            return new ArrayList<SubscriptionInfo>();
        }
        List<SubscriptionInfo> subInfoList = new ArrayList<SubscriptionInfo>(tmp);
        if (DBG) {
            Log.d(LOG_TAG, "subInfoList size:" + subInfoList.size());
        }
        return subInfoList;
    }
     ...
     */
    public static String getSubDisplayName(int subId) {
        CharSequence displayName = "";
        SubscriptionInfo subInfo = SubscriptionManager
                .from(PhoneGlobals.getInstance()).getActiveSubscriptionInfo(subId);
        if (subInfo != null) {
    ...
    public static boolean isValidSubId(int subId) {
        if (DBG) {
            Log.d(LOG_TAG, "isValidSubId subId:" + subId);
        }
    ...
    public static Phone getPhoneUsingSubId(int subId) {
        if (DBG) {
            Log.d(LOG_TAG, "getPhoneUsingSubId subId:" + subId);
        }
     ...
     */
    public static final boolean hasPhoneType(int type) {
        for (int i = 0; i < TelephonyManager.getDefault().getPhoneCount(); i++) {
            if (PhoneFactory.getPhone(i).getPhoneType() == type) {
                return true;
            }
        }
        return false;
     ...
     */
    public static final void setRadioPower(boolean enabled) {
        for (Phone phone : PhoneFactory.getPhones()) {
    ...
    /**
     * M: fix CR:ALPS03220165,getIccId by subscription info to get. @{
     */
    public static String getIccIdForPhone(Phone phone) {
        SubscriptionInfo subInfo = SubscriptionManager
                .from(phone.getContext()).getSubscriptionInfo(phone.getSubId());
        if (subInfo != null) {
