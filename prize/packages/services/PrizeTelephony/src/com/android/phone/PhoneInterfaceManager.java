...
public class PhoneInterfaceManager extends ITelephony.Stub {
    ...
    private static final class IccAPDUArgument {
        ...
        public IccAPDUArgument(int channel, int cla, int command,
            ...
            int slot = SubscriptionManager
                    .getSlotId(SubscriptionManager.getDefaultSubscriptionId());
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case CMD_ANSWER_RINGING_CALL:
                    ...
                    /// M: CC: Wake up the requesting thread [ALPS02014935] @{
                    synchronized (request) {
                        request.result = "OK";
                        request.notifyAll();
                    }
                ...
                case CMD_END_CALL:
                    ...
                    if (phone == null) {
                        loge("CMD_END_CALL: no phone for id: " + end_subId);
                        // Wake up the requesting thread
                ...
                case EVENT_LOAD_EF_LINEARFIXEDALL_DONE:
                    log("CMD_LOAD_EF_LINEARFIXEDALL");
                    ar = (AsyncResult) msg.obj;
                    request = (MainThreadRequest) ar.userObj;
                    if (ar.exception == null && ar.result != null) {
                ...
                case CMD_EXCHANGE_SIM_IO_EX:
                    ...
                    if (uiccCard == null) {
                        ...
                        onCompleted = obtainMessage(EVENT_EXCHANGE_SIM_IO_EX_DONE,
                                request);
                        uiccCard.iccExchangeSimIOEx(iccArgument.cla, /* cla is fileID here! */
                                iccArgument.command, iccArgument.p1, iccArgument.p2, iccArgument.p3,
    ...
    public boolean setPreferredNetworkType(int subId, int networkType) {
        enforceModifyPermissionOrCarrierPrivilege(subId);
        //fix for ALPS02822059, set network type into provider before send request.
    ...
    public int getTetherApnRequired() {
         return getTetherApnRequiredForSubscriber(mSubscriptionController.getDefaultDataSubId());
    }
     ...
     */
    @Override
    public int getTetherApnRequiredForSubscriber(int subId) {
        enforceModifyPermission();
        ...
        int phoneId = mSubscriptionController.getPhoneId(subId);
        log("getTetherApnRequired: subId=" + subId + " phoneId=" + phoneId);
        Phone phone = PhoneFactory.getPhone(phoneId);
        if (phone != null) {
    ...
    public void setDataEnabled(int subId, boolean enable) {
        enforceModifyPermission();
        int phoneId = mSubscriptionController.getPhoneId(subId);
        if (DBG) log("setDataEnabled: subId=" + subId + " phoneId=" + phoneId);
        Phone phone = PhoneFactory.getPhone(phoneId);
        if (phone != null) {
    ...
    public void factoryReset(int subId) {
        ...
        try {
            if (SubscriptionManager.isUsableSubIdValue(subId) && !mUserManager.hasUserRestriction(
                ...
                // Enable data
                /// M: OP09A keep data switch status to ensure at most one data switch is on.
                boolean isOP09A =
                        OPERATOR_OP09.equals(SystemProperties.get("persist.operator.optr"))
                        && SEGDEFAULT.equals(SystemProperties.get("persist.operator.seg"));
                boolean isOP02 = "OP02".equals(SystemProperties.get("persist.operator.optr"));
                if (!isOP09A && !isOP02) {
                    boolean retVal = "true".equalsIgnoreCase(SystemProperties.get(
                            "ro.com.android.mobiledata", "true"));
                    setDataEnabled(subId, retVal);
                }
    ...
    public byte[] iccExchangeSimIOEx(int subId, int fileID, int command,
        ...
        IccIoResult response =
                (IccIoResult) sendRequest(CMD_EXCHANGE_SIM_IO_EX,
                        new IccAPDUArgument(-1, fileID, command,
