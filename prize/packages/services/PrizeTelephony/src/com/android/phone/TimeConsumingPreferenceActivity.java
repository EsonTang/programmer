...
public class TimeConsumingPreferenceActivity extends PreferenceActivity
    ...
    protected Dialog onCreateDialog(int id) {
        ...
        if (id == RESPONSE_ERROR || id == RADIO_OFF_ERROR || id == EXCEPTION_ERROR
            ...
            /// M:[CT VOLTE]
            // finish the activity if error encountered
            if (this instanceof CdmaCallWaitingUtOptions) {
                dialog.setOnDismissListener(new OnDismissListener() {
                    @Override
                    public void onDismiss(DialogInterface dialog) {
                        Log.d(LOG_TAG, "CdmaCallWaitingUtOptions error, dismiss...");
                        finish();
                    }
                });
            }
    ...
    public void onException(Preference preference, CommandException exception) {
        ...
        /// M: for Call barring change password @{
        } else if (exception.getCommandError() == CommandException.Error.PASSWORD_INCORRECT) {
