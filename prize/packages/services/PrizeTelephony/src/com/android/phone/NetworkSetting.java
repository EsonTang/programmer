...
public class NetworkSetting extends PreferenceActivity
    ...
    protected void onCreate(Bundle icicle) {
        ...
        startService(intent);
        /** M: Add subid to intent instand of Google default way @{
    ...
    protected void onDestroy() {
        //M:3352515 On pause may get fail, dismiss the dialog and cancel the network search
        cancelNetworkSearch();
        ...
        try {
            ...
            log("onDestroy: PhoneGlobals exception");
        }
    ...
    protected Dialog onCreateDialog(int id) {
        ...
        /// M: [PLMN search alert] @{
        if (id == DIALOG_ALERT_PLMN_SEARCH) {
            Builder builder = new AlertDialog.Builder(this);
            builder.setMessage(R.string.msg_plmn_search_alert);
            builder.setPositiveButton(android.R.string.yes, null);
    ...
    private void loadNetworksList() {
        ...
        /// M: [PLMN search alert] @{
        boolean showAlert = getResources().getBoolean(R.bool.config_show_plmn_search_alert_dialog);
        if (mIsForeground && showAlert) {
            if (DBG) {
                log("show PLMN search alert...");
            }
            showDialog(DIALOG_ALERT_PLMN_SEARCH);
        }
        /// @}
    ...
    private void networksListLoaded(List<OperatorInfo> result, int status) {
        ...
        /// M: [PLMN search alert]
        removeDialog(DIALOG_ALERT_PLMN_SEARCH);
        ...
        if (status != NetworkQueryService.QUERY_OK) {
            ...
            if (result != null){
                displayEmptyNetworkList(false);
                //M: 03386456 avoid display of 46020 2g plmn
                Iterator itr = result.iterator();
                while (itr.hasNext()) {
                    OperatorInfo opInfo = (OperatorInfo) itr.next();
                    String forbiddenNetwork = opInfo.getOperatorNumeric();
                    if (forbiddenNetwork.equals("46020")) {
                        itr.remove();
                    }
                }
    ...
    /// M: [PLMN search alert]
    // pop a dialog that the data may not work during PLMN searching
    ...
    private void newManuSelectFemetocellPreference(PreferenceScreen root) {
        if (PhoneFeatureConstants.FeatureOption.isMtkFemtoCellSupport() &&
            ...
            mManuSelectFemtocell = new Preference(this);
            mManuSelectFemtocell.setTitle(R.string.sum_search_femtocell_networks);
            mManuSelectFemtocell.setPersistent(false);
