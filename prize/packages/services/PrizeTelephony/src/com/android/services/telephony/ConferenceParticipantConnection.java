...
public class ConferenceParticipantConnection extends Connection {
    ...
    public ConferenceParticipantConnection(
        ...
        setCapabilities();
        /// M: ALPS02075564. @{
        /// CallLogManager in Telecom can't distinguish call disconnected and conference SRVCC, so
        /// it checks if VOLTE property is changed to know SRVCC happened.
        /// If it is belong to SRVCC case, callLogManager will log it as the same conference.
        /// The other purpose of VOLTE property is that inCallUI will show the participants'
        /// status if there is this property on the child calls.
    ...
    private void setCapabilities() {
        ...
        if (mParentConnection != null && mParentConnection.getCall() != null) {
            ...
            if (phone != null
                    && phone.isFeatureSupported(Phone.FeatureType.VOLTE_CONF_REMOVE_MEMBER)) {
    ...
    protected FormattedLog.Builder configDumpLogBuilder(FormattedLog.Builder builder) {
        ...
        return builder.setServiceName("Telephony");
    }
    /// @}
}
