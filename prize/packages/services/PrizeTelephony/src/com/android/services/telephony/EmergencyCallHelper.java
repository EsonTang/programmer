...
public class EmergencyCallHelper implements EmergencyCallStateListener.Callback {
    ...
    private static final String TAG = "ECCCallHelper";
    ...
    private void setupListeners() {
        ...
        for (int i = 0; i < TelephonyManager.getDefault().getPhoneCount(); i++) {
            mListeners.get(i).setContext(mContext);
        }
    }
    /**
    ...
    public void cleanup() {
        log("cleanup()");
        List<EmergencyCallStateListener> listeners
                = new ArrayList<EmergencyCallStateListener>(mInProgressListeners);
        for (EmergencyCallStateListener l : listeners) {
            l.cleanup();
        }
    }
    /// @}
