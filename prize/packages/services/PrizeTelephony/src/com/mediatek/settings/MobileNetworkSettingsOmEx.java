...
public class MobileNetworkSettingsOmEx {
    ...
    public MobileNetworkSettingsOmEx(Context context) {
        ...
        mContext = context;
        mEng = ENG.equals(SystemProperties.get("ro.build.type"));
        log("mContext =" + mContext + " mEng = " + mEng);
    }
    ...
    public void initMobileNetworkSettings(PreferenceActivity activity, int currentTab) {
        ...
        if (mPreferenceScreen.findPreference(ENABLE_4G_DATA) != null) {
            log("remove ENABLE_4G_DATA");
            mPreferenceScreen.removePreference(mPreferenceScreen.findPreference(ENABLE_4G_DATA));
        }
    ...
    private String getSimOperator(int slotId) {
       ...
       if (isSimStateReady) {
           if (isGeminiSupport()) {
               ...
               subId = subscriptionManager.getSubIdUsingPhoneId(slotId);
