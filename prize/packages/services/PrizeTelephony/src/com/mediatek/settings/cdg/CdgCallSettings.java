...
public class CdgCallSettings {
    ...
    public boolean onPreferenceTreeClick(
        ...
        if (preference == mCallWaiting || preference == mDoNotDisturb) {
            ...
            int[] fcList =  CdgUtils.getVoiceMessageRetrieveFc(subId);
            log("fcList = " + fcList);
            if (fcList != null && fcList.length > 0) {
                String fc = Integer.toString(fcList[0]);
    ...
    private void removeCdgUi(PreferenceActivity activity) {
        mCallForwarding = (PreferenceScreen) activity.findPreference(CDG_CALL_FORWARDING_KEY);
        if (mCallForwarding != null) {
            activity.getPreferenceScreen().removePreference(mCallForwarding);
        }

        mCallWaiting = (PreferenceScreen) activity.findPreference(CDG_CALL_WAITING_KEY);
        if (mCallWaiting != null) {
            activity.getPreferenceScreen().removePreference(mCallWaiting);
        }

        mDoNotDisturb = (PreferenceScreen) activity.findPreference(CDG_DO_NOT_DISTURB_KEY);
        if (mDoNotDisturb != null) {
            activity.getPreferenceScreen().removePreference(mDoNotDisturb);
        }

        mVoiceMessageRetrieve = (PreferenceScreen) activity.findPreference(
                CDG_VOICE_MESSAGE_RETRIEVE_KEY);
        if (mVoiceMessageRetrieve != null) {
            activity.getPreferenceScreen().removePreference(mVoiceMessageRetrieve);
        }

        mCustomizedEcc = (PreferenceScreen)activity.findPreference(CDG_CUSTOMIZED_ECC_KEY);
        if (mCustomizedEcc != null) {
            activity.getPreferenceScreen().removePreference(mCustomizedEcc);
        }
