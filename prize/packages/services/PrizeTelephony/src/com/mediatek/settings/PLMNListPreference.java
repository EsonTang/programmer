...
public class PLMNListPreference extends TimeConsumingPreferenceActivity
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference pref) {
        ...
        Log.d(LOG_TAG, "get order: " + pref.getOrder());

        NetworkInfoWithAcT info = mPLMNList.get(pref.getOrder());
        /// M: ALPS00541579
        if (null == info) {
            return false;
        }
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        if (PhoneUtils.isValidSubId(mSubId)) {
            ...
            isShouldEnabled = isIdle && TelephonyUtils.isRadioOn(mSubId, this);
        }
        if (menu != null) {
    ...
    private void refreshPreference(ArrayList<NetworkInfoWithAcT> list) {
        ...
        int order = 0;
        //M: 03368200 avoid display of 46020 2g plmn
        Iterator itr = list.iterator();
        while (itr.hasNext()) {
            NetworkInfoWithAcT nAct = (NetworkInfoWithAcT) itr.next();
            String forbiddenNetwork = nAct.getOperatorNumeric();
            if (forbiddenNetwork.equals("46020") || forbiddenNetwork.equals("40496")) {
                itr.remove();
            }
        }
        for (NetworkInfoWithAcT network : list) {
            addPLMNPreference(network, order++);
    ...
    private void addPLMNPreference(NetworkInfoWithAcT network, int order) {
    ...
    private void setScreenEnabled() {
        ...
        boolean isIdle = (TelephonyManager.getDefault().getCallState(
                mSubId) == TelephonyManager.CALL_STATE_IDLE);
        isShouldEnabled = isIdle && (!mAirplaneModeEnabled) &&
                TelephonyUtils.isRadioOn(mSubId, this);
        ...
        mPhoneStateListener = new PhoneStateListener(mSubId) {
            ...
            public void onServiceStateChanged(ServiceState serviceState) {
                if (mState != serviceState.getState()) {
                    Log.d(LOG_TAG, "onServiceStateChanged state is " + serviceState.getState());
