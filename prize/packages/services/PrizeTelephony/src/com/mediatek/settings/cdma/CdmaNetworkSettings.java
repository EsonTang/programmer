...
public class CdmaNetworkSettings {
    ...
    private void updateSwitch() {
        ...
        /// TODO: Temple solution for 4G data only @{
        if (FeatureOption.isMtkTddDataOnlySupport()) {
            /// 4G data only item should be disabled when Enable 4G item unchecked.
            // to align with CT SPEC, disable Single4GData if data is off
            boolean isDataEnabled =
                (SubscriptionManager.getDefaultDataSubscriptionId() == mPhone.getSubId())
                && TelephonyManager.getDefault().getDataEnabled();
            /// [CT VOLTE]
            boolean isCtVolteOn = TelephonyUtilsEx.isCtVolteEnabled()
                    && TelephonyUtilsEx.isCt4gSim(mPhone.getSubId())
                    && ImsManager.isEnhanced4gLteModeSettingEnabledByUser(mActivity);
            Log.d(TAG, "isDataEnabled = " + isDataEnabled + ", isCtVolteOn = " + isCtVolteOn);
            mEnableSigle4GDataPreference.setEnabled(checked && isDataEnabled
                    && !TelephonyUtilsEx.isCdmaRoaming(mPhone)&& !isCtVolteOn);
