...
public class CallerInfoLookupHelperTest extends TelecomTestCase {
    ...
    Bitmap mBitmap;
    int mSubId = SubscriptionManager.getDefaultVoiceSubscriptionId();
    ...
    public void setUp() throws Exception {
        ...
        when(mFactory.startQuery(anyInt(), eq(mContext), anyString(),
                any(CallerInfoAsyncQuery.OnQueryCompleteListener.class), any(), anyInt()))
    ...
    public void testLookupWithEmptyHandle() {
        ...
        mCallerInfoLookupHelper.startLookup(Uri.EMPTY, listener, mSubId);
    ...
    public void testSimpleLookup() {
        ...
        mCallerInfoLookupHelper.startLookup(URI1, listener, mSubId);
        ...
        verify(mFactory).startQuery(anyInt(), eq(mContext), eq(URI1.getSchemeSpecificPart()),
                queryListenerCaptor.capture(), logSessionCaptor.capture(), eq(mSubId));
    ...
    public void testLookupWithTwoListeners() {
        ...
        mCallerInfoLookupHelper.startLookup(URI1, callListener, mSubId);
        mCallerInfoLookupHelper.startLookup(URI1, otherListener, mSubId);
        waitForActionCompletion();
        ...
        verify(mFactory, times(1)).startQuery(anyInt(), eq(mContext),
                ...
                logSessionCaptor.capture(), eq(mSubId));
