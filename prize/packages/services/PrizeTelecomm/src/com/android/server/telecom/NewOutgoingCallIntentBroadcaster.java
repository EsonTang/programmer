            ...
            try {
                ...
                synchronized (mLock) {
                    ...
                    if (endEarly) {
                        if (mCall != null) {
                            /// M: [log optimize]
                            Log.disconnectEvent(mCall, "Call end early from " +
                    ...
                    Uri resultHandleUri = Uri.fromParts(
                            ...
                            resultNumber, null);
                    /// M: For VoLTE @{
                    // If ims only request, revert above changes of Uri via
                    // get original uri from intent.
                    if (TelecomVolteUtils.isImsCallOnlyRequest(intent)) {
                        resultHandleUri = TelecomVolteUtils.getHandleFromIntent(
                                intent, resultHandleUri);
                    }
                    /// @}
    ...
    public int processIntent() {
        ...
        if (isVoicemailNumber) {
            if (Intent.ACTION_CALL.equals(action)
                ...
                Log.d(this, "Placing call immediately instead of waiting for "
        ...
        if (Intent.ACTION_CALL.equals(action)) {
            if (isPotentialEmergencyNumber) {
                if (!mIsDefaultOrSystemPhoneApp) {
                    Log.d(this, "Cannot call potential emergency number %s with CALL Intent %s "
            ...
            if (!isPotentialEmergencyNumber) {
                Log.d(this, "Cannot call non-potential-emergency number %s with EMERGENCY_CALL "
