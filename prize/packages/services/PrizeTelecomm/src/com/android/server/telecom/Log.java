...
public class Log {
    ...
    public final static class Events {
        ...
        /**
         * M: Call control events, operations.
         */
        private static final Set<String> ccOpEvents =
                new HashSet<String>() {{
                    add(REQUEST_HOLD);
                    add(REQUEST_UNHOLD);
                    add(REQUEST_DISCONNECT);
                    add(REQUEST_ACCEPT);
                    add(REQUEST_REJECT);
                    add(CONFERENCE_WITH);
                    add(SWAP);
                    add(START_CONNECTION);
                    add(BIND_CS);
                }};

        /**
         * M: Call control events, notifications from FWK.
         */
        private static final Set<String> ccNotifyEvents =
                new HashSet<String>() {{
                    add(SET_CONNECTING);
                    add(SET_DIALING);
                    add(SET_ACTIVE);
                    add(SET_HOLD);
                    add(SET_RINGING);
                    add(SET_DISCONNECTED);
                    add(SET_DISCONNECTING);
                    add(SET_SELECT_PHONE_ACCOUNT);
                    add(CS_BOUND);
                    add(ADD_CHILD);
                    add(REMOVE_CHILD);
                    add(SET_PARENT);
    ...
    public static class CallEventRecord {
        ...
        public void addEvent(String event, String sessionId, Object data) {
            ...
            /// M: dump more call info for debug.
            String dumpInfo = dumpCallInfoForEvent(event, mCall);
            Log.i("Event", "Call %s: %s, %s, %s", mCall.getId(), event, dumpInfo, data);
        }
    ...
    private static final String PREF_EXTENDED_LOGGING = "pref_extended_logging";
    /// @}
    /// M: make it possible to enable trace by adb commands
    public static final boolean SYSTRACE_DEBUG =
            Trace.isTagEnabled(Trace.TRACE_TAG_PERF); /* STOP SHIP if true */
    ...
    public static void setIsExtendedLoggingEnabled(boolean isExtendedLoggingEnabled) {
        ...
        /// M: save in preference to make it continue working after reboot.
        setExtendedLoggingMode(isExtendedLoggingEnabled);
    }
    ...
    public static synchronized void startSession(String shortMethodName,
        ...
        if (Log.SYSTRACE_DEBUG) {
            Trace.traceBegin(Trace.TRACE_TAG_PERF, shortMethodName);
        }
    ...
    private static synchronized Session createSubsession(boolean isStartedFromActiveSession) {
        ...
        /**
         * M: [Log optimize] Don't need SUBSESSION log.
    ...
    public static synchronized void endSession() {
        ...
        /// M: [log optimize] parent sessions themselves will end here.
        /// Parent sessions and sub sessions shares the same endSession() method,
        /// so we have to check parent here to filter sub sessions. @{
        if (completedSession.getParentSession() == null) {
            if (!completedSession.getChildSessions().isEmpty()) {
                Log.v(LOGGING_TAG, "MARK_AS_COMPLETED, but still waiting for children complete: "
                        + completedSession.getChildSessions());
            }
            if (Log.SYSTRACE_DEBUG) {
                Trace.traceEnd(Trace.TRACE_TAG_PERF);
            }
        }
        /// @}
    ...
    public static void event(Call call, String event, Object data) {
        ...
        if (call == null) {
            Log.d(TAG, "Non-call EVENT: %s, %s", event, data);
            return;
        }
    ...
    public static boolean isLoggable(int level) {
        /**
         * M: Don't use google's log checking.
         return FORCE_LOGGING || android.util.Log.isLoggable(TAG, level);
         */
        if (FORCE_LOGGING || TextUtils.equals(Build.TYPE, "eng")) {
            return true;
        }
        /// M: else checking EM configurations for non-eng build
        boolean loggable;
        switch (level) {
            case android.util.Log.VERBOSE:
            case android.util.Log.DEBUG:
                loggable = (1 == SystemProperties.getInt(PROP_FORCE_DEBUG_KEY, 0));
                break;
    ...
    public static String piiHandle(Object pii) {
        if (pii == null || VERBOSE || mIsUserExtendedLoggingEnabled) {
    ...
    /**
     * M: dump more call information in the event log for better debugging.
     *
     * @param event the event string.
     * @param call the dumping call.
     * @return the debug log string.
     */
    private static String dumpCallInfoForEvent(String event, Call call) {
        StringBuilder sb = new StringBuilder();
        if (Events.ccOpEvents.contains(event)) {
            sb.append("[Debug][CC][Telecom][OP]");
        } else if (Events.ccNotifyEvents.contains(event)) {
            sb.append("[Debug][CC][Telecom][Notify]");
        }
        sb.append("[").append(event).append("]");
        if (DEBUG) {
            sb.append("[").append(TelecomUtils.getNumberFromCall(call)).append("]");
        }
        sb.append("[").append(call.getId()).append("]");
        if (call.isCdmaCall()) {
            sb.append("[CDMA]");
        }
        if (call.hasProperty(Connection.PROPERTY_VOLTE)) {
            sb.append("[VoLTE]");
        }
        ...
        sb.append("[")
                .append(VideoProfile.videoStateToString(call.getVideoState()))
                .append("]");
        return sb.toString();
    }

    public static void disconnectEvent(Call call, String msg) {
        event(call, Events.REQUEST_DISCONNECT, msg);
    }

    /**
     * M: If user reboot the phone, the extended logging should continue.
     * Call this method to enable it after TelecomSystem init.
     */
    public static void initExtendedLoggingState() {
        SharedPreferences preferences = TelecomUtils.getExtendedPreferences();
        if (preferences != null) {
            setIsExtendedLoggingEnabled(preferences.getBoolean(PREF_EXTENDED_LOGGING, false));
        }
    }

    /**
     * M: save current extended logging to shared preferences.
     * @param isInExtendedLoggingMode
     */
    private static void setExtendedLoggingMode(boolean isInExtendedLoggingMode) {
        SharedPreferences preferences = TelecomUtils.getExtendedPreferences();
        if (preferences != null) {
            SharedPreferences.Editor editor = preferences.edit();
            editor.putBoolean(PREF_EXTENDED_LOGGING, isInExtendedLoggingMode);
            editor.apply();
        }
    }
}
