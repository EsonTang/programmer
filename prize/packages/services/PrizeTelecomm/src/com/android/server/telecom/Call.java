...
public class Call implements CreateConnectionResponse {
    ...
    ///M: when system time changed more than this gap, calibrate the connection time
    public static final long GAP_FOR_TIME_CALIBRATE = 500;
    ...
    ///M: System clock elapsed real time when call was created
    private long mCreationClockElapsedMillis = SystemClock.elapsedRealtime();

    /** The time this call was made active. */
    private long mConnectTimeMillis = 0;
    ///M: System clock elapsed real time when call was made active
    private long mConnectClockElapsedMillis = 0;

    /** The time this call was disconnected. */
    private long mDisconnectTimeMillis = 0;
    ///M: System clock elapsed real time when call was disconnected
    private long mDisconnectClockElapsedMillis = 0;
    ...
    /**
     * M: Indicates whether the CDMA MO call accepted by remote
     */
    private boolean mIsAcceptedCdmaMoCall = false;

    /**
     * M: Indicates whether this call is a CDMA call.
     */
    private boolean mIsCdmaCall = false;

    /**
     * M: Indicates whether the call can't co-exist with other calls if it is a ViLTE call.
     * TODO: Should mark this behavior in PhoneAccount CAPABILITIES instead of
     * calculating here.
     */
    private boolean mAllowsSingletonViLTE = false;

    /**
     * M: Cache the subId of this call if it is a SIM call.
     */
    ...
    public void setState(int newState, String tag) {
        if (mState != newState) {
            ...
            if (mState == CallState.ACTIVE || mState == CallState.ON_HOLD) {
                ...
                /// M: For revising call duration
                mDisconnectClockElapsedMillis = 0;
                ...
                /// M: For revising call duration
                mDisconnectClockElapsedMillis = SystemClock.elapsedRealtime();
                setLocallyDisconnecting(false);
                fixParentAfterDisconnect();
                /// M: For VoLTE @{
                if (TelecomVolteUtils.isMmiWithDataOff(mDisconnectCause)) {
                    TelecomVolteUtils.showNoDataDialog(mContext, mTargetPhoneAccountHandle);
                    // Reset DisconnectCause for Mmi, see ALPS02032054.
                    setDisconnectCause(new DisconnectCause(DisconnectCause.OTHER));
                }
                /// @}
            }
    ...
    /*
     * Change from "package" to "Public" for accessing from other package, like LogUtils.java.
     */
    ...
    public String getName() {
        return mCallerInfo == null ? null :
            /// M: ALPS01813758, If name is empty return the phone number.
            // If the phone number is voice mail, the mCallerInfo.phoneNumber
            // will be set as voice mail rather than actual phone number.
            mCallerInfo.name == null ? mCallerInfo.phoneNumber : mCallerInfo.name;
    }
    ...
    public void setTargetPhoneAccount(PhoneAccountHandle accountHandle) {
        if (!Objects.equals(mTargetPhoneAccountHandle, accountHandle)) {
            ...
            mIsCdmaCall = TelecomUtils.isCdmaPhoneAccount(mTargetPhoneAccountHandle);
            /// M: TODO: Should mark this behavior in PhoneAccount CAPABILITIES instead of
            /// calculating here.
            mAllowsSingletonViLTE = TelecomUtils.supportsSingletonVilteCall(
                    mTargetPhoneAccountHandle);
            /// M: cache the subId
            mSubId = TelecomUtils.getSubId(mContext, mTargetPhoneAccountHandle);
            for (Listener l : mListeners) {
                l.onTargetPhoneAccountChanged(this);
            }
            configureIsWorkCall();
            checkIfVideoCapable();
            /// M: [ALPS03110733]The video state should be reconsidered
            /// according to the change of TargetPhoneAccount.
    ...
    ///M: add API to judge whether call is outgoing
    ...
    private void checkIfVideoCapable() {
        ...
        Log.v(this, "[checkIfVideoCapable]%s supports video call: %s",
                mTargetPhoneAccountHandle, mIsVideoCallingSupported);
    }
    ...
    public long getAgeMillis() {
        if (mState == CallState.DISCONNECTED &&
            ...
            return SystemClock.elapsedRealtime() - mConnectClockElapsedMillis;
            /// @}
        }

        /// M: Original code:
        // return mDisconnectTimeMillis - mConnectTimeMillis; @{
        return mDisconnectClockElapsedMillis - mConnectClockElapsedMillis;
        /// @}
    }
    ...
    public long getCreationTimeMillis() {
        ...
        return System.currentTimeMillis()
                - (SystemClock.elapsedRealtime() - mCreationClockElapsedMillis);
        /// @}
    }
    ...
    long getConnectTimeMillis() {
        ...
        long calculatedConnectionTime =
                mConnectTimeMillis == 0 ? 0 : (System.currentTimeMillis()
                - (SystemClock.elapsedRealtime() - mConnectClockElapsedMillis));
        if ((calculatedConnectionTime - mConnectTimeMillis) > GAP_FOR_TIME_CALIBRATE
                || (mConnectTimeMillis - calculatedConnectionTime) > GAP_FOR_TIME_CALIBRATE) {
            return calculatedConnectionTime;
        }
        return mConnectTimeMillis;
        /// @}
    }
    ...
    void setConnectionCapabilities(int connectionCapabilities, boolean forceUpdate) {
        ...
        if (forceUpdate || mConnectionCapabilities != connectionCapabilities) {
            ...
            // remove hold capability
            updateConnectionCapabilitesForVilte();
            /// @}
    ...
    public void handleCreateConferenceSuccess(
        ...
        Log.v(this, "handleCreateConferenceSuccess %s", conference);
    ...
    void playDtmfTone(char digit) {
        if (mConnectionService == null) {
            ...
            Log.d(this, "Send playDtmfTone to connection service for call %s", this);
    ...
    void stopDtmfTone() {
        if (mConnectionService == null) {
            Log.d(this, "stopDtmfTone() request on a call without a connection service.");
        } else {
            Log.d(this, "Send stopDtmfTone to connection service for call %s", this);
    ...
    public void disconnect(boolean wasViaNewOutgoingCallBroadcaster) {
        /**
         * M: [log optimize] need more info for disconnect.
    ...
    public Uri getContactUri() {
        Log.d(this, "[Debug]getContactUri()...mCallerInfo : " + mCallerInfo);
        if (mCallerInfo == null || !mCallerInfo.contactExists) {
            Log.d(this, "[Debug]getContactUri()...return handle.");
            return getHandle();
        }
    ...
    private void addChildCall(Call call) {
        if (!mChildCalls.contains(call)) {
            ...
            /// M: Record the number who actually connected in conference @{
            if (isConferenceDial() && call.getHandle() != null) {
                String participant = call.getHandle().getSchemeSpecificPart();
                if (!TextUtils.isEmpty(participant)) {
                    Log.d(this, "addChildCall, participant is %s", participant);
                    mActualConferenceParticipants.add(participant);
                }
            }
            /// @}
    ...
    boolean isRespondViaSmsCapable() {
        ...
        if (getHandle() == null || TextUtils.isEmpty(
                getHandle().getSchemeSpecificPart())) {
    ...
    private boolean isRinging(String actionName) {
        ...
        Log.d(this, "Request to %s a non-ringing call %s", actionName, this);
        return false;
    }
    ...
    private void startCallerInfoLookup() {
        mCallerInfo = null;
        ///M: To distinguish which phone's voicemail number, so need pass subId to query
    ...
    private void setCallerInfo(Uri handle, CallerInfo callerInfo) {
        ...
        ///M: check callerInfo is null or not firstly
        if (callerInfo == null) {
            Log.i(this, "setCallerInfo: callerInfo is null, do not update");
            return;
        }
    ...
    public void setVideoProvider(IVideoProvider videoProvider) {
        Log.v(this, "setVideoProvider: %s", videoProvider);
    ...
    public void setVideoState(int videoState) {
        ...
        Log.v(this, "setVideoState: %s", videoState);
        ...
        /// M: [ALPS03110733]The mIsVideoCallSupported would be the init value false before
        /// target PhoneAccount set. We shouldn't force change the video state to voice then.
        /// adding mTargetPhoneAccountHandle != null checking.
        if (mTargetPhoneAccountHandle != null && !isVideoCallingSupported()) {
            /// M: If force changed specified video state, should print warning log. @{
            if (videoState != VideoProfile.STATE_AUDIO_ONLY) {
                Log.w(this, "[setVideoState]Force change %s -> voice due to %s " +
                        "not support video calling", videoState, mTargetPhoneAccountHandle);
            /// @}
                videoState = VideoProfile.STATE_AUDIO_ONLY;
            }
        }
        ...
        /// M: don't update that much if no change
        ...
        /// M: For block certain ViLTE @{
        // trigger update hold capability when upgrade or downgrade.
        updateConnectionCapabilitesForVilte();
        /// @}
    ...
    public void setStatusHints(StatusHints statusHints) {
        /// M: don't update that much if no change.
        if (Objects.equals(mStatusHints, statusHints)) {
            return;
        }
    ...
    void hangupAll() {
        ...
        Log.d(this, "hangupAll %s", this);
        mConnectionService.hangupAll(this);
    }
    /// @}

    /// M: For volte @{
    // Used to restore pau information from Telephony.
    private String mPauField = "";

    /**
     * This function used to handle some VoLTE-related info from Telephony.
     * we use mIsEmergencyCall to record that the call has been marked as Ecc,
     * and use mPauField to record the pau information from Telephony.
     * @param bundle
     */
    public void updateVolteExtras(Bundle bundle) {
        Log.d(this, "updateVolteExtras()... bundle: " + bundle);
        if (bundle == null) {
            Log.d(this, "updateExtras()... bundle is null, return directly.");
            return;
        }
        TelecomVolteUtils.dumpVolteExtra(bundle);

        if (TelecomVolteUtils.isEmergencyCallChanged(bundle, mIsEmergencyCall)) {
            mIsEmergencyCall = !mIsEmergencyCall;
            // trigger InCallController.updateCall();
            for (Listener l : mListeners) {
                l.onExtraUpdated(this);
            }
        }
    }
    ...
    public void inviteConferenceParticipants(List<String> numbers) {
        if (mConnectionService == null) {
            Log.w(this, "inviteConferenceParticipants()... without a connection service");
        } else {
            if (numbers != null) {
                Log.d(this, "inviteConferenceParticipants()... size: " + numbers.size());
            }
            mConnectionService.inviteConferenceParticipants(this, numbers);
        }
    }

    // ------------below are for conference dial & conference invite----------------
    // Indicate the request will be dialed to conference server directly.
    private boolean mIsConferenceDial = false;
    // Indicate participants (by number) who were desired to be invited into the conference.
    private List<String> mConferenceParticipants = new ArrayList<String>();
    // Indicate participants (by number) who were added into the conference.
    private List<String> mActualConferenceParticipants = new ArrayList<String>();

    public void setIsConferenceDial(boolean isConferenceDial) {
        mIsConferenceDial = isConferenceDial;
        // Also mark the call as conference call if it is a conference dial.
        if (isConferenceDial) {
            mIsConference = isConferenceDial;
        }
    }

    public boolean isConferenceDial() {
        return mIsConferenceDial;
    }

    public void setConferenceParticipants(List<String> numbers) {
        mConferenceParticipants.clear();
        if (numbers != null) {
            mConferenceParticipants.addAll(numbers);
        }
    }
    ...
    /**
     * M: Get the unconnected participant numbers
     * @return the unconnected participant numbers
     */
    public List<String> getUnconnectedParticipants() {
        List<String> participants = new ArrayList<String>();
        for (String strSrc : mConferenceParticipants) {
            boolean find = false;
            for (String strDes : mActualConferenceParticipants) {
                if (strSrc.equals(strDes) || PhoneNumberUtils.compare(strSrc, strDes)) {
                    find = true;
                    break;
                }
            }
            if (!find) {
                Log.d(this, "getUnconnectedParticipants, participant = " + strSrc);
                participants.add(strSrc);
            }
        }
        return participants;
    }

    // Indicate the MT call is from conference server, which need a different UI from nornal MT.
    private boolean mIsIncomingFromConfServer = false;

    public void setIsIncomingFromConfServer(boolean isIncomingFromConfServer) {
        mIsIncomingFromConfServer = isIncomingFromConfServer;
    }

    public boolean isIncomingFromConfServer() {
        return mIsIncomingFromConfServer;
    }
    /// @}

    ///M: For Ip dial @ {
    private boolean mIsIpCall = false;

    public void setIsIpCall(boolean ipCall) {
        mIsIpCall = ipCall;
    }
    ...
    /**
     * Wrapper function, used to make getTargetPhoneAccount() visible for other packages.
     * @return
     */
    public PhoneAccountHandle getTargetPhoneAccountEx() {
        return getTargetPhoneAccount();
    }
    /// @}

    /// M: Add for volte conference call callog. @{
    private long mConferenceCallLogId = 0;

    public long getConferenceCallLogId() {
        return mConferenceCallLogId;
    }

    public void setConferenceCallLogId(long conferenceCallLogId) {
        mConferenceCallLogId = conferenceCallLogId;
    }
    /// @}

    ///M: add for 3G VT @{
    public boolean isVideoCall(){
        Log.d(this, "isVideoCall = " + !VideoProfile.isAudioOnly(getVideoState()));
        return !VideoProfile.isAudioOnly(getVideoState());
    }
    /// @}

    /// M: for log parser @{
    /*
     * get id in ConnectionServiceWrapper
     */
    public String getCallIdEx() {
        String callId = "";
        if (mConnectionService != null) {
            CallIdMapper callIdMapper = mConnectionService.getCallIdMap();
            if (callIdMapper != null) {
                callId = callIdMapper.getCallId(this);
            }
        }
        return callId;
    }
    ...
    /// M: add for diff 3G and 4G VT @{
    /*
     * isVideoCall() will always be true under 3G/4G VT
     * add Capability_VOLTE to diff 3G or 4G VT
     */
    public boolean is3GVideoCall() {
        boolean is3GVTCall = false;

        if (isVideoCall() & (!hasProperty(Connection.PROPERTY_VOLTE))) {
            is3GVTCall = true;
        }

        Log.i(this, "is3GVTCall = " + is3GVTCall);
        return is3GVTCall;
    }
    /// @}
    ...
    public void setIsHeld(boolean held) {
        if (mIsHeld != held) {
            mIsHeld = held;
            // trigger notify to UI, here use callback of onExtrasChanged().
            for (Listener l : mListeners) {
                l.onExtrasChanged(this, SOURCE_CONNECTION_SERVICE, new Bundle());
            }
        }
    }
    ...
    /// M: For block certain ViLTE @{
    // remove hold capability
    private int mConnectionCapabilitiesRecord;

    /**
     * To record the capabilities from Telephony.
     * @param capabilities
     */
    public void setCapabilitiesRecord(int capabilities) {
        mConnectionCapabilitiesRecord = capabilities;
    }

    /**
     * If the call is ViLTE call under certain NW, remove CAPABILITY_HOLD.
     * If is VoLTE downgraded from ViLTE under certain NW, add CAPABILITY_HOLD back.
     */
    private void updateConnectionCapabilitesForVilte() {
        int capabilities = mConnectionCapabilities;
        if (allowsSingletonViLTE()) {
            if (isVideoCall() && (capabilities & Connection.CAPABILITY_HOLD) != 0) {
                Log.v(this, "updateConnectionCapabilites()...Remove CAPABILITY_HOLD!");
                capabilities &= ~Connection.CAPABILITY_HOLD;
            }
            if (!isVideoCall() && (capabilities & Connection.CAPABILITY_HOLD) == 0
                    && (mConnectionCapabilitiesRecord & Connection.CAPABILITY_HOLD) != 0) {
                // Maybe downgrade trigger this.
                Log.v(this, "updateConnectionCapabilites()...Add CAPABILITY_HOLD back!");
                capabilities |= Connection.CAPABILITY_HOLD;
            }
        }
    ...
    public void onConnectionEvent(String event, Bundle extras) {
        ...
        /// M: update remote side hold state @{
        } else if (TelecomManagerEx.EVENT_ON_REMOTE_HOLD.equals(event)) {
            setIsHeld(true);
        } else if (TelecomManagerEx.EVENT_ON_REMOTE_RESUME.equals(event)) {
            setIsHeld(false);
        /// @}
        } else {
            for (Listener l : mListeners) {
     ...
     * outgoing? or unhold.
     */
    void disconnect(String pendingCallAction) {
        disconnect(false, pendingCallAction);
    }

    /**
     * M: For DSDA.
     * Attempts to disconnect the call through the connection service, with pending call action.
     */
    void disconnect(boolean wasViaNewOutgoingCallBroadcaster, String pendingCallAction) {
        // Track that the call is now locally disconnecting.
        setLocallyDisconnecting(true);
        ...
        if (mState == CallState.NEW || mState == CallState.SELECT_PHONE_ACCOUNT ||
            ...
            if (mConnectionService == null) {
                ...
                // association between call and connection service severed, see
                // {@link CallsManager#markCallAsDisconnected}.
                mConnectionService.disconnect(this, pendingCallAction);
            }
        }
    }

    /// M: get subId of the call phone account @{
    public int getSubId() {
        return mSubId;
    }
    /// @}

    ///M: For CDMA MO call goes to active state before the call is really answered,
    // add APIs for CDMA call to set/judge whether a call is a CDMA dialing call@{
    public void setIsAcceptedCdmaMoCall(boolean isAcceptedCdmaMo) {
        mIsAcceptedCdmaMoCall = isAcceptedCdmaMo;
    }

    public boolean isCdmaDialingCall() {
        return mState == CallState.ACTIVE && !isIncoming()
                && mChildCalls.size() == 0
                && !mIsAcceptedCdmaMoCall
                && isCdmaCall()
                && !hasProperty(Connection.PROPERTY_VOLTE);
    }

    public boolean isCdmaCall() {
        return mIsCdmaCall;
    }
    /// @}

    public boolean allowsSingletonViLTE() {
        return mAllowsSingletonViLTE;
    }
}
