...
public class BluetoothPhoneServiceImpl {
    ...
    public final IBluetoothHeadsetPhone.Stub mBinder = new IBluetoothHeadsetPhone.Stub() {
        ...
        public boolean answerCall() throws RemoteException {
            synchronized (mLock) {
                ...
                try {
                    ...
                    if (call != null) {
                        // M: fix CR:ALPS03122393,can not answer 3GVT call by Bluetooth.
                        // (3GVT can not downgrade to voice call).
                        if (call.is3GVideoCall()) {
        ...
        public boolean hangupCall() throws RemoteException {
            synchronized (mLock) {
                ...
                try {
                    ...
                    if (call != null) {
                        /// M: [log optimize]
                        Log.disconnectEvent(call, "BluetoothPhoneServiceImpl.hangupCall: " +
                                "BT hangup foreground call");
                        mCallsManager.disconnectCall(call);
                        return true;
                    }
                    return false;
        ...
        /// M: MSMA @{
        @Override
        public boolean restrictDualTalk() throws RemoteException {
            synchronized (mLock) {
                enforceModifyPermission();

                long token = Binder.clearCallingIdentity();
                try {
                    Log.i(TAG, "restrictDualTalk");
                    if (TelecomUtils.isInDsdaMode()) {
                        return restrictMsma(false);
                    }
                    return true;
                } finally {
                    Binder.restoreCallingIdentity(token);
    ...
    public CallsManagerListener mCallsManagerListener = new CallsManagerListenerBase() {
        ...
        public void onCallStateChanged(Call call, int oldState, int newState) {
            ...
            /// M: marked google code for MTK modifications.
            ...
            /// M: If a call is being put on hold but still have pending call action, ignore this
            // temp HOLD status. Such as answer a call in 1A1W, start a new outgoing call when have
            // 1A or swap call in 1A1H.
            // When the second action finished, we will then send out the aggregated update. @{
            ...
            if (oldState == CallState.ACTIVE && newState == CallState.ON_HOLD) {
                for (Call otherCall : mCallsManager.getCalls()) {
                    ...
                    if (otherCall.getState() == CallState.CONNECTING
                            || otherCall.getState() == CallState.RINGING
                            || otherCall.getState() == CallState.ON_HOLD) {
                        return;
                    }
                }
            }
        ...
        public void onIncomingCallRejected(Call call,
                boolean rejectWithMessage, String textMessage) {
            mRingingAddress = null;
            mRingingAddressType = 0;
            Call ringingCall = mCallsManager.getFirstCallWithState(call, CallState.RINGING);
    ...
    private boolean processChld(int chld) {
        ...
        if (chld == CHLD_TYPE_RELEASEHELD) {
            if (ringingCall != null) {
                ...
                /// M: [log optimize]
                Log.disconnectEvent(heldCall,
                        "BluetoothPhoneServiceImpl.processChld: CHLD_TYPE_RELEASEHELD");
            ...
            if (activeCall != null) {
                /**
                 * M: Disconnect active call then answerCall or unHoldCall will make
                 *    telephony get into wrong state. Original code:
                ...
                if (ringingCall != null) {
                    ...
                    /// M: [log optimize]
                    Log.disconnectEvent(activeCall,
                            "BluetoothPhoneServiceImpl.processChld: " +
                                    "CHLD_TYPE_RELEASEACTIVE_ACCEPTHELD");
                    /// M: Disconnect active call will unhold the hold call by default.
                    mCallsManager.disconnectCall(activeCall);
                }
                /** @} */
                return true;
            /// M: If outgoing call exists, disconnect outgoing and answer background call. @{
                ...
                if (ringingCall != null) {
                    ...
                    /// M: [log optimize]
                    Log.disconnectEvent(outgoingCall,
                            "BluetoothPhoneServiceImpl.processChld: " +
                                    "CHLD_TYPE_RELEASEACTIVE_ACCEPTHELD");
                    /// M: Disconnect outgoing call will unhold the hold call by default.
                    mCallsManager.disconnectCall(outgoingCall);
                }
            /// M: If no active/outgoing call exists, answer waiting call/unhold holding call.@{
            } else {
                if (ringingCall != null) {
                    mCallsManager.answerCall(ringingCall, ringingCall.getVideoState());
                } else if (heldCall != null) {
    ...
    private void updateHeadsetWithCallState(boolean force) {
        ...
        if (isHfpConnected() && restrictMsma(true)) {
            Log.i(TAG, "updateHeadsetWithCallStateForMsma: return for restriction");
            return;
        }
        /// @}
        ...
        if (mBluetoothHeadset != null &&
            ...
            /// M: It seems that BT side can not handle null. @{
            if (mRingingAddress == null) {
                mRingingAddress = "";
            }
            /// @}
    ...
    ///M : MSMA @{
    /**
     * To judge if currently HFP is connected.
     */
    private boolean isHfpConnected() {
        ...
        Log.i(TAG, "isHfpConnected :" + isConnected);
        return isConnected;
    }
    /// @}

    /// M: restrict MSMA scenarios for HFP device @{
    private boolean restrictMsma(boolean hfpConnected) {
        Log.d(TAG, "restrictMsma > hfpConneccted:" + hfpConnected);

        boolean executed = false;

        // check if there is ringing call
        Call ringingCall = mCallsManager.getRingingCall();

        if (ringingCall != null) {
            //(I,I) restriction
            if (restrictIIIfNeeded(hfpConnected)) {
                executed = true;
            }
        }
        Log.d(TAG, "[restrictMsma] :" + executed);

        return executed;
    }

    /**
     * More than one incoming call is not reasonable to HFP.
     * So we try to detect if there are more than one incoming call
     * if HFP is just connected, hang up the background incoming call
     * if HFP is already connected, hang up the new incoming call
     */
    private boolean restrictIIIfNeeded(boolean hfpConnected) {
        Log.i(TAG, "restrictIIIfNeeded");
        List<Call> ringCallList = mCallsManager.getCallsWithStates(CallState.RINGING);

        if (ringCallList.size() <= 1) {
            return false;
        }

        Call ringingCall = mCallsManager.getRingingCall();
        Call callToReject = null;

        if (hfpConnected) {
            // when HFP already connected, reject the later incoming call
            for (Call rCall : ringCallList) {
                if (rCall != ringingCall) {

                    if (rCall.getAgeMillis() < ringingCall.getAgeMillis()) {
                        callToReject = rCall;
                    } else {
                        callToReject = ringingCall;
                        ringingCall = rCall;
                    }
                    break;
                }
            ...
            // when HFP just connected, reject the background incoming call
            // the foreground incoming call will be put in the front of the list
            List<Call> sortedInComingCallList = mCallsManager.getSortedInComingCallList();
            if (sortedInComingCallList != null && sortedInComingCallList.size() > 1) {
                callToReject = sortedInComingCallList.get(sortedInComingCallList.size() - 1);
                ringingCall = sortedInComingCallList.get(sortedInComingCallList.size() - 2);
            }
        }

        if (callToReject != null) {
            Log.i(TAG, "(I,I) conflict existed :" + callToReject);
            // reject incoming call with cause : MISSED
            callToReject.setDisconnectCause(new DisconnectCause(DisconnectCause.MISSED));
            mCallsManager.rejectCall(callToReject, false, null);
