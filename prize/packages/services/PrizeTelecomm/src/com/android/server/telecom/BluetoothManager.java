...
public class BluetoothManager {
    ...
    private void updateListenerOfBluetoothState(boolean canBePending) {
        ...
        Log.d(this, "mBluetoothState = " + mBluetoothState + " newState = " + newState);
        ...
        ///M: ALPS02782536 @{
        //bt audio connected, send bluetoothstatechange to CallAudioRoutePeripheralAdapter
        //to avoid audio path is not the same as UI icon
        } else if (newState == BLUETOOTH_AUDIO_CONNECTED
                && mBluetoothState == BLUETOOTH_AUDIO_CONNECTED) {
            Log.d(this, "force to update bluetooth state change");
            mBluetoothStateListener.onBluetoothStateChange(mBluetoothState, newState);
        }
        /// @}
    }
    ...
    public void connectBluetoothAudio() {
        ...
        /// M: Google timing issue: mBluetoothHeadset.connectAudio() -> setBluetoothStatePending().
        // Some times we receive confirmation of connectAudio() before setBluetoothStatePending().
        // Then will set mBluetoothState back as DEVICE_CONNECT in updateListenerOfBluetoothState(),
        // Which will trigger disconnectBluetoothAudio() in CallAudioRoutePeripheralAdapter.
        // (BLUETOOTH_AUDIO_PENDING -> BLUETOOTH_DEVICE_CONNECT). see ALPS03070980.
        // We change it as: setBluetoothStatePending() -> mBluetoothHeadset.connectAudio().
        if (mBluetoothHeadset != null) {
            if (!mBluetoothHeadset.connectAudio()) {
    ...
    private void setBluetoothStatePending() {
        Log.v(this, "setBluetoothStatePending()...");
