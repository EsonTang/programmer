...
public class VideoProviderProxy extends Connection.VideoProvider {
    ...
    private final class VideoCallListenerBinder extends IVideoCallback.Stub {
        ...
        public void changePeerDimensionsWithAngle(int width, int height, int rotation) {
            synchronized (mLock) {
                logFromVideoProvider("changePeerDimensionsWithAngle: width=" + width + " height=" +
                        height + " rotation=" + rotation);
                VideoProviderProxy.this.changePeerDimensionsWithAngle(width, height, rotation);
            }
        }
        /* M: ViLTE part end */
    ...
    public void onSetUIMode(int mode){
        synchronized (mLock) {
            logFromInCall("onSetUIMode: " + mode);
            try {
                mConectionServiceVideoProvider.setUIMode(mode);
            } catch (RemoteException e) {
            }
        }
    }
