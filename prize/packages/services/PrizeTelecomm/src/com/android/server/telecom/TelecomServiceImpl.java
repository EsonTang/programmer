    ...
    private final ITelecomService.Stub mBinderImpl = new ITelecomService.Stub() {
        ...
        public PhoneAccount getPhoneAccount(PhoneAccountHandle accountHandle) {
            ...
            synchronized (mLock) {
                ...
                try {
                    /**
                     * Move out of the lock to get more precise timing.
                    Log.startSession("TSI.gPA");
                     */
        ...
        public List<PhoneAccount> getAllPhoneAccounts() {
            ...
            synchronized (mLock) {
                ...
                try {
                    /**
                     * M: move it out of the lock for more precise timing.
                    Log.startSession("TSI.gAPA");
                     */
        ...
        public boolean handlePinMmiForPhoneAccount(PhoneAccountHandle accountHandle,
            ...
            try {
                ...
                try {
                    ...
                    retval = getTelephonyManager().handlePinMmiForSubscriber(
                            subId, dialString);
                } finally {
                    Binder.restoreCallingIdentity(token);
                }
        ...
        public void placeCall(Uri handle, Bundle extras, String callingPackage) {
            try {
                ...
                synchronized (mLock) {
                    ...
                    if (TelecomVolteUtils.isConferenceDialRequest(extras)) {
                        if (!TelecomUtils.checkCallingCtaPermission(
                                com.mediatek.Manifest.permission.CTA_CONFERENCE_CALL,
                                "Place conference directly")) {
                            Log.w(this, "[placeCall]Failed to start conference, no permission");
                            TelecomUtils.showToast(
                                    com.mediatek.R.string.denied_required_permission);
                            return;
                        }
                    }
                    /// @}
        ...
        public List<PhoneAccountHandle> getVolteCallCapablePhoneAccounts() {
            synchronized (mLock) {
                try {
                    return mPhoneAccountRegistrar.getVolteCallCapablePhoneAccounts();
                } catch (Exception e) {
                    Log.e(this, e, "getVolteCallCapablePhoneAccounts");
                    throw e;
                }
            }
        }
        ...
        public List<PhoneAccountHandle> getVideoCallCapablePhoneAccounts() {
            synchronized (mLock) {
                try {
                    return mPhoneAccountRegistrar.getVideoCallCapablePhoneAccounts();
                } catch (Exception e) {
                    Log.e(this, e, "getVolteCallCapablePhoneAccounts");
                    throw e;
                }
        ...
        /// @}
        /**
         * M: return if current has live ongoing video call or not;
         * @see android.telecom.TelecomManager#isInVideoCall
         */
        @Override
        public boolean isInVideoCall(String callingPackage) {
            try {
                Log.startSession("TSI.iIVC");
                if (!canReadPhoneState(callingPackage, "isInVideoCall")) {
                    return false;
                }
                synchronized (mLock) {
    ...
    private boolean endCallInternal() {
        ...
        if (call != null) {
            if (call.getState() == CallState.RINGING) {
                ...
                /// M: [log optimize]
                Log.disconnectEvent(call, "TelecomServiceImpl.endCallInternal: " +
                        "called from TelecomManager");
