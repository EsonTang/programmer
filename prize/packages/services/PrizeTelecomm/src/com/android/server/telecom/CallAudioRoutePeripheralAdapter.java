            ...
            case BluetoothManager.BLUETOOTH_AUDIO_PENDING:
                switch (newState) {
                    case BluetoothManager.BLUETOOTH_DISCONNECTED:
                        ...
                        break;
                    case BluetoothManager.BLUETOOTH_DEVICE_CONNECTED:
                        if (oldState == BluetoothManager.BLUETOOTH_AUDIO_PENDING) {
                            Log.d(this, "return, because there has pending connect audio");
                            return;
                        }
                        mCallAudioRouteStateMachine.sendMessageWithSessionInfo(
                        ...
                        break;
                    ///M: ALPS02782536 @{
                    // send Switch_Bluetooth message to CallAudioRouteStateMachine
                    // to notify InCallUI refresh audio path icon
                    case BluetoothManager.BLUETOOTH_AUDIO_CONNECTED:
                        Log.d(this, "force to switch_bt to refresh UI icon");
                        mCallAudioRouteStateMachine.sendMessageWithSessionInfo(
                                CallAudioRouteStateMachine.SWITCH_BLUETOOTH);
                        break;
                    /// @}
