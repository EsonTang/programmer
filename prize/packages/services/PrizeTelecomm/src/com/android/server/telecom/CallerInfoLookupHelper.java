...
public class CallerInfoLookupHelper {
    ...
    public void startLookup(final Uri handle, OnQueryCompleteListener listener, final int subId) {
        if (handle == null) {
        ...
        Log.d(this, "[startLookup]lookup " + Log.piiHandle(handle) + " for listener: " + listener);
        synchronized (mLock) {
            if (mQueryEntries.containsKey(handle)) {
                ...
                Log.d(this, "[startLookup]record new query for " + Log.piiHandle(handle));
        ...
        mHandler.post(new Runnable("CILH.sL", mLock) {
            ...
            public void loggedRun() {
                Log.v(CallerInfoLookupHelper.this, "[logedRun]startLookup");
                Session continuedSession = Log.createSubsession();
                try {
                    ///M: To distinguish which phone's voicemail number, pass subId to query
                    CallerInfoAsyncQuery query = mCallerInfoAsyncQueryFactory.startQuery(
                            0, mContext, number,
                            makeCallerInfoQueryListener(handle), continuedSession, subId);
                    if (query == null) {
                ...
                try {
                    if (mQueryEntries.containsKey(handle)) {
                        ...
                        for (OnQueryCompleteListener l : info.listeners) {
                            Log.d(CallerInfoLookupHelper.this, "[onQueryComplete]notifying: " + l);
    ...
    private void startPhotoLookup(final Uri handle, final Uri contactPhotoUri) {
        mHandler.post(new Runnable("CILH.sPL", mLock) {
            @Override
            public void loggedRun() {
                Log.v(CallerInfoLookupHelper.this, "[loggedRun]startPhotoLookup");
                Session continuedSession = Log.createSubsession();
                try {
            ...
            synchronized (mLock) {
                ...
                try {
                    if (mQueryEntries.containsKey(handle)) {
                        ...
                        Log.v(CallerInfoLookupHelper.this, "[onImageLoadComplete]Photo get," +
                                " remove record for " + Log.piiHandle(handle));
                        mQueryEntries.remove(handle);
                    } else {
