...
public class ParcelableCallUtils {
    ...
    public static ParcelableCall toParcelableCall(
        ...
        /// M: For VoLTE conference invite @{
        // original code:
        // if (call.isConference()) {
        if (call.isConference() || call.isIncomingFromConfServer()) {
        /// @}
        ...
        /// M: For ViLTE @{
        if (call.isHeld()) {
            properties |= android.telecom.Call.Details.PROPERTY_HELD;
        }
        /// @}

        /// M: For phone record @{
        if (PhoneRecorderHandler.shouldAppendOrRemovePhoneRecordCapability(call)) {
            capabilities |= android.telecom.Call.Details.CAPABILITY_VOICE_RECORD;
        } else {
            capabilities &= ~android.telecom.Call.Details.CAPABILITY_VOICE_RECORD;
        }
        /// @}
       ...
       // if (call.isRespondViaSmsCapable() && isDefaultSmsAccount) {
        if (call.isRespondViaSmsCapable()) { //wangyunhe modify 20161121
        ...
        /// M: For VoLTE @{
        // If conference do have connect time, use it instead of child's connect time.
        // consider SRVCC, child's connect time will be from 0 again.
        if (!childCalls.isEmpty() && call.getConnectTimeMillis() > 0) {
            connectTimeMillis = call.getConnectTimeMillis();
        }
        /// @}
    ...
    private static final int[] CONNECTION_TO_CALL_CAPABILITY = new int[] {
        ...
        android.telecom.Call.Details.CAPABILITY_VOICE_RECORD,

        Connection.CAPABILITY_ECT,
        android.telecom.Call.Details.CAPABILITY_ECT,
        /* M: CC part end */

        /// M: For VoLTE @{
        Connection.CAPABILITY_INVITE_PARTICIPANTS,
        android.telecom.Call.Details.CAPABILITY_INVITE_PARTICIPANTS,
        /// @}

       /// M: For blind/assured ECT @{
        Connection.CAPABILITY_BLIND_ASSURED_ECT,
        android.telecom.Call.Details.CAPABILITY_BLIND_ASSURED_ECT,
        /// @}

        /// M: For video ringtone @{
        Connection.CAPABILITY_VIDEO_RINGTONE,
