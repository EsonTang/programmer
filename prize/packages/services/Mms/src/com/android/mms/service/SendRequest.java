...
public class SendRequest extends MmsRequest {
    ...
    protected Uri persistIfRequired(Context context, int result, byte[] response) {
        ...
        if (response != null && response.length > 0) {
            GenericPdu sendConfPdu = (new PduParser(response, false)).parse();
            if (sendConfPdu != null
                    && sendConfPdu instanceof SendConf
                    && ((SendConf)sendConfPdu).getMessageId() != null) {
                LogUtil.d(requestId, "persistIfRequired: messageId = "
                        + new String(((SendConf)sendConfPdu).getMessageId()));
            }
        }
    ...
    /**
     * Update the sender Address of MO MMS before sending.
     */
    private void updateSenderAddress(final GenericPdu pdu) {
        final String requestId = getRequestId();
        if (pdu == null) {
        ...
        try {
            ...
            if (pdu instanceof SendReq) {
                String selfNumber = telephonyManager.getLine1Number(mSubId);
                EncodedStringValue from = null;
                if (selfNumber != null && !selfNumber.isEmpty()) {
                    from = new EncodedStringValue(selfNumber);
                    LogUtil.w(requestId, "updateSenderAddress set from: " + selfNumber);
                    pdu.setFrom(from);
