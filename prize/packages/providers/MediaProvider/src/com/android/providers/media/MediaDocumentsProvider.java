...
public class MediaDocumentsProvider extends DocumentsProvider {
    ...
    private static final String AUDIO_MIME_TYPES = joinNewline(
            "audio/*", "application/ogg", "application/x-flac", "application/x-ogg");
    ...
    /// M: Add to support OMA DRM {@
    private static final int DRM_METHOD_SD = 1 << 2;
    /// @}
    ...
    private void includeImage(MatrixCursor result, Cursor cursor) {
        ...
        /// M: add to support drm
        row.add(MediaStore.MediaColumns.DATA, cursor.getString(ImageQuery.DATA));
        row.add(MediaStore.MediaColumns.IS_DRM, cursor.getInt(ImageQuery.IS_DRM));
        row.add(MediaStore.MediaColumns.DRM_METHOD, cursor.getInt(ImageQuery.DRM_METHOD));
        row.add(Document.COLUMN_FLAGS,
    ...
    private void includeVideo(MatrixCursor result, Cursor cursor) {
        ...
        /// M: add to support drm
        row.add(MediaStore.MediaColumns.DATA, cursor.getString(VideoQuery.DATA));
        row.add(MediaStore.MediaColumns.IS_DRM, cursor.getInt(VideoQuery.IS_DRM));
        row.add(MediaStore.MediaColumns.DRM_METHOD, cursor.getInt(VideoQuery.DRM_METHOD));
        row.add(Document.COLUMN_FLAGS,
                Document.FLAG_SUPPORTS_THUMBNAIL | Document.FLAG_SUPPORTS_DELETE);
    }
    ...
    private void includeAudio(MatrixCursor result, Cursor cursor) {
        ...
        /// M: add to support drm
        row.add(MediaStore.MediaColumns.DATA, cursor.getString(SongQuery.DATA));
        row.add(MediaStore.MediaColumns.IS_DRM, cursor.getInt(SongQuery.IS_DRM));
        row.add(MediaStore.MediaColumns.DRM_METHOD, cursor.getInt(SongQuery.DRM_METHOD));
    ...
    private long getImageForBucketCleared(long bucketId) throws FileNotFoundException {
        ...
        try {
            ...
            /// M: add to support drm, only show METHOD_SD type drm image thumbnail. {@
            if (cursor != null && cursor.getCount() > 0) {
                cursor.moveToPosition(-1);
                while (cursor.moveToNext()) {
                    boolean isDrm = cursor.getInt(ImagesBucketThumbnailQuery.IS_DRM) > 0;
                    if (!isDrm || (isDrm && cursor.getInt(ImagesBucketThumbnailQuery.DRM_METHOD)
                            == DRM_METHOD_SD)) {
    ...
    private AssetFileDescriptor openOrCreateImageThumbnailCleared(
        ...
        /// M: When thumbnail record in database but realy file not exsit, we may need try get it again.{@
        ParcelFileDescriptor pfd = null;
        try {
            pfd = openImageThumbnailCleared(id, signal);
        } catch (FileNotFoundException e) {
            /// thumbnail file may be delete, so generate new one.
        }
        /// @}
    ...
    private long getVideoForBucketCleared(long bucketId)
        ...
        try {
            ...
            /// M: add to support drm, only show METHOD_SD type drm video thumbnail. {@
            if (cursor != null && cursor.getCount() > 0) {
                cursor.moveToPosition(-1);
                while (cursor.moveToNext()) {
                    boolean isDrm = cursor.getInt(VideosBucketThumbnailQuery.IS_DRM) > 0;
                    if (!isDrm || (isDrm && cursor.getInt(VideosBucketThumbnailQuery.DRM_METHOD)
                            == DRM_METHOD_SD)) {
    ...
    private int queryOrientationForImage(long id, CancellationSignal signal) {
        ...
        try {
            ...
            if (cursor.moveToFirst()) {
                ...
                MtkLog.w(TAG, "Missing orientation data for " + id);
                return 0;
            }
        } finally {
            IoUtils.closeQuietly(cursor);
