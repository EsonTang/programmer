...
public class MediaScannerThreadPool extends ThreadPoolExecutor {
    ...
    protected void afterExecute(Runnable r, Throwable t) {
        ...
        if ((r instanceof SingleTypeScanTask) && !mHasExecutedAllTask
            ...
            if(MediaUtils.LOG_SCAN) {
                MtkLog.v(TAG, "Audio/video or image singleScanTask"
                    +" has finish, execute all tasks to threadpool");
            }
            executeAllTask();
        }
    }
    ...
    protected void terminated() {
        if(MediaUtils.LOG_SCAN) {
            MtkLog.v(TAG, "All task(" + getTaskCount() +  ") scan finish,"
                +" send message to insert all to database.");
        }
    ...
    public synchronized void stopScan() {
        if(MediaUtils.LOG_SCAN) {
            MtkLog.w(TAG, "stopScan in threadpool, clear work queue,"
                +" stop insert and mark all task executed");
        }
    ...
    public void parseScanTask() {
        ...
        try {
            synchronized(MediaScannerThreadPool.this) {
    ...
    private class ParseTask extends Task {
        ...
        private int mQuitParseLayerSize = 10; //set as 10 to avoid recursion over stack
        ...
        public void run() {
            ...
            if(MediaUtils.LOG_SCAN) {
                MtkLog.v(TAG, "parse finsih in " + Thread.currentThread().getName()
                    + ": folder = " + mPath
                    + ", cost = " + (end - start) + "ms");
            }
        }
    ...
    private class SingleTypeScanTask extends Task {
        ...
        public void run() {
            ...
            long end = System.currentTimeMillis();
            if(MediaUtils.LOG_SCAN) {
                MtkLog.v(TAG, "SingleTypeScanTask() finished in "
    ...
    private void scan(String path, String volume, int priority, int size) {
        long start = System.currentTimeMillis();
        try {
            MediaScanner scanner = createMediaScanner(volume);
    ...
    private MediaScanner createMediaScanner(String volume) {
        MediaScanner scanner = new MediaScanner(mContext,volume);
        return scanner;
    }
    ...
    private class InsertFolderTask extends Task {
        ...
        public void run() {
            ...
            while (iterator.hasNext()) {
                ...
                if (!isExistInDatabase(folderPath)) {
                    /// If folder has been deleted right now, remove from map and do next check.
            ...
            long end = System.currentTimeMillis();
            if(MediaUtils.LOG_SCAN) {
                MtkLog.v(TAG, "Insert all folder entries finsih in "
                     + Thread.currentThread().getName()
                     + ": folder size = " + mFolderMap.size()
                     + ", insert num = " + folderList.size()
                     + ", cost = " + (end - start) + "ms");
                }
        }
        ...
        private void flush(Uri tableUri, List<ContentValues> list) {
            ...
            long end = System.currentTimeMillis();
            if(MediaUtils.LOG_SCAN) {
                MtkLog.d(TAG, "flush() " + tableUri + " with size " + size
                    + " which cost " + (end - start) + "ms");
            }
        }
    }
}
