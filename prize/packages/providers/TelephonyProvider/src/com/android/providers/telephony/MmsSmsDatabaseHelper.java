...
//            + Mms.THREAD_ID + "; " +
     ...
     */
    static final int DATABASE_VERSION = 641002;
...
//                                            + " LIMIT 1";
    ...
    protected static void removeOrphanedAddresses(SQLiteDatabase db) {
        ...
        MmsSmsProvider.MmsProviderLog.dpi(TAG, "removeOrphanedAddresses, recipient ids = " + ids);
...
//                   "END;");
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int currentVersion) {
        ...
        case 641000:
            if (currentVersion <= 641000){
                return;
            }
            db.beginTransaction();
            try {
                upgradeDatabaseToVersion641001(oldVersion, db);
                db.setTransactionSuccessful();
            } catch (Throwable ex) {
                Log.e(TAG, ex.getMessage(), ex);
                break;
        ...
        case 641001:
            if (currentVersion <= 641001){
                return;
            }
            db.beginTransaction();
            try {
                upgradeDatabaseToVersion641002(oldVersion, db);
                db.setTransactionSuccessful();
            } catch (Throwable ex) {
                Log.e(TAG, ex.getMessage(), ex);
                break;
...
//        createRCSTable(db);
    ...
    private void upgradeDatabaseToVersion641001(int oldVersion, SQLiteDatabase db) {
        if (MTK_RCS_SUPPORT && oldVersion < 601023) {
            db.execSQL("ALTER TABLE " + RCSProvider.TABLE_CONVERSATIONS + " ADD COLUMN " +
                    RCSProvider.DAPI_CONVERSATION_STATUS + " INTEGER DEFAULT 0");
            createDAPIRCSThreadView(db);
            // TODO need verify
            createRcsMessageStatusTriggers(db);
        }
    }

    private void upgradeDatabaseToVersion641002(int oldVersion, SQLiteDatabase db) {
        if (MTK_RCS_SUPPORT && oldVersion < 601024) {
            db.execSQL("ALTER TABLE " + RCSProvider.TABLE_CONVERSATIONS + " ADD COLUMN " +
                RCSProvider.CONVERSATION_COLUMN_AT_ME + " INTEGER DEFAULT 0");
            createDAPIRCSThreadView(db);
            db.execSQL("ALTER TABLE " + RCSProvider.TABLE_MESSAGE + " ADD COLUMN " +
                RCSProvider.MESSAGE_COLUMN_AT_ME + " INTEGER DEFAULT 0");
            createRcsAtMeTrigger(db);
            upgradeDatabaseToVersion601011(db);
        }
    }
    ...
    /// M: Add for OP01 RCS feature: @{
    // Trigger to update rcs_conversations when insert rcs message.
    ...
    private static String UPDATE_RCS_THREAD_AFTER_INSERT_RCS_MESSAGE_DAPI =
            ...
            "   AND NEW." + RCSProvider.MESSAGE_COLUMN_ISBLOCKED + "=0 " +
            " BEGIN " +
            "  UPDATE " + RCSProvider.TABLE_CONVERSATIONS + " " +
            "    SET " +
            // BODY
            ...
            // TYPE
            "      " + RCSProvider.DAPI_CONVERSATION_TYPE + "= " +
            "           (CASE WHEN " + RCSProvider.TABLE_CONVERSATIONS + "." +
                                       RCSProvider.DAPI_CONVERSATION_TIMESTAMP + " <=NEW." +
                                       RCSProvider.MESSAGE_COLUMN_TIMESTAMP +
            "           THEN NEW."   + RCSProvider.MESSAGE_COLUMN_TYPE +
            "           ELSE "       + RCSProvider.TABLE_CONVERSATIONS + "." +
                                       RCSProvider.DAPI_CONVERSATION_TYPE + " END), " +
           // MESSAGE_STATUS
           "      " + RCSProvider.DAPI_CONVERSATION_STATUS + "= " +
           "           (CASE WHEN " + RCSProvider.TABLE_CONVERSATIONS + "." +
                                      RCSProvider.DAPI_CONVERSATION_TIMESTAMP + " <=NEW." +
                                      RCSProvider.MESSAGE_COLUMN_TIMESTAMP +
           "           THEN NEW."   + RCSProvider.MESSAGE_COLUMN_MESSAGE_STATUS +
           "           ELSE "       + RCSProvider.TABLE_CONVERSATIONS + "." +
                                      RCSProvider.DAPI_CONVERSATION_STATUS + " END) " +
            "  WHERE " + RCSProvider.TABLE_CONVERSATIONS + "." +
                         RCSProvider.CONVERSATION_COLUMN_CONVERSATION + "=NEW." +
                         RCSProvider.MESSAGE_COLUMN_CONVERSATION + "; " +
            " END;";
    ...
    private void createRCSTables(SQLiteDatabase db) {
        ...
        createDAPIRCSThreadView(db);
        // TODO need verify if use trigger to update status
        createRcsMessageStatusTriggers(db);
    }
    ...
    private void createRCSThreadView(SQLiteDatabase db) {
        ...
        " SELECT " +
        "    threads._id AS _id " +
        "    ,CASE WHEN T5." + RCSProvider.CONVERSATION_COLUMN_TIMESTAMP + ">threads.date " +
        "         THEN T5." + RCSProvider.CONVERSATION_COLUMN_BODY +
        "         ELSE threads.snippet END AS snippet " +
        "    ,CASE WHEN T5." + RCSProvider.CONVERSATION_COLUMN_TIMESTAMP + ">threads.date " +
        "         THEN 0 ELSE threads.snippet_cs END AS snippet_cs " +
        "    ,0 AS date_sent " +
        "    ,threads.type " +
        "    ,CASE WHEN T5." + RCSProvider.CONVERSATION_COLUMN_TIMESTAMP + ">threads.date " +
        "         THEN T5." + RCSProvider.CONVERSATION_COLUMN_TIMESTAMP +
        "         ELSE threads.date END AS date " +
        "    ,T5." + RCSProvider.CONVERSATION_COLUMN_MESSAGE_COUNT + " - " +
        "         T5." + RCSProvider.CONVERSATION_COLUMN_UNREAD_COUNT + "+threads.readcount " +
        "         AS readcount " +
        ...
        "    ,threads.archived" +
        "    ,CASE WHEN T5." + RCSProvider.CONVERSATION_COLUMN_TIMESTAMP + ">threads.date " +
        "          THEN T5." + RCSProvider.CONVERSATION_COLUMN_CLASS +
        "          ELSE 0 END AS class " +
        ...
        "    ,CASE WHEN T5." + RCSProvider.CONVERSATION_COLUMN_TIMESTAMP + ">threads.date " +
        "         THEN T5." + RCSProvider.CONVERSATION_COLUMN_TYPE +
        "         ELSE 0 END AS CHATMESSAGE_TYPE " +
        "    ,CASE WHEN T5." + RCSProvider.CONVERSATION_COLUMN_TIMESTAMP + ">threads.date " +
        "         THEN T5." + RCSProvider.CONVERSATION_COLUMN_MIME_TYPE +
        "         ELSE NULL END AS CHATMESSAGE_MIME_TYPE " +
        "    ,T5." + RCSProvider.CONVERSATION_COLUMN_AT_ME +
        " FROM " +
            RCSProvider.TABLE_CONVERSATIONS + " T5, threads " +
        " WHERE threads._id=T5." + RCSProvider.CONVERSATION_COLUMN_CONVERSATION + "; ");
    }

    private void createDAPIRCSThreadView(SQLiteDatabase db) {
        db.execSQL("DROP VIEW IF EXISTS rcs_threads_dapi;");
        db.execSQL(
        "CREATE VIEW " + RCSProvider.VIEW_RCS_THREADS_DAPI + " AS " +
        " SELECT " +
        "    threads._id AS " + RCSProvider.DAPI_THREADS_CONVERSATION_ID +
        "    ,T." + RCSProvider.CONVERSATION_COLUMN_RECIPIENTS +
        "         AS " + RCSProvider.DAPI_THREADS_RECIPIENTS +
        "    ,CASE WHEN T." + RCSProvider.DAPI_CONVERSATION_TIMESTAMP + ">threads.date " +
        "         THEN T." + RCSProvider.DAPI_CONVERSATION_BODY +
        "         ELSE threads.snippet END AS " + RCSProvider.DAPI_THREADS_BODY +
        "    ,CASE WHEN T." + RCSProvider.DAPI_CONVERSATION_TIMESTAMP + ">threads.date " +
        "         THEN T." + RCSProvider.DAPI_CONVERSATION_TIMESTAMP +
        "         ELSE threads.date END AS " + RCSProvider.DAPI_THREADS_TIMESTAMP +
        "    ,T." + RCSProvider.CONVERSATION_COLUMN_FLAG + " AS " + RCSProvider.DAPI_THREADS_FLAG +
        "    ,CASE WHEN T." + RCSProvider.DAPI_CONVERSATION_TIMESTAMP + ">threads.date " +
        "         THEN T." + RCSProvider.DAPI_CONVERSATION_TYPE +
        "         ELSE 0 END AS " + RCSProvider.DAPI_THREADS_TYPE +
        "    ,T." + RCSProvider.DAPI_CONVERSATION_UNREAD_COUNT + " + " +
        "         threads.message_count-threads.readcount" +
        "         AS " + RCSProvider.DAPI_THREADS_UNREAD_COUNT +
        "    ,T." + RCSProvider.DAPI_CONVERSATION_MESSAGE_COUNT + "+threads.message_count " +
        "         AS " + RCSProvider.DAPI_THREADS_MESSAGE_COUNT +
        "    ,CASE WHEN T." + RCSProvider.DAPI_CONVERSATION_TIMESTAMP + ">threads.date " +
        "         THEN T." + RCSProvider.DAPI_CONVERSATION_MIMETYPE +
        "         ELSE NULL END AS " + RCSProvider.DAPI_THREADS_MIMETYPE +
        "    ,T." + RCSProvider.DAPI_CONVERSATION_STATUS +
        "        AS " + RCSProvider.DAPI_THREADS_MESSAGE_STATUS +
        " FROM " +
            RCSProvider.TABLE_CONVERSATIONS + " T, threads " +
        " WHERE threads._id=T." + RCSProvider.CONVERSATION_COLUMN_CONVERSATION +
        " UNION " +
        " SELECT " +
        "    T." + RCSProvider.CONVERSATION_COLUMN_CONVERSATION + " AS " +
        "           " + RCSProvider.DAPI_THREADS_CONVERSATION_ID +
        "   ,T." + RCSProvider.CONVERSATION_COLUMN_RECIPIENTS + " AS " +
        "           " + RCSProvider.DAPI_THREADS_RECIPIENTS +
        "   ,T." + RCSProvider.DAPI_CONVERSATION_BODY + " AS " +
        "           " + RCSProvider.DAPI_THREADS_BODY +
        "   ,T." + RCSProvider.DAPI_CONVERSATION_TIMESTAMP + " AS " +
        "           " + RCSProvider.DAPI_THREADS_TIMESTAMP +
        "   ,T." + RCSProvider.CONVERSATION_COLUMN_FLAG + " AS " +
        "           " + RCSProvider.DAPI_THREADS_FLAG +
        "   ,T." + RCSProvider.DAPI_CONVERSATION_TYPE + " AS " +
        "           " + RCSProvider.DAPI_THREADS_TYPE +
        "   ,T." + RCSProvider.DAPI_CONVERSATION_UNREAD_COUNT + " AS " +
        "           " + RCSProvider.DAPI_THREADS_UNREAD_COUNT +
        "   ,T." + RCSProvider.DAPI_CONVERSATION_MESSAGE_COUNT + " AS " +
        "           " + RCSProvider.DAPI_THREADS_MESSAGE_COUNT +
        "   ,T." + RCSProvider.DAPI_CONVERSATION_MIMETYPE + " AS " +
        "           " + RCSProvider.DAPI_THREADS_MIMETYPE +
        "   ,T." + RCSProvider.DAPI_CONVERSATION_STATUS + " AS " +
        "           " + RCSProvider.DAPI_THREADS_MESSAGE_STATUS +
        " FROM " + RCSProvider.TABLE_CONVERSATIONS + " T " +
        " WHERE T." + RCSProvider.CONVERSATION_COLUMN_FLAG + " = " + RCSProvider.FLAG_OFFICIAL +
        " ; ");
    }
    ...
    private void createRCSTriggers(SQLiteDatabase db) {
        ...
        db.execSQL(
            ...
            " END;");
        // Block message START
        String whereSelection = RCSProvider.MESSAGE_COLUMN_CLASS + "=" + RCSProvider.CLASS_NORMAL +
                " AND " + RCSProvider.MESSAGE_COLUMN_CONVERSATION +
                "= NEW." + RCSProvider.MESSAGE_COLUMN_CONVERSATION + " AND " +
                RCSProvider.MESSAGE_COLUMN_ISBLOCKED + "=0";

        db.execSQL("DROP TRIGGER IF EXISTS update_rcs_thread_after_update_rcs_blocked_dapi;");
        db.execSQL("CREATE TRIGGER update_rcs_thread_after_update_rcs_blocked_dapi " +
                " AFTER UPDATE OF " + RCSProvider.MESSAGE_COLUMN_ISBLOCKED + " ON " +
                    RCSProvider.TABLE_MESSAGE +
                " WHEN NEW." + RCSProvider.MESSAGE_COLUMN_CLASS + "=" + RCSProvider.CLASS_NORMAL +
                " BEGIN " +
                "   UPDATE " + RCSProvider.TABLE_CONVERSATIONS +
                ...
                "     SET " +
                       RCSProvider.DAPI_CONVERSATION_MESSAGE_COUNT + "=(" +
                "          SELECT COUNT(" + RCSProvider.COLUMN_ID + ") " +
                "          FROM " + RCSProvider.TABLE_MESSAGE + " WHERE " + whereSelection + "), " +
                // BODY
                       RCSProvider.DAPI_CONVERSATION_BODY + "=(" +
                "          SELECT " + RCSProvider.MESSAGE_COLUMN_BODY +
                "          FROM " + RCSProvider.TABLE_MESSAGE + " WHERE " + whereSelection + " " +
                "          ORDER BY " + RCSProvider.MESSAGE_COLUMN_TIMESTAMP + " DESC LIMIT 1), " +
                // TIMESTAMP
                       RCSProvider.DAPI_CONVERSATION_TIMESTAMP + "=(" +
                "          SELECT " + RCSProvider.MESSAGE_COLUMN_TIMESTAMP +
                "          FROM " + RCSProvider.TABLE_MESSAGE + " WHERE " + whereSelection + " " +
                "          ORDER BY " + RCSProvider.MESSAGE_COLUMN_TIMESTAMP + " DESC LIMIT 1), " +
                // TYPE
                       RCSProvider.DAPI_CONVERSATION_TYPE + "=(" +
                "          SELECT " + RCSProvider.MESSAGE_COLUMN_TYPE +
                "          FROM " + RCSProvider.TABLE_MESSAGE + " WHERE " + whereSelection + " " +
                "          ORDER BY " + RCSProvider.MESSAGE_COLUMN_TIMESTAMP + " DESC LIMIT 1), " +
                // MIMETYPE
                       RCSProvider.DAPI_CONVERSATION_MIMETYPE + "=(" +
                "          SELECT " + RCSProvider.MESSAGE_COLUMN_MIME_TYPE +
                "          FROM " + RCSProvider.TABLE_MESSAGE + " WHERE " + whereSelection + " " +
                "          ORDER BY " + RCSProvider.MESSAGE_COLUMN_TIMESTAMP + " DESC LIMIT 1) " +
                "    WHERE " + RCSProvider.TABLE_CONVERSATIONS + "." +
                                RCSProvider.CONVERSATION_COLUMN_CONVERSATION + "=NEW." +
    ...
    private void createRcsAtMeTrigger(SQLiteDatabase db) {
        String updateOperation =
                " UPDATE " + RCSProvider.TABLE_CONVERSATIONS +
                "  SET " + RCSProvider.CONVERSATION_COLUMN_AT_ME + "=" +
                "    (SELECT COUNT(_id) FROM " + RCSProvider.TABLE_MESSAGE +
                "     WHERE " + RCSProvider.MESSAGE_COLUMN_CONVERSATION +
                      "=NEW." + RCSProvider.MESSAGE_COLUMN_CONVERSATION +
                      " AND " + RCSProvider.MESSAGE_COLUMN_AT_ME + "=1)" +
                "  WHERE " + RCSProvider.TABLE_CONVERSATIONS + "." +
                      RCSProvider.CONVERSATION_COLUMN_CONVERSATION + "=NEW." +
                      RCSProvider.MESSAGE_COLUMN_CONVERSATION + "; ";
        db.execSQL("DROP TRIGGER IF EXISTS update_rcs_thread_at_me_after_insert_rcs_message;");
        db.execSQL("CREATE TRIGGER update_rcs_thread_at_me_after_insert_rcs_message " +
            " AFTER INSERT ON " + RCSProvider.TABLE_MESSAGE +
            " WHEN NEW." + RCSProvider.MESSAGE_COLUMN_AT_ME + "=1" +
            " BEGIN " + updateOperation + " END;");
        db.execSQL("DROP TRIGGER IF EXISTS update_rcs_thread_at_me_after_update_rcs_message;");
        db.execSQL("CREATE TRIGGER update_rcs_thread_at_me_after_update_rcs_message " +
            " AFTER UPDATE OF " + RCSProvider.MESSAGE_COLUMN_AT_ME +
            "    ON " + RCSProvider.TABLE_MESSAGE +
            "    WHEN OLD." + RCSProvider.MESSAGE_COLUMN_AT_ME + "=1 " +
            " BEGIN " + updateOperation + " END;");
    }
    // TODO Need to decide whether to use trigger to finish update column status
    private void createRcsMessageStatusTriggers(SQLiteDatabase db) {
        String updateForSmsMms = String.format(RCSProvider.RCS_UPDATE_MESSAGE_STATUS,
                String.format(RCSProvider.RCS_UNION_QUERY, "NEW.thread_id",
                        "NEW.thread_id", "NEW.thread_id"), "NEW.thread_id");
        String updateForRcs = String.format(RCSProvider.RCS_UPDATE_MESSAGE_STATUS,
                String.format(RCSProvider.RCS_UNION_QUERY,
                        "NEW." + RCSProvider.MESSAGE_COLUMN_CONVERSATION,
                        "NEW." + RCSProvider.MESSAGE_COLUMN_CONVERSATION,
                        "NEW." + RCSProvider.MESSAGE_COLUMN_CONVERSATION),
                        "NEW." + RCSProvider.MESSAGE_COLUMN_CONVERSATION);
        ...
        db.execSQL("CREATE TRIGGER rcs_update_status_on_insert_sms AFTER INSERT ON sms " +
                "BEGIN " + updateForSmsMms + " ;END;");
        db.execSQL("CREATE TRIGGER rcs_update_status_on_update_sms_read AFTER UPDATE OF read " +
                "ON sms BEGIN " + updateForSmsMms + " ;END;");
        db.execSQL("CREATE TRIGGER rcs_update_status_on_update_sms_type AFTER UPDATE OF type " +
                "ON sms BEGIN " + updateForSmsMms + " ;END;");
        db.execSQL("CREATE TRIGGER rcs_update_status_on_insert_mms AFTER INSERT ON pdu " +
                "BEGIN " + updateForSmsMms + " ;END;");
        db.execSQL("CREATE TRIGGER rcs_update_status_on_update_mms_read AFTER UPDATE OF read " +
                "ON pdu BEGIN " + updateForSmsMms + " ;END;");
        db.execSQL("CREATE TRIGGER rcs_update_status_on_update_mms_msg_box AFTER UPDATE OF " +
                "msg_box ON pdu BEGIN " + updateForSmsMms + " ;END;");
//        db.execSQL("CREATE TRIGGER rcs_update_status_on_insert_rcs AFTER INSERT ON rcs_message " +
//                "BEGIN " + updateForRcs + " ;END;");
        db.execSQL("CREATE TRIGGER rcs_update_status_on_update_rcs AFTER UPDATE OF " +
                "CHATMESSAGE_MESSAGE_STATUS ON rcs_message " +
                "BEGIN " + updateForRcs + " ;END;");
    }
    /// @}
}
