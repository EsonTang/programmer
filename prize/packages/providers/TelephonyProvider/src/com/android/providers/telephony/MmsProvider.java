...
public class MmsProvider extends ContentProvider {
    ...
    public Cursor query(Uri uri, String[] projection,
        ...
        if (ret != null) {
            Log.d(TAG, "query getReadableDatabase query end cursor count =" + ret.getCount());
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        int match = sURLMatcher.match(uri);
        if (LOCAL_LOGV) {
            MmsSmsProvider.MmsProviderLog.vpi(TAG, "Insert uri=" + uri + ", match=" + match);
        }
        ...
        if (notify) {
            Log.d(TAG, "insert getWritebleDatabase notify");
            /// M: Code analyze 002, fix bug ALPS00262044, not show out unread message
            /// icon after restore messages. notify mms application about unread messages
            /// number after insert operation.
            notifyUnread = false;
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        int match = sURLMatcher.match(uri);
        Log.d(TAG, "delete begin, uri = " + uri + ", selection = " + selection);
        SQLiteDatabase db = mOpenHelper.getWritableDatabase();
        int deletedRows = 0;
 ...
 public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        MmsSmsProvider.MmsProviderLog.dpi(TAG, "update begin, uri = " + uri + ", values = " + values
                + ", selection = " + selection);
        boolean notify = false;
        String msgId = null;
        String table;
