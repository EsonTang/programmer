...
//@yanlin, should fix before OP09 ready.
...
//    private static final Uri ICC_URI_FOUR = Uri.parse("content://sms/icc4");
    ...
    private static final Uri ICC_URI_GEMINI_INTERNATIONAL = Uri
            .parse("content://sms/icc2_international");
    ...
    */
    ...
    public Cursor query(Uri uri, String[] projectionIn, String selection,
        ...
        MmsSmsProvider.MmsProviderLog.dpi(TAG, "query begin, uri = "
        ...
        switch (match) {
            ...
            case SMS_ALL_ICC_INTERNATIONAL:
              //@yanlin, should fix before OP09 ready.
//                return getAllMessagesFromIccInternational(uri, PhoneConstants.SIM_ID_1);
            /// @}
            case URI_THREAD_ID:
    ...
    private Object[] convertIccToSms(SmsMessage message,
        ...
        if (mIsInternationalCardNotActivate) {
            if (concatSmsIndex == null) {
                try {
                    ...
                    Log.e(TAG, "concatSmsIndex bad number");
                }
            } else {
                row[6] = concatSmsIndex;
            }
        ...
        Log.d(TAG, "convertIccToSms; contactSmsIndex:" + row[6]);
    ...
    private Uri insertInner(Uri url, ContentValues initialValues, int callerUid, String callerPkg) {
        ...
        Log.d(TAG, "insertInner match url end");
        ...
        try {
            if (table.equals(TABLE_SMS)) {
                ...
                if (initialValues == null) {
                    ...
                    /// M: Code analyze 005, fix bug ALPS00245352, it cost long time
                    /// to restore messages. remove useless operation and add transaction
                    /// while import sms. @{
                ...
                /// after restored. set the date as the right value when import.
                    ...
                    Log.d(TAG, "insert sms date " + date);
                /// @}
                }
                ...
                if (((threadId == null) || (threadId == 0)) && (!TextUtils.isEmpty(address))) {
                    ...
                    /// to restore messages. remove useless operation and add transaction
                    /// while import sms. @{
                    Log.d(TAG, "insert sms getThreadId start");
                    long id = 0;
                    if (importSms) {
                    ...
                    Log.d(TAG, "insert getContentResolver getOrCreateThreadId end id = " + id);
                    /// @}
                }
                ...
                /// M: Code analyze 006, fix bug ALPS00252799, it cost long time
                /// to restore messages. support batch processing while restore messages. @{
            ...
            /// remove useless operation and add transaction while import sms. @{
            if (Log.isLoggable(TAG, Log.VERBOSE)) {
                Log.v(TAG, "insert table body end");
            }
            // if (!importSms){
...
//                setThreadStatus(db, values, 0);
            ...
            if (table == TABLE_SMS) {
                ...
                if (Log.isLoggable(TAG, Log.VERBOSE)) {
                    Log.v(TAG, "insert TABLE_WORDS end");
                }
            }
            db.setTransactionSuccessful();
        } finally {
            db.endTransaction();
            Log.d(TAG, "insert sms transacton end");
        }
        ...
        if (rowID > 0) {
            ...
            Log.d(TAG, "insertInner succeed, uri = " + uri);
            return uri;
        } else {
    ...
    public int delete(Uri url, String where, String[] whereArgs) {
        Log.d(TAG, "delete begin, uri = " + url + ", selection = " + where);
        ...
        switch (match) {
            case SMS_ALL:
                Log.d(TAG, "Call delete case SMS_ALL");
                if (where != null && where.equals(FOR_MULTIDELETE)) {
                    ...
                    count = db.delete(TABLE_SMS, where, whereArgs);
                         Log.d(TAG, "SMS_ALL: delete count = " + count);
            ...
            case SMS_AUTO_DELETE:
                ...
                if (whereArgs != null) {
                  ...
                  //  Log.d(TAG, "selectids = "  + selectids);
                    db.execSQL("delete from words where table_to_use=1 and source_id in "
                            + selectids);
                    Log.d(TAG, "delete words end");
                    for (int i = 0; i < whereArgs.length; ) {
                        if (i % 100 == 0) {
                            Log.d(TAG, "delete sms1 beginTransaction i = " + i);
                          // db.beginTransaction();
                        }
...
//                            Log.d(TAG, "delete sms1 endTransaction i = " + i);
//                            db.endTransaction();
//                        }
                    }
                } else {
                    if (where != null) {
                        ...
                        if (args.length > 1) {
                            ...
                            Log.d(TAG, "SMS_CONVERSATIONS_ID args[1] = " + args[1]);
                            id = Integer.parseInt(finalid);
                            Log.d(TAG, "SMS_CONVERSATIONS_ID id = " + id);

                            for (int i = 1; i < id; i++) {
                                if (i % 30 == 0 || i == id - 1) {
                                    Log.d(TAG, "delete sms2 beginTransaction i = " + i);
                                    where = "locked=0 AND type<>3 AND ipmsg_id<=0 AND _id>"
                                        + (i - 30) + " AND _id<=" + i;
                                    where = DatabaseUtils.concatenateWhere("thread_id=" + threadID,
                                            where);
                                    count += db.delete(TABLE_SMS, where, null);
                                    Log.d(TAG, "delete sms2 endTransaction i = " + i
                                            + " count=" + count);
                                }
                            }
                        }
    ...
    public int update(Uri url, ContentValues values, String where, String[] whereArgs) {
        final int callerUid = Binder.getCallingUid();
        final String callerPkg = getCallingPackage();
        Log.d(TAG, "update begin, uri = " + url + ", values = " + values
                + ", selection = " + where);
        int count = 0;
        String table = TABLE_SMS;
    ...
    private void notifyChange(boolean notifyIfNotDefault, Uri uri,
            final String callingPackage, boolean notify) {
        ...
        cr.notifyChange(Uri.parse("content://mms-sms/conversations/"), null, true,
                UserHandle.USER_ALL);
        if (notify && sURLMatcher.match(uri) != SMS_RAW_MESSAGE_ID) {
...
//        long token = Binder.clearCallingIdentity();
//        try {
//            Log.d(TAG, "getAllMessagesFromIccInternational slotId =" + slotId);
//            MatrixCursor mc = null;
//            SmsManager smsManager = SmsManager.getDefault();
//            if (isDual()) {
//                int activateMode = TelephonyManagerEx.getDefault()
//                .getPhoneType(PhoneConstants.GEMINI_SIM_1);
...
//                    messagesActivate = smsManager.getAllMessagesFromIcc((long)slotId,
//                            PhoneConstants.PHONE_TYPE_GSM);
//                    messagesNotActivate = smsManager.getAllMessagesFromIcc((long)slotId,
//                                PhoneConstants.PHONE_TYPE_CDMA);
//                } else {
//                    messagesActivate = smsManager.getAllMessagesFromIcc(slotId,
//                            PhoneConstants.PHONE_TYPE_CDMA);
//                    messagesNotActivate = smsManager.getAllMessagesFromIcc(slotId,
//                                PhoneConstants.PHONE_TYPE_GSM);
//                }
//                mc = getAllMessagesForInternationalCardMatrix(slotId, url,
//                                messagesActivate, messagesNotActivate);
...
//        ITelephonyEx mTelephony = ITelephonyEx.Stub.asInterface(
//                ServiceManager.checkService("phoneEx"));
//        try {
//            if (mTelephony != null) {
//                cardType = mTelephony.getInternationalCardType(
//                        SystemProperties.getInt("ril.external.md", 0)-1);
...
//            Log.d(TAG, "get International cardType failed");
//            return false;
//        }
//    }
...
//            Log.e(TAG, "getAllMessagesFromIccInternational messagesActivate is null");
...
//                SimInfoManager.SimInfoRecord si = SimInfoManager.getSimInfoBySlot(
//                        getContext(), slotId);
//                if (null == si) {
//                    Log.d(TAG, "getSingleMessageFromIcc:SIMInfo is null for slot " + slotId);
//                    return null;
//                }
//                int countActivate = messagesActivate.size();
//                int countNotActivate = messagesNotActivate.size();
//                MatrixCursor cursor = new MatrixCursor(ICC_COLUMNS, countActivate
//                        + countNotActivate);
...
//                            SmsHeader smsHeader = message.getUserDataHeader();
//                            if (null != smsHeader && null != smsHeader.concatRef) {
//                                concatSmsIndexAndBody = getConcatSmsIndexAndBody(
//                                        messagesNotActivate, i);
//                            }
//                        }
//                        cursor.addRow(convertIccToSms(message, concatSmsIndexAndBody, i,
//                                    (int) si.mSimInfoId));
//                    }
//                }
//                mIsInternationalCardNotActivate = false;
...
//                            }
//                        }
//                        cursor.addRow(convertIccToSms(message, concatSmsIndexAndBody,
//                                i + countNotActivate, (int) si.mSimInfoId));
//                    }
//                }
//                return cursor;
//            }
...
//    }
    ...
    //@yanlin, should fix before OP09 ready.
//    private MatrixCursor getAllMessagesForInternationalCard(int slotId, Uri url,
//                    ArrayList<SmsMessage> messages) {
//        if (messages == null || messages.isEmpty()) {
//            Log.e(TAG, "getAllMessagesFromIccInternational messages is null");
//            return null;
//        }
//        ArrayList<String> concatSmsIndexAndBody = null;
//        /// M: convert slotId to simId
//        SimInfoManager.SimInfoRecord si = SimInfoManager.getSimInfoBySlot(getContext(), slotId);
//        if (null == si) {
//            Log.d(TAG, "getSingleMessageFromIcc:SIMInfo is null for slot " + slotId);
//            return null;
//        }
...
//        else {
            cursor.setNotificationUri(getContext().getContentResolver(),
                    Uri.parse("content://sms/" + subId + "/icc"));
//        }
        return cursor;
    }
    /// @}
    ...
    private long getThreadIdInternal(String recipient, SQLiteDatabase db, boolean importSms) {
        ...
        "sms insert, getThreadIdInternal, recipientId = " + recipientId);
        ...
        try {
              if (cursor != null && cursor.getCount() == 0) {
                   ...
                   /// M: modified for RCS feature, add parameter recipient
                   return insertThread(recipientId, db, recipient);
     ...
     */
    /// M: modified for RCS feature, add parameter recipient
    private long insertThread(long recipientIds, SQLiteDatabase db, String recipient) {
        ...
        if (MTK_RCS_SUPPORT) {
            ...
            db.update("threads", v1, whereClause, null);

            ContentValues v2 = new ContentValues();
            v2.put(RCSProvider.CONVERSATION_COLUMN_TIMESTAMP, 0);
            v2.put(RCSProvider.CONVERSATION_COLUMN_RECIPIENTS, recipient);
            v2.put(RCSProvider.CONVERSATION_COLUMN_FLAG, RCSProvider.FLAG_OTO);
            v2.put(RCSProvider.CONVERSATION_COLUMN_CONVERSATION, threadId);
            db.insert(RCSProvider.TABLE_CONVERSATIONS, null, v2);
        }
        /// @} end
    ...
    private String getWordIds(SQLiteDatabase db, String boxType, String selectionArgs) {
        StringBuffer content = new StringBuffer("(");
        String res = "";
        String rawQuery = String.format("select _id from sms where _id NOT IN (select _id " +
                "from sms where type IN %s AND _id NOT IN %s)", boxType, selectionArgs);
        Cursor cursor = null;
        try {
            ...
            Log.d(TAG, "getWordIds cursor content = " + res + " COUNT " + cursor.getCount());
        } finally {
            if (cursor != null) {
                cursor.close();
            }
