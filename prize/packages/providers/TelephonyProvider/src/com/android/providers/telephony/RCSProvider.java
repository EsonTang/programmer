...
public class RCSProvider extends ContentProvider {
    ...
    private static final String PERMISSION_MESSAGE_READ = "com.cmcc.ccs.READ_CCS_MESSAGE";
    private static final String PERMISSION_MESSAGE_WRITE = "com.cmcc.ccs.WRITE_CCS_MESSAGE";
    ...
    private static final int URI_DAPI_MESSAGE           = 101;
    private static final int URI_DAPI_MESSAGE_ID        = 102;
    private static final int URI_DAPI_CONVERSATION      = 103;
    private static final int URI_DAPI_CONVERSATION_ID   = 104;
    ...
    private static final String DAPI_AUTHORITY_CONVERSATION = "com.cmcc.ccs.message_conversation";
    private static final Uri CONTENT_URI_DAPI_CONVERSATION =
                        Uri.parse("content://com.cmcc.ccs.message_conversation");
    ...
    static {
        ...
        uriMatcher.addURI(DAPI_AUTHORITY_CONVERSATION, null, URI_DAPI_CONVERSATION);
        uriMatcher.addURI(DAPI_AUTHORITY_CONVERSATION, "#", URI_DAPI_CONVERSATION_ID);
    ...
    public int delete(Uri uri, String where, String[] whereArgs) {
        ...
        if (count > 0) {
            removeObsoleteGroupThreads(getContext(), db, deletedThreads);
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch(match) {
            ...
            case URI_MESSAGE_ID:
                ...
                boolean atMe = "true".equals(uri.getQueryParameter("at_me"));
                if (atMe) {
                    values.put(MESSAGE_COLUMN_AT_ME, 1);
                }
                id = db.insert(TABLE_MESSAGE, null, values);
                break;
            case URI_DAPI_MESSAGE:
            case URI_DAPI_MESSAGE_ID:
                if ("true".equals(uri.getQueryParameter("createThread"))) {
                    if (!values.containsKey(MESSAGE_COLUMN_CONVERSATION)) {
                        long threadId = ensureThreadId(uri);
                        Log.d(TAG, "after ensureThreadId, threadId = " + threadId);
                        values.put(MESSAGE_COLUMN_CONVERSATION, threadId);
                    }
                    id = db.insert(TABLE_MESSAGE, null, values);
                } else {
                    throw new UnsupportedOperationException("Not Support URI " + uri);
    ...
    public Cursor query(Uri uri, String[] projection, String selection,
        ...
        switch(match) {
            ...
            case URI_DAPI_CONVERSATION:
//                String[] columns = {COLUMN_ID, MESSAGE_COLUMN_CONVERSATION};
//                Cursor c = db.query(TABLE_MESSAGE, columns,
//                        MESSAGE_COLUMN_CLASS + "=" + CLASS_NORMAL, null,
//                        MESSAGE_COLUMN_CONVERSATION, null, null);
//                String selectionDapi = DAPI_THREADS_CONVERSATION_ID + " IN (";
//                try {
//                    if (c != null && c.getCount() > 0) {
//                        while (c.moveToNext()) {
//                            selectionDapi = selectionDapi + c.getLong(1) + ",";
//                        }
//                        selectionDapi = selectionDapi.substring(0, selectionDapi.length() - 1);
//                    }
//                    selectionDapi += ")";
//                } finally {
//                    if (c != null) {
...
//                    }
//                }
                String selectionDapi = DAPI_THREADS_MESSAGE_COUNT + ">0";
                selection = concatSelections(selection, selectionDapi);
                cursor = db.query(VIEW_RCS_THREADS_DAPI, projection, selection, selectionArgs,
                        null, null, sortOrder);
                break;
            case URI_DAPI_CONVERSATION_ID:
                selectionId = DAPI_THREADS_CONVERSATION_ID + "=" +
    ...
    public int update(Uri uri, ContentValues values, String where,
        ...
        switch(match) {
            case URI_MESSAGE:
                if (values.containsKey(MESSAGE_COLUMN_MESSAGE_STATUS) &&
                        values.getAsInteger(MESSAGE_COLUMN_MESSAGE_STATUS) == MESSAGE_STATUS_READ) {
                    values.put(MESSAGE_COLUMN_AT_ME, 0);
                }
    ...
    private long ensureThreadId(Uri uri) {
        Log.d(TAG, "ensureThreadId, start, uri=" + uri.toString());
        ...
        try {
            ...
            if (!TextUtils.isEmpty(chatId)) {
                long threadId = getGroupConversationId(chatId);
                if (threadId > 0) {
                    return threadId;
                }
                u = Uri.parse("content://mms-sms/rcs/thread");
                uriBuilder = u.buildUpon();
                uriBuilder.appendQueryParameter("chatId", chatId);
                Uri returnUri = resolver.insert(uriBuilder.build(), new ContentValues());
                return returnUri == null ? 0 : Long.parseLong(returnUri.getLastPathSegment());
            } else {
                u = Uri.parse("content://mms-sms/threadID");
                uriBuilder = u.buildUpon();
                ...
                c = resolver.query(uriBuilder.build(), projection, null, null, null);
    ...
    private long getGroupConversationId(String chatId) {
        long conversationId = 0;
        if (!TextUtils.isEmpty(chatId)) {
            String[] projection = {COLUMN_ID};
            String selection = CONVERSATION_COLUMN_RECIPIENTS + "='" + chatId + "' AND " +
                    CONVERSATION_COLUMN_FLAG + "=" + FLAG_MTM;
            Uri uri = Uri.parse("content://mms-sms-rcs/conversations");
            ...
            Log.d(TAG, "getGroupConversationId, chatId is null");
        }
    ...
    static void updateConversation(SQLiteDatabase db, long thread_id) {
        ...
        String updateSQL =
                ...
                "       (SELECT " + MESSAGE_COLUMN_MIME_TYPE + " FROM " + TABLE_MESSAGE +
                "       WHERE " + selection +
                "       ORDER BY " + CONVERSATION_COLUMN_TIMESTAMP + " DESC LIMIT 1), " +
                CONVERSATION_COLUMN_AT_ME + " = " +
                "       (SELECT COUNT(_id) FROM " + TABLE_MESSAGE +
                "       WHERE " + selection + " AND " + MESSAGE_COLUMN_AT_ME + "=1) " +
                " WHERE " + CONVERSATION_COLUMN_CONVERSATION + " = " + thread_id;
        db.execSQL(updateSQL);
        // Update Device api values
        String dapiSelection = MESSAGE_COLUMN_CONVERSATION + " = " + thread_id +
                " AND " + MESSAGE_COLUMN_CLASS + "=" + CLASS_NORMAL;
        String updateDapiSQL =
                " UPDATE rcs_conversations SET " +
                // BODY
                DAPI_CONVERSATION_BODY + " = " +
                "       (SELECT " + MESSAGE_COLUMN_BODY + " FROM " + TABLE_MESSAGE +
                "       WHERE " + dapiSelection +
                "       ORDER BY " + CONVERSATION_COLUMN_TIMESTAMP + " DESC LIMIT 1), " +
                // TIMESTAMP
                DAPI_CONVERSATION_TIMESTAMP + " = " +
                "       (SELECT " + CONVERSATION_COLUMN_TIMESTAMP + " FROM " + TABLE_MESSAGE +
                "       WHERE " + dapiSelection +
                "       ORDER BY " + CONVERSATION_COLUMN_TIMESTAMP + " DESC LIMIT 1), " +
                // TYPE
                DAPI_CONVERSATION_TYPE + " = " +
                "       (SELECT " + CONVERSATION_COLUMN_TYPE + " FROM " + TABLE_MESSAGE +
                "       WHERE " + dapiSelection +
                "       ORDER BY " + CONVERSATION_COLUMN_TIMESTAMP + " DESC LIMIT 1), " +
                // MESSAGE_COUNT
                DAPI_CONVERSATION_MESSAGE_COUNT + " = (SELECT COUNT(_id) FROM " + TABLE_MESSAGE +
                "       WHERE " + dapiSelection + "), " +
                // UNREAD_COUNT
                DAPI_CONVERSATION_UNREAD_COUNT + " = (SELECT COUNT(_id) FROM " + TABLE_MESSAGE +
                "       WHERE " + dapiSelection +
                "       AND " + MESSAGE_COLUMN_MESSAGE_STATUS + "=" + MESSAGE_STATUS_UNREAD + ")," +
                // MIME_TYPE
                DAPI_CONVERSATION_MIMETYPE + " = " +
                "       (SELECT " + MESSAGE_COLUMN_MIME_TYPE + " FROM " + TABLE_MESSAGE +
                "       WHERE " + selection +
                "       ORDER BY " + CONVERSATION_COLUMN_TIMESTAMP + " DESC LIMIT 1) " +
                " WHERE " + CONVERSATION_COLUMN_CONVERSATION + " = " + thread_id;
        db.execSQL(updateDapiSQL);
        // MESSAGE_STATUS
        db.execSQL(String.format(RCS_UPDATE_MESSAGE_STATUS,
                String.format(RCS_UNION_QUERY, thread_id, thread_id, thread_id), thread_id));
    ...
    static int removeObsoleteGroupThreads(Context context, SQLiteDatabase db, long[] threadIds) {
        String in = null;
        ...
        String querySelection = CONVERSATION_COLUMN_FLAG + "=" + FLAG_MTM;
        String threadsSelection = " status <> 0 ";
        String selection = " _id NOT IN " +
        ...
        if (in != null) {
            selection = " _id " + in + " AND " +
                    " _id NOT IN " +
                    "    (SELECT DISTINCT " + RCSProvider.MESSAGE_COLUMN_CONVERSATION +
                    "        AS thread_id FROM " + RCSProvider.TABLE_MESSAGE + " where " +
                             RCSProvider.MESSAGE_COLUMN_CONVERSATION + " NOT NULL)";
        }
        String[] queryProjection = {CONVERSATION_COLUMN_RECIPIENTS};
        Cursor c = db.query(TABLE_CONVERSATIONS, queryProjection,
                concatSelections(querySelection, selection), null, null, null, null);
        String deleteGroupChatIn = null;
        try {
            if (c != null && c.getCount() > 0) {
                boolean first = true;
                deleteGroupChatIn = " IN (";
                while (c.moveToNext()) {
                    if (first) {
                        deleteGroupChatIn = deleteGroupChatIn + "'" + c.getString(0) + "'";
                        first = false;
                    } else {
                        deleteGroupChatIn = deleteGroupChatIn + ", '" + c.getString(0) + "'";
                    }
                ...
                deleteGroupChatIn += " )";
                deleteGroupChatIn = "chat_id" + deleteGroupChatIn;
            }
        } finally {
            if (c != null) {
                c.close();
        ...
        int count = db.delete(MmsSmsProvider.TABLE_THREADS,
                concatSelections(threadsSelection, selection), null);
        if (count > 0) {
            MmsSmsDatabaseHelper.removeOrphanedAddresses(db);
            context.getContentResolver().delete(Uri.parse("content://rcs-groupchat"),
                MmsSmsProvider.concatSelections("status<0", deleteGroupChatIn), null);
        }
