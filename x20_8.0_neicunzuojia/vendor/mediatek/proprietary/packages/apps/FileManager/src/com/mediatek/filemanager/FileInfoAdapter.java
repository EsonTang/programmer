...
//longliang
				...
				if(MountPointManager.getInstance().isInternalMountPath(path)){
					//totalSpaces = FileUtils.sizeToString(34359738368L);//32 GB (32*1024*1024*1024)
					float JTY_ROM_MULTIPLE = Float.parseFloat(SystemProperties.get("ro.jty.fat.expandNum", "1"));
					totalSpaces = FileUtils.sizeToString((long)(MountPointManager.getInstance().getMountPointTotalSpace(fileInfo.getFileAbsolutePath()) * JTY_ROM_MULTIPLE)); 
					long usedSpaces = MountPointManager.getInstance().getMountPointTotalSpace(fileInfo.getFileAbsolutePath())
										- MountPointManager.getInstance().getMountPointFreeSpace(fileInfo.getFileAbsolutePath());
                    //totalSpaces = "64.00GB";
					freeSpaceString = FileUtils.sizeToString((long)(MountPointManager.getInstance().getMountPointTotalSpace(fileInfo.getFileAbsolutePath()) * JTY_ROM_MULTIPLE) - usedSpaces); 
				}
//end 
