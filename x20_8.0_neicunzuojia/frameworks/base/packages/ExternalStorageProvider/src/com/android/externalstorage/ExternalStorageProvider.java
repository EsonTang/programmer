...
public class ExternalStorageProvider extends FileSystemProvider {
    ...
    public Cursor queryRoots(String[] projection) throws FileNotFoundException {
		...
		String jtypath = root.path.getAbsolutePath();
	if (SystemProperties.get("ro.mtk_shared_sdcard").equals("1")){
		if (StorageManagerEx.isExternalSDCard(jtypath) || StorageManagerEx.isUSBOTG(jtypath)){
		JTY_ROM_MULTIPLE = 1.0F;
		}else{
		JTY_ROM_MULTIPLE = Float.parseFloat(SystemProperties.get("ro.jty.fat.expandNum", "1"));
		}
	}else{
		 if (("/storage/sdcard0".equals(jtypath)) || ("/mnt/sdcard".equals(jtypath)) || ("/storage/emulated/legacy".equals(jtypath))){
			JTY_ROM_MULTIPLE = Float.parseFloat(SystemProperties.get("ro.jty.fat.expandNum", "1"));
		}
	}
                ...
                row.add(Root.COLUMN_AVAILABLE_BYTES,
                        root.reportAvailableBytes ? (long) (root.path.getFreeSpace() * JTY_ROM_MULTIPLE): -1);
            }
        }
        return result;
    }
