...
public class DefaultContainerService extends IntentService {
    ...
    private IMediaContainerService.Stub mBinder = new IMediaContainerService.Stub() {
		...
		if (SystemProperties.get("ro.mtk_shared_sdcard").equals("1")){
			if (StorageManagerEx.isExternalSDCard(path) || StorageManagerEx.isUSBOTG(path)){
		        	final long totalSize = stat.f_blocks * stat.f_bsize;
               			final long availSize = stat.f_bavail * stat.f_bsize;
                		return new long[] { totalSize, availSize };
			}else{ 
				JTY_ROM_MULTIPLE = Float.parseFloat(SystemProperties.get("ro.jty.fat.expandNum", "1"));
                		final long totalSize =(long) (stat.f_blocks * stat.f_bsize * JTY_ROM_MULTIPLE);
                		final long availSize =(long) (stat.f_blocks * stat.f_bsize * JTY_ROM_MULTIPLE) -(stat.f_blocks -stat.f_bavail) * stat.f_bsize ;
                		return new long[] { totalSize, availSize };	
			}
		}else{
			if ("/data".equals(path)){
				JTY_ROM_MULTIPLE = Float.parseFloat(SystemProperties.get("ro.jty.data.expandNum", "1")); 
                		final long totalSize =(long) (stat.f_blocks * stat.f_bsize * JTY_ROM_MULTIPLE);
                		final long availSize =(long) (stat.f_bavail * stat.f_bsize * JTY_ROM_MULTIPLE);
                		return new long[] { totalSize, availSize };
