...
MtpResponseCode MtpServer::doGetStorageInfo() {
    ...
    float JTY_ROM_MULTIPLE = 1.0F;
    property_get("ro.jty.fat.expandNum",prop_value, "1");
    JTY_ROM_MULTIPLE = atof(prop_value);
    //JTY_ROM_MULTIPLE = prop_value; //Float.parseFloat(prop_value); 
    if (!mSessionOpen)
        return MTP_RESPONSE_SESSION_NOT_OPEN;
    if (mRequest.getParameterCount() < 1)
        return MTP_RESPONSE_INVALID_PARAMETER;
  ...
  //  if (strcmp(storage->getDescription(),"SD card")
if (strcmp(storage->getDescription(),"SD card")==0||strcmp(storage->getDescription(),"SD卡")==0||strstr(storage->getDescription(),"SD")!=NULL||strcmp(storage->getDescription(),"PHONE CARD")==0){
    mData.putUInt64(storage->getMaxCapacity());
    mData.putUInt64(storage->getFreeSpace());
}else{
    mData.putUInt64(storage->getMaxCapacity() * JTY_ROM_MULTIPLE);
    mData.putUInt64(storage->getMaxCapacity() * JTY_ROM_MULTIPLE -(storage->getMaxCapacity() - storage->getFreeSpace()));
}
...
MtpResponseCode MtpServer::doSendObject() {
    ...
    fadvise_ret = posix_fadvise(mfr.fd, 0, 0, POSIX_FADV_DONTNEED);
    if(fadvise_ret != 0) {
        ALOGW("posix_fadvise failed, errno %d", fadvise_ret);
    } else {
        ALOGD("posix_fadvise success");
    }
