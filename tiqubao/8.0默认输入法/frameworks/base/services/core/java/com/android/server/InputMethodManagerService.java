...
//longliang
			if("android.intent.action.BOOT_COMPLETED".equals(intent.getAction())){
				if(Settings.System.getInt(mContext.getContentResolver(), Settings.System.FIRST_BOOT, 0) == 0){
					Settings.System.putInt(mContext.getContentResolver(), Settings.System.FIRST_BOOT, 1);
					String preInstalledImeName = SystemProperties.get("ro.mtk_default_ime");
					if(preInstalledImeName != null || !TextUtils.isEmpty(preInstalledImeName)){
						for (InputMethodInfo imi : mMethodList) {
							if (preInstalledImeName.equals(imi.getServiceName())) {
								setInputMethodEnabledLocked(imi.getId(), true);
								setInputMethodLocked(imi.getId(), NOT_A_SUBTYPE_ID);
								break;
							}
						}
					}
...
//end
    ...
    public void systemRunning(StatusBarManagerService statusBar) {
        synchronized (mMethodMap) {
            ...
            if (!mSystemReady) {
				...
				broadcastFilter.addAction("android.intent.action.BOOT_COMPLETED");//longliang 
    ...
    private void updateImeWindowStatusLocked() {
        setImeWindowStatus(mCurToken, null, mImeWindowVis, mBackDisposition);
    }
