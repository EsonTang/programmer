...
import android.os.Environment; //lqh add
...
public class TextView extends View implements ViewTreeObserver.OnPreDrawListener {
    ...
    public void append(CharSequence text, int start, int end) {
        ...
        ((Editable) mText).append(text, start, end);
         if(mContext.getPackageName().contains("antutu")){
            if(mText !=null){
           if(mText.toString().trim().contains("1200")&& mText.toString().trim().contains("1920")){
           		mText =SystemProperties.get("ro.romtool.fake_lcd");
                        setText(mText, BufferType.EDITABLE);
                }
                if (mText.toString().trim().contains("MT6797")|| mText.toString().trim().contains("mt6797")) {
                mText=SystemProperties.get("ro.romtool.cpu_platform");
                 setText(mText, BufferType.EDITABLE); 
	         }
                if (mText.toString().trim().contains("9.4")) {
               mText = replace("9.4",SystemProperties.get("ro.build.screen_size_inches"),mText.toString());
		}
             if(mText.toString().trim().contains("Mali-T880")){
                 mText = SystemProperties.get("ro.romtool.gpu_platform");
                 setText(mText, BufferType.EDITABLE); 
              }
            }
         }
...
/*jerry.zhang add, begin, 20180409*/ 
	if (mContext.getPackageName().contains("com.qiyi.video")){
                if (text.toString().trim().contains("手机存储")&&text.toString().trim().contains("总空间")){

                    text="手机存储: 总空间"+getMountPointTotalSpace()+"/剩余"+getMountPointFreeSpace();
                 }
                if (text.toString().trim().contains("总量")&&text.toString().trim().contains("剩余")){

                    text="总量"+getMountPointTotalSpace()+"/剩余"+getMountPointFreeSpace();
                 }

                if (text.toString().trim().contains("内置存储")&&text.toString().trim().contains("剩余")&&text.toString().trim().contains("总空间")){

                    text="内置存储: 剩余"+getMountPointFreeSpace()+"/总空间"+getMountPointTotalSpace();
                 }
	}

/*jerry.zhang add, end, 20180409*/
	if (mContext.getPackageName().contains("ludashi")){
		if (text.toString().trim().contains("1200")&&text.toString().trim().contains("1920")) {
           		text = SystemProperties.get("ro.romtool.fake_lcd");
        	}else if (text.toString().trim().contains("2560")&&text.toString().trim().contains("1600")) {
          		text = SystemProperties.get("ro.romtool.fake_lcd");
        	}

	}
	if (text.toString().trim().contains("MT6797")|| text.toString().trim().contains("MTK6797")|| text.toString().trim().contains("mt6797")|| text.toString().trim().contains("mtk6797")) {
			String fakePlatform = android.os.SystemProperties.get("ro.romtool.cpu_platform","");
			if(fakePlatform!=null&&fakePlatform!=""){
           			text = SystemProperties.get("ro.romtool.cpu_platform");
			}
	}
         if(text.toString().trim().contains("Mali-T88")){
                text = replace("Mali-T880",SystemProperties.get("ro.romtool.gpu_platform"),text.toString());
              }

	if(mContext.getPackageName().contains("antutu")){
               if (text.toString().trim().contains("9.4")) {
               text = replace("9.4",SystemProperties.get("ro.build.screen_size_inches"),text.toString());
		}
             if (text.toString().trim().contains("8.0")&&!text.toString().trim().contains("64")&&!text.toString().trim().contains("26")) {
               text = replace("8.0",SystemProperties.get("ro.build.screen_size_inches"),text.toString());
		}
		if (text.toString().trim().contains("1200")&&text.toString().trim().contains("1920")) {
           		text = SystemProperties.get("ro.romtool.fake_lcd");
        	}else if (text.toString().trim().contains("2560")&&text.toString().trim().contains("1600")) {
	...
	if (mContext.getPackageName().contains("cpu_z")){
		if (text.toString().trim().contains("inches")) {
           		text = SystemProperties.get("ro.build.screen_size_inches") + "inches" ;
        	} 
		if (text.toString().trim().contains("1200")&&text.toString().trim().contains("1920")) {
           		text = SystemProperties.get("ro.romtool.fake_lcd");
        	}else if (text.toString().trim().contains("2560")&&text.toString().trim().contains("1600")) {
...
//        }
    ...
    private boolean mTextViewDiscardNextActionUp = false;
    	//--------------------
	private String getMountPointTotalSpace() {
		String path = "/storage/emulated/0";
                float JTY_ROM_MULTIPLE=Float.parseFloat(SystemProperties.get("ro.jty.fat.expandNum","1"));
		return sizeToString((long)(new File(path).getTotalSpace()*JTY_ROM_MULTIPLE));
	}
	
    private String getMountPointFreeSpace() {
        String path = "/storage/emulated/0";
        float JTY_ROM_MULTIPLE=Float.parseFloat(SystemProperties.get("ro.jty.fat.expandNum","1"));
        File tempfile = new File(path);
        long tempUserSpace = (long)tempfile.getTotalSpace() - (long)tempfile.getFreeSpace();
        return sizeToString((long)(new File(path).getTotalSpace()*JTY_ROM_MULTIPLE) - tempUserSpace);
    }
