...
import java.text.NumberFormat;  //longliang
...
public class PhoneStatusBar extends BaseStatusBar implements DemoMode,
	...
	private static final int MSG_DELAY_SCREENSHOT = 1111;//longliang
	
	//longliang
    private static final String KEY_OPEN_BATTERY_LEVEL = "open_battery_level";
    private static final String KEY_CLOSE_BATTERY_LEVEL = "close_battery_level";
    private int mLevel=0;
	private TextView mBatteryLevel;
    //end
	
	//longliang 
	private boolean mCalculateNetSpeed = false;
	private static final String INTENT_SHOW_NET_SPEED = "intent_show_net_speed";
    private static final String INTENT_HIDE_NET_SPEED = "intent_hide_net_speed";
	private static final int CALCULATE_NET_SPEED_DELAY = 1000;
	private static final int MSG_SHOW_NET_SPEED = 1112;
	private long preRxBytes, preSeBytes;
	private long rxBytes, seBytes;
	private LinearLayout mNetSpeedLayout;
	private TextView mDownloadSpeedView;
	private TextView mUploadSpeedView;
	//end 
    ...
    protected PhoneStatusBarView makeStatusBarView() {
		...
		mBatteryLevel = (TextView) mStatusBarView.findViewById(R.id.battery_level);//longliang
		
		//longliang 
		mNetSpeedLayout = (LinearLayout) mStatusBarView.findViewById(R.id.net_speed);
		mDownloadSpeedView = (TextView) mStatusBarView.findViewById(R.id.download_speed);
		mUploadSpeedView = (TextView) mStatusBarView.findViewById(R.id.upload_speed);
		//end 
        ...
        mBatteryController.addStateChangedCallback(new BatteryStateChangeCallback() {
            ...
            public void onBatteryLevelChanged(int level, boolean pluggedIn, boolean charging) {
                ...
                if(Settings.System.getInt(mContext.getContentResolver(),
                    Settings.System.BATTERY_LEVEL_ENABLE, 1) != 0){
                    mLevel=level;
		...
		//longliang
        filter.addAction(KEY_OPEN_BATTERY_LEVEL);
        filter.addAction(KEY_CLOSE_BATTERY_LEVEL);
        //end
		//longliang
		filter.addAction(Intent.ACTION_BOOT_COMPLETED);
		filter.addAction(INTENT_SHOW_NET_SPEED);
        filter.addAction(INTENT_HIDE_NET_SPEED);
		//end 
...
//longliang   
    private View.OnClickListener mScreenShotActionListener = new View.OnClickListener() {
        public void onClick(View v) {
			mHandler.sendEmptyMessageDelayed(MSG_DELAY_SCREENSHOT,400);
        }
    };
...
//longliang 
        ...
        screenShotButton.setOnClickListener(mScreenShotActionListener);
//end 
        /// M: BMW  restore button @{
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            }else if(KEY_OPEN_BATTERY_LEVEL.equals(action)){//longliang
                if(mBatteryLevel==null){
                    mBatteryLevel = (TextView) mStatusBarView.findViewById(R.id.battery_level);
                }
                String percentage = NumberFormat.getPercentInstance().format((double) mLevel / 100.0);//add code
                mBatteryLevel.setText(percentage);
                mBatteryLevel.setVisibility(View.VISIBLE);
            }else if(KEY_CLOSE_BATTERY_LEVEL.equals(action)){
                if(mBatteryLevel!=null){
                    mBatteryLevel.setVisibility(View.GONE);
                }
            }  //end
			//longliang 
				...
				if(Settings.System.getInt(mContext.getContentResolver(), Settings.System.NET_SPEED_ENABLE, 0) != 0){
					mNetSpeedLayout.setVisibility(View.VISIBLE);
					mHandler.sendEmptyMessage(MSG_SHOW_NET_SPEED);
				}
			}
			else if(INTENT_SHOW_NET_SPEED.equals(action)){
				if(mNetSpeedLayout != null){
					mNetSpeedLayout.setVisibility(View.VISIBLE);
					mHandler.sendEmptyMessage(MSG_SHOW_NET_SPEED);
				}
			}else if(INTENT_HIDE_NET_SPEED.equals(action)){
				if(mNetSpeedLayout != null){
					mNetSpeedLayout.setVisibility(View.GONE);
					mHandler.removeMessages(MSG_SHOW_NET_SPEED);
					mCalculateNetSpeed = false;
				}
			}
			//end 
        }
	...
	//longliang
	final Object mNetSpeedLock = new Object();
	
	private void calculateNetSpeed() {
		synchronized (mNetSpeedLock) {  
			if(!mCalculateNetSpeed){
				mCalculateNetSpeed = true;
				preRxBytes = TrafficStats.getTotalRxBytes(); 
				preSeBytes = TrafficStats.getTotalTxBytes();
				mHandler.sendEmptyMessageDelayed(MSG_SHOW_NET_SPEED, CALCULATE_NET_SPEED_DELAY);
				return;
			}
			rxBytes = TrafficStats.getTotalRxBytes(); 
			seBytes = TrafficStats.getTotalTxBytes(); 
			Log.i("longliang", "preRxBytes =" + preRxBytes + ",preSeBytes =" + preSeBytes + ", rxBytes = " + rxBytes + ",seBytes = " + seBytes);
			long downloadSpeed = rxBytes - preRxBytes;
			long uploadSpeed = seBytes - preSeBytes;
			preRxBytes = rxBytes;
			preSeBytes = seBytes;
			...
			if(downloadSpeed == 224 && uploadSpeed == 224){
				downloadSpeed = 0;
				uploadSpeed = 0;
			}
			...
			if (downloadSpeed > 1024 * 1024) {
				...
				downSpeed = downloadSpeed + "MB/s";
			} else if (downloadSpeed > 1024) {
				downloadSpeed /= (1024);
				downSpeed = downloadSpeed + "kB/s";
			} else {
				downSpeed = downloadSpeed + "B/s";
			}
			if (uploadSpeed > 1024 * 1024) {
				uploadSpeed /= (1024 * 1024);
				upSpeed = uploadSpeed + "MB/s";
			} else if (uploadSpeed > 1024) {
				uploadSpeed /= (1024);
				upSpeed = uploadSpeed + "kB/s";
			} else {
				upSpeed = uploadSpeed + "B/s";
			}
/*
			if(downloadSpeed == 0 && uploadSpeed == 0){
				mDownloadSpeedView.setVisibility(View.GONE);
				mUploadSpeedView.setVisibility(View.GONE);
			}else{
				mDownloadSpeedView.setVisibility(View.VISIBLE);
				mUploadSpeedView.setVisibility(View.VISIBLE);
			}
			*/
			mDownloadSpeedView.setText(downSpeed);
			mUploadSpeedView.setText(upSpeed);
