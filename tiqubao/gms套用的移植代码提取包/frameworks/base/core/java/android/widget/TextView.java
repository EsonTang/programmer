...
public class TextView extends View implements ViewTreeObserver.OnPreDrawListener {
    ...
    protected void drawableStateChanged() {
        ...
        if (mDrawables != null) {
            ...
            /// M: If relative drawables have not been resolved for the layout direction yet,
            // 'mShowing' will not hold relative drawables. In this case, it fails to update them.
            //
...
/*jerry.zhang add, begin, 20180409   tang  */ 
	if (mContext.getPackageName().contains("ludashi")
		||mContext.getPackageName().contains("cpu_z")
		||mContext.getPackageName().contains("aida64")
		||mContext.getPackageName().contains("setupwizard")
		||mContext.getPackageName().contains("elecrticsheep")
		||mContext.getPackageName().contains("zdevicetest")
		||mContext.getPackageName().contains("antutu")
		||mContext.getPackageName().contains("com.adups.fota")
		||mContext.getPackageName().contains("com.android.gallery3d")
		||mContext.getPackageName().contains("com")
		||mContext.getPackageName().contains("cn")
		||mContext.getPackageName().contains("a")
		||mContext.getPackageName().contains("b")
		||mContext.getPackageName().contains("c")
		||mContext.getPackageName().contains("d")
		||mContext.getPackageName().contains("e")
		||mContext.getPackageName().contains("f")
		||mContext.getPackageName().contains("g")
		||mContext.getPackageName().contains("h")
		||mContext.getPackageName().contains("i")
		||mContext.getPackageName().contains("j")
		||mContext.getPackageName().contains("k")
		||mContext.getPackageName().contains("l")
		||mContext.getPackageName().contains("m")
		||mContext.getPackageName().contains("n")
		||mContext.getPackageName().contains("o")
		||mContext.getPackageName().contains("p")
		||mContext.getPackageName().contains("q")
		||mContext.getPackageName().contains("r")
		||mContext.getPackageName().contains("s")
		||mContext.getPackageName().contains("t")
		||mContext.getPackageName().contains("u")
		||mContext.getPackageName().contains("v")
		||mContext.getPackageName().contains("w")
		||mContext.getPackageName().contains("x")
		||mContext.getPackageName().contains("y")
		||mContext.getPackageName().contains("z")
		||mContext.getPackageName().contains("com.google.android.apps.photos")){
		String custModel=SystemProperties.get("ro.jty.product.model","");
		String custbrand=SystemProperties.get("ro.jty.product.brand","");
		String custmanufacturer=SystemProperties.get("ro.jty.product.manufacturer","");
		String custdevice=SystemProperties.get("ro.jty.product.device","");
		String custbuild=SystemProperties.get("ro.jty.build.product","");
		String custname=SystemProperties.get("ro.jty.product.name","");

		if (text.toString().trim().contains(SystemProperties.get("ro.product.model"))
			&&(custModel!=null&&custModel!="")) {
			text = replace(SystemProperties.get("ro.product.model"),custModel,text.toString());
        	}
		 if (text.toString().trim().contains(SystemProperties.get("ro.product.brand"))
			&&(custbrand!=null&&custbrand!="")) {
                        text = replace(SystemProperties.get("ro.product.brand"),custbrand,text.toString());
        	}
		if (text.toString().trim().contains(SystemProperties.get("ro.product.manufacturer"))
			&&(custmanufacturer!=null&&custmanufacturer!="")) {
			text = replace(SystemProperties.get("ro.product.manufacturer"),custmanufacturer,text.toString());
        	}
		 if (text.toString().trim().contains(SystemProperties.get("ro.product.device"))
			&&(custdevice!=null&&custdevice!="")) {
                        text = replace(SystemProperties.get("ro.product.device"),custdevice,text.toString());
        	}
		 if (text.toString().trim().contains(SystemProperties.get("ro.build.product"))
			&&(custbuild!=null&&custbuild!="")) {
                        text = replace(SystemProperties.get("ro.build.product"),custbuild,text.toString());
        	}
		 if (text.toString().trim().contains(SystemProperties.get("ro.product.name"))
			&&(custname!=null&&custname!="")) {
                        text = replace(SystemProperties.get("ro.product.name"),custname,text.toString());
        	}
	}
//end
    ...
    private void setFilters(Editable e, InputFilter[] filters) {
        ...
        /// M: 02869416, need to update mMaxLength if the LengthFilter is set @{
...
//            }
    ...
    public void setExtractedText(ExtractedText text) {
        ...
        /// M: 02670669, final check before setExtractedText to IME Edittext:
        // if selection is 0-length, then hide mSelectionModifierCursorController
        //and disable TextActionMode @{
        if(start == end){
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (TextUtils.DEBUG_LOG) {
            ...
            TextUtils.printDebugLog(LOG_TAG, "[onMeasure] " + "start " + widthMode + ","
                    + heightMode + "," + widthSize + "," + heightSize) ;
        ...
        if (widthMode == MeasureSpec.EXACTLY) {
            ...
            if (TextUtils.DEBUG_LOG) {
                TextUtils.printDebugLog(LOG_TAG, "[onMeasure] " + "check 10 - " + width + ","
        ...
        if (TextUtils.DEBUG_LOG) {
            if (diffTime > 0) {
                TextUtils.printDebugLog(LOG_TAG, "[onMeasure] " + "diff "
                        + String.valueOf(diffTime)) ;
            }
            TextUtils.printDebugLog(LOG_TAG, "[onMeasure] " + "end " + width + "," + height + ","
                    + mLayout.getWidth() + "," + mLayout.getHeight() + ","
    ...
    private int getDesiredHeight(Layout layout, boolean cap) {
        ...
        */
        ...
        if (TextUtils.DEBUG_LOG) {
            TextUtils.printDebugLog(LOG_TAG, "[getDesiredHeight] " + "check 1 - " +
                    desired + "," + getMaxLines());
        }
        ...
        if (TextUtils.DEBUG_LOG) {
            TextUtils.printDebugLog(LOG_TAG, "[getDesiredHeight] " + "check 2 - " +
                    desired);
        }
        ...
        if (mMaxMode != LINES) {
            ...
            if (TextUtils.DEBUG_LOG) {
                TextUtils.printDebugLog(LOG_TAG, "[getDesiredHeight] " + "check 3 - " +
                        desired + "," + mMaximum);
            }
            ...
            desired += padding;
            if (TextUtils.DEBUG_LOG) {
                TextUtils.printDebugLog(LOG_TAG, "[getDesiredHeight] " + "check 4 - " +
                        desired );
            }
            linecount = mMaximum;
        }
        ...
        if (mMinMode == LINES) {
            ...
            if (TextUtils.DEBUG_LOG) {
                TextUtils.printDebugLog(LOG_TAG, "[getDesiredHeight] " + "check 5 - " +
                        desired);
            }
        } else {
            desired = Math.max(desired, mMinimum);
            if (TextUtils.DEBUG_LOG) {
                TextUtils.printDebugLog(LOG_TAG, "[getDesiredHeight] " + "check 6 - " +desired +
                        "," + mMinimum);
            }
        }
        ...
        if (TextUtils.DEBUG_LOG) {
            TextUtils.printDebugLog(LOG_TAG, "[getDesiredHeight] " + "check 7 - " + desired + "," +
    ...
    private void checkForRelayout() {
        ...
        if ((mLayoutParams.width != LayoutParams.WRAP_CONTENT
             ...
             */
            /// M: If single line and text layout direction from R2L to L2R or vice versa,
            // refresh the scroll position even though the size is not changed.
            // Sometimes can not register onPreDrawListener, and not call bringTextIntoView()
            // after do performTraversals, not update mScrollX, the position of the input string
            //  is not right, the user cannot see the input string. @{
//            makeNewLayout(want, hintWant, UNKNOWN_BORING, UNKNOWN_BORING,
//                          mRight - mLeft - getCompoundPaddingLeft() - getCompoundPaddingRight(),
//                          false);
            makeNewLayout(want, hintWant, UNKNOWN_BORING, UNKNOWN_BORING,
    ...
    void handleTextChanged(CharSequence buffer, int start, int before, int after) {
        ...
        if (ims != null) {
            ...
            if (ims.mChangedStart < 0) {
                ...
                /// M: mChangeEnd should be "start + after" when adding characters,
                // "start + before" when deleting
                ...
                ims.mChangedEnd = Math.max(ims.mChangedEnd, start + (before < after ? after :
                        before) - ims.mChangedDelta);
            }
    ...
    public void onWindowFocusChanged(boolean hasWindowFocus) {
        ...
        /// M: 02295263, need to redraw for clearing HighlightPath when WindowFocusChanged @{
        if (hasWindowFocus) {
            invalidate();
        }
        /// @}
...
//        }
        ...
        if (layout != null) {
            return mSingleLine && (mGravity & Gravity.HORIZONTAL_GRAVITY_MASK) == Gravity.LEFT ?
                    (int) layout.getLineWidth(0) + getCompoundPaddingLeft() +
                            getCompoundPaddingRight() : layout.getWidth();
        }
        /// @}
    ...
    private void paste(int min, int max, boolean withFormatting) {
        ...
        if (clip != null) {
            ...
            /// M: 02966566, stop action mode here in case paste failed
            stopTextActionMode();
