...
public class UsbConfirmActivity extends AlertActivity
    ...
    public void onCreate(Bundle icicle) {
		...
		// chenyl modify start for orion 
		  Log.d("chenyl", "UsbConfirmActivity mPackageName = "+mResolveInfo.activityInfo.packageName);
		  if(mResolveInfo.activityInfo.packageName != null 
		  	&& !mResolveInfo.activityInfo.packageName.equals("com.sunstoneengineering.orion.s2.v250")){
			  setupAlert(); 
		  }else { 
					  try {
						  ...
						  if (mDevice != null) {
							  newIntent = new Intent(UsbManager.ACTION_USB_DEVICE_ATTACHED);
							  newIntent.putExtra(UsbManager.EXTRA_DEVICE, mDevice);
							  ...
							  newIntent = new Intent(UsbManager.ACTION_USB_ACCESSORY_ATTACHED);
							  newIntent.putExtra(UsbManager.EXTRA_ACCESSORY, mAccessory);
						  ...
						  startActivityAsUser(newIntent, new UserHandle(userId));
					  } catch (Exception e) {
