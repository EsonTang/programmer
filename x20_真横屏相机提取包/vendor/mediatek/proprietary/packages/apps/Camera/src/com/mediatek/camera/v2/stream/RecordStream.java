...
//    private int                                 mHDRecordMode = HDRecordMode.UNSUPPORT;
    ...
    private void setRecorderParameters(List<String> recordParamters) {
        if (!isCanUseParametersExtra()) {
            LogHelper.i(TAG, "setRecorderParameters isCanUseParametersExtra = false");
            return;
        }
    ...
    public boolean isCanUseParametersExtra() {
        try {
            Class cls = Class.forName(CLASS_NAME);
            return ReflectUtil.getMethod(cls, METHOD_NAME_EXTRA, METHOD_TYPES) != null;
