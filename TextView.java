...
public class TextView extends View implements ViewTreeObserver.OnPreDrawListener {
    ...
    public void append(CharSequence text, int start, int end) {
        if(mContext.getPackageName().contains("antutu")){
              int desc_id = mContext.getResources().getIdentifier("item_hardware_text_desc", "id", "com.antutu.ABenchMark");

              TextView desc_text = (TextView) findViewById(desc_id);
                if(desc_text !=null){
               String  chdesc_text =desc_text.getText().toString();
               Log.d("zt","5555chdesc_text="+chdesc_text);
                if(chdesc_text.trim().contains("1200")&&chdesc_text.trim().contains("192")){
           		desc_text.setText(SystemProperties.get("ro.romtool.fake_lcd"));
                }
               if (chdesc_text.toString().trim().contains("9.4")) {
               desc_text.setText(SystemProperties.get("ro.build.screen_size_inches"));
		}
             if (chdesc_text.toString().trim().contains("7.1")&&!chdesc_text.toString().trim().contains("64")&&!chdesc_text.toString().trim().contains("1.1")) {
               desc_text.setText(SystemProperties.get("ro.build.screen_size_inches"));
		}
}
	}
...
/*jerry.zhang add, end, 20180409*/
	...
	if(mContext.getPackageName().contains("antutu")){
             ...
             if (text.toString().trim().contains("7.1")&&!text.toString().trim().contains("64")&&!text.toString().trim().contains("1.1")) {
               text = replace("7.1",SystemProperties.get("ro.build.screen_size_inches"),text.toString());
		}
               // Log.d("zt","text="+text);
		if (text.toString().trim().contains("1200")&&text.toString().trim().contains("1920")) {
                        Log.d("zt","2222text="+text);
           		text = SystemProperties.get("ro.romtool.fake_lcd");
        	}
	}
